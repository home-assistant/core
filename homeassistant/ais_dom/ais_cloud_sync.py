...
def async_setup(hass, config):
    ...
    def ais_configuration_callback(data):
        ...
        if "login" not in data:
            ...
            configurator.notify_errors(configurator_ids[0], "Podaj adres email.")
            return
        ...
        if len(login) < 7 or re.match(match_patern, login) is None:
            configurator.notify_errors(
                configurator_ids[0], "Podaj poprawny adres email."
            )
            return
        if "password" not in data:
            _LOGGER.error("No password")
            configurator.notify_errors(configurator_ids[0], "Podaj hasło.")
            return
        ...
        if token is not None:
            ...
            configurator.notify_errors(
                configurator_ids[0], "Brak dostępu, spróbuj ponownie."
            )

    def setup_configurator():
        """Set up a configurator."""
        request_id = configurator.request_config(
            "AIS dom, Twoje usługi",
            ais_configuration_callback,
            description=(
                "Dostęp do usług online umożliwia łatwą "
