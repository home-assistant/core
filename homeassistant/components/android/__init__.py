"""Native Home Assistant Android app component."""
import datetime
import logging
...
CONFIGURATION_FILE = '.android.conf'
...
async def async_setup(hass, config):
    ...
    android_config = await hass.async_add_executor_job(
        load_json, hass.config.path(CONFIGURATION_FILE))

    if android_config == {}:
        android_config[ATTR_DEVICES] = {}

    android_config[CONF_PUSH] = (conf or {}).get(CONF_PUSH, {})

    hass.data[DOMAIN] = android_config
...
async def async_setup_entry(hass, entry):
    """Set up an Android entry."""
    hass.async_create_task(
        hass.config_entries.async_forward_entry_setup(entry, 'sensor'))

    hass.http.register_view(
        AndroidIdentifyDeviceView(hass.config.path(CONFIGURATION_FILE)))
    hass.http.register_view(AndroidPushConfigView(hass.data[DOMAIN][CONF_PUSH]))

    return True


# pylint: disable=invalid-name
class AndroidPushConfigView(HomeAssistantView):
    """A view that provides the push categories configuration."""

    url = '/api/android/push'
    name = 'api:android:push'
...
class AndroidIdentifyDeviceView(HomeAssistantView):
    """A view that accepts device identification requests."""

    url = '/api/android/identify'
    name = 'api:android:identify'
    ...
    async def post(self, request):
        ...
        # Commented for now while Android app is getting frequent updates
        # try:
...
config_entry_flow.register_discovery_flow(
    DOMAIN, 'Home Assistant Android', lambda *_: True,
