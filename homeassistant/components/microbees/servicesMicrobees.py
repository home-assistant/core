...
async def sendCommand(self, data):
    ...
    async with aiohttp_client.async_create_clientsession(self.hass) as session:
        async with session.post(SENDCOMMAND_URL, json=data, headers=headers) as resp:
            await resp.text()


async def getBees(hass: HomeAssistant, token):
    headers = {
        "Content-Type": "application/json",
        "Authorization": "Bearer %s" % token,
    }
    async with aiohttp_client.async_create_clientsession(hass) as session:
        async with session.post(GETMYBEES_URL, headers=headers) as resp:
