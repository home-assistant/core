...
class PS4Device(MediaPlayerEntity):
    ...
    if status is not None:
        ...
        if status.get("status") == "Ok":
            self._parse_ok_status(status)
        else:
            self._handle_non_ok_status()
...
def _parse_ok_status(self, status: dict) -> None:
    """Handle status when it's 'Ok'."""
...
def _handle_non_ok_status(self) -> None:
    """Handle status when it's not 'Ok'."""
