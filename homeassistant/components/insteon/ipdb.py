...
DEVICE_PLATFORM: dict[type, dict[Platform, list[int] | range]] = {
...
def map_config_to_platform(prop: DeviceFlagBase):
    """Map a configuration flag to a platform."""
...
def get_device_platforms(device) -> dict[Platform, list[int] | range]:
    """Return the HA platforms for a device type."""
    return DEVICE_PLATFORM.get(type(device), {})


def get_device_platform_groups(device: Device, platform: Platform) -> list[int] | range:
    """Return the list of device groups for a platform."""
    return get_device_platforms(device).get(platform, [])


def get_device_config_plaforms(device: Device) -> dict[Platform, list[str]]:
    """Return the platforms for device configuration."""
    prop: DeviceFlagBase
    platforms: dict[Platform, list[str]] = {}
    for name, prop in device.configuration.items():
        if prop.is_read_only or prop.property_type not in [
            PropertyType.STANDARD,
            PropertyType.DERIVED,
        ]:
            continue
        if platform := map_config_to_platform(prop):
            if platform not in platforms:
                platforms[platform] = []
            platforms[platform].append(name)
    return platforms


def get_device_config_platform_props(device: Device, platform: Platform) -> list[str]:
    """Return a list of platforms for config entities of the device."""
    return get_device_config_plaforms(device).get(platform, [])
