...
class HiFiBerryMediaPlayer(MediaPlayerEntity):
    ...
    async def _async_command(self, command: str) -> None:
        """Send a playback command to media player."""
        capabilities = self._client.active_player_capabilities
        if (
            command in ("play", "pause")
            and command not in capabilities
            and "play_pause" in capabilities
        ):
            await self._client.async_command("play_pause")
            return
    ...
    async def async_media_next_track(self) -> None:
        ...
        await self._async_command("next")

    @override
    async def async_media_previous_track(self) -> None:
        """Send media_previous command to media player."""
        await self._async_command("previous")
    ...
    async def async_media_play_pause(self) -> None:
        ...
        if "play_pause" in self._client.active_player_capabilities:
            await self._client.async_command("play_pause")
            return
    ...
    async def async_mute_volume(self, mute: bool) -> None:
        ...
        if mute:
            ...
            if not self._muted and current_volume is not None:
