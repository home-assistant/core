...
class AlexaRangeController(AlexaCapability):
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        if self.instance in {
            ...
            # Common case for percentage-based attributes
            labels = ["Percentage", AlexaGlobalCatalog.SETTING_FAN_SPEED] if fan.DOMAIN in self.instance else ["Position", AlexaGlobalCatalog.SETTING_OPENING]
            min_value, max_value, precision = self.get_min_max_precision(self.entity, fan.ATTR_PERCENTAGE_STEP, "max_value")
            return self.create_resource(labels, min_value, max_value, precision)

        if self.instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}":
            min_value, max_value, precision = self.get_min_max_precision(self.entity, humidifier.ATTR_MIN_HUMIDITY, humidifier.ATTR_MAX_HUMIDITY, 10, 90)
            return self.create_resource(["Humidity", "Percentage", "Target humidity"], min_value, max_value, precision)

        if self.instance in (f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}", f"{number.DOMAIN}.{number.ATTR_VALUE}"):
            ...
            resource = AlexaPresetResource(["Value", get_resource_by_unit_of_measurement(self.entity)], min_value, max_value, precision, unit)
            resource.add_preset(value=min_value, labels=[AlexaGlobalCatalog.VALUE_MINIMUM])
            resource.add_preset(value=max_value, labels=[AlexaGlobalCatalog.VALUE_MAXIMUM])
        ...
        if self.instance == f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}":
            ...
            resource = AlexaPresetResource([AlexaGlobalCatalog.SETTING_FAN_SPEED], min_value=0, max_value=max_value, precision=1)
            for index, speed in enumerate(speed_list):
                ...
                resource.add_preset(value=index, labels=labels)
            return resource.serialize_capability_resources()

        return {}

    def get_min_max_precision(self, entity, domain_attr_min, domain_attr_max, default_min=0, default_max=100, default_precision=1):
        """Get min and max values."""
        min_value = entity.attributes.get(domain_attr_min, default_min)
        max_value = entity.attributes.get(domain_attr_max, default_max)
        precision = entity.attributes.get("precision", default_precision)
        return min_value, max_value, precision

    def create_resource(self, labels, min_value, max_value, precision=1, unit=AlexaGlobalCatalog.UNIT_PERCENT):
