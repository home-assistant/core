...
class AlexaThermostatController(AlexaCapability):
    ...
    def get_property(self, name: str) -> Any:
        ...
        if name in ("targetSetpoint", "lowerSetpoint", "upperSetpoint"):
            return self._get_setpoint(name)

        raise UnsupportedProperty(name)

    def _get_thermostat_mode(self, name: str) -> dict[str, str] | str | None:
    ...
    def _get_setpoint(self, name: str) -> dict[str, float | str] | None:
        """Return the setpoint property."""
        unit = self.hass.config.units.temperature_unit
...
class AlexaRangeController(AlexaCapability):
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        """Return capabilityResources object."""
        # Try each resource helper in order, return early if a result is found.
        for helper in (
            self._fan_speed_percentage_resources,
            self._humidifier_target_humidity_resources,
            self._cover_position_resources,
            self._cover_tilt_resources,
            self._input_number_value_resources,
            self._number_value_resources,
            self._vacuum_fan_speed_resources,
            self._valve_position_resources,
        ):
            result = helper()
    ...
    def _fan_speed_percentage_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}":
    ...
    def _humidifier_target_humidity_resources(
        self,
    ...
    def _cover_position_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
            self._resource = AlexaPresetResource(
    ...
    def _cover_tilt_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{cover.DOMAIN}.tilt":
            self._resource = AlexaPresetResource(
    ...
    def _input_number_value_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}":
    ...
    def _number_value_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{number.DOMAIN}.{number.ATTR_VALUE}":
    ...
    def _vacuum_fan_speed_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}":
    ...
    def _valve_position_resources(self) -> dict[str, list[dict[str, Any]]] | None:
        if self.instance == f"{valve.DOMAIN}.{valve.ATTR_POSITION}":
            self._resource = AlexaPresetResource(
