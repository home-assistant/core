...
class AlexaModeController(AlexaCapability):
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        instance_handlers: dict[str, Callable[[], dict[str, list[dict[str, Any]]]]] = {
            f"{fan.DOMAIN}.{fan.ATTR_DIRECTION}": self._capability_fan_direction,
            f"{fan.DOMAIN}.{fan.ATTR_PRESET_MODE}": self._capability_fan_preset_mode,
            f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}": self._capability_humidifier_modes,
            f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}": self._capability_water_heater_modes,
            f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}": self._capability_remote_activity,
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": self._capability_cover_position,
            f"{valve.DOMAIN}.state": self._capability_valve_state,
        }

        handler = instance_handlers.get(self.instance or "")
        if handler is not None:
            return handler()
        return {}

    def _capability_fan_direction(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _capability_fan_preset_mode(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _capability_humidifier_modes(self) -> dict[str, list[dict[str, Any]]]:
    ...
    def _capability_water_heater_modes(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaModeResource([AlexaGlobalCatalog.SETTING_MODE], False)
        operation_modes = (
            self.entity.attributes.get(water_heater.ATTR_OPERATION_LIST) or []
        )
    ...
    def _capability_remote_activity(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaModeResource([AlexaGlobalCatalog.SETTING_MODE], False)
    ...
    def _capability_cover_position(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaModeResource(
    ...
    def _capability_valve_state(self) -> dict[str, list[dict[str, Any]]]:
        supported_features = self.entity.attributes.get(ATTR_SUPPORTED_FEATURES, 0)
        self._resource = AlexaModeResource(
