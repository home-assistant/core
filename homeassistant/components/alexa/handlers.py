...
def _calculate_range_delta(
    range_delta: float, range_delta_default: bool, default_step: int = 20
) -> int:
    """Calculate the adjusted range delta based on default step."""
    return int(range_delta * default_step) if range_delta_default else int(range_delta)


def _get_current_value_or_raise(
    entity: ha.State, attr_name: str, entity_id: str, value_type: str
) -> float:
    """Get current value from entity attributes or raise error if not found."""
    current = entity.attributes.get(attr_name)
    if current is None:
        msg = f"Unable to determine {entity_id} current {value_type}"
        raise AlexaInvalidValueError(msg)
    return float(current)


def _clamp_position_value(
    value: float, min_val: float = 0, max_val: float = 100
) -> float:
    """Clamp a position value between min and max bounds."""
    return min(max_val, max(min_val, value))


def _handle_position_adjustment(
    entity: ha.State,
    range_delta: int,
    position_attr: str,
    value_type: str,
    ...
    """Handle position-based adjustments (covers, valves)."""
    current = _get_current_value_or_raise(
        entity, position_attr, entity.entity_id, value_type
    )
    position = _clamp_position_value(range_delta + current)

    if position == 100:
        return open_service, {}, position
    if position == 0:
        return close_service, {}, position
    return set_service, {data_attr: position}, position


def _handle_percentage_adjustment(
    entity: ha.State,
    range_delta: int,
    percentage_attr: str,
    value_type: str,
    set_service: str,
    turn_off_service: str,
    data_attr: str,
    min_val: float = 0,
    max_val: float = 100,
) -> tuple[str, dict[str, Any], float]:
    """Handle percentage-based adjustments (fan, humidifier)."""
    current = _get_current_value_or_raise(
        entity, percentage_attr, entity.entity_id, value_type
    )

    # Get custom min/max if provided
    if min_val == 0 and max_val == 100:
        percentage = _clamp_position_value(range_delta + current)
    else:
        percentage = _clamp_position_value(range_delta + current, min_val, max_val)

    if percentage and set_service:
        return set_service, {data_attr: percentage}, percentage
    if turn_off_service:
        return turn_off_service, {}, percentage
    return set_service, {data_attr: percentage}, percentage


def _handle_number_adjustment(
    entity: ha.State,
    range_delta: float,
    min_attr: str,
    max_attr: str,
    ...
    """Handle number/input_number adjustments."""
    min_value = float(entity.attributes[min_attr])
    max_value = float(entity.attributes[max_attr])
    current = float(entity.state if use_state else entity.attributes.get(data_attr, 0))

    new_value = _clamp_position_value(range_delta + current, min_value, max_value)
    return service, {data_attr: new_value}, new_value


def _handle_vacuum_fan_speed_adjustment(
    entity: ha.State, range_delta: int
) -> tuple[str, dict[str, Any], float]:
    """Handle vacuum fan speed adjustments."""
    speed_list = entity.attributes[vacuum.ATTR_FAN_SPEED_LIST]
    ...
    new_speed_index = _clamp_position_value(
        current_speed_index + range_delta, 0, len(speed_list) - 1
    )
    speed = speed_list[int(new_speed_index)]
    return vacuum.SERVICE_SET_FAN_SPEED, {vacuum.ATTR_FAN_SPEED: speed}, speed
...
async def async_api_adjust_range(
    ...
    """Process an adjust range value request."""
    entity = directive.entity
    instance = directive.instance
    domain = entity.domain
    ...
    data: dict[str, Any] = {ATTR_ENTITY_ID: entity.entity_id}
    HandlerMapType = dict[str, dict[str, Any]]
    HANDLER_MAP: HandlerMapType = {
        f"{cover.DOMAIN}.{cover.ATTR_POSITION}": {
            "handler": _handle_position_adjustment,
            "args": lambda e, d, r, rd_def: (
                e,
                _calculate_range_delta(r, rd_def),
        ...
        f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}": {
            "handler": _handle_percentage_adjustment,
            "args": lambda e, d, r, rd_def: (
                e,
                _calculate_range_delta(
                    r, rd_def, e.attributes.get(fan.ATTR_PERCENTAGE_STEP) or 20
                ),
                fan.ATTR_PERCENTAGE,
                "fan speed",
        ...
        f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}": {
            ...
            "args": lambda e, d, r, rd_def: (
                e,
                _calculate_range_delta(r, rd_def, 5),
                humidifier.ATTR_HUMIDITY,
                "target humidity",
                humidifier.SERVICE_SET_HUMIDITY,
                None,
                humidifier.ATTR_HUMIDITY,
                e.attributes.get(humidifier.ATTR_MIN_HUMIDITY, 10),
                e.attributes.get(humidifier.ATTR_MAX_HUMIDITY, 90),
            ),
        },
        f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}": {
            "handler": _handle_number_adjustment,
            "args": lambda e, d, r, rd_def: (
                e,
                float(r),
        ...
        f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}": {
            "handler": _handle_vacuum_fan_speed_adjustment,
            "args": lambda e, d, r, rd_def: (e, int(r)),
        },
    ...
    # Get the correct handler based on instance
    if instance is None:
        raise AlexaInvalidDirectiveError(DIRECTIVE_NOT_SUPPORTED)

    config_entry = HANDLER_MAP.get(instance)
    if config_entry is None:
        raise AlexaInvalidDirectiveError(DIRECTIVE_NOT_SUPPORTED)

    # MyPy now knows config_entry is not None
    assert config_entry is not None
    handler = config_entry["handler"]
    args_func = config_entry["args"]

    args = args_func(entity, domain, range_delta, range_delta_default)
    service, data_update, response_value = handler(*args)

    # Update data and call service
