...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    LOGGER.debug("Attempting to connect to %s", host)
    spa = BalboaSpaWifi(host)
    connected = await spa.connect()
    if not connected:
        LOGGER.error("Failed to connect to spa at %s", host)
    ...
    async def _async_balboa_update_cb() -> None:
        ...
        LOGGER.debug("Primary update callback triggered")
    ...
    LOGGER.debug("Starting listener and monitor tasks")
    ...
    def stop_monitoring() -> None:
        ...
        LOGGER.debug("Canceling listener and monitor tasks")
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    LOGGER.debug("Disconnecting from spa")
...
async def async_setup_time_sync(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    LOGGER.debug("Setting up daily time sync")
    spa: BalboaSpaWifi = hass.data[DOMAIN][entry.entry_id]

    async def sync_time(now: datetime) -> None:
        LOGGER.debug("Syncing time with Home Assistant")
