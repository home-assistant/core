...
async def websocket_lovelace_config(
    ...
    if not connection.user.is_admin:
        ...
        deserialized_config["views"] = list(filter(lambda view : any(user["user"] == connection.user.id for user in view["visible"]) if "visible" in view else True, deserialized_config["views"]))

        config = json_fragment(orjson.dumps(deserialized_config))
