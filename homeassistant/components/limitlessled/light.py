...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        voluptuous.Optional(light_bridge.CONF_BRIDGES): voluptuous.All(
            ...
            [
                {
                    voluptuous.Required(
                        light_bridge.CONF_HOST
                    ): config_validation.string,
                    voluptuous.Optional(
                        light_bridge.CONF_VERSION, default=light_bridge.DEFAULT_VERSION
                    ): config_validation.positive_int,
                    voluptuous.Optional(
                        light_bridge.CONF_PORT, default=light_bridge.DEFAULT_PORT
                    ): config_validation.port,
                    voluptuous.Required(light_bridge.CONF_GROUPS): voluptuous.All(
                        config_validation.ensure_list,
                        [
                            {
                                voluptuous.Required(
                                    light_bridge.CONF_NAME
                                ): config_validation.string,
                                voluptuous.Optional(
                                    light_bridge.CONF_TYPE,
                                    default=light_bridge.DEFAULT_LED_TYPE,
                                ): voluptuous.In(light_bridge.LED_TYPE),
                                voluptuous.Required(
                                    light_bridge.CONF_NUMBER
                                ): config_validation.positive_int,
                                voluptuous.Optional(
                                    light_bridge.CONF_FADE,
                                    default=light_bridge.DEFAULT_FADE,
