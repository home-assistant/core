...
class TeslemetryVehicleSwitchEntity(TeslemetryRootEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        await handle_vehicle_command(
            self.hass, self.entity_description.on_func(self.api)
        )
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        self.raise_for_scope(self.entity_description.scopes[0])
        await handle_vehicle_command(
            self.hass, self.entity_description.off_func(self.api)
        )
...
class TeslemetryStormModeSwitchEntity(TeslemetryEnergyInfoEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        """Turn on the Switch."""
        self.raise_for_scope(Scope.ENERGY_CMDS)
        await handle_command(self.hass, self.api.storm_mode(enabled=True))
        self._attr_is_on = True
        self.async_write_ha_state()
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn off the Switch."""
        self.raise_for_scope(Scope.ENERGY_CMDS)
        await handle_command(self.hass, self.api.storm_mode(enabled=False))
        self._attr_is_on = False
        self.async_write_ha_state()
