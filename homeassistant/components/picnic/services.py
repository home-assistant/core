...
from .const import ATTR_AMOUNT, ATTR_DEVICE_ID, ATTR_PRODUCT_ID, ATTR_PRODUCT_NAME, CONF_API, DOMAIN, \
...
async def async_register_services(hass: HomeAssistant) -> None:
    ...
    if not hass.services.has_service(DOMAIN, SERVICE_ADD_PRODUCT_TO_CART):
        ...
        hass.services.async_register(
            DOMAIN, SERVICE_ADD_PRODUCT_TO_CART, async_add_product_service, schema=vol.Schema({
    ...
    if not hass.services.has_service(DOMAIN, SERVICE_SEARCH_PRODUCT):
        ...
        hass.services.async_register(
            DOMAIN, SERVICE_SEARCH_PRODUCT, async_search_product_service, schema=vol.Schema({
                vol.Optional(ATTR_DEVICE_ID): cv.string,
    ...
    for item in search_result[0]['items']:
        if 'name' in item:
            ...
            decorators = {
                ...
                for d in item.get('decorators', []) if d['type'] in PRODUCT_DECORATOR_MAPPING
            }
