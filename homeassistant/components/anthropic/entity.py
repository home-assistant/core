...
class AnthropicBaseLLMEntity(CoordinatorEntity[AnthropicCoordinator]):
    ...
    async def _async_handle_chat_log(  # noqa: C901
        ...
        if options.get(CONF_CODE_EXECUTION, DEFAULT[CONF_CODE_EXECUTION]):
            ...
            if model.startswith(
                ...
                not options.get(CONF_WEB_SEARCH, DEFAULT[CONF_WEB_SEARCH])
                and not options.get(CONF_WEB_FETCH, DEFAULT[CONF_WEB_FETCH])
            ):
                tools.append(
                    CodeExecutionTool20250825Param(
                        name="code_execution",
            ...
            if model.startswith(
                ...
                tools.append(
                    WebFetchTool20250910Param(
                        ...
                        max_uses=options.get(CONF_WEB_FETCH_MAX_USES),
                    )
                )
            else:
                tools.append(
