"""Services for the Tag component."""
import logging

import voluptuous as vol

from homeassistant.config_entries import ConfigEntry
from homeassistant.core import callback
from homeassistant.helpers import device_registry as device_reg
import homeassistant.helpers.config_validation as cv
from homeassistant.helpers.device_registry import DeviceEntry, DeviceRegistry
from homeassistant.helpers.typing import HomeAssistantType

from .const import DOMAIN as DOMAIN

ATTR_TAG_ID = "tag_id"

SERVICE_REGISTER_TAG = "register_tag"
SERVICE_TAG_SCANNED = "tag_scanned"
ATTR_MANUFACTURER = "manufacturer"
ATTR_MODEL = "model"
ATTR_NAME = "name"
ATTR_TAG_FORMAT = "tag_format"

_LOGGER = logging.getLogger(__name__)
_UNDEF = object()

SERVICE_SCHEMAS = {
    SERVICE_REGISTER_TAG: vol.Schema(
        {
            vol.Required(ATTR_TAG_ID, default=None): cv.string,
            vol.Required(ATTR_MANUFACTURER, default=None): cv.string,
            vol.Required(ATTR_MODEL, default=None): cv.string,
            vol.Optional(ATTR_NAME, default=None): cv.string,
            vol.Optional(ATTR_TAG_FORMAT, default=None): cv.string,
        }
    )
}


@callback
def async_load_services(hass: HomeAssistantType):
    """Load the services exposed by the Tag component."""

    async def async_register_tag(service):
        """Create a device to store the Tag tag id in HA."""

        entry: ConfigEntry = hass.data[DOMAIN]["config_entry"]
        device_registry: DeviceRegistry = await device_reg.async_get_registry(hass)

        tag_id: str = service.data.get(ATTR_TAG_ID)
        manufacturer: str = service.data.get(ATTR_MANUFACTURER)
        model: str = service.data.get(ATTR_MODEL)
        custom_name: str = service.data.get(ATTR_NAME)
        tag_format: str = service.data.get(ATTR_TAG_FORMAT)

        device: DeviceEntry = device_registry.async_get_or_create(
            config_entry_id=entry.entry_id,
            identifiers={(DOMAIN, tag_id)},
            name=f"{manufacturer} - {model}".format(manufacturer, model),
            manufacturer=manufacturer,
            model=model,
            sw_version=_UNDEF if tag_format is None else tag_format,
        )

        if custom_name is not None and device.name_by_user != custom_name:
            device_registry.async_update_device(device.id, name_by_user=custom_name)

    hass.helpers.service.async_register_admin_service(
        DOMAIN,
        SERVICE_REGISTER_TAG,
        async_register_tag,
        schema=SERVICE_SCHEMAS[SERVICE_REGISTER_TAG],
    )

    async def async_tag_scanned(service):
        """Fire an event when a tag is scanned."""
        device_registry: DeviceRegistry = await device_reg.async_get_registry(hass)
        tag_id: str = service.data.get(ATTR_TAG_ID)
        _LOGGER.info("Tag scanned with id: %s", tag_id)
        device: DeviceEntry = device_registry.async_get_device({(DOMAIN, tag_id)}, {})
        hass.bus.async_fire("tag-scanned", {"tag_device_id": device.id})

    hass.helpers.service.async_register_admin_service(
        DOMAIN,
        SERVICE_TAG_SCANNED,
        async_tag_scanned,
        schema=SERVICE_SCHEMAS[SERVICE_REGISTER_TAG],
    )


@callback
def async_unload_services(hass):
    """Unload the tag component services."""
    hass.services.async_remove(DOMAIN, SERVICE_REGISTER_TAG)
