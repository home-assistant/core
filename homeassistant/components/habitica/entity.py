...
class HabiticaPartyMemberBase(HabiticaBase):
    ...
    def user(self) -> UserData:
        ...
        return self.party_coordinator.data.members[self.subentry.unique_id]
...
class HabiticaPartyBase(CoordinatorEntity[HabiticaPartyCoordinator]):
    ...
    def __init__(
        ...
        unique_id = f"{config_entry.unique_id}_{coordinator.data.group.id!s}"
        self.entity_description = entity_description
        self._attr_unique_id = f"{unique_id}_{entity_description.key}"
        self._attr_device_info = DeviceInfo(
            entry_type=DeviceEntryType.SERVICE,
            manufacturer=MANUFACTURER,
            model=NAME,
            name=coordinator.data.group.summary,
            identifiers={(DOMAIN, unique_id)},
