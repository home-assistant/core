...
CURRENT_STATE_MAP = {
    0: AlarmControlPanelEntityState.ARMED_HOME,
    1: AlarmControlPanelEntityState.ARMED_AWAY,
    2: AlarmControlPanelEntityState.ARMED_NIGHT,
    3: AlarmControlPanelEntityState.DISARMED,
    4: AlarmControlPanelEntityState.TRIGGERED,
}

TARGET_STATE_MAP = {
    AlarmControlPanelEntityState.ARMED_HOME: 0,
    AlarmControlPanelEntityState.ARMED_AWAY: 1,
    AlarmControlPanelEntityState.ARMED_NIGHT: 2,
    AlarmControlPanelEntityState.DISARMED: 3,
}
...
class HomeKitAlarmControlPanelEntity(HomeKitEntity, AlarmControlPanelEntity):
    ...
    async def async_alarm_disarm(self, code: str | None = None) -> None:
        """Send disarm command."""
        await self.set_alarm_state(AlarmControlPanelEntityState.DISARMED, code)

    async def async_alarm_arm_away(self, code: str | None = None) -> None:
        """Send arm command."""
        await self.set_alarm_state(AlarmControlPanelEntityState.ARMED_AWAY, code)

    async def async_alarm_arm_home(self, code: str | None = None) -> None:
        """Send stay command."""
        await self.set_alarm_state(AlarmControlPanelEntityState.ARMED_HOME, code)

    async def async_alarm_arm_night(self, code: str | None = None) -> None:
        """Send night command."""
        await self.set_alarm_state(AlarmControlPanelEntityState.ARMED_NIGHT, code)

    async def set_alarm_state(
        self, state: AlarmControlPanelEntityState, code: str | None = None
    ) -> None:
        """Send state command."""
