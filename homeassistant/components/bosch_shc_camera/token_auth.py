...
from . import async_get_bosch_cloud_session, ir
...
class TokenAuthCoordinatorMixin:
    ...
    def token(self: Any) -> str:
        """Return the current bearer token (in-memory refreshed value wins over the config entry)."""
        # Prefer in-memory refreshed token over config entry (avoids stale reads)
        return (  # type: ignore[no-any-return]
            getattr(self, "_refreshed_token", None)
            or self.entry.data.get(  # value is correct at runtime; HA/external source is Any-typed
                "bearer_token", ""
            )
        )
    ...
    def refresh_token(self: Any) -> str:
        """Return the current refresh token (in-memory refreshed value wins over the config entry)."""
        return (  # type: ignore[no-any-return]
            getattr(self, "_refreshed_refresh", None)
            or self.entry.data.get(  # value is correct at runtime; HA/external source is Any-typed
                "refresh_token", ""
            )
        )
    ...
    def options(self: Any) -> dict[str, Any]:
        """Return the cached options snapshot for this coordinator's lifetime."""
    ...
    async def ensure_valid_token(self: Any, observed_token: str | None = None) -> str:
        ...
        async with self._token_refresh_lock:
            return await self._refresh_token_locked(  # type: ignore[no-any-return]
                observed_token
    ...
    async def _refresh_token_locked(
        ...
        # Local import (not top-level): config_flow.py imports DEFAULT_OPTIONS/
        ...
        # other caller of `ensure_valid_token` (including all other
        # 401-recovery paths in this integration) blocks. Bounding the
        # whole loop to a single 15s ceiling caps the lock hold time
    ...
    async def _async_token_failure_alert(self: Any, message: str) -> None:
        ...
        for svc in self._get_alert_services("system"):
            ...
            if self.hass.services.has_service(domain, name):
                ...
                except Exception as err:  # noqa: BLE001 — per-notify-target loop; one target's failure must not skip remaining configured notify services
    ...
    async def _proactive_refresh(self: Any) -> None:
        ...
        except Exception as err:  # noqa: BLE001 — background-task safety net; ensure_valid_token can raise HomeAssistantError/ConfigEntryAuthFailed/UpdateFailed/aiohttp errors, none of which may escape this fire-and-forget timer callback
            _LOGGER.warning(
