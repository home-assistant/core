...
_SSL_CONTEXT_DATA_KEY = "bosch_shc_camera_cloud_ssl_context"
_SSL_CONTEXT_LOCK_DATA_KEY = "bosch_shc_camera_cloud_ssl_context_lock"
_SESSION_LOCK_DATA_KEY = "bosch_shc_camera_cloud_session_lock"
...
async def async_get_bosch_cloud_ssl_context(hass: HomeAssistant) -> ssl.SSLContext:
    ...
    Cached on ``hass.data`` (not a module-level global) so state never leaks
    between separate ``HomeAssistant`` instances in the same process, as
    happens routinely across parallel test runs.
    """
    existing = cast("ssl.SSLContext | None", hass.data.get(_SSL_CONTEXT_DATA_KEY))
    if existing is not None:
        return existing
    # Lazily create the lock on first call (must be on the event loop)
    lock = cast("asyncio.Lock | None", hass.data.get(_SSL_CONTEXT_LOCK_DATA_KEY))
    if lock is None:
        lock = asyncio.Lock()
        hass.data[_SSL_CONTEXT_LOCK_DATA_KEY] = lock
    async with lock:
        # Double-check inside the lock (another coroutine may have built it)
        existing = cast("ssl.SSLContext | None", hass.data.get(_SSL_CONTEXT_DATA_KEY))
        if existing is None:
            existing = await hass.async_add_executor_job(_build_ssl_context)
            hass.data[_SSL_CONTEXT_DATA_KEY] = existing
    return existing
...
async def async_get_bosch_cloud_session(hass: HomeAssistant) -> aiohttp.ClientSession:
    ...
    # Lazily create the lock on first call (must be on the event loop).
    lock = cast("asyncio.Lock | None", hass.data.get(_SESSION_LOCK_DATA_KEY))
    if lock is None:
        lock = asyncio.Lock()
        hass.data[_SESSION_LOCK_DATA_KEY] = lock
    async with lock:
