...
async def build_data_and_dispatch(
    ...
    for cam_id in cam_ids:
        ...
        status = coordinator._cached_status.get(cam_id, "UNKNOWN")
        ...
        if do_events and events:
            ...
            if prev_id is None:
                ...
                with coordinator._fcm_lock:
                    _last_push = getattr(coordinator, "_fcm_last_push", float("-inf"))
                    _started_at = getattr(coordinator, "_fcm_started_at", float("-inf"))
                    ...
                    if (
                        ...
                        and getattr(coordinator, "_fcm_running", False)
                        and getattr(coordinator, "_fcm_healthy", False)
                        and _ref != float("-inf")
                        and _push_age > FCM_DELIVERY_DEAD_AFTER_SEC
                    ):
                        coordinator._fcm_healthy = False
                        coordinator._fcm_force_hard_heal = True
                        _ago = (
                            "never"
                ...
                _dedup_skip = (
                    coordinator._alert_sent_ids.get(newest_id, float("-inf"))
                    > _now_mono - 60.0
                )
                coordinator._last_event_ids[cam_id] = newest_id
                if _dedup_skip:
                    ...
                    coordinator._alert_sent_ids[newest_id] = _now_mono
                    ...
                    if len(coordinator._alert_sent_ids) > 64:
                        # Mutate in place — a dict-comprehension rebind
                        # (coordinator._alert_sent_ids = {...}) would detach
                        ...
                        # any later reader of coordinator._alert_sent_ids —
                        # allowing a duplicate alert for the same event
                        # (bug-hunt 2026-07-03).
                        _cutoff = _now_mono - 120.0
                        for _k in [
                            k
                            for k, v in coordinator._alert_sent_ids.items()
                            if v < _cutoff
                        ]:
                            del coordinator._alert_sent_ids[_k]
                    _LOGGER.debug(
