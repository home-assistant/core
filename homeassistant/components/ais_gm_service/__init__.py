...
async def async_get_keys(hass):
    ...
    try:
        json_ws_resp = await aisCloud.async_key("gm_user_key", hass)
        GM_USER = json_ws_resp["key"]
        json_ws_resp = await aisCloud.async_key("gm_pass_key", hass)
        GM_PASS = json_ws_resp["key"]
        try:
            json_ws_resp = await aisCloud.async_key("gm_dev_key", hass)
