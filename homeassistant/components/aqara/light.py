...
from .const import AQARA_DISCOVERY_NEW, AQARA_HA_SIGNAL_UPDATE_ENTITY, DOMAIN
...
class AqaraLightEntityDescription(LightEntityDescription):
    ...
    color_model: ColorMode | None = None
...
class AqaraLightEntity(AqaraEntity, LightEntity):
    ...
    def turn_on(self, **kwargs: Any) -> None:
        ...
        if kwargs.get("brightness") is not None:
            bright = kwargs.get("brightness")
            brightness = bright if bright is not None else 0
            commands = {
            ...
            rgbw: tuple[int, int, int, int] = (
                color if color is not None else {0, 0, 0, 0}
            )
            argb = rgbw_to_argb(rgbw)
            commands = {self.entity_description.rgbw_color_res_id: str(argb)}
        elif kwargs.get("rgb_color") is not None:
            rgbcolor = kwargs.get("rgb_color")
            rgb: tuple[int, int, int] = rgbcolor if rgbcolor is not None else {0, 0, 0}
