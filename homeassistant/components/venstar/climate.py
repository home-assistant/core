...
class VenstarThermostat(ClimateDevice):
    ...
    def set_temperature(self, **kwargs):
        ...
        if set_temp:
            if operation_mode == self._client.MODE_HEAT or operation_mode == HVAC_MODE_HEAT:
                success = self._client.set_setpoints(temperature, self._client.cooltemp)
            elif operation_mode == self._client.MODE_COOL or operation_mode == HVAC_MODE_COOL:
                success = self._client.set_setpoints(self._client.heattemp, temperature)
            elif operation_mode == self._client.MODE_AUTO or operation_mode == HVAC_MODE_AUTO:
