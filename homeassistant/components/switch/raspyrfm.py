...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    controlunit_manufacturer = config.get(CONF_CONTROLUNIT_MANUFACTURER)
    controlunit_model = config.get(CONF_CONTROLUNIT_MODEL)
    channel_config = config.get(CONF_CHANNEL_CONFIG)
    ...
    # TODO: automatically detect clients
    #
    # if host is None:
    #    # search for gateways on local network
    #    gateways = raspyrfm_client.search()
    #    for gateway in gateways:
    #        if gateway.get_manufacturer() is Manufacturer(gateway_manufacturer)
    #
    # else:
    #    pass
