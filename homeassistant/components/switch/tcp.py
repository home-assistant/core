...
https://home-assistant.io/components/switch.tcp/
"""
import logging
import socket
import select
...
from homeassistant.const import (
    CONF_NAME, CONF_HOST, CONF_PORT, CONF_PAYLOAD, CONF_PAYLOAD_ON, CONF_PAYLOAD_OFF,
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Required(CONF_PAYLOAD_ON): cv.string,
    vol.Required(CONF_PAYLOAD_OFF): cv.string,
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    """Set up the TCP switch."""
    add_devices([TcpSwitch(hass, config)])


class TcpSwitch(SwitchDevice):
    """Implementation of a TCP socket based switch."""
    ...
    def __init__(self, hass, config):
        ...
        self._config = {
            ...
            CONF_PAYLOAD_ON: config.get(CONF_PAYLOAD_ON),
            CONF_PAYLOAD_OFF: config.get(CONF_PAYLOAD_OFF),
    ...
    def name(self):
        ...
        if name is not None:
            return name
        return super(TcpSwitch, self).name
    ...
    def update(self):
        ...
        if self._config[CONF_VALUE_TEMPLATE] is not None:
            try:
                value = self._config[CONF_VALUE_TEMPLATE].render(
                    value=value)
    ...
    def turn_on(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            try:
                sock.send(self._config[CONF_PAYLOAD_ON].encode())
            except socket.error as err:
                _LOGGER.error(
                    "Unable to send payload %r to %s on port %s: %s",
                    self._config[CONF_PAYLOAD_ON], self._config[CONF_HOST],
                    self._config[CONF_PORT], err)
                return
            ...
            if not readable:
                _LOGGER.warning(
                    "Timeout (%s second(s)) waiting for a response after "
                    "sending %r to %s on port %s.",
                    self._config[CONF_TIMEOUT], self._config[CONF_PAYLOAD_ON],
                    self._config[CONF_HOST], self._config[CONF_PORT])
                return

            sock.recv(self._config[CONF_BUFFER_SIZE]).decode()

        #_LOGGER.warning("value = %d", value)
        #self._state = value
        #self._state = True
    ...
    def turn_off(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            try:
                sock.send(self._config[CONF_PAYLOAD_OFF].encode())
            except socket.error as err:
                _LOGGER.error(
                    "Unable to send payload %r to %s on port %s: %s",
                    self._config[CONF_PAYLOAD_OFF], self._config[CONF_HOST],
                    self._config[CONF_PORT], err)
                return
            ...
            if not readable:
                _LOGGER.warning(
                    "Timeout (%s second(s)) waiting for a response after "
                    "sending %r to %s on port %s.",
                    self._config[CONF_TIMEOUT], self._config[CONF_PAYLOAD_OFF],
                    self._config[CONF_HOST], self._config[CONF_PORT])
                return
