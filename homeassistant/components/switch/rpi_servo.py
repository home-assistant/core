...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for port, name in ports.items():
        switches.append(RPiGPIOServo(name, port, inactive_position, position_on, position_off, enabled_duration))
...
class RPiGPIOServo(ToggleEntity):
    ...
    def __init__(self, name, port, inactive_position, position_on, position_off, enabled_duration):
    ...
    def _run_with_duty(self, duty):
        """Turn the servo to the given duty, and back to inactive unless duration is 0"""
