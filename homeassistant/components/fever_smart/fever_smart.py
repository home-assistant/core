...
class FeverSmartAdvParser(BluetoothData):
    ...
    def _start_update(self, service_info: BluetoothServiceInfo) -> None:
        ...
        raw_adv = FEVER_SMART_MANUFACTURER_ID.to_bytes(length=2, byteorder="little") + adv_value
    ...
    def decrypt(
        ...
        hashedKey = make_key(key, raw_device_id, mac_size_bytes)
    ...
    def process(raw_offset, key):
        ...
        device_id = parse_device_id(raw_device_id, False)
        nonce_int = int(nonce_b, 16) # BigInteger bigInteger = new BigInteger(this.scanRecord.substring(26, 42), 16);

        message = decrypt(
            key,
            raw_device_id,
            nonce_a + nonce_b, # scanRecord.substring(6, 16) + scanRecord.substring(26, 42), nonce
            raw_offset[10:26], #scanRecord.substring(10, 26), AEADParameters.associatedText
