...
async def _read_zone_state(system_id: int, zone_id: int, tty: str) -> _ZoneState:
    ...
    return _ZoneState(curr_temp=curr_temp, htsp=htsp, clsp=clsp)
...
class EvolutionCoordinator(DataUpdateCoordinator[EvolutionState]):
    ...
    def read_current_temperature(self, sys_id: int, zone_id: int) -> int:
        ...
        return self.data.systems[sys_id].zones[zone_id].curr_temp

    def read_fan_mode(self, sys_id: int) -> str:
        """Return the current fan mode."""
        return self.data.systems[sys_id].fan_mode

    def read_hvac_mode(self, sys_id: int) -> HVACMode:
        """Return the current HVAC mode."""
        evolution_mode = self.data.systems[sys_id].hvac_mode
        mode_enum = {
            "HEAT": HVACMode.HEAT,
            "COOL": HVACMode.COOL,
    ...
    def read_target_temperatures(
        ...
        match mode:
            case HVACMode.HEAT:
                return (self.data.systems[sys_id].zones[zone_id].htsp, None, None)
            case HVACMode.COOL:
                return (self.data.systems[sys_id].zones[zone_id].clsp, None, None)
            case HVACMode.HEAT_COOL:
                htsp = self.data.systems[sys_id].zones[zone_id].htsp
                clsp = self.data.systems[sys_id].zones[zone_id].clsp
    ...
    def read_hvac_action(self, sys_id: int, zone_id: int) -> HVACAction:
        ...
        if not self.data.systems[sys_id].is_hvac_active:
