...
def get_uptime_from_hostinfo(hostinfo: StateInfo) -> int:
    """Return the uptime duration in seconds."""
    return math.ceil(timedelta(microseconds=(hostinfo.uptime / 1000)).total_seconds())


def hev_cycle_status(hev_cycle: StateHevCycle) -> tuple[int, int, bool]:
    """Return the current HEV cycle status."""
    return (hev_cycle.duration, hev_cycle.remaining, hev_cycle.last_power)


def last_hev_cycle_result_str(last_hev_cycle: StateLastHevCycleResult) -> str:
