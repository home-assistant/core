...
This component can be used to track which device is closest to a given zone
from a set of devices (like the entities 'home' and 'work' in the example) or
to track which zone is closest to one device (like the entity 'nick' in the
example).
The first child of the proximity dictionary will be used as the `entity_id` of
a proximity entity. The first entity of the example can be accessed by using
`proximity.home`.
The config for tracking the distance to a zone is the following:
- you can add a `zone` key, if you don't put it in, it will use the `home` zone
- you can add `ignored_zones`, when a device is in an `ignored_zone`, that
  device will not be used for the calculation. This might be helpful if you
  want to control your thermostat based on the distance between your house and
  the closest device, but your work is too close to your house and you still
  want to monitor the distance with another device.
- you must add the `devices` key and add at least one device in it
- you can change the `tolerance` to have a larger tolerance when determining
  the direction of travel. if you don't put this key, it will use 1.
- you can change the `name` of the entity to change how the entity will be
  represented on the frontend
- you can say the `type` of the proximity entity is a `zone` tracker. If you
  don't put it, it will automatically use `type: zone`
The config for tracking which zone is closest to a device is the following:
- you need to add the `type` key and say it's a device tracker
- you need to add the `device` key to tell which device it is you want to
  monitor
- you need to add the `zones` key and add  at least one zone. These are the
  zones you're comparing the distance to. There are no `ignored_zones` for this
  tracker, if you want to ignore a zone, just don't add it in the `zones` key
- you can add the `tolerance` and `name` keys just like with the zone tracker
...
class Proximity(Entity):  # pylint: disable=too-many-instance-attributes
    ...
    def check_proximity_device(self, entity, old_state, new_state):
        ...
        if (closest_zone != self.nearest or old_state is None or
