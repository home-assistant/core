...
def async_register_commands(
    ...
    async_reg(hass, handle_get_state)
...
@callback
@decorators.websocket_command(
    {vol.Required("type"): "get_state", vol.Required("entity_id"): cv.entity_id}
)
def handle_get_state(
    hass: HomeAssistant, connection: ActiveConnection, msg: dict[str, Any]
) -> None:
    """Handle get state command."""
    entity_ids = msg.get("entity_id", [])
    states = hass.states.get(entity_ids)
    print(states)
    # JSON serialize here so we can recover if it blows up due to the
    # state machine containing unserializable data. This command is required
    # to succeed for the UI to show.
