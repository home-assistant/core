...
class ModernFormsLightEntity(LightEntity, ModernFormsDeviceEntity):
    ...
    def unique_id(self) -> str:
        ...
        return f"{self.coordinator.data.info.mac_address}_light"
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn off the light."""
        await cast(
            ModernFormsDataUpdateCoordinator, self.coordinator
        ).modernforms.light(on=LIGHT_POWER_OFF)
    ...
    async def async_set_light_sleep_timer(
        ...
        await cast(
            ModernFormsDataUpdateCoordinator, self.coordinator
        ).modernforms.light(sleep=sleep_time * 60)
    ...
    async def async_clear_light_sleep_timer(
        ...
        await cast(
            ModernFormsDataUpdateCoordinator, self.coordinator
        ).modernforms.light(sleep=CLEAR_TIMER)
