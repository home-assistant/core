...
class MillEntity(CoordinatorEntity[MillDataCoordinator]):
    ...
    def __init__(self, coordinator: MillDataCoordinator, device_id: str, capability: EDeviceCapability | None = None):
        """Initialize the Mill entity."""
        super().__init__(coordinator)
        self._device_id = device_id
        self._capability = capability 

        device_data = coordinator.data.get(self._device_id) if coordinator.data else None
        device_name = device_data.get("customName", self._device_id) if device_data else self._device_id
        model_name = DeviceMetric.get_device_type(device_data) if device_data else "Unknown Mill Device"
        
        if model_name is None:
            _LOGGER.warning("Model name could not be determined for device %s at init, defaulting to 'Unknown Mill Device'", device_id)
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self._device and self._attr_device_info:
            ...
            elif not model_name and self._attr_device_info.get("model") != "Unknown Mill Device":
