...
async def async_setup_entry(
    ...
    for device_id, device_data in coordinator.data.items():
        if not device_data:
            _LOGGER.warning("Device data missing for ID %s in climate setup.", device_id)
            continue

        device_type_name = DeviceMetric.get_device_type(device_data)
        if not device_type_name:
            _LOGGER.warning("Could not determine device type for climate entity: %s", device_id)
            continue
        ...
        try:
            ...
            _LOGGER.warning("Unsupported device type for climate entity: '%s' for device %s", device_type_name, device_id)
            continue 
        ...
        if has_target_temp and has_measure_temp and has_onoff:
            entities.append(MillClimate(coordinator, api, device_id, capabilities, device_type_enum))
        else:
            missing_caps = []
    ...
    if entities:
        ...
        _LOGGER.info("No climate entities were added. Check previous INFO logs for reasons.")
...
class MillClimate(MillEntity, ClimateEntity):
    ...
    def __init__(
        ...
        self._is_socket = self._device_type in [
            EDeviceType.SOCKET_GEN2, EDeviceType.SOCKET_GEN3, EDeviceType.SOCKET_GEN4
        ]
        ...
        if self._is_socket:
            ...
            if self._device_type == EDeviceType.SOCKET_GEN4 and EDeviceCapability.COOLING_MODE in self._capabilities:
    ...
    def _update_internal_state(self) -> None:
        ...
        current_temp_val = DeviceMetric.get_capability_value(self._device, EDeviceCapability.MEASURE_TEMPERATURE)
        try:
            self._attr_current_temperature = float(current_temp_val) if current_temp_val is not None else None
        except (ValueError, TypeError):
            self._attr_current_temperature = None

        server_target_temp_val = DeviceMetric.get_capability_value(self._device, EDeviceCapability.TARGET_TEMPERATURE)
        try:
            server_target_temperature = float(server_target_temp_val) if server_target_temp_val is not None else None
            if self._attr_target_temperature is None or self._attr_target_temperature != server_target_temperature:
        ...
        if power_on:
            if self._device_type == EDeviceType.SOCKET_GEN4 and EDeviceCapability.COOLING_MODE in self._capabilities:
                is_cooling_mode_active = bool(DeviceMetric.get_capability_value(self._device, EDeviceCapability.COOLING_MODE))
                ...
                is_cooling_capable = EDeviceCapability.COOLING_MODE in self._capabilities
        ...
        if self._attr_hvac_mode == HVACMode.HEAT:
            if power_on:
                if not self._is_socket and self._device.get("lastMetrics", {}).get("heaterFlag") == 1:
                    new_hvac_action = HVACAction.HEATING
                elif current_temp is not None and target_temp is not None and current_temp < target_temp:
                    new_hvac_action = HVACAction.HEATING if not self._is_socket else HVACAction.IDLE 
                    if self._is_socket: new_hvac_action = HVACAction.IDLE 
                                                                    
                    if self._is_socket: new_hvac_action = HVACAction.HEATING 
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        if EDeviceCapability.INDIVIDUAL_CONTROL in self._capabilities:
            individual_control_active = DeviceMetric.get_capability_value(self._device, EDeviceCapability.INDIVIDUAL_CONTROL)
            if not individual_control_active:
                ...
                try:
                    await self._api.set_switch_capability(self._device_id, EDeviceCapability.INDIVIDUAL_CONTROL.value, True, self._device)
                    _LOGGER.info("Individual control enabled for %s to set temperature.", self.entity_id)
                    self.hass.async_create_task(self._delayed_refresh(3)) # Schedule refresh
                except Exception as e:
                    _LOGGER.error("Failed to enable individual control for %s: %s", self.entity_id, e)
                    return 

        original_target_temp = self._attr_target_temperature
        self._attr_target_temperature = float(temp) 
        if self._attr_hvac_mode == HVACMode.OFF:
            if self._device_type == EDeviceType.SOCKET_GEN4 and EDeviceCapability.COOLING_MODE in self._capabilities and \
                bool(DeviceMetric.get_capability_value(self._device, EDeviceCapability.COOLING_MODE)):
        ...
        try:
            ...
            await self._api.set_number_capability(self._device_id, EDeviceCapability.TARGET_TEMPERATURE.value, temp, self._device)
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        if not self._device:
            _LOGGER.error("Cannot set HVAC mode for %s, device data not available.", self.entity_id)
            return
        ...
        try:
            if hvac_mode == HVACMode.OFF:
                ...
                await self._api.set_device_power(self._device_id, True, self._device)
                if self._device_type == EDeviceType.SOCKET_GEN4 and EDeviceCapability.COOLING_MODE in self._capabilities:
                    current_cooling_state = DeviceMetric.get_capability_value(self._device, EDeviceCapability.COOLING_MODE)
                    if current_cooling_state:
                        await self._api.set_switch_capability(self._device_id, EDeviceCapability.COOLING_MODE.value, False, self._device)
            elif hvac_mode == HVACMode.COOL: 
                if EDeviceCapability.COOLING_MODE in self._capabilities:
                    await self._api.set_device_power(self._device_id, True, self._device)
                    await self._api.set_switch_capability(self._device_id, EDeviceCapability.COOLING_MODE.value, True, self._device)
                else: 
                    _LOGGER.warning("Attempted to set HVACMode.COOL for %s which does not support it.", self.entity_id)
            ...
            _LOGGER.error("Error setting HVAC mode for %s to %s: %s", self._device_id, hvac_mode, e)
            self._attr_hvac_mode = original_hvac_mode 
            if self._attr_hvac_mode == HVACMode.OFF: self._attr_hvac_action = HVACAction.OFF
            self.async_write_ha_state()
