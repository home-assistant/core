"""Support for number capabilities."""
...
async def async_setup_entry(
    ...
    for device_id, device_data in coordinator.data.items():
        ...
        if not device_type_name:
            _LOGGER.warning(
                "Could not determine device type for number entity: %s", device_id
            )
            continue
        ...
        try:
            ...
            _LOGGER.warning(
                "Unsupported device type for number entity: %s for device %s",
                device_type_name,
                device_id,
            )
            continue
        ...
        for cap_enum, (
            ...
            if cap_enum in capabilities:
                if cap_enum == EDeviceCapability.TARGET_TEMPERATURE:
                    if (
                        EDeviceCapability.MEASURE_TEMPERATURE not in capabilities
                        or EDeviceCapability.ONOFF not in capabilities
                    ):
                        entities.append(
                            MillNumber(
                                coordinator,
...
class MillNumber(MillEntity, NumberEntity):
    ...
    def _update_internal_state(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Could not parse number value '%s' for %s", value, self.entity_id
            )
            new_native_value = None
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        if not self._device:
            _LOGGER.error(
                "Cannot set number value for %s, device data not available.",
                self.entity_id,
            )
            return
        try:
