...
from .device_capability import DEVICE_CAPABILITY_MAP, EDeviceCapability, EDeviceType, EPurifierFanMode 
...
SELECT_TYPES: dict[EDeviceCapability, tuple[str, list[str]]] = {
    EDeviceCapability.PURIFIER_MODE: ( 
        "Mode", list(PURIFIER_FAN_MODES.keys()) 
    ),
}
...
async def async_setup_entry(
    ...
    for device_id, device_data in coordinator.data.items():
        ...
        if not device_type_name:
            _LOGGER.warning("Could not determine device type for select entity: %s", device_id)
            continue
        
        try:
            device_type_enum = EDeviceType(device_type_name)
        except ValueError:
            _LOGGER.warning("Unsupported device type for select entity: %s for device %s", device_type_name, device_id)
            continue
...
class MillSelect(MillEntity, SelectEntity):
    ...
    def _update_internal_state(self) -> None:
        ...
        if str_value in self._attr_options:
            ...
            _LOGGER.warning("Device %s reported purifier mode '%s' which is not in defined options %s. Sensor may show unknown state.", 
                            self.entity_id, str_value, self._attr_options)
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        if not self._device:
            _LOGGER.error("Cannot select option for %s, device data not available.", self.entity_id)
            return

        _LOGGER.info("Attempting to set purifier mode to: %s for device %s", option, self._device_id)
        ...
        try:
            await self._api.set_select_capability(self._device_id, self._capability.value, option, self._device)
            self.hass.async_create_task(self._delayed_refresh(3))
        except Exception as e:
            _LOGGER.error("Error selecting option '%s' for %s on %s: %s", option, self._capability.value, self._device_id, e)
