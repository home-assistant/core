"""Support for sensor capabilities."""
...
SENSOR_TYPES: dict[
    EDeviceCapability,
    tuple[
        str, str | None, SensorDeviceClass | None, SensorStateClass | None, str | None
    ],
] = {
...
async def async_setup_entry(
    ...
    if not coordinator.data:
        _LOGGER.warning(
            "No data in coordinator during sensor setup, skipping sensor entity creation."
        )
        return
    ...
    for device_id, device_data in coordinator.data.items():
        ...
        if not device_type_name:
            _LOGGER.warning(
                "Could not determine device type for %s when setting up sensors",
                device_id,
            )
            continue
        ...
        try:
            ...
            _LOGGER.warning(
                "Unsupported device type for sensor platform: '%s' for device %s",
                device_type_name,
                device_id,
            )
            continue
...
class MillSensor(MillEntity, SensorEntity):
    ...
    def _update_internal_state(self) -> None:
        ...
        if value is not None:
            if self._capability == EDeviceCapability.MEASURE_FILTER_STATE:
            ...
            elif self._attr_state_class in [
                SensorStateClass.MEASUREMENT,
                SensorStateClass.TOTAL_INCREASING,
            ] or self._attr_device_class in [
                SensorDeviceClass.TEMPERATURE,
                SensorDeviceClass.HUMIDITY,
                SensorDeviceClass.POWER,
                SensorDeviceClass.ENERGY,
                ...
                try:
                    ...
                    _LOGGER.debug(
                        "Could not parse sensor value '%s' as float for %s (%s)",
                        value,
                        self.entity_id,
                        self._capability,
                    )
