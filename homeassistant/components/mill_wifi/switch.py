...
SWITCH_TYPES: dict[EDeviceCapability, tuple[str, SwitchDeviceClass | None, str | None]] = {
    ...
    EDeviceCapability.OPEN_WINDOW: ("Open Window Detection", None, "mdi:window-open-variant"),
    EDeviceCapability.PREDICTIVE_HEATING: ("Predictive Heating", None, "mdi:chart-timeline-variant"),
    ...
    EDeviceCapability.INDIVIDUAL_CONTROL: ("Individual Control", None, "mdi:tune-variant"),
...
async def async_setup_entry(
    ...
    for device_id, device_data in coordinator.data.items():
        ...
        if not device_type_name:
            _LOGGER.warning("Could not determine device type for switch entity: %s", device_id)
            continue
        
        try:
            device_type_enum = EDeviceType(device_type_name)
        except ValueError:
            _LOGGER.warning("Unsupported device type for switch platform: %s for device %s", device_type_name, device_id)
            continue

        capabilities = DEVICE_CAPABILITY_MAP.get(device_type_enum, set())

        for capability_enum, (name_suffix, device_class, icon) in SWITCH_TYPES.items(): # Unpack icon
            if capability_enum in capabilities:
                if capability_enum == EDeviceCapability.ONOFF and \
                    EDeviceCapability.TARGET_TEMPERATURE in capabilities and \
                    EDeviceCapability.MEASURE_TEMPERATURE in capabilities:
                        _LOGGER.debug("Skipping ONOFF switch for %s as it will be part of Climate entity.", device_id)
                        continue
...
class MillSwitch(MillEntity, SwitchEntity):
    ...
    def _update_internal_state(self) -> None:
        if self._device:
            new_state = DeviceMetric.get_capability_value(self._device, self._capability)
    ...
    async def _common_switch_action(self, turn_on: bool):
        ...
        if not self._device:
            _LOGGER.error(f"Cannot change state for {cap_name}, device data not available for {self.entity_id}.")
            return
        ...
        try:
            await self._api.set_switch_capability(self._device_id, self._capability.value, turn_on, self._device)
            self.hass.async_create_task(self._delayed_refresh(2)) 
        except Exception as e:
            _LOGGER.error(f"Error {'turning on' if turn_on else 'turning off'} {cap_name} for {self._device_id}: {e}")
