...
def ban_middleware(app, handler):
    ...
    if KEY_BANNED_IPS not in app:
        hass = app['hass']
        app[KEY_BANNED_IPS] = yield from hass.async_add_job(load_ip_bans_config, hass.config.path(IP_BANS_FILE))
...
def process_wrong_login(request):
    ...
    if (request.app[KEY_FAILED_LOGIN_ATTEMPTS][remote_addr] >
        ...
        yield from hass.async_add_job(update_ip_bans_config, hass.config.path(IP_BANS_FILE),
            new_ban)
