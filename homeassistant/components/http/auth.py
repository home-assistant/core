...
async def async_setup_auth(
    ...
    async def auth_middleware(
        ...
        if (
            ...
            and (
                resp := await _async_check_strict_connection(
                    hass, request, strict_connection_mode_non_cloud
                )
            )
        ):
            return resp
...
async def _async_check_strict_connection(
    hass: HomeAssistant,
    request: Request,
    strict_connection_mode_non_cloud: StrictConnectionMode,
) -> StreamResponse | None:
    """Check if strict connection mode is enabled and perform action."""
    guard_page = None
    if is_cloud_connection(hass):
        from homeassistant.components.cloud.util import (  # pylint: disable=import-outside-toplevel
            get_strict_connection_mode,
            read_strict_connection_guard_page as read_cloud_strict_connection_guard_page,
        )

        strict_connection_mode = get_strict_connection_mode(hass)
        strict_connection_func = _async_perform_strict_connection_action
        if strict_connection_mode is StrictConnectionMode.GUARD_PAGE:
            guard_page = read_cloud_strict_connection_guard_page
    else:
        strict_connection_mode = strict_connection_mode_non_cloud
        strict_connection_func = _async_perform_strict_connection_action_on_non_local
        if strict_connection_mode is StrictConnectionMode.GUARD_PAGE:
            guard_page = _read_strict_connection_guard_page
    ...
    if (
        ...
        and (resp := await strict_connection_func(hass, request, guard_page))
        is not None
    ):
        return resp
...
async def _async_perform_strict_connection_action_on_non_local(
    hass: HomeAssistant,
    request: Request,
    guard_page: Callable[[HomeAssistant], Awaitable[str]] | None,
) -> StreamResponse | None:
    ...
    - If guard_page is not None, return a response with the content of the callable
...
@singleton.singleton(f"{DOMAIN}_{_STRICT_CONNECTION_GUARD_PAGE_NAME}")
