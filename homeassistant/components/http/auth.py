...
def auth_middleware(app, handler):
    ...
    def auth_middleware_handler(request):
        ...
        if (HTTP_HEADER_HA_AUTH in request.headers and validate_password(
                request, request.headers[HTTP_HEADER_HA_AUTH])):
            # A valid auth header has been set
            authenticated = True

        elif (DATA_API_PASSWORD in request.query and validate_password(
                request, request.query[DATA_API_PASSWORD])):
            authenticated = True

        elif (hdrs.AUTHORIZATION in request.headers
              and validate_authorization_header(request)):
            authenticated = True
...
def is_trusted_ip(request):
    ...
    return ip_addr and any(
        ...
        for trusted_network in request.app[KEY_TRUSTED_NETWORKS])
