...
async def async_setup_entry(
    ...
    for description in FORECAST_SENSORS:
        if dict_nested_value(coordinator.data, description.keys) is not None:
            entities.append(
                AemetSensor(
                    name,
                    coordinator,
...
class AemetSensor(CoordinatorEntity[WeatherUpdateCoordinator], SensorEntity):
    ...
    def native_value(self):
        ...
        value = dict_nested_value(
            self.coordinator.data,
            self.entity_description.keys,
        )
        if self.entity_description.keys[-1] == AOD_CONDITION:
            value = CONDITIONS_MAP.get(value)
        elif self.entity_description.keys[-1] == AOD_TIMESTAMP:
