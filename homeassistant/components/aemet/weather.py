...
async def async_setup_entry(
    ...
    if entity_registry.async_get_entity_id(
        WEATHER_DOMAIN,
        DOMAIN,
        f"{config_entry.unique_id} {FORECAST_MODES[AOD_FORECAST_HOURLY]}",
    ):
        for mode, mode_id in FORECAST_MODES.items():
...
class AemetWeather(SingleCoordinatorWeatherEntity[WeatherUpdateCoordinator]):
    ...
    def get_aemet_value(self, keys: list[str]) -> Any:
        """Get aemet coordinator values."""
        return dict_nested_value(self.coordinator.data, keys)
