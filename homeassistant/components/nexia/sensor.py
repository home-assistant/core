...
from . import (ATTR_MODEL, ATTR_FIRMWARE, ATTR_THERMOSTAT_NAME, ATTR_THERMOSTAT_ID, ATTR_ZONE_ID,
               ATTRIBUTION,
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for thermostat_id in thermostat.get_thermostat_ids():
        ...
        if thermostat.has_outdoor_temperature(thermostat_id):
            ...
            sensors.append(
                NexiaSensor(thermostat, scan_interval, thermostat_id, "get_outdoor_temperature",
                            "Outdoor Temperature",
                            DEVICE_CLASS_TEMPERATURE, unit))

        if thermostat.has_relative_humidity(thermostat_id):
            sensors.append(
                NexiaSensor(thermostat, scan_interval, thermostat_id, "get_relative_humidity",
        ...
        for zone in thermostat.get_zone_ids(thermostat_id):
            ...
            unit = (TEMP_CELSIUS if thermostat.get_unit(
                thermostat_id) == thermostat.UNIT_CELSIUS else TEMP_FAHRENHEIT)
            sensors.append(NexiaZoneSensor(thermostat, scan_interval, thermostat_id, zone,
                                           "get_zone_temperature",
                                           f"{name} Temperature", DEVICE_CLASS_TEMPERATURE, unit,
                                           None))
            sensors.append(
                NexiaZoneSensor(thermostat, scan_interval, thermostat_id, zone, "get_zone_status",
...
class NexiaZoneSensor(NexiaSensor):
    ...
    def __init__(self, device, scan_interval, thermostat_id, zone, sensor_call, sensor_name,
                 sensor_class, sensor_unit,
                 modifier=None):
        super().__init__(device, scan_interval, thermostat_id, sensor_call, sensor_name,
                         sensor_class, sensor_unit,
                         modifier)
        self._zone = zone
