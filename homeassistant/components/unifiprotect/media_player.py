...
class ProtectMediaPlayer(ProtectDeviceEntity, MediaPlayerEntity):
    ...
    def _async_updated_event(self, device: ProtectModelWithId) -> None:
        ...
        if (
            ...
            _LOGGER.debug("Updating state [%s (%s)] %s (%s, %s) -> %s (%s, %s)", device.name, device.mac,
                previous_state, previous_available, previous_volume_level,
                self._attr_state, self._attr_available, self._attr_volume_level
            )
