...
class ProtectEventBinarySensor(EventEntityMixin, BinarySensorEntity):
    ...
    def _async_updated_event(self, device: ProtectModelWithId) -> None:
        ...
        if (
            ...
            _LOGGER.debug("Updating state [%s (%s)] %s (%s, %s) -> %s (%s, %s)",
                device.name,
                device.mac,
                previous_is_on, previous_available, previous_extra_state_attributes,
                self._attr_is_on, self._attr_available, self._attr_extra_state_attributes)
