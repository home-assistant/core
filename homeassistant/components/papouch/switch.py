...
class PapouchSwitch(PapouchEntity, SwitchEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{entry.entry_id}_{DATA_SWITCH}_{self.item_id}"
        self._attr_name = switch_data["name"]

    @property
    def is_on(self) -> bool | None:
        """Return True if the switch is on."""
        val = self.coordinator.data.get(DATA_SWITCH, {}).get(self.item_id)
