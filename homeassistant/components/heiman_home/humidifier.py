"""Humidifier entities for Heiman Home Integration."""
...
async def async_setup_entry(
    ...
    """Set up Heiman Home humidifiers."""
    entry_id = config_entry.entry_id
    devices_dict = hass.data[DOMAIN]["devices"].get(entry_id, {})

    _LOGGER.info("Setting up Heiman Home humidifiers for entry: %s", entry_id)
    ...
    for device in devices:
        ...
        humidifier_entities = heiman_device.get_humidifier_entities(
            devices_config=devices_config,
        )
        entities.extend(humidifier_entities)

    if entities:
        async_add_entities(entities)


class HeimanHumidifierEntity(CoordinatorEntity, HumidifierEntity):
    """Representation of a Heiman humidifier."""
    ...
    def __init__(
        ...
        if sw_version:
            ...
            _LOGGER.debug(
                "Added firmware version %s to humidifier device info for %s",
                sw_version,
                device_id,
            )
