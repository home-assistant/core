"""Media Player and Notify entities for Heiman Home."""
...
async def async_setup_entry(
    ...
    """Set up Heiman Home media players and notify entities."""
    ...
    for device in devices:
        ...
        # Add media player and notify entities
        entities.extend(
            heiman_device.get_media_player_entities(devices_config=devices_config),
        )
        entities.extend(
            heiman_device.get_notify_entities(devices_config=devices_config),
        )
...
class HeimanMediaPlayerEntity(CoordinatorEntity, MediaPlayerEntity):
    """Representation of a Heiman media player."""
    ...
    def __init__(
        ...
        if sw_version:
            ...
            _LOGGER.debug(
                "Added firmware version %s to media player device info for %s",
                sw_version,
                device_id,
            )
...
class HeimanNotifyEntity(CoordinatorEntity, NotifyEntity):
    """Representation of a Heiman notify entity."""
    ...
    def __init__(
        ...
        if sw_version:
            device_info_dict["sw_version"] = sw_version
            _LOGGER.debug(
                "Added firmware version %s to notify device info for %s",
                sw_version,
                device_id,
            )
        if device_config.get("area_id"):
