...
class QubeHub:
    ...
    async def async_read_value(self, ent: EntityDef) -> Any:
        ...
        if ent.platform == "binary_sensor":
            return await self._read_binary_sensor(ent)
        if ent.platform == "switch":
            return await self._read_switch(ent)
        # sensor
        return await self._read_sensor(ent)

    async def _read_binary_sensor(self, ent: EntityDef) -> bool | None:
        ...
        if input_type in (
            ...
            rr = await self._call(
                "read_holding_registers", address=ent.address, count=1
            )
    ...
    async def _read_switch(self, ent: EntityDef) -> bool | None:
        ...
        if write_type in (
            ...
            try:
                if data_type == "float32" and len(regs) >= 2:
                    raw = struct.pack(
                        ">HH", int(regs[0]) & 0xFFFF, int(regs[1]) & 0xFFFF
                    )
        ...
        try:
            ...
            if write_type in (
                ...
                await self.async_write_register(
                    ent.address, value, ent.data_type or "uint16"
                )
                return
        except ModbusException:
            fallback_addr = ent.address - 1
            if fallback_addr >= 0:
                logging.getLogger(__name__).info(
                    "Modbus write failed @ %s, retrying @ %s (fallback)",
                    ent.address,
                    fallback_addr,
                )
                if write_type in ("coil", "coils"):
                ...
                if write_type in (
                    ...
                    await self.async_write_register(
                        fallback_addr, value, ent.data_type or "uint16"
                    )
                    return
            raise
