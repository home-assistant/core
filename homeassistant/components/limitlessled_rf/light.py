...
from homeassistant.util.color import color_hs_to_RGB, color_RGB_to_hs, color_temperature_mired_to_kelvin, color_temperature_kelvin_to_mired
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
	voluptuous.Optional(CONF_RADIO_SECTION): voluptuous.All({
		voluptuous.Optional(CONF_RADIO_GPIO_PIN): config_validation.positive_int,
	...
	voluptuous.Optional(CONF_REMOTES_SECTION): voluptuous.All(config_validation.ensure_list, [
		{
			voluptuous.Optional(CONF_REMOTE_START): config_validation.positive_int,
			voluptuous.Optional(CONF_REMOTE_TYPE): config_validation.string,
			voluptuous.Optional(CONF_REMOTE_COUNT): config_validation.positive_int,
			voluptuous.Optional(CONF_REMOTE_ZONES): config_validation.ensure_list,
			voluptuous.Optional(CONF_REMOTE_RETRIES): config_validation.positive_int,
			voluptuous.Optional(CONF_REMOTE_FORMAT): config_validation.match_all,
			voluptuous.Optional(CONF_ZONE_FORMAT): config_validation.match_all
		}
...
def _init_radio_lt8900(radio_config):
	...
	radio = lt8900_spi.Radio(spi_bus, spi_dev, config = {
		'reset_command': reset_module_via_gpio,
		'reset_command_gpio': reset_gpio,
		'use_software_tx_queue': True,
		'__DISABLED__debug_log_command': (lambda message: _debug_log("LT8900", message)),
...
def setup_platform(hass, config, add_entities, discovery_info = None):
	...
	for remote_config in config.get(CONF_REMOTES_SECTION, []):
		...
		for remote_id_index in range(remote_id_count):
			...
			remote_global_name = _format_entity_name(remote_id, 0, remote_format, zone_format)

			# Compute configuration for this remote
			remote_config_parameters = {
				'__DISABLED__debug_log_command': (lambda message: _debug_log("REMOTE " + remote_global_name, message)),
				'radio_queue': remote_global_name
			}
			if zone_retries != 0:
				remote_config_parameters['retries'] = zone_retries

			remote = limitlessled_rf.Remote(radio, remote_type, remote_id, config = remote_config_parameters)
			...
			for zone_id in remote_zone_ids:
				...
				remote_zone_name = _format_entity_name(remote_id, zone_id, remote_format, zone_format)
			...
			if 0 in remote_zone_ids:
				remotes_zones.append(LimitlessLED_RF_HASS(remote_global_name, remote, None, remote_zones))

	# Register an event handler to pair or unpair a bulb
	hass.bus.listen("limitlessled_rf_pair", (lambda event: _pair_bulb(remotes_zones, event)))
	hass.bus.listen("limitlessled_rf_unpair", (lambda event: _unpair_bulb(remotes_zones, event)))
...
class LimitlessLED_RF_HASS(Light):
	...
	def propagate_copy(self, other, attrs_to_copy):
		self._debug_log("Copying state ({}) from parent (parent is {})".format(attrs_to_copy, other.name))
	...
	def min_mireds(self):
		...
		warmest_color_temp_mired = color_temperature_kelvin_to_mired(warmest_color_temp_kelvins)
		coolest_color_temp_mired = color_temperature_kelvin_to_mired(coolest_color_temp_kelvins)
	...
	def turn_on(self, **kwargs):
		...
		if attr_hs_color is not None:
			...
			self._color = _rgb_list_to_int(color_hs_to_RGB(attr_hs_color[0], attr_hs_color[1]))
		...
		if 'color' in attrs_to_copy:
			if self._color == 0xffffff:
				self._color = _rgb_list_to_int([self._brightness, self._brightness, self._brightness])
		...
		if attr_color_temp is not None:
			...
			self._debug_log("Computed input temperature of {} mired to {} kelvin".format(attr_color_temp, self._temperature))
