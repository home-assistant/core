...
def _er_request(data=None):
    ...
    if cmd == WATER_ON:
        ...
        url = '{0}/result.cgi?xi=0:{1}:{2}:{3}:{4}:{5}:{6}:{7}:{8}'.format(ER['server_origin'], valves[1], valves[2], valves[3], valves[4], valves[5], valves[6], valves[7], valves[8] )
...
#
def get_state(valve):
    """Get a state from the EtherRain API service."""
    data={}
    data["valve"] = valve
    data["duration"] = 0
    data["command"] = STATE
    ...
    # ri contains the last valve to run.  os is the current state of that valve. (ready or busy)
    # (XXX: The valve number returned is 0-7 but the watering command takes 1-8)
    if 'os' in status and status['os'] == 'WT':
        # _LOGGER.info("valve is {0} and is waiting".format(valve, int(status['ri'])))
        return 1
    if 'ri' in status and int(status['ri']) == valve-1:
        if 'os' in status and status['os'] == 'RD':
            # _LOGGER.info("last valve is {0} and is ready".format(valve, int(status['ri'])))
            return 0
        if 'os' in status and status['os'] == 'BZ':
            # _LOGGER.info("valve is {0} and is busy".format(valve, int(status['ri'])))
            return 1
    else:
        return 0
