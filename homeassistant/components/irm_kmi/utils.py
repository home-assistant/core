...
def get_config_value_data(
    config_entry: ConfigEntry, key: str, default: Any = None
) -> Any:
...
def get_config_value_options(
    config_entry: ConfigEntry, key: str, default: Any = None
) -> Any:
    """Get the value of key in the options."""
...
def preferred_language(hass: HomeAssistant, config_entry: ConfigEntry | None) -> str:
    ...
    if (
        ...
        or get_config_value_options(config_entry, CONF_LANGUAGE_OVERRIDE) == "none"
    ):
        return hass.config.language if hass.config.language in LANGS else "en"

    return get_config_value_options(config_entry, CONF_LANGUAGE_OVERRIDE, "en")
