...
class NeatoConnectedVacuum(NeatoEntity, StateVacuumEntity):
    ...
    def update(self) -> None:
        ...
        self._handle_state(self._state)
        ...
        self._handle_mapdata(self._mapdata[self._robot_serial]["maps"][0])
    ...
    def _handle_state(self, state: dict[str, Any]) -> None:
        ...
        robot_alert = ALERTS.get(state["alert"]) if "alert" in state else None
        if robot_alert is not None:
            self._status_state = robot_alert

        if state["state"] == 2 and robot_alert is None:
            self._attr_state = STATE_CLEANING
            self._status_state = (
            ...
            if (
                ...
                self._status_state += f" {state['cleaning']['boundary']['name']}"
        ...
        if state["state"] == 4:
            ...
            self._status_state = ERRORS.get(state["error"])

    def _handle_mapdata(self, mapdata: dict[str, Any]) -> None:
