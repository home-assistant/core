...
def setup(hass, config):
    """Setup hot water heater controller."""
    ...
    def away_mode_set_service(service):
        """Set away mode on target hot water heater devices."""
        ...
        for geyser in target_geyser:
            if away_mode:
                geyser.turn_away_mode_on()
            else:
                geyser.turn_away_mode_off()
    ...
    def guest_mode_set_service(service):
        """Set guest mode on target hot water heater devices."""
        target_geyser = component.extract_from_service(service)
    ...
    def holiday_mode_set_service(service):
        """Set holiday mode on target hot water heater devices."""
        target_geyser = component.extract_from_service(service)
    ...
    def operation_set_service(service):
        """Set operating mode on the target hot water heater devices."""
        target_geyser = component.extract_from_service(service)
        ...
        for geyser in target_geyser:
            geyser.set_operation_mode(operation_mode)
...
class GeyserDevice(Entity):
    """Representation of a hot water heater device."""

    # pylint: disable=too-many-public-methods,no-self-use
    @property
    def geyser_class(self):
        """Return the class of this geyser, from GEYSER_CLASSES."""
        return None
    ...
    def _convert_for_display(self, temp):
        ...
        if self.hass.config.units.temperature_unit is TEMP_CELSIUS:
            # Geyser controllers unlikely to supply decimals
            decimal_count = 0
        else:
