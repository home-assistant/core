...
from homeassistant.const import CONF_LIGHTS, CONF_NAME, CONF_BRIGHTNESS, CONF_SLAVE, CONF_COLOR_TEMP
from homeassistant.core import HomeAssistant
from homeassistant.helpers.entity_platform import AddEntitiesCallback
from homeassistant.helpers.typing import ConfigType, DiscoveryInfoType

from . import get_hub
from .const import CONF_COLOR_TEMP_KELVIN, DEFAULT_MIN_KELVIN, DEFAULT_MAX_KELVIN, MAX_BRIGHTNESS, MODBUS_MAX, \
    MODBUS_MIN, CONF_MIN_TEMP, CONF_MAX_TEMP, DEFAULT_BRIGHTNESS, CALL_TYPE_WRITE_REGISTER, CALL_TYPE_REGISTER_HOLDING
...
class ModbusLight(BaseSwitch, LightEntity):
    ...
    def __init__(self, hass: HomeAssistant, hub: ModbusHub, config: dict[str, Any]) -> None:
        ...
        if self._attr_color_mode == ColorMode.COLOR_TEMP:
            self._attr_min_color_temp_kelvin = config.get(CONF_MIN_TEMP, DEFAULT_MIN_KELVIN)
            self._attr_max_color_temp_kelvin = config.get(CONF_MAX_TEMP, DEFAULT_MAX_KELVIN)
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if CONF_BRIGHTNESS in kwargs:
            await self.async_set_brightness(kwargs.get(CONF_BRIGHTNESS, self._brightness))

        if CONF_COLOR_TEMP_KELVIN in kwargs:
            await self.async_set_color_temp(kwargs.get(CONF_COLOR_TEMP_KELVIN, self._color_temp_kelvin))
    ...
    async def async_set_brightness(self, brightness: int) -> None:
        ...
        if address:
            conv_brightness = await self._convert_brightness_to_modbus(brightness=brightness)
            await self._hub.async_pb_call(unit=slave, address=address,
                                          value=conv_brightness, use_call=CALL_TYPE_WRITE_REGISTER)
    ...
    async def async_set_color_temp(self, color_temp_kelvin: int) -> None:
        ...
        slave = int(self.config.get(CONF_SLAVE))
        if address:
            conv_color_temp_kelvin = await self._convert_color_temp_to_modbus(kelvin=color_temp_kelvin)
            await self._hub.async_pb_call(unit=slave, address=address,
                                          value=conv_color_temp_kelvin, use_call=CALL_TYPE_WRITE_REGISTER)
    ...
    async def _async_update(self) -> None:
        ...
        if brightness_address:
            brightness_result = await self._hub.async_pb_call(unit=slave, value=1, address=brightness_address,
                                                              use_call=CALL_TYPE_REGISTER_HOLDING)
            if brightness_result.registers:
                self._brightness = await self.percent_to_brightness(brightness_result.registers[0])

        color_address = int(self.config.get(CONF_COLOR_TEMP))
        if color_address:
            color_result = await self._hub.async_pb_call(unit=slave, value=1, address=color_address,
                                                         use_call=CALL_TYPE_REGISTER_HOLDING)
            if color_result.registers:
                self._color_temp_kelvin = await self.percent_to_temperature(color_result.registers[0])
    ...
    async def percent_to_temperature(self, percent: int) -> int:
        ...
        return round(self._attr_min_color_temp_kelvin + (
                percent / MODBUS_MAX * (self._attr_max_color_temp_kelvin - self._attr_min_color_temp_kelvin)))
    ...
    async def _convert_color_temp_to_modbus(kelvin: int) -> int:
        ...
        return round(MODBUS_MIN + (kelvin - DEFAULT_MIN_KELVIN) * (MODBUS_MAX - MODBUS_MIN) /
