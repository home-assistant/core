...
from homeassistant.const import ATTR_TEMPERATURE, CONF_NAME, CONF_SLAVE, TEMP_CELSIUS, TEMP_FAHRENHEIT
...
class ModbusThermostat(ClimateDevice):
    ...
    def set_temperature(self, **kwargs):
        ...
        target_temperature = int((kwargs.get(ATTR_TEMPERATURE)  - self._offset) / self._scale)
    ...
    def read_register(self, register):
        ...
        try:
            result = self._hub.read_holding_registers(self._slave, register, self._count)
        except AttributeError as ex:
            _LOGGER.error(ex)
        byte_string = b"".join([x.to_bytes(2, byteorder="big") for x in result.registers])
        val = struct.unpack(self._structure, byte_string)[0]
        register_value = format((self._scale * val) + self._offset, f".{self._precision}f")
