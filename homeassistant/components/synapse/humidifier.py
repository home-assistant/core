...
from .const import DOMAIN, SynapseHumidifierDefinition
...
async def async_setup_entry(
    ...
    entities = bridge.config_data.get("humidifier")
    if entities is not None:
      async_add_entities(SynapseHumidifier(hass, bridge, entity) for entity in entities)


class SynapseHumidifier(SynapseBaseEntity, HumidifierEntity):
    def __init__(
        self,
        hass: HomeAssistant,
        hub: SynapseBridge,
