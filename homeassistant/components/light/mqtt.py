...
from homeassistant.const import (
    ...
    CONF_VALUE_TEMPLATE, CONF_WHITE_VALUE, CONF_XY)
...
class MqttLight(Light):
    ...
    def __init__(self, name, effect_list, topic, templates, qos, retain,
                 payload, optimistic, brightness_scale, white_value_scale):
        ...
        self._optimistic_effect = (optimistic
                                   or topic[CONF_EFFECT_STATE_TOPIC] is None)
        ...
        self._supported_features |= (topic[CONF_RGB_COMMAND_TOPIC] is not None
                                     and SUPPORT_RGB_COLOR)
        self._supported_features |= (
            topic[CONF_BRIGHTNESS_COMMAND_TOPIC] is not None
            and SUPPORT_BRIGHTNESS)
        self._supported_features |= (
            topic[CONF_COLOR_TEMP_COMMAND_TOPIC] is not None
            and SUPPORT_COLOR_TEMP)
        self._supported_features |= (topic[CONF_EFFECT_STATE_TOPIC] is not None
                                     and SUPPORT_EFFECT)
        self._supported_features |= (
            topic[CONF_WHITE_VALUE_COMMAND_TOPIC] is not None
            and SUPPORT_WHITE_VALUE)
        self._supported_features |= (topic[CONF_XY_COMMAND_TOPIC] is not None
                                     and SUPPORT_XY_COLOR)
    ...
    def async_added_to_hass(self):
        ...
        def rgb_received(topic, payload, qos):
            ...
            self._rgb = [
                int(val) for val in templates[CONF_RGB](payload).split(',')
            ]
            self.async_schedule_update_ha_state()

        if self._topic[CONF_RGB_STATE_TOPIC] is not None:
            yield from mqtt.async_subscribe(self.hass,
                                            self._topic[CONF_RGB_STATE_TOPIC],
        ...
        def xy_received(topic, payload, qos):
            ...
            self._xy = [
                float(val) for val in templates[CONF_XY](payload).split(',')
            ]
            self.async_schedule_update_ha_state()

        if self._topic[CONF_XY_STATE_TOPIC] is not None:
            yield from mqtt.async_subscribe(self.hass,
                                            self._topic[CONF_XY_STATE_TOPIC],
                                            xy_received, self._qos)
            self._xy = [1, 1]
    ...
    def async_turn_on(self, **kwargs):
        ...
        if ATTR_XY_COLOR in kwargs and \
           self._topic[CONF_XY_COMMAND_TOPIC] is not None:
            ...
            mqtt.async_publish(
                self.hass, self._topic[CONF_XY_COMMAND_TOPIC], '{},{}'.format(
                    *kwargs[ATTR_XY_COLOR]), self._qos, self._retain)
