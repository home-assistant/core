...
class LightEntity(ToggleEntity, cached_properties=CACHED_PROPERTIES_WITH_ATTR_):
    ...
    def _light_internal_convert_color(
        ...
        if color_mode is ColorMode.HS and (hs_color := self.hs_color):
            data[ATTR_HS_COLOR] = (round(hs_color[0], 3), round(hs_color[1], 3))
            data[ATTR_RGB_COLOR] = color_util.color_hs_to_RGB(*hs_color)
            data[ATTR_XY_COLOR] = color_util.color_hs_to_xy(*hs_color)
        elif color_mode is ColorMode.XY and (xy_color := self.xy_color):
        ...
        elif color_mode is ColorMode.RGB and (rgb_color := self.rgb_color):
        ...
        elif color_mode is ColorMode.RGBW and (
            ...
            data[ATTR_XY_COLOR] = color_util.color_RGB_to_xy(*rgb_color)
        elif color_mode is ColorMode.RGBWW and (rgbww_color := self.rgbww_color):
            rgb_color = color_util.color_rgbww_to_rgb(
            ...
            data[ATTR_XY_COLOR] = color_util.color_RGB_to_xy(*rgb_color)
        elif color_mode is ColorMode.COLOR_TEMP and (
    ...
    def __validate_color_mode(
        ...
        if color_mode is None or color_mode is ColorMode.UNKNOWN:
