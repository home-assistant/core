...
from .const import CONF_STATE_ADDRESS, DATA_KNX_CONFIG, DOMAIN, KNX_ADDRESS
...
class KNXText(KnxEntity, TextEntity, RestoreEntity):
    ...
    def __init__(self, xknx: XKNX, config: ConfigType) -> None:
        ...
        self._attr_pattern = (
            r"[\u0000-\u00ff]*"  # Latin-1
            if issubclass(self._device._message.dpt_class, DPTLatin1)
            else r"[\u0000-\u007f]*"  # ASCII
        )
        self._attr_entity_category = config.get(CONF_ENTITY_CATEGORY)
        self._attr_unique_id = str(self._device._message.group_address)

    async def async_added_to_hass(self) -> None:
        """Restore last state."""
        # pylint: disable=protected-access
        await super().async_added_to_hass()
        if not self._device._message.readable and (
            last_state := await self.async_get_last_state()
        ):
            if last_state.state not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
                self._device._message.value = last_state.state
