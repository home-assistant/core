...
DATA: HassKey[EntityComponent[BaseAutomationEntity]] = HassKey(DOMAIN)
...
def _automations_with_x(
    ...
    return [
        ...
        for automation_entity in hass.data[DATA].entities
...
def _x_in_automation(
    ...
    if (automation_entity := hass.data[DATA].get_entity(entity_id)) is None:
        return []
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    hass.data[DATA] = component = EntityComponent[BaseAutomationEntity](
        LOGGER, DOMAIN, hass
    )
...
def websocket_config(
    ...
    automation = hass.data[DATA].get_entity(msg["entity_id"])
