...
class VoiceCommandSegmenter:
    ...
    def process(self, chunk_seconds: float, speech_probability: float | None) -> bool:
        ...
        if not self.in_command:
            # Before command
            self._handle_before_command(speech_probability, chunk_seconds)

        # In command
        elif self._handle_in_command(speech_probability, chunk_seconds) is False:
            return False
        return True

    def _reset_silence_counter(self, chunk_seconds: float) -> None:
    ...
    def _handle_before_command(
        self, speech_probability: float, chunk_seconds: float
    ) -> None:
    ...
    def _handle_in_command(
        ...
        if not is_speech:
            ...
            self._reset_silence_counter(chunk_seconds)
        return None
