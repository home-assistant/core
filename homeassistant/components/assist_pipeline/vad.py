...
class VoiceCommandSegmenter:
    ...
    def process_in_command(self, chunk_seconds: float, is_speech: bool | None) -> bool | None:
        if not is_speech:
            # Silence in command
            self._reset_seconds_left = self.reset_seconds
    ...
    def process_not_in_command(self, chunk_seconds: float, is_speech: bool | None) -> None:
        if is_speech:
            self._reset_seconds_left = self.reset_seconds
    ...
    def process(self, chunk_seconds: float, is_speech: bool | None) -> bool:
        ...
        if not self.in_command:
            self.process_not_in_command(chunk_seconds, is_speech)
        else:
            done = self.process_in_command(chunk_seconds, is_speech)
            if done == False:
                return False
