...
class VoiceCommandSegmenter:
    ...
    def process(self, chunk_seconds: float, speech_probability: float | None) -> bool:
        ...
        if not self.in_command:
            return self._handle_before_command(chunk_seconds, speech_probability)

        return self._handle_in_command(chunk_seconds, speech_probability)

    # Helper methods

    def _handle_before_command(self, chunk_seconds: float, speech_probability: float) -> bool:
        """Handle voice activity before command starts."""
    ...
    def _handle_in_command(self, chunk_seconds: float, speech_probability: float) -> bool:
        """Handle voice activity while in command."""
        is_speech = speech_probability > self.in_command_speech_threshold

        if not is_speech:
            return self._handle_silence_in_command(chunk_seconds)

        return self._handle_speech_in_command(chunk_seconds)

    def _start_command(self) -> None:
        """Transition into active voice command."""
    ...
    def _handle_silence_in_command(self, chunk_seconds: float) -> bool:
        """Handle silence during an active voice command."""
        self._reset_seconds_left = self.reset_seconds
    ...
    def _handle_speech_in_command(self, chunk_seconds: float) -> bool:
        """Handle speech continuation during an active voice command."""
        self._reset_seconds_left -= chunk_seconds
        self._command_seconds_left -= chunk_seconds
