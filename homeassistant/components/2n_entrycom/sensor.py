            ...
            try:
                ...
                with async_timeout.timeout(10, loop=self._hass.loop):
                    response = await websession.get(
                        self._subscribe_url,
                        auth=aiohttp.BasicAuth(self._uname,
                                               self._passw),
                        verify_ssl=False)
                ...
                _LOGGER.info(
                    "exception when setting up subscription: {}".format(sys.exc_info()[0]))
                    ...
                    with async_timeout.timeout(310, loop=self._hass.loop):
                        ...
                        if(resp.status == 200):
                            ...
                            if (
                                ...
                                _LOGGER.info(
                                    'Set event on 2n: {}'.format(self._state))
                            else:
                                self._state = False
                        ...
                        with async_timeout.timeout(10, loop=self._hass.loop):
                            resp = await websession.get(
                                ...
                                auth=aiohttp.BasicAuth(
                                    self._uname, self._passw),
                                verify_ssl=False)
                    except Exception:
                        _LOGGER.info(
                            'exception while trying to end subscription')
                        _LOGGER.info("exception: {}".format(sys.exc_info()[0]))
                    break
