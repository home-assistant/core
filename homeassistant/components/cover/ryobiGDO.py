...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for device_id, device_config in devices.items():
        apiKey = retrieve_apiKey(device_config.get(CONF_USERNAME),device_config.get(CONF_PASSWORD),device_config.get(CONF_DEVICE, device_id))
        device_id_valid = check_device_id(device_config.get(CONF_USERNAME),device_config.get(CONF_PASSWORD),device_config.get(CONF_DEVICE, device_id))
        if apiKey != None and device_id_valid:
            ...
            covers.append(RyobiCover(hass, args, supported_features=(SUPPORT_OPEN | SUPPORT_CLOSE)))
...
def retrieve_apiKey(username, password, device_id):
    ...
    if resp.status_code == 200:
        ...
        _LOGGER.error("auth KO. No apiKey retrieved. cover " + device_id +" will not be add")
        return None
...
def check_device_id (username, password, device_id):
    ...
    if device_found == True:
        ...
        _LOGGER.error("Device_id " + device_id +" is not among your devices. It will not be add")
        return False
...
class RyobiCover(CoverDevice):
    ...
    def get_ws(self):
        ...
        if self._ws is None:
            try:
                ...
                authMssg=json.dumps({'jsonrpc':'2.0','id':3,'method':'srvWebSocketAuth','params': {'varName':self._username,'apiKey':self._api_key}})
    ...
    def send_message(self, command, value):
        ...
        try:
            ...
            payLoad = json.dumps({'jsonrpc':'2.0','method':'gdoModuleCommand','params': {'msgType':16,'moduleType':5,'portId':7,'moduleMsg':{command:value},'topic':self._device_id}})
    ...
    def update(self):
        ...
        doorState = GDOstatus['result'][0]['deviceTypeMap']['garageDoor_7']['at']['doorState']['value']
        lightState = GDOstatus['result'][0]['deviceTypeMap']['garageLight_7']['at']['lightState']['value']
        backupBatteryLevel = GDOstatus['result'][0]['deviceTypeMap']['backupCharger_8']['at']['chargeLevel']['value']
        _LOGGER.info("Cover " + self._device_id + " status: doorState: " + str(doorState) + ", LightState: "+ str(lightState) + ", BackupBatteryLevel: "+ str(backupBatteryLevel))
        if doorState == 1:
            self._door_state = STATE_OPEN
            self.schedule_update_ha_state()
        if doorState == 0:
