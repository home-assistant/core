...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for surepy_entity in spc.states.values():
        ...
        if surepy_entity.type in [
            ...
            entities.append(SureLock(surepy_entity.id, spc))
...
class SureLock(SensorEntity):
    ...
    def __init__(self, _id: int, spc: SurePetcareAPI) -> None:
        ...
        self._attr_name = f"{surepy_entity.type.name.capitalize()} {surepy_entity.name.capitalize()} Lock"
        self._attr_unique_id = f"{surepy_entity.household_id}-{surepy_entity.id}-lock"

    @callback
    def _async_update(self) -> None:
        """Get the latest data and update the state."""
        surepy_entity = self._spc.states[self._id]
        state = surepy_entity.state
