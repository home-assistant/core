...
def _get_store(hass: HomeAssistant, unique_id: str) -> Store:
    return Store(hass, 1, "hassmpris" + os.path.sep + unique_id, True)
...
async def load_cert_data(
    ...
    it is absent.
    """
    data = await _get_store(hass, unique_id).async_load()
    if data is None:
...
async def save_cert_data(
    ...
    data = {
        CONF_CLIENT_CERT: client_cert.public_bytes(serialization.Encoding.PEM).decode(
            "ascii"
        ),
    ...
    await _get_store(hass, unique_id).async_save(data)
