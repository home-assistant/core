...
from .application_credentials import HisenseApplicationCredentials
from .diagnostics import async_get_config_entry_diagnostics, async_get_device_diagnostics
from .reauth import HisenseReauthFlowHandler, HisenseReauthFlowManager
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    # Register OAuth2 implementation for legacy support
    ...
    app_credentials = HisenseApplicationCredentials(hass)
    await app_credentials.async_get_auth_implementation(hass, DOMAIN, {})
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Check if using Application Credentials or legacy OAuth2
    if "auth_implementation" in entry.data:
        _LOGGER.debug("Using Application Credentials")
        # Using Application Credentials - create API client with config entry
        api_client = HisenseApiClient(hass, config_entry=entry)
    else:
        _LOGGER.debug("Using legacy OAuth2")
        # Legacy OAuth2 support
        ...
        api_client = HisenseApiClient(hass, oauth_session=oauth_session)
