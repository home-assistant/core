...
from .auth import HisenseAuthProvider
...
class HisenseApiClient:
    ...
    def __init__(
        ...
        self.auth_provider = HisenseAuthProvider(hass) if config_entry else None
    ...
    async def _api_request(
        ...
        try:
            # Get access token using appropriate method
            if self.auth_provider and self.config_entry:
                access_token = await self.auth_provider.get_access_token(self.config_entry)
            elif self.oauth_session:
                await self.oauth_session.async_ensure_token_valid()
                access_token = await self.oauth_session.async_get_access_token()
            else:
                raise HisenseApiError("No authentication method available")
    ...
    async def _get_system_parameters(self, access_token: str = None) -> dict[str, Any]:
