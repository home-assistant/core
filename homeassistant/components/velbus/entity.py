...
class VelbusEntity(Entity):
    ...
    def __init__(self, channel: VelbusChannel) -> None:
        ...
        self._module_adress = channel.get_module_address()
        ...
        if self._channel.is_sub_device():
            self._attr_device_info["via_device"] = (
                DOMAIN,
                str(self._module_adress),
            )
        serial = channel.get_module_serial() or str(self._module_adress)
        self._attr_unique_id = f"{serial}-{channel.get_channel_number()}"

    def _get_identifier(self) -> str:
        """Return the identifier of the entity."""
        if not self._channel.is_sub_device():
            return str(self._module_adress)
