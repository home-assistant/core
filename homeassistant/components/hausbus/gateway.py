...
async def async_get_home_server(hass: HomeAssistant) -> HomeServer:
    """Return the Haus-Bus HomeServer for this Home Assistant instance.

    HomeServer.__new__ always returns the same process-wide singleton, but
    its __init__ unconditionally reruns on every HomeServer() call and
    starts new background threads that are never stopped, orphaning the
    previous ones. Building it directly on the event loop would also block
