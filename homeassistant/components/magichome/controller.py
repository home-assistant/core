"""Support for the MagicHome controller."""
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    """Set up MagicHome controller platform."""
    ...
    for dev_id in dev_ids:
        ...
        if device is None:
            continue
        devices.append(MagicHomeController(device))
    add_entities(devices)


class MagicHomeController(MagicHomeDevice, Light):
    """MagicHome controller device."""

    def __init__(self, magichome):
        """Init MagicHome controller device."""
        super().__init__(magichome)
        self.entity_id = ENTITY_ID_FORMAT.format(magichome.object_id())

    @property
    def brightness(self):
        """Return the brightness of the controller."""
        return int(self.magichome.brightness())

    @property
    def hs_color(self):
        """Return the hs_color of the controller."""
        return tuple(map(int, self.magichome.hs_color()))

    @property
    def color_temp(self):
        """Return the color_temp of the controller."""
    ...
    @property
    def is_on(self):
        """Return true if controller is on."""
