...
from .const import CONF_LIB_PATH, DOMAIN, LOGGER
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        encoder = OpenH264Encoder(
            width=640, height=480, fps=30, bitrate=2000000, lib_path=lib_path
        )
        LOGGER.info(
            "OpenH264 encoder initialized successfully: %s", encoder.get_version()
        )
    except Exception as e:
        LOGGER.warning(
            "Failed to initialize OpenH264 encoder: %s. Services will work in fallback mode.",
            e,
        )
        encoder = None
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        LOGGER.info(
            "OpenH264 Nedis Camera integration unloaded for entry %s", entry.entry_id
        )
...
async def _async_register_services(hass: HomeAssistant) -> None:
    ...
    async def handle_encode_file(call: ServiceCall):
        ...
        LOGGER.info(
            "encode_file service called: %s -> %s (bitrate=%s, fps=%s, gop=%d, prefer_ffmpeg=%s)",
            input_path,
            output_path,
            bitrate,
            fps,
        ...
        try:
            ...
            if prefer_ffmpeg:
                ...
                if success:
                    LOGGER.info(
                        "File encoded successfully with ffmpeg+libopenh264: %s",
                        output_path,
                    )
                    return
                LOGGER.warning(
                    "ffmpeg+libopenh264 encoding failed, trying shim fallback"
                )

            # Path B: Fallback to our shim with yuv4mpeg pipeline
            await _encode_with_shim(hass, input_path, output_path, bitrate, fps, gop)
    ...
    async def _encode_with_ffmpeg_openh264(
        input_path: str, output_path: str, bitrate: str, fps: int, gop: int
    ) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                bitrate,
                "-bufsize",
                f"{int(bitrate.rstrip('MmKk')) * 2}M"
                if bitrate.endswith(("M", "m"))
                else f"{int(bitrate.rstrip('MmKk')) * 2000}k",
                "-pix_fmt",
                "yuv420p",
                "-profile:v",
                "baseline",
    ...
    async def _encode_with_shim(
        hass, input_path: str, output_path: str, bitrate: str, fps: int, gop: int
    ) -> None:
        ...
        if not ffmpeg_path:
            raise HomeAssistantError(
                "ffmpeg is required for video decoding but not found"
            )
        ...
        LOGGER.info(
            "Video info: %dx%d@%dfps, target bitrate: %d bps",
            width,
            height,
            target_fps,
            bitrate_bps,
        ...
        try:
            ...
            await _process_video_with_shim(
                ffmpeg_path, input_path, output_path, encoder
            )
    ...
    def _parse_video_info(stderr_text: str) -> tuple[int, int, int]:
        ...
        video_match = re.search(
            r"Stream.*Video:.*?(\d+)x(\d+).*?(\d+(?:\.\d+)?)\s*fps", stderr_text
        )
        if video_match:
    ...
    async def _process_video_with_shim(
        ffmpeg_path: str, input_path: str, output_path: str, encoder
    ) -> None:
        ...
        try:
            ...
            with open(h264_temp_path, "wb") as h264_file:
                ...
                while True:
                    ...
                    if len(frame_data) != frame_size:
                        LOGGER.warning(
                            "Short frame read: expected %d, got %d",
                            frame_size,
                            len(frame_data),
                        )
                        break
                    ...
                    try:
                        ...
                        if frame_count % 30 == 0:  # Log every 30 frames (~1 second)
                            LOGGER.info(
                                "Processed %d frames, %d bytes encoded",
                                frame_count,
                                total_encoded_size,
                            )
            ...
            LOGGER.info(
                "Encoded %d frames to H.264 (%d bytes total)",
                frame_count,
                total_encoded_size,
            )
            ...
            await _wrap_h264_in_mp4(
                ffmpeg_path, h264_temp_path, output_path, encoder.fps
            )
    ...
    async def _wrap_h264_in_mp4(
        ffmpeg_path: str, h264_path: str, output_path: str, fps: int
    ) -> None:
    ...
    async def handle_capture_snapshot(call: ServiceCall):
        ...
        LOGGER.info(
            "capture_snapshot service called: %s -> %s (format: %s)",
            entity_id,
            filename,
            image_format,
        )
        ...
        if not entity_id or not filename:
            ...
            raise HomeAssistantError(
                "Missing required parameters: entity_id and filename"
            )
        ...
        try:
            ...
            processed_filename = filename.format(
                entity_id=entity_id.replace(".", "_"), timestamp=timestamp
            )
            ...
            LOGGER.info(
                "Snapshot saved successfully: %s (%d bytes)",
                processed_filename,
    ...
    async def handle_record_clip(call: ServiceCall):
        ...
        LOGGER.info(
            "record_clip service called: %s -> %s (%ds, lookback=%ds)",
            entity_id,
            filename,
            duration,
            lookback,
        ...
        try:
            ...
            await hass.services.async_call(
                "camera", "record", record_data, blocking=True
            )
            ...
            LOGGER.info(
                "Recording completed successfully: %s (%d bytes)", filename, final_size
            )
