...
class StateVacuumEntity[SegmentType = Any](
    ...
    async def async_internal_clean_area(
        ...
        area_mapping: dict[str, list[SegmentType]] = options.get("area_mapping", {})

        # We use a dict to preserve the order of segments.
        segment_ids: dict[SegmentType, None] = {}
    ...
    def clean_area(self, segment_ids: list[SegmentType], **kwargs: Any) -> None:
        """Perform an area clean."""
        raise NotImplementedError

    async def async_clean_area(
        self, segment_ids: list[SegmentType], **kwargs: Any
    ) -> None:
        """Perform an area clean."""
