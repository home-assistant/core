...
def _get_options_schema(type: str) -> vol.Schema:
...
async def get_options(handler: SchemaCommonFlowHandler) -> vol.Schema:
    """Return schema for options step."""

    return _get_options_schema(handler.options[CONF_TYPE])
...
async def ws_start_preview(
    ...
    try:
        schema = _get_options_schema(conf_type)
        validated_data = schema(msg["user_input"])
