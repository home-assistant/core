...
async def async_setup_entry(
    ...
    """Set up the Qingping sensors."""
    coordinator = entry.runtime_data
    if isinstance(coordinator, QingpingMqttCoordinator):
        mac = entry.data[CONF_MAC]
        model = entry.data[CONF_MODEL]
        async_add_entities(
            QingpingMqttSensor(coordinator, description, mac, model)
            for description in MQTT_SENSOR_DESCRIPTIONS[model]
        )
        return
...
MQTT_SENSOR_DESCRIPTIONS: Final[
