...
from homeassistant.components.binary_sensor import DOMAIN as BINARY_SENSOR, DEVICE_CLASS_CONNECTIVITY
...
MIKROTIK_SCHEMA = vol.All(
    vol.Schema({
        ...
        vol.Optional(CONF_WAN_PORT, default=MTK_DEFAULT_WAN_PORT): cv.string,
    })
)
...
class MikrotikAPI:
    ...
    def get_polling_method(self, host):
        ...
        try:
            capsman_exist = self._client[host](cmd=MIKROTIK_SERVICES[CAPSMAN])
    ...
    async def update_device_tracker(self, host):
        ...
        for result in arp:
            if 'mac-address' in result and result['invalid'] == False:
                self.hass.data[MIKROTIK][ARP][result['mac-address']] = result

        for device in data:
            mac = device['mac-address']
            if method == DHCP:
                if not 'active-address' in device:
                    continue
            ...
            if mac in self.hass.data[MIKROTIK][ARP]:
                attributes['ip_address'] = self.hass.data[MIKROTIK][ARP][mac]['address']
        ...
        for index in range(len(data)):
            ...
            for key in result:
                ...
                if key in BINARY_SENSORS[sensor_type][5]:
                    binary_sensors[index]['attrib'][slugify(key)] = result[key]

        self.hass.data[MIKROTIK][host][CONF_BINARY_SENSORS][sensor_type] = binary_sensors
