...
from .climate import CLIMATE_MONITORED_EPCS
from .const import (
...
from .poller import HemsPropertyPoller
from .types import HemsConfigEntry, HemsRuntimeData, RuntimeHealth
...
async def async_setup(hass: HomeAssistant, _config: ConfigType) -> bool:
    """Set up the HEMS integration and ensure a config entry exists."""
...
async def async_setup_entry(hass: HomeAssistant, entry: HemsConfigEntry) -> bool:
    ...
    _LOGGER.debug("Setting up HEMS with interface %s", interface)

    # Load device definitions from bundled JSON and vendor YAML
    config_dir = Path(hass.config.config_dir) if hass.config.config_dir else None
    try:
        definitions = await async_get_definitions_registry(config_dir)
    ...
    # Add climate-specific monitored EPCs
    for class_code, epcs in CLIMATE_MONITORED_EPCS.items():
    ...
    async def _async_restart_runtime() -> None:
        ...
        async with restart_lock:
            ...
            try:
                ...
                _LOGGER.debug("Failed to stop HEMS runtime client: %s", err)
            try:
                await client.start()
            except OSError as err:
                _LOGGER.error("Failed to restart HEMS runtime client: %s", err)
    ...
    def _handle_runtime_event(event: RuntimeEvent) -> None:
        ...
        if isinstance(event, HemsFrameEvent):
            ...
            hass.async_create_task(_process_frame(), name="hems_process_frame")
            return
        ...
        if isinstance(event, HemsInstanceListEvent):
            ...
            # blocking receiver loop
            hass.async_create_task(
                _process_instance_list(), name="hems_process_instance_list"
            )
            return
        ...
        if isinstance(event, HemsErrorEvent):
            ...
            _LOGGER.warning("HEMS runtime client encountered an error: %s", event.error)
            ...
            hass.async_create_task(_handle_error(), name="hems_handle_error")
    ...
    property_poller = HemsPropertyPoller(
...
async def _async_update_listener(hass: HomeAssistant, entry: HemsConfigEntry) -> None:
