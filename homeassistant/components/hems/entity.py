...
class HemsEntity(CoordinatorEntity[HemsCoordinator]):
    """Base entity bound to a HEMS node."""
    ...
    def __init__(
        ...
        name = node.product_code or f"HEMS node {node.device_key}"
...
class HemsEntityDescription(EntityDescription):
    """Base entity description for HEMS entities with common fields.

    This class provides common ECHONET-related fields and methods for all HEMS
    ...
    def should_create(self, node: HemsNodeState) -> bool:
...
class HemsDescribedEntity[DescriptionT: HemsEntityDescription](HemsEntity):
    """Base class for HEMS entities with EntityDescription.
    ...
    Climate entities should inherit from HemsEntity directly since they manage
    ...
    def __init__(
        ...
        """Initialize a described HEMS entity.
...
def setup_hems_platform[DescriptionT: HemsEntityDescription](
    entry: HemsConfigEntry,
    async_add_entities: AddConfigEntryEntitiesCallback,
    platform_type: PlatformType,
    description_factory: Callable[[int, EntityDefinition, DecoderSpec], DescriptionT],
    entity_factory: Callable[[HemsCoordinator, HemsNodeState, DescriptionT], Entity],
    platform_name: str,
) -> None:
    """Set up common entity platform setup pattern for HEMS.
