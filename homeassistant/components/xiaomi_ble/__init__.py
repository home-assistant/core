        ...
        if address is not None:
            ...
            if not is_s400:
                ...
                device_entry = device_registry.async_get_device_by_identifier(
                    (BLUETOOTH_DOMAIN, address), entry.entry_id
                )
...
def _async_is_known_s400(
    ...
    device_entry = dr.async_get(hass).async_get_device_by_identifier(
        (BLUETOOTH_DOMAIN, address), entry_id
    )
...
def _async_purge_stale_s400_impedance_restore_cache(
    ...
    if _async_is_known_s400(hass, entry.entry_id, address, coordinator):
...
def _async_recover_interrupted_s400_migration(
    ...
    if legacy_entity_id is None or not _async_is_known_s400(
        hass, entry.entry_id, address, coordinator
    ):
        return
