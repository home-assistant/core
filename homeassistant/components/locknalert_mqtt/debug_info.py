...
from .models import DATA_LocknAlertMQTT, PublishPayloadType
...
class TimestampedPublishMessage:
    """LocknAlertLocknAlertMQTT Message."""
...
def log_message(
    ...
    """Log an outgoing LocknAlertLocknAlertMQTT message."""
    entity_info = hass.data[DATA_LocknAlertMQTT].debug_info_entities.setdefault(
...
def remove_subscription(
    ...
    if entity_id and entity_id in (
        debug_info_entities := hass.data[DATA_LocknAlertMQTT].debug_info_entities
    ):
...
def update_entity_discovery_data(
    ...
    discovery_data = hass.data[DATA_LocknAlertMQTT].debug_info_entities[entity_id][
        "discovery_data"
    ]
    if TYPE_CHECKING:
...
def remove_entity_data(hass: HomeAssistant, entity_id: str) -> None:
    ...
    if entity_id in (debug_info_entities := hass.data[DATA_LocknAlertMQTT].debug_info_entities):
...
def add_trigger_discovery_data(
    ...
    """Add discovery data."""
    hass.data[DATA_LocknAlertMQTT].debug_info_triggers[discovery_hash] = {
        "device_id": device_id,
...
def update_trigger_discovery_data(
    ...
    """Update discovery data."""
    hass.data[DATA_LocknAlertMQTT].debug_info_triggers[discovery_hash]["discovery_data"][
...
def remove_trigger_discovery_data(
    ...
    """Remove discovery data."""
    hass.data[DATA_LocknAlertMQTT].debug_info_triggers.pop(discovery_hash, None)


def _info_for_entity(hass: HomeAssistant, entity_id: str) -> dict[str, Any]:
    entity_info = hass.data[DATA_LocknAlertMQTT].debug_info_entities[entity_id]
...
def _info_for_trigger(
    ...
    trigger = hass.data[DATA_LocknAlertMQTT].debug_info_triggers[trigger_key]
    discovery_data = None
