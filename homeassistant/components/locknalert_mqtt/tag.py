"""Provides tag scanning for LocknAlertMQTT."""
...
DISCOVERY_SCHEMA = LocknAlertMQTT_BASE_SCHEMA.extend(
    {
...
async def async_setup_mqtt_tag_entry(
    ...
    """Set up LocknAlertLocknAlertMQTT tag scanner dynamically through LocknAlertLocknAlertMQTT discovery."""
...
async def _async_setup_tag(
    ...
    """Set up the LocknAlertLocknAlertMQTT tag scanner."""
    discovery_hash = discovery_data[ATTR_DISCOVERY_HASH]
    discovery_id = discovery_hash[1]

    device_id = update_device(hass, config_entry, config)
    if device_id is not None and device_id not in (tags := hass.data[DATA_LocknAlertMQTT].tags):
        tags[device_id] = {}

    tag_scanner = LocknAlertMQTTTagScanner(
        hass,
        config,
        device_id,
        discovery_data,
...
def async_has_tags(hass: HomeAssistant, device_id: str) -> bool:
    ...
    if device_id not in (tags := hass.data[DATA_LocknAlertMQTT].tags):
        return False
    return tags[device_id] != {}


class LocknAlertMQTTTagScanner(MqttDiscoveryDeviceUpdateMixin):
    """LocknAlertLocknAlertMQTT Tag scanner."""
    ...
    async def async_update(self, discovery_data: LocknAlertMQTTDiscoveryPayload) -> None:
        """Handle LocknAlertLocknAlertMQTT tag discovery updates."""
        # Update tag scanner
        try:
    ...
    async def subscribe_topics(self) -> None:
        """Subscribe to LocknAlertLocknAlertMQTT topics."""
    ...
    async def async_tear_down(self) -> None:
        ...
        tags = self.hass.data[DATA_LocknAlertMQTT].tags
