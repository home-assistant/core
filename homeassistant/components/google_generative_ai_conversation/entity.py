...
def _create_thinking_config(model: str, thinking_budget: int) -> ThinkingConfig | None:
    ...
    if _is_gemini_3_model(model):
        ...
        # Thinking cannot be fully disabled on Gemini 3.
        if thinking_budget == -1:
            return ThinkingConfig(
                include_thoughts=True, thinking_level=ThinkingLevel.HIGH
            )
        if thinking_budget == 0:
            # Attempt to disable: use LOW as the minimum available level
            return ThinkingConfig(
                include_thoughts=True, thinking_level=ThinkingLevel.LOW
            )
        ...
        return ThinkingConfig(include_thoughts=True, thinking_level=ThinkingLevel.HIGH)
    ...
    if thinking_budget == 0:
        # Disable thinking (not supported for gemini-2.5-pro, minimum is 128)
        name = model.removeprefix("models/")
...
class GoogleGenerativeAILLMBaseEntity(Entity):
    ...
    def create_generate_content_config(
        ...
        thinking_config = _create_thinking_config(
            model,
            options.get(CONF_THINKING_BUDGET, RECOMMENDED_THINKING_BUDGET),
        )
