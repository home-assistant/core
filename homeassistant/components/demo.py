...
def async_setup(hass, config):
    ...
    tasks.append(
        bootstrap.async_setup_component(hass, 'input_boolean', {
            'input_boolean': {
                'notify': {
                    'icon': 'mdi:car',
                    'initial': False,
    ...
    # Set up input boolean
    tasks.append(
        bootstrap.async_setup_component(hass, 'input_number', {
            'input_number': {
                'noise_allowance': {
                    'icon': 'mdi:bell-ring',
                    'min': 0,
    ...
    # Set up weblink
    tasks.append(
        bootstrap.async_setup_component(hass, 'weblink', {
            'weblink': {
                'entities': [{
                    'name': 'Router',
    ...
    # Set up scenes
    tasks2.append(
        bootstrap.async_setup_component(hass, 'scene', {
            'scene': [
                {
    ...
    tasks2.append(
        group.Group.async_create_group(hass, 'Living Room', [
            lights[1], switches[0], 'input_select.living_room_preset',
            'cover.living_room_window', media_players[
                1], 'scene.romantic_lights'
        ]))
    tasks2.append(
        group.Group.async_create_group(hass, 'Bedroom', [
            lights[0], switches[1], media_players[0],
            'input_number.noise_allowance'
        ]))
    tasks2.append(
        group.Group.async_create_group(hass, 'Kitchen', [
            lights[2], 'cover.kitchen_window', 'lock.kitchen_door'
        ]))
    tasks2.append(
        group.Group.async_create_group(hass, 'Doors', [
            'lock.front_door', 'lock.kitchen_door',
            'garage_door.right_garage_door', 'garage_door.left_garage_door'
        ]))
    tasks2.append(
        group.Group.async_create_group(hass, 'Automations', [
            'input_select.who_cooks',
            'input_boolean.notify',
        ]))
    tasks2.append(
        group.Group.async_create_group(hass, 'People', [
    ...
    def hue_configuration_callback(data):
        ...
        if len(configurator_ids) == 1:
            configurator.notify_errors(configurator_ids[0],
