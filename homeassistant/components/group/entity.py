...
from .const import ATTR_AUTO, ATTR_ORDER, DATA, DOMAIN, GROUP_ORDER, REG_KEY
...
def async_get_component(hass: HomeAssistant) -> EntityComponent[Group]:
    ...
    if (component := hass.data.get(DATA)) is None:
        component = hass.data[DATA] = EntityComponent[Group](
