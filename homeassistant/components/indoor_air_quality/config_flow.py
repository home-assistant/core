...
from .helpers import unique_id_from_sources
...
PM25_DEVICE_CLASS_VALUE = str(SensorDeviceClass.PM25)
...
TVOC_PARTS_DEVICE_CLASS_VALUE = str(SensorDeviceClass.VOLATILE_ORGANIC_COMPOUNDS_PARTS)
...
SOURCE_SELECTORS = {
    ...
    CONF_PM25: selector.EntitySelector(
        selector.EntitySelectorConfig(
            domain=SENSOR_DOMAIN,
...
def _is_tvoc_entry(labels: str, device_classes: set[str]) -> bool:
    ...
    if TVOC_PARTS_DEVICE_CLASS_VALUE in device_classes:
        return False
...
def _sources_from_device(hass: HomeAssistant, device_id: str) -> dict[str, Any]:
    ...
    for entry in er.async_entries_for_device(entity_registry, device_id):
        ...
        if PM25_DEVICE_CLASS_VALUE in device_classes:
            pm25_source = pm25_source or entry.entity_id
            continue
...
class IndoorAirQualityConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _async_create_config_entry(
        ...
        standard = user_input.get(CONF_STANDARD, DEFAULT_STANDARD)
        data: dict[str, Any] = {
        ...
        await self.async_set_unique_id(unique_id_from_sources(sources, standard))
    ...
    async def async_step_sources(
        ...
        if user_input is not None:
            ...
            if self.source != config_entries.SOURCE_RECONFIGURE:
                sources = self._detected_sources | sources
            errors.update(_validate_voc_sources(sources))
    ...
    async def async_step_reconfigure(
        ...
        if user_input is not None:
            ...
            if not errors:
                standard = user_input.get(CONF_STANDARD, current_standard)
                unique_id = unique_id_from_sources(sources, standard)
