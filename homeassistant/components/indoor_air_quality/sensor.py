...
def _device_entry_from_device_id(
    hass: HomeAssistant, device_id: str | None
) -> AnyDeviceEntry | None:
    """Return the selected source device, when configured."""
    if not device_id:
        return None
...
async def async_setup_entry(
    ...
    parent_device = _device_entry_from_device_id(hass, device_id)
...
class IndoorAirQualitySensor(SensorEntity):
    ...
    def __init__(
        ...
        sensor_type: str,
        parent_device: AnyDeviceEntry | None,
    ) -> None:
        ...
        if parent_device is not None:
            self.device_entry = parent_device
        else:
