...
from .const import CONF_SOURCES, CONF_STANDARD
...
def unique_id_from_sources(sources: Mapping[str, str], standard: str) -> str:
    """Return a stable identity for a methodology and source set."""
    payload = json.dumps(
        {CONF_SOURCES: sources, CONF_STANDARD: standard}, sort_keys=True
    ).encode()
...
_MGM3_UNITS: Final = frozenset(
    {UnitOfDensity.MILLIGRAMS_PER_CUBIC_METER, "mg/m3", "mg/m^3"}
)
...
def normalise_unit(unit: str | None) -> str | None:
    ...
    if unit in _MGM3_UNITS:
        return UnitOfDensity.MILLIGRAMS_PER_CUBIC_METER
    return unit
...
def resolve_state(
    ...
    if not math.isfinite(value):
        _LOGGER.debug("State of entity %s is not finite: %r", entity_id, entity.state)
        return None
