...
from homeassistant.const import PERCENTAGE, UnitOfDensity, UnitOfTemperature
...
SOURCE_SPECS: Final[dict[str, SourceSpec]] = {
    ...
    CONF_HUMIDITY: SourceSpec(maximum=100, valid_units=frozenset({PERCENTAGE})),
    CONF_CO2: SourceSpec(target_units=UNIT_PPM, molar_mass=MOLAR_MASS_CO2),
    CONF_TVOC: SourceSpec(target_units=UNIT_MGM3),
    CONF_VOC_INDEX: SourceSpec(),
    CONF_PM25: SourceSpec(
        converter=MassVolumeConcentrationConverter,
        target_unit=UnitOfDensity.MICROGRAMS_PER_CUBIC_METER,
    ),
    CONF_NO2: SourceSpec(
    ...
    CONF_CO: SourceSpec(
        ...
        target_unit=UnitOfDensity.MILLIGRAMS_PER_CUBIC_METER,
    ),
...
class IndoorAirQualityController:
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if self._indexes:
            attrs[ATTR_LIMITING_SOURCE] = min(
                self._indexes, key=lambda source: self._indexes[source]
            )
    ...
    def update(self) -> None:
        ...
        for src in self._sources:
            ...
            if (spec.minimum is not None and value < spec.minimum) or (
                ...
                _LOGGER.debug(
                    "[%s] Source %s value %s is outside its physical range",
                    self._entry_id,
                    src,
                    value,
                )
        ...
        if used == len(self._sources):
            ...
            self._reset_state()
            _LOGGER.debug(
                "[%s] Not all configured sources are usable, IAQ index cleared",
                self._entry_id,
            )
    ...
    def _single_entity_id(self, source: str) -> str | None:
        """Return a configured source entity id."""
