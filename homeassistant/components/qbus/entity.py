...
from .qbus_mqtt import QbusMqttDevice, QbusMqttOutput
...
class QbusEntity(Entity):
    ...
    def __init__(
        ...
        self._command_topic = (
            f"cloudapp/QBUSMQTTGW/{mqtt_device.id}/{mqtt_output.id}/setState"
        )
        self._state_topic = (
            f"cloudapp/QBUSMQTTGW/{mqtt_device.id}/{mqtt_output.id}/state"
        )
