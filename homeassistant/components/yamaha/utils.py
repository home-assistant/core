...
async def check_yamaha_ssdp(location: str, hass: HomeAssistant) -> bool:
    """Check if the Yamaha receiver has a valid control URL."""
    details: RxvDetails | None = await get_rxv_details(location, hass)
    return (details and details.ctrl_url) is not None


async def get_rxv_details(location: str, hass: HomeAssistant) -> RxvDetails:
