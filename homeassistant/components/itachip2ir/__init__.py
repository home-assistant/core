...
PLATFORMS: list[Platform] = [Platform.INFRARED, Platform.REMOTE]
...
async def async_setup_entry(hass: HomeAssistant, entry: ItachConfigEntry) -> bool:
    ...
    try:
        ...
        raise ConfigEntryNotReady(
            f"Could not connect to iTach at {host}:{port}"
        ) from err
        ...
        raise ConfigEntryNotReady(
            f"Could not validate iTach IR module at {host}:{port}: {err}"
        ) from err
    ...
    if not ir_enabled_ports:
        ...
        raise ConfigEntryNotReady(
            f"No iTach IR output ports are available at {host}:{port}"
        )
