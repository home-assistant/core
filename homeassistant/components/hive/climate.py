...
async def async_setup_entry(
    ...
    devices = coordinator.hive.session.deviceList.get("climate")
    if devices:
        async_add_entities(
            (HiveClimateEntity(coordinator, dev) for dev in devices), True
        )
...
class HiveClimateEntity(HiveEntity, ClimateEntity):
    ...
    @refresh_system
    async def async_heating_boost_on(self, time_period: int, temperature: float) -> None:
    ...
    def _update_state_from_device(self) -> None:
        """Update climate attributes from device data."""
