...
async def async_validate_input(
    data: dict[str, Any], yale_gateway: YaleGateway
) -> dict[str, Any]:
    ...
    """
    assert yale_gateway.authenticator is not None
    authenticator = yale_gateway.authenticator
    ...
    try:
        ...
        if code is None:
            await yale_gateway.authenticator.async_send_verification_code()
        raise

    return {
        "title": data.get(CONF_USERNAME),
        "data": yale_gateway.config_entry(),
    }
...
class YaleConfigFlow(config_entry_oauth2_flow.AbstractOAuth2FlowHandler, domain=DOMAIN):
    ...
    def __init__(self) -> None:
        ...
        self._yale_gateway: YaleGateway | None = None
    ...
    @callback
    def _async_get_gateway(self) -> YaleGateway:
        """Set up the gateway."""
        if self._yale_gateway is not None:
            return self._yale_gateway
        self._aiohttp_session = async_create_yale_clientsession(self.hass)
        self._yale_gateway = YaleGateway(
    ...
    async def _async_reset_access_token_cache_if_needed(
        self, gateway: YaleGateway, username: str, access_token_cache_file: str | None
    ) -> None:
