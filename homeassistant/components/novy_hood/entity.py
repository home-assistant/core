"""Common entity for the Novy Hood integration."""
...
class NovyHoodEntity(Entity):
    """Novy Hood base entity."""
    ...
    async def _async_send_repeated(
        self,
        command_factory: type[RadioFrequencyCommand],
        times: int,
        gap: float = 0.1,
    ) -> None:
        """Send `command_factory()` `times` times, sleeping `gap` seconds between sends."""
        for i in range(times):
            if i:
                await asyncio.sleep(gap)
            await self._async_send(command_factory())
