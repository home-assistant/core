...
class SeventeenTrackDataCoordinator(DataUpdateCoordinator):
    ...
    async def async_update(self) -> dict[str, list[Package] | dict]:
        """Update SeventeenTrack data."""
        try:
            packages: list[Package] = await self.client.profile.packages(
            ...
            if self.show_archived:
                archived_packages = await self.client.profile.packages(
                    show_archived=True, tz=dt_util.DEFAULT_TIME_ZONE
                )
    ...
    async def async_setup(self) -> bool:
        ...
        async def async_add_package(service: ServiceCall) -> None:
            ...
            if not (device := device_registry.async_get(service.data[CONF_DEVICE_ID])):
                _LOGGER.error("17Track device not found")
                return
            for entry_id in device.config_entries:
                config_entry = self.hass.config_entries.async_get_entry(entry_id)
                if config_entry and config_entry.domain == DOMAIN:
                    client: Client = self.hass.data[DOMAIN][entry_id].client
                    break
            try:
...
async def get_client(hass: HomeAssistant, entry) -> Client:
    """Return SeventeenTrack client."""
    session = aiohttp_client.async_get_clientsession(hass)
    client = Client(session=session)
    if not await client.profile.login(entry[CONF_USERNAME], entry[CONF_PASSWORD]):
