...
from .const import (SERV_SECURITY_SYSTEM, CHAR_CURRENT_SECURITY_STATE, CHAR_TARGET_SECURITY_STATE)
...
STATE_TO_SERVICE = {STATE_ALARM_DISARMED: 'alarm_disarm', STATE_ALARM_ARMED_HOME: 'alarm_arm_home',
                    STATE_ALARM_ARMED_AWAY: 'alarm_arm_away', STATE_ALARM_ARMED_NIGHT: 'alarm_arm_night'}
...
class SecuritySystem(HomeAccessory):
    ...
    def __init__(self, hass, entity_id, display_name):
        ...
        self.char_current_state = self.service_alarm.get_characteristic(CHAR_CURRENT_SECURITY_STATE)
        self.char_target_state = self.service_alarm.get_characteristic(CHAR_TARGET_SECURITY_STATE)
    ...
    def run(self):
        ...
        async_track_state_change(self._hass, self._entity_id, self.update_security_state)

    def set_security_state(self, value):
        """Move security state to value if call came from homekit."""
        if value != self.current_security_state and value in HOMEKIT_TO_HASS:
            _LOGGER.debug("%s: Set security state to %d", self._entity_id, value)
            self.homekit_target_security_state = value
            hass_value = HOMEKIT_TO_HASS[value]
            service = STATE_TO_SERVICE[hass_value]

            self._hass.services.call('alarm_control_panel', service, {'entity_id': self._entity_id})

    def update_security_state(self, entity_id=None, old_state=None, new_state=None):
        """Update security state after state changed."""
        if new_state is None:
            return

        hass_state = new_state.state
        _LOGGER.debug("%s: Want to update current state to %s", self._entity_id, hass_state)
        if hass_state in HASS_TO_HOMEKIT:
            ...
            _LOGGER.debug("%s: Updated current state to %s (%d)", self._entity_id, hass_state, self.current_security_state)

            if self.homekit_target_security_state is None or \
                    self.homekit_target_security_state == self.current_security_state:
