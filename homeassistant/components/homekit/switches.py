...
class Switch(HomeAccessory):
    ...
    def run(self):
        ...
        async_track_state_change(self._hass, self._entity_id, self.update_state)

    def set_state(self, value):
        """Move switch state to value if call came from homekit."""
        if value != self.current_on:
            _LOGGER.debug("%s: Set switch state to %s", self._entity_id, value)
            self.homekit_target_on = value
            service = 'turn_on' if value else 'turn_off'
            self._hass.services.call(self._domain, service, {'entity_id': self._entity_id})
    ...
    def update_state(self, entity_id=None, old_state=None, new_state=None):
        ...
        _LOGGER.debug("%s: Want to update current state to %s", self._entity_id, new_state.state)
        self.current_on = (new_state.state == 'on')
        self.char_on.set_value(self.current_on)
        _LOGGER.debug("%s: Updated current state to %s (%s)", self._entity_id, new_state.state, self.current_on)
        if self.homekit_target_on is None or self.homekit_target_on == self.current_on:
            self.homekit_target_on = None
