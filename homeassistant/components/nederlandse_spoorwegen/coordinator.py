...
class NSDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(
        ...
        self.stations_updated: str | None = None
    ...
    def _get_routes(self) -> list[dict[str, Any]]:
        """Get routes from config entry options/data."""
        if self.config_entry is None:
            return []

        routes_data = self.config_entry.options.get(
            CONF_ROUTES, self.config_entry.data.get(CONF_ROUTES, [])
        )

        # Handle both list format (legacy) and dict format (current)
        if isinstance(routes_data, dict):
            # Convert dict of routes to list of route configs
            return list(routes_data.values())
    ...
    async def _get_trips_for_route(self, route: dict[str, Any]) -> list[Any]:
        ...
        # Only validate stations if station data is available
        # This allows initial setup to proceed without station validation
        station_codes = self.get_station_codes()
        if station_codes and not self._validate_route_stations(normalized_route):
            return []
    ...
    def _normalize_route_stations(self, route: dict[str, Any]) -> dict[str, Any]:
        ...
        Station codes are normalized to uppercase for consistent internal operations.
        The station cache contains both original and uppercase variants for compatibility.
        """
        normalized_route = route.copy()

        # Use centralized station code normalization (converts to uppercase)
    ...
    def _validate_route_stations(self, route: dict[str, Any]) -> bool:
        ...
        if from_station not in valid_station_codes:
            _LOGGER.error(
                "From station '%s' not found in NS station list for route: %s. Available stations: %d",
                from_station,
                route,
                len(valid_station_codes),
            )
            # Log some similar station codes to help with debugging
            if valid_station_codes:
                similar_codes = [
                    code
                    for code in valid_station_codes
                    if from_station.lower() in code.lower()
                    or code.lower() in from_station.lower()
                ]
                if similar_codes:
                    _LOGGER.debug("Similar station codes found: %s", similar_codes[:5])
            return False

        if to_station not in valid_station_codes:
            _LOGGER.error(
                "To station '%s' not found in NS station list for route: %s. Available stations: %d",
                to_station,
                route,
                len(valid_station_codes),
            )
            ...
            if valid_station_codes:
                similar_codes = [
                    code
                    for code in valid_station_codes
                    if to_station.lower() in code.lower()
                    or code.lower() in to_station.lower()
                ]
                if similar_codes:
                    _LOGGER.debug("Similar station codes found: %s", similar_codes[:5])
            return False
        ...
        if via_station and via_station not in valid_station_codes:
            _LOGGER.error(
                "Via station '%s' not found in NS station list for route: %s. Available stations: %d",
                via_station,
                route,
                len(valid_station_codes),
            )
            ...
            if valid_station_codes:
                similar_codes = [
                    code
                    for code in valid_station_codes
                    if via_station.lower() in code.lower()
                    or code.lower() in via_station.lower()
                ]
                if similar_codes:
                    _LOGGER.debug("Similar station codes found: %s", similar_codes[:5])
            return False
    ...
    def get_station_codes(self) -> set[str]:
        ...
        if (
            ...
            station_codes = self.api_wrapper.get_station_codes(
                self.config_entry.runtime_data.stations
            )
            _LOGGER.debug(
                "Available station codes: %d stations loaded", len(station_codes)
            )
            # Debug: Log first few station codes to help with troubleshooting
            if station_codes:
                sample_codes = sorted(station_codes)[:10]
                _LOGGER.debug("Sample station codes: %s", sample_codes)
            return station_codes
        _LOGGER.warning("No station data available in runtime_data")
        return set()
