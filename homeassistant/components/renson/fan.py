...
async def async_setup_entry(
    ...
    async_add_entities([RensonFan(api, coordinator), RensonBreezeFan(api, coordinator)])
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if level == Level.BREEZE:
            self._attr_percentage = ranged_value_to_percentage(
                SPEED_RANGE, SPEED_MAPPING[Level.OFF.value]
            )
        else:
            level = self.api.parse_value(
...
class RensonBreezeFan(RensonEntity, FanEntity):
    """Representation of the Renson breeze fan platform."""
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn off the breeze (to away)."""
        await self.async_set_percentage(0)

    async def async_set_percentage(self, percentage: int) -> None:
        """Set breeze speed percentage."""
        _LOGGER.debug("Changing fan speed percentage to %s", percentage)
        ...
        if cmd != Level.HOLIDAY:
            await self.hass.async_add_executor_job(
                self.api.set_manual_level, Level.BREEZE
            )
