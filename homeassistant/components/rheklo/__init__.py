...
from .coordinator import RhekloConfigEntry, RhekloRuntimeData, RhekloUpdateCoordinator

PLATFORMS = [Platform.SENSOR]
_LOGGER = logging.getLogger(__name__)


async def async_setup_entry(hass: HomeAssistant, entry: RhekloConfigEntry) -> bool:
    """Set up Rheklo from a config entry."""
    websession = async_get_clientsession(hass)
    rheklo = AioKem(session=websession)
    ...
    rheklo.set_refresh_token_callback(async_refresh_token_update)
    rheklo.set_retry_policy(retry_count=3, retry_delays=[5, 10, 20])

    try:
        await rheklo.authenticate(
    ...
    homes = await rheklo.get_homes()

    entry.runtime_data = RhekloRuntimeData(
...
async def async_unload_entry(hass: HomeAssistant, entry: RhekloConfigEntry) -> bool:
    """Unload a config entry."""
    await entry.runtime_data.rheklo.close()
