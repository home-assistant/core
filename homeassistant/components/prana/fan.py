...
class PranaSendSpeed:
    ...
    async def send_speed_percentage(self, percentage: int) -> None:
        ...
        request_data = {"speed": speed * 10, "fanType": effective_type}
        _LOGGER.debug("Sending speed request data: %s", request_data)

        async with (
            ClientSession() as session,
            session.post(
                f"http://{self.coordinator.entry.data.get('host')}:80/setSpeed",
                json=request_data,
            ) as resp,
        ):
            if resp.status != 200:
    ...
    async def send_speed_is_on(self, value: bool) -> None:
        ...
        request_data = {"value": value, "fanType": effective_type}
        async with (
            ClientSession() as session,
            session.post(
                f"http://{self.coordinator.entry.data.get('host')}:80/setSpeedIsOn",
                json=request_data,
            ) as resp,
        ):
            _LOGGER.debug("Sent speed is_on=%s for fan type %s", value, effective_type)
            if resp.status != 200:
                raise HomeAssistantError(f"HTTP {resp.status}")
