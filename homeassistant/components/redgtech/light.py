                ...
                async with session.get(f'{API_URL}/home_assistant?access_token={access_token}') as response:
                    if response.status == 200:
                        ...
                        for item in data.get("boards", []):
                            ...
                            if 'dim' in endpoint_id:  # Adiciona apenas dispositivos com controle de brilho
...
class RedgtechLight(LightEntity):
    ...
    def __init__(self, data, token):
        ...
        self._previous_brightness = self._brightness  # Armazena o brilho anterior
    ...
    async def async_turn_on(self, **kwargs):
        ...
        brightness = kwargs.get(CONF_BRIGHTNESS, self._previous_brightness)  # Usa o brilho anterior como padrão
        await self._set_state(STATE_ON, brightness)

    async def async_turn_off(self, **kwargs):
        """Turn the light off."""
        self._previous_brightness = self._brightness  # Armazena o brilho atual antes de desligar
    ...
    async def _set_state(self, state, brightness=None):
        ...
        brightness_value = int((brightness / 255) * 100) if brightness else 0  # Converte para escala de 0 a 100
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    self._state = state
                    if state == STATE_ON:
                        self._brightness = brightness or 255  # Define brilho máximo ao ligar
                    else:
                        self._brightness = 0  # Define brilho 0 ao desligar
