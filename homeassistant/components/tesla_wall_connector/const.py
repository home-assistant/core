"""Constants for the Tesla Wall Connector integration."""

DOMAIN = "tesla_wall_connector"
CONF_SCAN_INTERVAL_CHARGING = "scan_interval_charging"
DEFAULT_SCAN_INTERVAL = 30
DEFAULT_SCAN_INTERVAL_CHARGING = 5

WALLCONNECTOR_CLIENT = "api"
WALLCONNECTOR_DATA_UPDATE_COORDINATOR = "DataUpdateCoordinator"
WALLCONNECTOR_HOST = "ip"
WALLCONNECTOR_PART_NUMBER = "part_number"
WALLCONNECTOR_FIRMWARE_VERSION = "firmware_version"
WALLCONNECTOR_SERIAL_NUMBER = "serial_number"

WALLCONNECTOR_DATA_VITALS = "vitals"
WALLCONNECTOR_DATA_LIFETIME = "lifetime"

WALLCONNECTOR_DEVICE_NAME = "Tesla Wall Connector"
