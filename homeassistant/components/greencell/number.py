...
class EVSEMaxCurrent(NumberEntity):
    def __init__(
        ...
        if self._device_is_habu_den():
            if max_current > DEFAULT_MAX_CURRENT_HABU_DEN:
                _LOGGER.warning(
                    f"Max current for Habu Den is limited to {DEFAULT_MAX_CURRENT_HABU_DEN} A"
                )
                max_current = DEFAULT_MAX_CURRENT_HABU_DEN
        else:
            if max_current > DEFAULT_MAX_CURRENT_OTHER:
                _LOGGER.warning(
                    f"Max current for unknown device type is limited to {DEFAULT_MAX_CURRENT_OTHER} A"
                )
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        if value < DEFAULT_MIN_CURRENT or value > self._attr_native_max_value:
            _LOGGER.error(f"Value {value} is out of range for {self._serial}")
            return
...
async def async_setup_platform(
    ...
    max_current = (
        int(discovery_info.get("max_current", 32))
        if discovery_info
        else int(config.get("max_current", 32))
    )
...
async def async_setup_entry(
    ...
    max_current = (
        int(discovery_info.get("max_current", 32))
        if discovery_info
        else int(entry.data.get("max_current", 32))
    )
