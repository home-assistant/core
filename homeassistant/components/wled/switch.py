...
from homeassistant.components.wled.const import DATA_WLED_CLIENT, DOMAIN
...
class WLEDSwitch(ToggleEntity, WLEDDeviceEntity):
    """Defines a WLED switch."""

    def __init__(self, entry_id: str, wled: WLED, name: str, icon: str, key: str):
    ...
    def unique_id(self) -> str:
        ...
        return f"{DOMAIN}_{self.wled.device.info.mac_address}_switch_{self._key}"
...
class WLEDNightlightSwitch(WLEDSwitch):
    ...
    async def _wled_update(self) -> None:
        ...
        self._attributes = {
            "duration": self.wled.device.state.nightlight.duration,
            "fade": self.wled.device.state.nightlight.fade,
            "target_brightness": self.wled.device.state.nightlight.target_brightness,
        }
...
class WLEDSyncSendSwitch(WLEDSwitch):
    ...
    async def _wled_update(self) -> None:
        ...
        self._attributes = {"udp_port": self.wled.device.info.udp_port}
