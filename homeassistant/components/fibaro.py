...
class FibaroController():
    ...
    def __init__(self, hass, username, password, url):
        ...
        self.client.add_event_handler('value', self.value_change_handler)

        # used by venetian blind covers for tilt
        self.client.add_event_handler('value2', self.value2_change_handler)

        # used by RGB led strips, which also use value
        self.client.add_event_handler('brightness', self.value_change_handler)
        ...
        self.client.add_event_handler('Humidity', self.dummy_handler)
        self.client.add_event_handler('Pressure', self.dummy_handler)
        self.client.add_event_handler('Temperature', self.dummy_handler)
        self.client.add_event_handler('Wind', self.dummy_handler)
        self.client.add_event_handler('ui.Current_Weather_Label.caption',
                                      self.dummy_handler)

        # these could be useful, but not implemented yet
        self.client.add_event_handler('lastUpdated', self.dummy_handler)
        self.client.add_event_handler('lastOutdoorUpdated', self.dummy_handler)
        self.client.add_event_handler('logTemp', self.dummy_handler)
        self.client.add_event_handler('color', self.dummy_handler)
        self.client.add_event_handler('batteryLevel', self.dummy_handler)
        self.client.add_event_handler('lastColorSet', self.dummy_handler)
        self.client.add_event_handler('power', self.dummy_handler)
        self.client.add_event_handler('energy', self.dummy_handler)
    ...
    def value2_change_handler(self, **kwargs):
        """Process the value2 change event of a device."""
        try:
            change_id = kwargs.get('id', None)
            change_value = kwargs.get('value', None)
            with self.devices_lock:
