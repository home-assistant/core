...
class FibaroController(FibaroClient):
    ...
    def __init__(self, hass, username, password, url):
        ...
        if my_login is None or my_login.status is False:
            _LOGGER.error("Invalid login for Fibaro HC. Please check username and password.")
            raise ValueError("Invalid login for Fibaro HC. Please check username and password.")

        self._read_rooms()
        self._read_devices()
        self.add_event_handler('value', self.value_change_handler)      # used by most devices
        self.add_event_handler('value2', self.value2_change_handler)    # used by venetian blind covers for tilt
        self.add_event_handler('brightness', self.value_change_handler) # used by RGB led strips, which also use value
        self.add_event_handler('log', self.log_handler)                 # this is used for log messages
        ...
        self.add_event_handler('ui.Current_Weather_Label.caption', self.dummy_handler)
    ...
    def lastUpdated_change_handler(self, **kwargs):
        """Callback for the last update event of a device."""
        pass
    ...
    def power_change_handler(self, **kwargs):
        """Callback for the power update event of a device."""
        pass
