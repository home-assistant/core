...
from .const import CONF_PROTOCOL, CONF_USE_HTTPS, DEFAULT_TIMEOUT, SUBSCRIPTION_RENEW_THRESHOLD
...
class ReolinkHost:
    ...
    async def renew(self) -> bool:
        ...
        elif timer <= SUBSCRIPTION_RENEW_THRESHOLD:
    ...
    async def register_webhook(self) -> bool:
        ...
        try:
            webhook.async_register(DOMAIN, event_id, self.webhook_id, handle_webhook)
        except ValueError:
            _LOGGER.debug("Error registering webhook %s. Trying to unregister it first and re-register again.", self.webhook_id)
            webhook.async_unregister(self.webhook_id)
            try:
                webhook.async_register(DOMAIN, event_id, self.webhook_id, handle_webhook)
            except ValueError:
                _LOGGER.error("Error registering a webhook %s for %s", self.webhook_id, self.api.nvr_name)
                self.webhook_id = None
                return False
        ...
        try:
            ...
            try:
                ...
                _LOGGER.error("Error registering URL for webhook %s: HomeAssistant URL is not available.", self.webhook_id)
                webhook.async_unregister(self.webhook_id)
                self.webhook_id = None
                return False
