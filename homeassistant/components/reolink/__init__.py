...
def migrate_entity_ids(
    ...
    for device in devices:
        ...
        if host.api.supported(None, "UID") and device_uid[0] != host.unique_id:
            ...
            _LOGGER.debug("Updating Reolink device UID from %s to %s", device_uid, new_device_id)
    ...
    for entity in entities:
        ...
        if host.api.supported(None, "UID") and not entity.unique_id.startswith(
            ...
            _LOGGER.debug("Updating Reolink entity unique_id from %s to %s", entity.unique_id, new_id)
