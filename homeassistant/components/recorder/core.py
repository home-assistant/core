...
class Recorder(threading.Thread):
    ...
    def _process_state_changed_event_into_session(
        ...
        dbstate_args: DBStateArgs = {}
        add_pending_db_state = False
        ...
        if pending_states_meta := states_meta_manager.get_pending(entity_id):
            dbstate_args["states_meta_rel"] = pending_states_meta
        elif metadata_id := states_meta_manager.get(entity_id, session, True):
            dbstate_args["metadata_id"] = metadata_id
            ...
            dbstate_args["states_meta_rel"] = states_meta

        # Map the event data to the StateAttributes table
        shared_attrs = shared_attrs_bytes.decode("utf-8")
        dbstate_args["attributes"] = None
        # Matching attributes found in the pending commit
        if pending_state_attributes := state_attributes_manager.get_pending(
            shared_attrs
        ):
            dbstate_args["state_attributes"] = pending_state_attributes
        # Matching attributes id found in the cache
        elif (
            ...
            dbstate_args["attributes_id"] = attributes_id
        else:
            # No matching attributes found, save them in the DB
            ...
            dbstate_args["state_attributes"] = dbstate_attributes

        dbstate = States.from_event(event, **dbstate_args)
        if add_pending_db_state:
