...
from . import api
from . import const
from .const import DOMAIN, DATA_ZWAVE_DEVICES, DATA_ZWAVE_NETWORK
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    if discovery_info is None or DATA_ZWAVE_NETWORK not in hass.data:
        return False

    device = hass.data[DATA_ZWAVE_DEVICES].pop(
...
def setup(hass, config):
    ...
    network = hass.data[DATA_ZWAVE_NETWORK] = \
        ZWaveNetwork(options, autostart=False)
    hass.data[DATA_ZWAVE_DEVICES] = {}
...
class ZWaveDeviceEntityValues():
    ...
    def _check_entity_ready(self):
        ...
        def discover_device(component, device, dict_id):
            ...
            self._hass.data[DATA_ZWAVE_DEVICES][dict_id] = device
