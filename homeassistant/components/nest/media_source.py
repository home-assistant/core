...
class NestMediaSource(MediaSource):
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        events = self._get_events(device)
    ...
    async def async_browse_media(self, item: MediaSourceItem) -> BrowseMediaSource:
        ...
        if media_id is None:
            ...
            for device_id, child_device in devices.items():
                browse_root.children.append(
                    self._browse_device(MediaId(device_id), child_device)
                )
            return browse_root
        ...
        if media_id.event_id is None:
            ...
            browse_device = self._browse_device(media_id, device)
            ...
            for child_event in events.values():
                ...
                browse_device.children.append(
                    self._browse_event(event_id, device, child_event)
                )
        ...
        return self._browse_event(media_id, device, event)
    ...
    def _browse_device(self, device_id: MediaId, device: Device) -> BrowseMediaSource:
    ...
    def _browse_event(
        self, event_id: MediaId, device: Device, event: ImageEventBase
    ) -> BrowseMediaSource:
    ...
    def _get_events(self, device: Device) -> Mapping[str, ImageEventBase]:
