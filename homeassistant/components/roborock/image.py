...
class RoborockMap(RoborockCoordinatedEntity, ImageEntity):
    ...
    def __init__(
        ...
        if self.coordinator.api.is_available:
...
async def create_coordinator_maps(
    ...
    # This won't be None at this point as the coordinator will have run first unless we are using cached data
    if coord.api.is_available:
        assert cur_map is not None
    else:
        # If we are using cached data it doesn't really matter - and we can just do any int for sorting.
        cur_map = 0
    # Sort the maps so that we start with the current map and we can skip the
    # load_multi_map call.
    maps_info = sorted(
        coord.maps.items(), key=lambda data: data[1] == cur_map, reverse=True
    )
    for map_name, map_flag in maps_info:
        ...
        if coord.api.is_available:
            ...
            # Make it empty when we can't reach api.
            api_data = b""
        entities.append(
            RoborockMap(
