...
async def async_device_discovered(hass, config_entry, tasmota_mqtt, payload, mac):
    ...
    async_dispatcher_send(hass, TASMOTA_DISCOVERY_DEVICE, tasmota_device_config, mac)
    ...
    for component, component_key in SUPPORTED_COMPONENTS.items():
        ...
        for (idx, tasmota_entity_config) in enumerate(tasmota_entities):
            ...
            if not tasmota_entity_config:
                ...
                if entity_id:
                    _LOGGER.debug("Removing entity: %s %s", component, discovery_hash)
...
async def async_start(
    ...
    await tasmota_discovery.start_discovery(
        functools.partial(async_device_discovered, hass, config_entry, tasmota_mqtt)
    )
