"""Support for Meppel Afvalkalender."""
import asyncio
...
async def _update_meppel_afvalkalender(
    hass: HomeAssistantType, unique_id: Optional[str]
) -> None:
    """Update Meppel Afvalkalender."""
    if unique_id is not None:
        meppel_afvalkalender = hass.data[DOMAIN].get(unique_id)
        if meppel_afvalkalender is not None:
            await meppel_afvalkalender.update()
            async_dispatcher_send(hass, DATA_UPDATE, unique_id)
    else:
        tasks = []
        for meppel_afvalkalender in hass.data[DOMAIN].values():
            tasks.append(meppel_afvalkalender.update())
...
async def async_setup(hass: HomeAssistantType, config: ConfigType) -> bool:
    """Set up the Meppel Afvalkalender components."""

    async def update(call) -> None:
        """Service call to manually update the data."""
        unique_id = call.data.get(CONF_ID)
        await _update_meppel_afvalkalender(hass, unique_id)
...
async def async_setup_entry(hass: HomeAssistantType, entry: ConfigEntry) -> bool:
    """Set up Meppel Afvalkalender from a config entry."""
    ...
    hass.data.setdefault(DOMAIN, {})[unique_id] = meppel_afvalkalender
    ...
    async def _interval_update(now=None) -> None:
        """Update Meppel Afvalkalender data."""
        await _update_meppel_afvalkalender(hass, unique_id)
...
async def async_unload_entry(hass: HomeAssistantType, entry: ConfigEntry) -> bool:
    """Unload Meppel Afvalkalender config entry."""
