...
from homeassistant.const import (
    CONF_NAME, STATE_OFF, STATE_ON, STATE_IDLE, STATE_PAUSED, STATE_PLAYING, STATE_STANDBY,
    STATE_UNKNOWN,)
...
class MQTT_MEDIA(MediaPlayerDevice):
    """Representation of a MQTT_MEDIA Receiver."""

    def __init__(self, hass, name, min_volume, max_volume, power_dict, vol_dict, mute_dict, source_dict):
        ...
        mqtt.subscribe(hass, self._source_dict["status_topic"], self.message_source_selected, self._qos)
        mqtt.subscribe(hass, self._power_dict["status_topic"], self.message_power_changed, self._qos)
        mqtt.subscribe(hass, self._vol_dict["status_topic"], self.message_volume_changed, self._qos)
        mqtt.subscribe(hass, self._mute_dict["status_topic"], self.message_mute_changed, self._qos)
    ...
    def message_volume_changed(self, topic, payload, qos):
        ...
        vol=abs(self._min_volume - int(payload)) / abs(self._min_volume - self._max_volume)
        self._volume=vol
        self.update_ha_state()
    ...
    def turn_off(self):
        ...
        mqtt.publish(self.hass, self._power_dict["command_topic"], self._power_dict["payload_off"], self._power_dict["qos"], self._power_dict["retain"])

    def turn_on(self):
        """Turn the media player on."""
        mqtt.publish(self.hass, self._power_dict["command_topic"], self._power_dict["payload_on"], self._power_dict["qos"], self._power_dict["retain"])

    def volume_up(self):
        """Volume up the media player."""
        mqtt.publish(self.hass, self._vol_dict["command_up_topic"], self._vol_dict["payload"], self._vol_dict["qos"], self._vol_dict["retain"])

    def volume_down(self):
        """Volume down the media player."""
        mqtt.publish(self.hass, self._vol_dict["command_down_topic"], self._vol_dict["payload"], self._vol_dict["qos"], self._vol_dict["retain"])

    def set_volume_level(self, volume):
        """Set volume level, range 0..1."""
        vol=self._min_volume + round(abs(self._min_volume - self._max_volume) * volume)
        mqtt.publish(self.hass, self._vol_dict["command_set_topic"], vol, self._vol_dict["qos"], self._vol_dict["retain"])

    def select_source(self, source):
        """Select input source."""
        mqtt.publish(self.hass, self._source_dict["command_topic"], source, self._source_dict["qos"], self._source_dict["retain"])
    ...
    def mute_volume(self, mute):
        ...
        if self._mute:
            mqtt.publish(self.hass, self._mute_dict["command_topic"], self._mute_dict["payload_on"], self._mute_dict["qos"], self._mute_dict["retain"])
        else:
            mqtt.publish(self.hass, self._mute_dict["command_topic"], self._mute_dict["payload_off"], self._mute_dict["qos"], self._mute_dict["retain"])
