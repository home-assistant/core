...
class MerossDevice(Entity):
    ...
    def __init__(self,
        ...
        if supplementary_classifiers is not None:
            self._id = calculate_id(platform=platform, uuid=device.uuid, channel=channel,
    ...
    async def async_update(self):
        try:
            ...
            LOGGER.warning(f"async_update:{self.device.uuid} {self.device.device_type}, e: {e}")


    async def _async_push_notification_received(self,namespace: Namespace, data: dict, uuid: str):
        full_update = True
        await self.device.async_update_push_state(namespace=namespace, data=data,uuid=uuid)
        self.async_schedule_update_ha_state(force_refresh=full_update)

    async def async_added_to_hass(self) -> None:
        self.device.register_push_notification_handler_coroutine(self._async_push_notification_received)
        self.hass.data[DOMAIN]["ADDED_ENTITIES_IDS"].add(self.unique_id)

    async def async_will_remove_from_hass(self) -> None:
        self.device.unregister_push_notification_handler_coroutine(self._async_push_notification_received)
