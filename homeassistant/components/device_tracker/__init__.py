...
from homeassistant.const import (
    ...
    DEVICE_DEFAULT_NAME, STATE_HOME, STATE_NOT_HOME, ATTR_ENTITY_ID, CONF_ICON,
    ATTR_ICON)
...
def see(hass: HomeAssistantType,
        mac: str = None,
        dev_id: str = None,
        host_name: str = None,
    ...
    data = {
        key: value
        for key, value in ((ATTR_MAC, mac), (ATTR_DEV_ID, dev_id),
                           (ATTR_HOST_NAME, host_name), (ATTR_LOCATION_NAME,
                                                         location_name),
                           (ATTR_GPS, gps), (ATTR_GPS_ACCURACY,
                                             gps_accuracy), (ATTR_BATTERY,
                                                             battery))
...
class DeviceTracker(object):
    ...
    def async_setup_group(self):
        ...
        """
        entity_ids = [
            dev.entity_id for dev in self.devices.values() if dev.track
        ]
    ...
    def async_setup_tracked_device(self):
        ...
        for device in self.devices.values():
            if device.track and not device.last_seen:
                tasks.append(
                    self.hass.async_add_job(async_init_single_device(device)))
