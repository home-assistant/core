    ...
    def validate_payload(payload, data_type):
        ...
        if not isinstance(data, dict) or data.get('_type') != data_type:
            _LOGGER.debug('Skipping %s update for following data '
                          ...
                          data_type, data)
            return None
        # Apparently an "acc":0 value from OwnTracks payloads means
        # accuracy can not be measured, hence we have to ignore that edge
        # case too
        if max_gps_accuracy is not None and \
            (convert(data.get('acc'), float, 0.0) == 0.0 or
