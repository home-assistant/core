...
def get_scanner(hass, config):
    """Validate the configuration and return a HiWiFi Device Scanner."""
    scanner = HiWiFiDeviceScanner(config[DOMAIN])

    return scanner if scanner.success_init else None


class HiWiFiDeviceScanner(DeviceScanner):
    """This class queries a HiWiFi router.
    ...
    def __init__(self, config):
        ...
        self.last_results = {}
        self.token, self.cookies = _get_token(self.host, self.username, self.password)
    ...
    def _retrieve_list_with_retry(self):
        ...
        result = _retrieve_list(self.host, self.token, cookies=self.cookies)
        if result:
            _LOGGER.info("Number of devices found: %s", len(result))
            return result

        _LOGGER.info("Refreshing token and retrying device list refresh")
        self.token, self.cookies = _get_token(self.host, self.username, self.password)
        return _retrieve_list(self.host, self.token, cookies=self.cookies)
...
def _retrieve_list(host, token, **kwargs):
    ...
    url = "http://{}/cgi-bin/turbo{}/proxy/call?_initializationDatas"
    url = url.format(host, token)
    data = {"muticall": "1", "mutiargs": [{"method": "device.sta.list", "data": {}}], "lang": "zh-CN", "version": "v1"}
    try:
        res = requests.post(url, json=data, timeout=5, **kwargs)
    ...
    if hiwifi_code == 0:
        try:
            return result['data']['results'][0]['result']['data']['list']
        except KeyError:
            _LOGGER.exception("No list in response from mi router. %s", result)
            return
    else:
        _LOGGER.info(
            "Receive wrong HiWiFi code %s, expected 0 in response %s",
            hiwifi_code, result)
        return
...
def _get_token(host, username, password):
    ...
    url = 'http://{}/cgi-bin/turbo/api/login/login_admin'.format(host)
    ...
    if res.status_code == 200:
        ...
        try:
            ...
            error_message = "HiWiFi token cannot be refreshed, response from " \
                            + "url: [%s] \nwith parameter: [%s] \nwas: [%s]"
