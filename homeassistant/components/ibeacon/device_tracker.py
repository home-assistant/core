...
from .data import IBeaconScanner, signal_seen, signal_unavailable
...
async def async_setup_entry(
    ...
    ibeacon_scanner: IBeaconScanner = hass.data[DOMAIN]

    @callback
    def _async_device_new(
        unique_id: str, parsed: iBeaconAdvertisement, rssi_by_address: dict[str, int]
    ) -> None:
        """Signal a new device."""
        async_add_entities(
            [IBeaconTrackerEntity(ibeacon_scanner, unique_id, parsed, rssi_by_address)]
        )
...
class IBeaconTrackerEntity(BaseTrackerEntity):
    ...
    def __init__(
        self,
        ibeacon_scanner: IBeaconScanner,
        unique_id: str,
        ...
        self._ibeacon_scanner = ibeacon_scanner
        self._parsed = parsed
