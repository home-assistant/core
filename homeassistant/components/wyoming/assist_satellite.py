...
from homeassistant.core import HomeAssistant, HomeAssistantError, callback, Event as HassEvent
...
class WyomingAssistSatellite(WyomingSatelliteEntity, AssistSatelliteEntity):
    ...
    def on_pipeline_event(self, event: PipelineEvent) -> None:
        ...
        if event.type == assist_pipeline.PipelineEventType.RUN_START:
            ...
            self.config_entry.async_create_background_task(self.hass, self._client.write_event(Detect().event()), f"{self.entity_id} {event.type}")
            ...
            if event.data and (wake_word_output := event.data.get("wake_word_output")):
                ...
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(detection.event()), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.STT_START:
            self.device.set_is_active(True)
            if event.data:
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(Transcribe(language=event.data["metadata"]["language"]).event()), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.STT_VAD_START:
            if event.data:
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(VoiceStarted(timestamp=event.data["timestamp"]).event()), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.STT_VAD_END:
            if event.data:
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(VoiceStopped(timestamp=event.data["timestamp"]).event()), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.STT_END:
            if event.data:
                stt_text = event.data["stt_output"]["text"]
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(Transcript(text=stt_text).event()), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.INTENT_PROGRESS:
            if (event.data and event.data.get("tts_start_streaming") and self._tts_stream_token and (stream := tts.async_get_stream(self.hass, self._tts_stream_token))):
                self._is_tts_streaming = True
                self.config_entry.async_create_background_task(self.hass, self._stream_tts(stream), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.TTS_START:
            if event.data:
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(Synthesize(text=event.data["tts_input"], voice=SynthesizeVoice(name=event.data.get("voice"), language=event.data.get("language"))).event()), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.TTS_END:
            if (event.data and (tts_output := event.data.get("tts_output")) and not self._is_tts_streaming and (stream := tts.async_get_stream(self.hass, tts_output["token"]))):
                self.config_entry.async_create_background_task(self.hass, self._stream_tts(stream), f"{self.entity_id} {event.type}")
        elif event.type == assist_pipeline.PipelineEventType.ERROR:
            if event.data:
                self.config_entry.async_create_background_task(self.hass, self._client.write_event(Error(text=event.data["message"], code=event.data["code"]).event()), f"{self.entity_id} {event.type}")
        ...
        for intent in intents.intents.values():
            for intent_data in intent.data:
                for sentence in intent_data.sentences:
                    self._collect_list_references(sentence, wildcard_names)
    ...
    def _collect_list_references(self, expression: Expression, list_names: set[str]) -> None:
    ...
    async def async_announce(self, announcement: AssistSatelliteAnnouncement) -> None:
        ...
        # This method now just calls the reusable helper
    ...
    async def _run_pipeline_loop(self) -> None:
        assert self._client is not None
        client_info: Info | None = None; wake_word_phrase: str | None = None; run_pipeline: RunPipeline | None = None; send_ping = True
            ...
            async with asyncio.timeout(_PING_TIMEOUT):
                ...
                if pipeline_ended_task in done:
                    ...
                    if (run_pipeline is not None) and run_pipeline.restart_on_end:
                        ...
                        continue
                if client_event_task not in done: continue
                client_event = client_event_task.result()
                if client_event is None: raise ConnectionResetError("Satellite disconnected")
                if Pong.is_type(client_event.type): send_ping = True
    ...
    def _run_pipeline_once(self, run_pipeline: RunPipeline, wake_word_phrase: str | None = None) -> None:
        ...
        if self._is_asking_question:
            _LOGGER.debug("ask_question is active. Forcing pipeline end_stage to STT.")
            end_stage = assist_pipeline.PipelineStage.STT
        
        if start_stage is None: raise ValueError(f"Invalid start stage: {start_stage}")
        if end_stage is None: raise ValueError(f"Invalid end stage: {end_stage}")
    ...
    async def _stream_tts(self, tts_result: tts.ResultStream) -> None:
        ...
        if tts_result.extension != "wav":
            raise ValueError(f"Cannot stream audio format to satellite: {tts_result.extension}")
        total_seconds = 0.0
        start_time = time.monotonic()
        try:
            header_data = b""
            header_complete = False
            sample_rate: int | None = None; sample_width: int | None = None; sample_channels: int | None = None; timestamp = 0
            async for data_chunk in tts_result.async_stream_result():
                if not header_complete:
                    header_data += data_chunk
                    if (len(header_data) >= 44) and (audio_info := _try_parse_wav_header(header_data)):
                        sample_rate, sample_width, sample_channels, data_chunk = (audio_info)
                ...
                assert sample_rate is not None and sample_width is not None and sample_channels is not None
                audio_chunk = AudioChunk(rate=sample_rate, width=sample_width, channels=sample_channels, audio=data_chunk, timestamp=timestamp)
            ...
            self.config_entry.async_create_background_task(self.hass, self._tts_timeout(timeout_seconds, self._run_loop_id), name="wyoming TTS timeout")
    ...
    async def _tts_timeout(self, timeout_seconds: float, run_loop_id: str | None) -> None:
        await asyncio.sleep(timeout_seconds + _TTS_TIMEOUT_EXTRA)
        if run_loop_id != self._run_loop_id: return
        self.tts_response_finished()

    @callback
    def _handle_timer(self, event_type: intent.TimerEventType, timer: intent.TimerInfo) -> None:
        assert self._client is not None
    ...
    # --- This helper method is copied VERBATIM from the base AssistSatelliteEntity class ---
    async def _resolve_announcement_media_id(self, message: str, media_id: str | None, preannounce_media_id: str | None = None) -> AssistSatelliteAnnouncement:
        ...
        media_id_source: Literal["url", "media_id", "tts"] | None = None; tts_token: str | None = None
        if media_id:
            original_media_id = media_id
        else:
            media_id_source = "tts"; pipeline_id = self._resolve_pipeline(); pipeline = async_get_pipeline(self.hass, pipeline_id)
            engine = tts.async_resolve_engine(self.hass, pipeline.tts_engine)
            if engine is None: raise HomeAssistantError(f"TTS engine {pipeline.tts_engine} not found")
            tts_options: dict[str, Any] = {}; 
            if pipeline.tts_voice is not None: tts_options[tts.ATTR_VOICE] = pipeline.tts_voice
            if self.tts_options is not None: tts_options.update(self.tts_options)
            stream = tts.async_create_stream(self.hass, engine=engine, language=pipeline.tts_language, options=tts_options)
            stream.async_set_message(message)
            tts_token = stream.token; media_id = stream.url
            original_media_id = tts.generate_media_source_id(self.hass, message, engine=engine, language=pipeline.tts_language, options=tts_options)
        if media_source.is_media_source_id(media_id):
            if not media_id_source: media_id_source = "media_id"
            media = await media_source.async_resolve_media(self.hass, media_id, None)
            media_id = media.url
        if not media_id_source: media_id_source = "url"
        media_id = async_process_play_media_url(self.hass, media_id)
        if preannounce_media_id:
            if media_source.is_media_source_id(preannounce_media_id):
                preannounce_media = await media_source.async_resolve_media(self.hass, preannounce_media_id, None)
                preannounce_media_id = preannounce_media.url
            preannounce_media_id = async_process_play_media_url(self.hass, preannounce_media_id)
        return AssistSatelliteAnnouncement(
            message=message, media_id=media_id, original_media_id=original_media_id, tts_token=tts_token,
            media_id_source=media_id_source, preannounce_media_id=preannounce_media_id
        )
