...
from .raw_parser import ElectricityValue
...
class _RawElectricityValueWrapper(DPCodeBase64Wrapper):
    """Custom DPCode Wrapper for extracting ElectricityValue from base64."""

    def _convert(self, value: ElectricityValue) -> float:
    ...
    def read_device_status(self, device: CustomerDevice) -> float | None:
        ...
        if (raw_value := super().read_bytes(device)) is None or (
            value := ElectricityValue.from_bytes(raw_value)
        ) is None:
            return None
        return self._convert(value)


class _RawElectricityCurrentWrapper(_RawElectricityValueWrapper):
    ...
    def _convert(self, value: ElectricityValue) -> float:
        """Extract specific value from ElectricityValue."""
        return value.current


class _RawElectricityPowerWrapper(_RawElectricityValueWrapper):
...
class _RawElectricityVoltageWrapper(_RawElectricityValueWrapper):
