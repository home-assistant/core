...
class TuyaHaClimate(TuyaHaDevice, ClimateEntity):
    ...
    def set_fan_mode(self, fan_mode):
        ...
        self.tuyaDeviceManager.sendCommands(
            self.tuyaDevice.id, [{"code": DPCODE_FAN_SPEED_ENUM, "value": fan_mode}]
        )

    def set_humidity(self, humidity):
        """Set new target humidity."""
        response = self.tuyaDeviceManager.sendCommands(
            self.tuyaDevice.id, [{"code": DPCODE_HUMIDITY_SET, "value": int(humidity)}]
        )
    ...
    def set_temperature(self, **kwargs):
        ...
        response = self.tuyaDeviceManager.sendCommands(
            self.tuyaDevice.id, [{"code": code, "value": int(kwargs["temperature"])}]
        )
        if response.get("success", False):
    ...
    def current_humidity(self) -> int:
        ...
        return int(self.tuyaDevice.status.get(DPCODE_HUMIDITY_CURRENT, 0))
    ...
    def hvac_modes(self) -> List:
        ...
        modes = json.loads(self.tuyaDevice.function.get(DPCODE_MODE, {}).values).get(
            "range"
        )
    ...
    def fan_modes(self) -> List[str]:
        ...
        data = json.loads(
            self.tuyaDevice.function.get(DPCODE_FAN_SPEED_ENUM, {}).values
        ).get("range")
        return data
    ...
    def swing_mode(self) -> str:
        ...
        mode = 0
        if (
            DPCODE_SWITCH_HORIZONTAL in self.tuyaDevice.status
            and self.tuyaDevice.status.get(DPCODE_SWITCH_HORIZONTAL)
        ):
            mode += 1
        if (
            DPCODE_SWITCH_VERTICAL in self.tuyaDevice.status
            and self.tuyaDevice.status.get(DPCODE_SWITCH_VERTICAL)
        ):
            mode += 2
    ...
    def supported_features(self):
        ...
        supports = 0
        if (
            DPCODE_TEMP_SET in self.tuyaDevice.status
            or DPCODE_TEMP_SET_F in self.tuyaDevice.status
        ):
        ...
        if (
            DPCODE_SWITCH_HORIZONTAL in self.tuyaDevice.status
            or DPCODE_SWITCH_VERTICAL in self.tuyaDevice.status
        ):
