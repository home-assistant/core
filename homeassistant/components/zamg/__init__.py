...
from .const import CONF_STATION_ID, LOGGER
from .coordinator import ZamgConfigEntry, ZamgDataUpdateCoordinator

PLATFORMS = (Platform.SENSOR, Platform.WEATHER)


async def async_setup_entry(hass: HomeAssistant, entry: ZamgConfigEntry) -> bool:
...
async def async_unload_entry(hass: HomeAssistant, entry: ZamgConfigEntry) -> bool:
    """Unload ZAMG config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def _async_migrate_entries(
    hass: HomeAssistant, config_entry: ZamgConfigEntry
) -> bool:
    """Migrate old entry."""
