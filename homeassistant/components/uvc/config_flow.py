...
def _get_data_schema(user_input: dict) -> vol.Schema:
    user_input = user_input or {}
    return vol.Schema(
        {
            vol.Required(CONF_HOST, default=user_input.get(CONF_HOST)): str,
            vol.Required(
                CONF_PORT, default=user_input.get(DEFAULT_PORT) or DEFAULT_PORT
            ): int,
            vol.Required(
                CONF_SSL, default=user_input.get(DEFAULT_SSL) or DEFAULT_SSL
            ): bool,
            vol.Required(CONF_API_KEY, default=user_input.get(CONF_API_KEY)): str,
            vol.Required(
                CONF_PASSWORD,
                default=user_input.get(DEFAULT_PASSWORD) or DEFAULT_PASSWORD,
            ): str,
        }
    )
