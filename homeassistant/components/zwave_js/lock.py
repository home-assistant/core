...
from . import access_control_helpers
...
class ZWaveLock(ZWaveBaseEntity, LockEntity):
    ...
    async def async_set_user(
        ...
        try:
            await access_control_helpers.async_set_user(
                self.info.node,
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="set_user_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_clear_user(self, user_index: int) -> None:
        """Delete a single access-control user from the lock."""
        try:
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="clear_user_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_clear_all_users(self) -> None:
        """Delete all access-control users from the lock."""
        try:
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="clear_all_users_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_get_credential_capabilities(self) -> ServiceResponse:
        """Query credential management capabilities of the lock."""
        try:
            return await access_control_helpers.async_get_credential_capabilities(
                self.info.node
            )
        except HomeAssistantError:
            raise
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="get_credential_capabilities_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_get_users(self) -> ServiceResponse:
        """List all access-control users on the lock."""
        try:
            return await access_control_helpers.async_get_users(self.info.node)
        except HomeAssistantError:
            raise
        except Exception as err:
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="get_users_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_set_credential(
        ...
        """Add or update a credential on the lock."""
        try:
    ...
    async def async_clear_credential(
        ...
        """Delete a single credential from the lock."""
        try:
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="clear_credential_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_clear_all_credentials(self, user_index: int) -> None:
        """Delete all credentials for a user on the lock."""
        try:
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="clear_all_credentials_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
    ...
    async def async_get_credential_status(
        ...
        """Query the status of a credential slot on the lock."""
        try:
            ...
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="get_credential_status_failed",
                translation_placeholders={
                    "entity_id": self.entity_id,
                    "error": str(err),
                },
