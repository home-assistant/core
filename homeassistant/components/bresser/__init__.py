...
type BresserConfigEntry = ConfigEntry[CCLDevice]


async def async_setup_entry(hass: HomeAssistant, entry: BresserConfigEntry) -> bool:
    """Set up a config entry for a single Bresser device."""
    new_device = CCLDevice(entry.data[CONF_WEBHOOK_ID])
    new_coordinator = entry.runtime_data = BresserCoordinator(hass, new_device)
    ...
    async def register_webhook() -> None:
        ...
        def handle_webhook(
            ...
            """Handle incoming requests from Bresser devices."""
    ...
    entry.async_create_background_task(
        hass, register_webhook(), "bresser_register_webhook"
    )
...
async def async_unload_entry(hass: HomeAssistant, entry: BresserConfigEntry) -> bool:
