...
from homeassistant.components.rfxtrx import (ATTR_AUTOMATIC_ADD, ATTR_NAME,
                                             ATTR_OFF_DELAY, ATTR_FIREEVENT,
                                             ATTR_DATA_BITS, CONF_DEVICES)
from homeassistant.const import (CONF_DEVICE_CLASS, CONF_COMMAND_ON,
                                 CONF_COMMAND_OFF)
...
def setup_platform(hass, config, add_devices_callback, discovery_info=None):
    ...
    for packet_id, entity in config['devices'].items():
        ...
        device = RfxtrxBinarySensor(
            ...
            entity[ATTR_FIREEVENT], entity[ATTR_OFF_DELAY],
            entity[ATTR_DATA_BITS], entity[CONF_COMMAND_ON],
    ...
    def binary_sensor_update(event):
        ...
        if sensor is None:
            ...
            if poss_dev is not None:
                ...
                _LOGGER.info("Found possible matching deviceid %s.", poss_id)
            ...
            _LOGGER.info("Added binary sensor %s "
                         "(Device_id: %s Class: %s Sub: %s)", pkt_id,
