...
class FanEntity(ToggleEntity):
    ...
    async def async_increase_speed(self, percentage_step: Optional[float]) -> None:
        """Increase the speed of the fan."""
        await self._async_adjust_speed(1, percentage_step)

    async def async_decrease_speed(self, percentage_step: Optional[float]) -> None:
        """Decrease the speed of the fan."""
        await self._async_adjust_speed(-1, percentage_step)

    async def _async_adjust_speed(
        self, modifier: int, percentage_step: Optional[float]
    ) -> None:
