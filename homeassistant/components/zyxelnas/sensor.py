...
ATTRIBUTION = 'Data provided by Zyxel'

CONF_VOLUMES = 'volumes'
DEFAULT_NAME = 'ZyxelNAS'
DEFAULT_PORT = 80
...
_UTILISATION_MON_COND = {
    ...
    'memory_available_real': ['Memory Available', 'Mb', 'mdi:memory'],
    'network_up': ['Network Up', 'Kbps', 'mdi:upload'],
    'network_down': ['Network Down', 'Kbps', 'mdi:download'],
    'system_status': ['System Status', None, 'mdi:doctor'],
    'system_status_fan': ['Fan Speed', 'RPM', 'mdi:fan'],
    'system_status_temp': ['CPU Temp', '°C', 'mdi:thermometer'],
}
_STORAGE_VOL_MON_COND = {
    'volume_status': ['Status', None, 'mdi:checkbox-marked-circle-outline'],
    'volume_device_type': ['Volume Type', None, 'mdi:harddisk'],
    'volume_size_total': ['Volume Total Size', None, 'mdi:chart-pie'],
    'volume_size_used': ['Volume Used Space', None, 'mdi:chart-pie'],
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    def run_setup(event):
        ...
        api = ZyxelAPI(host, port, username, password, unit, use_ssl)

        sensors = [ZyxelNASUtilSensor(
...
class ZyxelAPI:
    """Class to interface with ZyxelNAS API."""
    ...
    def __init__(self, host, port, username, password, temp_unit, use_ssl):
        ...
        try:
            self._api = ZyxelNAS(host, port, username, password,
                                 use_https=use_ssl)
        except:  # noqa: E722 pylint: disable=bare-except
            _LOGGER.error("Error setting up ZyxelNAS")
...
class ZyxelNASSensor(Entity):
    """Representation of a ZyxelNAS Sensor."""

    def __init__(self, api, variable, variable_info, monitor_device=None):
        """Initialize the sensor."""
        self.var_id = variable
        self.var_name = DEFAULT_NAME + "_" + variable_info[0]
...
class ZyxelNASUtilSensor(ZyxelNASSensor):
    """Representation a Zyxel Utilisation Sensor."""

    @property
    def state(self):
        """Return the state of the sensor."""
        network_sensors = ['network_up', 'network_down']
        memory_sensors = ['memory_size', 'memory_real_usage',
...
class ZyxelNASStorageSensor(ZyxelNASSensor):
    """Representation a Zyxel Utilisation Sensor."""
