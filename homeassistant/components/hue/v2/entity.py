...
from ..const import DOMAIN as HUE_DOMAIN, LOGGER
...
class HueBaseEntity(Entity):
    ...
    def __init__(
        ...
        self._attr_device_info = DeviceInfo(
            identifiers={(HUE_DOMAIN, self.device.id)},
        )
    ...
    def _handle_event(self, event_type: EventType, resource: CLIPResource) -> None:
        ...
        if event_type == EventType.RESOURCE_DELETED and resource.id == self.resource.id:
            LOGGER.debug("Received delete for %s", self.entity_id)
            ...
            LOGGER.debug("Received status update for %s", self.entity_id)
            self.on_update()
