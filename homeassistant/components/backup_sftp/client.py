...
class AsyncFileIterator:
    ...
    as context manager.
    """
    def __init__(self, cfg: "SFTPConfigEntryData", file_path: str, buffer_size: int = BUF_SIZE) -> None:
        ...
        LOGGER.warning(
            "Opening file: %s in Async File Iterator ...",
            file_path
        )
    ...
    async def _initialize(self) -> None:
        ...
        self._fileobj = await self._client._sftp.open(self.file_path, "rb")
...
class BackupAgentClient:
    ...
    def __init__(self, config: "SFTPConfigEntryData") -> None:
        if (bool(config.private_key_file) is False) and (bool(config.password) is False):
    ...
    async def __aenter__(self) -> "BackupAgentClient":
        """Configures SSH and SFTP Client connections on start of async context manager."""
        ...
        self._ssh = await connect(
            ...
            options=SSHClientConnectionOptions(
                ...
                client_keys = [self.cfg.private_key_file] if self.cfg.private_key_file else None
            )
        )
    ...
    async def __aexit__(self, exc_type, exc, traceback) -> None:
        ...
        if self._sftp:
            await self._sftp.__aexit__(exc_type, exc, traceback)
    ...
    def _initialized(self, file_path: str | None = None) -> None:
        """
        Checks if SSH Connection is initialized.
        ...
        if file_path and not file_path.startswith(self.cfg.backup_location):
            raise RuntimeError(f"Attempted to access file outside of configured backup location: {file_path}")

    async def async_delete_backup(self, backup: AgentBackup) -> None:
        """Delete backup archive."""

        assert backup is not None, "None object passed to `BackupAgentClient.async_delete_backup`."

        file_path = f"{self.cfg.backup_location}/{suggested_filename(backup)}"
        assert await self._sftp.exists(file_path), f"File at provided remote path: {file_path} does not exist."
        assert await self._sftp.isfile(file_path), f"Provided remote path: {file_path} is not file."

        LOGGER.debug("Removing file at path: %s", file_path)
        await self._sftp.unlink(file_path)
    ...
    async def async_list_backups(self) -> list[AgentBackup]:
        ...
        for file in await self.list_backup_location():
            ...
            async with self._sftp.open(file, "rb") as rfile:
                adapter = AsyncSSHFileWrapper(rfile, loop)
                try:
                    metadata = await loop.run_in_executor(None, process_tar_from_adapter, adapter, file)
                    if metadata is None:
                        continue
                except Exception as e:
                    LOGGER.exception(e)
                    LOGGER.error(
                        "Failed to load backup metadata from file: %s. Ignoring.",
                        file
                    )
                    continue
    ...
    async def async_upload_backup(
        ...
        assert backup is not None, "None object passed to `BackupAgentClient.async_upload_backup`."
        file_path = f"{self.cfg.backup_location}/{suggested_filename(backup)}"
        async with self._sftp.open(file_path, "wb") as f:
    ...
    async def iter_file(self, backup: AgentBackup) -> AsyncFileIterator:
        """Returns Async File Iterator object, that iterates"""

        assert backup is not None, "None object passed to `BackupAgentClient.iter_file`."
        file_path = f"{self.cfg.backup_location}/{suggested_filename(backup)}"
        if not await self._sftp.isfile(file_path):
            raise BackupAgentError("Backup not found.")
        return AsyncFileIterator(self.cfg, file_path, BUF_SIZE)

    async def list_backup_location(self) -> list[str]:
        """Returns .tar files located in backup location."""
        self._initialized()
        files = []
        LOGGER.debug("Changing directory to: `%s`", self.cfg.backup_location)
        await self._sftp.chdir(self.cfg.backup_location)

        for file in await self._sftp.listdir():
            LOGGER.debug(
    ...
    @asynccontextmanager
    async def open(self, file_path: str, mode: str = "rb") -> AsyncGenerator[SFTPClientFile]:
        """Open a remote file
        ...
        async with self._sftp.open(file_path, mode) as f:
            LOGGER.debug("Opening `%s` in `%s` mode.", file_path, mode)
            yield f

    async def rename(self, file_path: str, new_name: str) -> str:
        """Renames `file_path` to `new_name`. Returns full path to new file name."""
        self._initialized(file_path=file_path)
        ...
        await self._sftp.rename(file_path, new_name)
        return new_name
