...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    add_devices([TeamspeakSensor(name,host,username,password,virtualserver_id)])
...
class TeamspeakSensor(Entity):
    ...
    def update(self):
        ...
        if not self._ts3conn.is_connected():
            if not self.connect():
                return
        usersOnline = int(self._ts3conn.serverlist()[self._virtual_server_id]['virtualserver_clientsonline']) - int(self._ts3conn.serverlist()[self._virtual_server_id]['virtualserver_queryclientsonline'])
