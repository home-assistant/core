...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    _RESOURCE = 'https://www.worldtides.info/api?extremes&length=86400&start=%s&lat=%s&lon=%s&key=%s' % (
    ...
    def state(self):
        ...
        if self.data.data:
            if "High" in str(self.data.data['extremes'][0]['type']):
                tidetime = time.strftime('%I:%M %p', time.localtime(self.data.data['extremes'][0]['dt']))
                return "High tide at %s" % (tidetime)
            elif "Low" in str(self.data.data['extremes'][0]['type']):
                tidetime = time.strftime('%I:%M %p', time.localtime(self.data.data['extremes'][1]['dt']))
...
class WorldTidesInfoData(object):
    ...
    def update(self):
        ...
        try:
            ...
            _LOGGER.debug("Tide data queried with start time set to: %s", (STARTTIME))
