...
def setup_platform(hass, config, add_devices_callback, discovery_info=None):
    ...
    calib_value = util.convert(config.get(CONF_CALIBRATION_FACTOR), float, None)
    
    if None in (indoor_temp_sensor, outdoor_temp_sensor, indoor_humidity_sensor):
        _LOGGER.error('Missing required key %s, %s or %s', 
                        CONF_INDOOR_TEMP, CONF_OUTDOOR_TEMP, CONF_INDOOR_HUMIDITY)
        return False
        
    add_devices_callback([MoldIndicator(hass, name, indoor_temp_sensor, outdoor_temp_sensor,
...
class MoldIndicator(Entity):
    ...
    def __init__(self, hass, name, indoor_temp_sensor, outdoor_temp_sensor, indoor_humidity_sensor, calib_value):
        self._state = "-"
        self._hass = hass
        self._name = name
        ...
        # Read initial state
        self._sensor_changed(indoor_temp_sensor, None, hass.states.get(indoor_temp_sensor))
        self._sensor_changed(outdoor_temp_sensor, None, hass.states.get(outdoor_temp_sensor))
        self._sensor_changed(indoor_humidity_sensor, None, hass.states.get(indoor_humidity_sensor))
    ...
    def _update_temp_sensor(self, state):
        ...
        if unit == TEMP_FAHRENHEIT:
            ...
            _LOGGER.error('Temp sensor has unsupported unit: %s (allowed: %s, %s)',
    ...
    def _update_hum_sensor(self, state):
        ...
        # check unit
        if unit != "%":
            _LOGGER.error('Humidity sensor has unsupported unit: %s (allowed: %)',
                          unit)

        # check range
        if hum > 100 or hum < 0:
            _LOGGER.error('Humidity sensor out of range: %s (allowed: 0-100%)',
                          hum)
    ...
    def _calc_dewpoint(self):
        ...
        _LOGGER.debug('Dewpoint: %f °C', self._dewpoint)

    def _calc_moldindicator(self):
        """ Calculates the humidity at the (cold) calibration point """

        if None in (self._dewpoint, self._calib_value) or self._calib_value == 0:
            _LOGGER.debug('Invalid inputs - dewpoint: %s, calibration-Value: %s',
                           self._dewpoint, self._calib_value)
            self._state = "-"
            return

        # first calculate the approximate temperature at the calibration point
        self._wallTemp = self._outdoor_temp + (self._indoor_temp - self._outdoor_temp) / self._calib_value
        _LOGGER.debug('Wall temperature: %f °C', self._wallTemp)

        # Then calculate the humidity at this point
        alpha = MAGNUS_K2 * self._wallTemp / (MAGNUS_K3 + self._wallTemp)
        beta = MAGNUS_K2 * MAGNUS_K3 / (MAGNUS_K3 + self._wallTemp)

        wallHum = math.exp((self._dewpoint * beta - MAGNUS_K3 * alpha) / (self._dewpoint + MAGNUS_K3)) * 100.0
        ...
        if wallHum > 100:
            ...
            self._state = '{0:.2f}'.format(wallHum)
    ...
    def state_attributes(self):
        ...
        if self._dewpoint:
            return {
                ATTR_DEWPOINT: '{0:.2f}'.format(self._dewpoint) + ' ' + TEMP_CELCIUS,
                ATTR_WALLTEMP: '{0:.2f}'.format(self._wallTemp) + ' ' + TEMP_CELCIUS,
            }
