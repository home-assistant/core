...
from . import GarniConfigEntry
from .coordinator import GarniCoordinator
from .entity import GarniEntity

PARALLEL_UPDATES = 0

GARNI_SENSOR_DESCRIPTIONS: dict[str, SensorEntityDescription] = {
...
async def async_setup_entry(
    ...
    def _new_sensor(sensor: CCLSensor) -> None:
        ...
        entity_description = dataclasses.replace(
            GARNI_SENSOR_DESCRIPTIONS[sensor.sensor_type],
            key=sensor.key,
            name=sensor.name,
        )
        async_add_entities([GarniSensorEntity(coordinator, entity_description)])
...
class GarniSensorEntity(GarniEntity, SensorEntity):
    ...
    def __init__(
        ...
        """Initialize a GARNI Sensor Entity."""
