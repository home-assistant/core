...
class IndevoltNumberEntity(CoordinatorEntity[IndevoltCoordinator], NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        """Set new value."""
        try:
            await self.coordinator.async_push_data(self._get_write_key(), int(value))
...
class DischargeLimitNumber(IndevoltNumberEntity):
    ...
    def _get_write_key(self) -> str:
        """Get the data point (key)  for writing Emergency Power value."""
        return "1142"
...
class MaxACOutputPowerNumber(IndevoltNumberEntity):
    ...
    def _get_write_key(self) -> str:
        """Get the data point (key)  for writing Max AC Output Power value."""
        return "1147"
...
class InverterInputLimit(IndevoltNumberEntity):
    ...
    def _get_write_key(self) -> str:
        """Get the data point (key) for writing Inverter Input Limit value."""
        return "1138"
...
class FeedinPowerLimit(IndevoltNumberEntity):
    ...
    def _get_write_key(self) -> str:
        """Get the data point (key) for writing Feed-in Power Limit value."""
        return "1146"
