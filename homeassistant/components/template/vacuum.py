...
class TemplateStateVacuumEntity(TemplateEntity, AbstractTemplateVacuum):
    ...
    def _update_state(self, result):
        ...
        self._attr_activity = tcv.enum(
            self, CONF_STATE, VacuumActivity, none_on_unknown_unavailable=True
        )(result)
...
class TriggerVacuumEntity(TriggerEntity, AbstractTemplateVacuum):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if (rendered := self._rendered.get(CONF_STATE)) is not None:
            self._attr_activity = tcv.enum(self, CONF_STATE, VacuumActivity)(rendered)
        for key, updater in (
