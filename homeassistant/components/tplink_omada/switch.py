    ...
    if gateway_coordinator:
        for gateway in gateway_coordinator.data.values():
            entities.extend(
                ...
                ](gateway_coordinator, gateway, p.port_number, str(p.port_number), desc)
...
class OmadaDevicePortSwitchEntity(
    ...
    async def _async_turn_on_off(self, enable: bool) -> None:
        ...
        if self.entity_description.refresh_after_set:
    ...
    def _do_update(self) -> None:
        port = self.entity_description.coordinator_update_func(
