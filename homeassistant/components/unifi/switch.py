...
Support for controlling power supply of ports that provide Power over Ethernet (PoE).
...
from .unifi_port import UniFiPort, find_poe_ports
...
async def async_setup_entry(
    ...
    def items_added(
        ...
        add_poe_entities(controller, async_add_entities, devices)
...
@callback
def add_poe_entities(controller, async_add_entities, devices):
    """Add new switch entities from the controller."""
    switches = []

    for device, port in find_poe_ports(controller, devices):
        switches.append(UniFiPoEPortSwitch(device, port, controller))
...
class UniFiPoEPortSwitch(UniFiPort, SwitchEntity):
    ...
    @property
    def name(self) -> str:
        """Return the name for the switch."""
        return f"{super().name} PoE"
    ...
    def available(self):
        ...
        """
        return super().available and self.port.poe_mode is not None

    async def async_turn_on(self, **kwargs):
        """Enable PoE for port."""
        await self.device.set_port_poe_mode(self.port.port_idx, "auto")

    async def async_turn_off(self, **kwargs):
        """Disable PoE for port."""
        await self.device.set_port_poe_mode(self.port.port_idx, "off")
