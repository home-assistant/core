...
CONF_PAYLOAD_OTHER_ZONES = "payload_other_zones"
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(mqtt.SCHEMA_BASE).extend(
    {
        ...
        vol.Optional(CONF_PAYLOAD_OTHER_ZONES, default=dict()): {cv.string: cv.string},
...
async def async_setup_scanner(hass, config, async_see, discovery_info=None):
    ...
    qos = config[CONF_QOS]
    payload_other_zones = config[CONF_PAYLOAD_OTHER_ZONES]
    ...
    for dev_id, topic in devices.items():
        ...
        def async_message_received(msg, dev_id=dev_id):
            ...
            elif msg.payload in payload_other_zones:
                location_name = payload_other_zones[msg.payload]
            else:
