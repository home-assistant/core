...
class MqttLight(MqttEntity, LightEntity, RestoreEntity):
    ...
    async def _subscribe_topics(self):  # noqa: C901
        ...
        restore_state(ATTR_RGB_COLOR, self._optimistic_rgb)
        if (
    ...
    async def async_turn_on(self, **kwargs):
        ...
        def set_optimistic(attribute, value, condition):
            if not condition:
                return False
        ...
        if (
            ...
            and self.supported_color_modes is None
        ):
            # Convert HS to RGB
            rgb = scale_rgbx(color_util.color_hsv_to_RGB(*kwargs[ATTR_HS_COLOR], 100))
            rgb_s = render_rgbx(rgb, CONF_RGB_COMMAND_TEMPLATE)
            publish(CONF_RGB_COMMAND_TOPIC, rgb_s)
            should_update = set_optimistic(
                ATTR_HS_COLOR, kwargs[ATTR_HS_COLOR], self._optimistic_rgb
            )
        ...
        if ATTR_HS_COLOR in kwargs and self._topic[CONF_HS_COMMAND_TOPIC] is not None:
            ...
            should_update = set_optimistic(ATTR_HS_COLOR, hs_color, self._optimistic_hs)
        ...
        if (
            ...
            should_update = set_optimistic(ATTR_HS_COLOR, hs_color, self._optimistic_xy)
        ...
        if (
            ...
            should_update = set_optimistic(
                ATTR_BRIGHTNESS, kwargs[ATTR_BRIGHTNESS], self._optimistic_brightness
            )
        elif (
            ATTR_BRIGHTNESS in kwargs
        ...
        if ATTR_EFFECT in kwargs and self._topic[CONF_EFFECT_COMMAND_TOPIC] is not None:
            ...
            if effect in self._config.get(CONF_EFFECT_LIST):
                ...
                should_update = set_optimistic(
                    ATTR_EFFECT, effect, self._optimistic_effect
                )
        ...
        if (
            ...
            should_update = set_optimistic(
                ATTR_WHITE_VALUE, kwargs[ATTR_WHITE_VALUE], self._optimistic_white_value
            )
