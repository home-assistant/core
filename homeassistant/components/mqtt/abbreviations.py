...
def check_abbreviations_file(path, regex):
    ...
    with open(path) as file:
        ...
        for match in matches:
            if (match[1] not in ABBREVIATIONS.values() and
                    ...
                    match[0] not in WHITE_LIST):
                print("{}: no abbreviation for {} ({})".format(
    ...
    for root, _, fnames in os.walk(script_dir):
        for fname in fnames:
            if fname.endswith('.py'):
                check_abbreviations_file(os.path.join(root, fname), regex)
