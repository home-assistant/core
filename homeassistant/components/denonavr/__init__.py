...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry.options.get(CONF_USE_TELNET, DEFAULT_USE_TELNET):
        receiver: DenonAVR = hass.data[DOMAIN][entry.entry_id][CONF_RECEIVER]
        await receiver.async_telnet_disconnect()
    ...
    for registry_entry in registry_entries:
        if registry_entry.unique_id == zone2_id and not entry.options.get(CONF_ZONE2):
            entity_registry.async_remove(registry_entry.entity_id)
            _LOGGER.debug("Removing zone2 from DenonAvr")
        if registry_entry.unique_id == zone3_id and not entry.options.get(CONF_ZONE3):
            entity_registry.async_remove(registry_entry.entity_id)
