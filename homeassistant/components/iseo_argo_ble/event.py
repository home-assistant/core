...
class IseoAccessLogEvent(EventEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        # outran the wait for a long read — is reported rather than lost. Not
        # from here: states written before the entity has finished being added
        # are dropped, so several held entries would collapse into the single
        # write that follows this method and only the last would be seen.
        if self.hass.data.get(PENDING_LOG_ENTRIES, {}).get(self._entry.entry_id):
            self._replay_task = self.hass.async_create_task(
                self._async_report_pending(), eager_start=False
            )
            self.async_on_remove(self._cancel_replay)

    @callback
    def _cancel_replay(self) -> None:
        """Stop a replay this entity will no longer be here to write."""
        if self._replay_task is not None:
            self._replay_task.cancel()

    async def _async_report_pending(self) -> None:
        """Report what was drained while this entity was away.

        Taken here rather than when the replay is scheduled: an entity removed
        before this runs is cancelled, and the entries have to survive that —
        they are already marked read on the lock.
        """
        pending = self.hass.data.get(PENDING_LOG_ENTRIES, {}).pop(
            self._entry.entry_id, None
        )
        for event_type, attributes in pending or ():
