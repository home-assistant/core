...
async def async_setup_entry(
    ...
    if device.api.type == "LB1":
        lights.append(BroadlinkLight1(device))

    if device.api.type == "LB2":
        lights.append(BroadlinkLight2(device))

    async_add_entities(lights)


class BroadlinkLight1(BroadlinkEntity, LightEntity):
...
class BroadlinkLight2(BroadlinkEntity, LightEntity):
    """Representation of a Broadlink light."""
    ...
    def _update_state(self, data):
        ...
        if "colortemp" in data:
            self._attr_color_temp = round(500 - (data["colortemp"] - 2700) / 10.951)
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if self.supported_color_modes == {COLOR_MODE_BRIGHTNESS}:
            ...
            color_temp = kwargs[ATTR_COLOR_TEMP]
            state["colortemp"] = (500 - color_temp) * 10.951 + 2700
            state["bulb_colormode"] = BROADLINK_COLOR_MODE_WHITE
