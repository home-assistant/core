...
class LevelWebsocketManager:
    ...
    def __init__(
        ...
        *,
        ...
        on_integration_removed: Callable[[str], Awaitable[None]] | None = None,
    ) -> None:
        ...
        self._on_integration_removed = on_integration_removed
    ...
    async def _run_connection(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if self._stop_event.is_set():
                break
            # Detect auth failure from server-initiated close (4401/4403)
            if close_code in (4401, 4403) or auth_failed:
                LOGGER.warning(
                    "WebSocket auth failure detected (close_code=%s)", close_code
                )
                if self._on_auth_failure is not None:
                    await self._on_auth_failure()
                break
            # Notify connection lost if we were previously connected
    ...
    async def _handle_text_message(self, data: str) -> None:
        ...
        if msg_type == "device_state_changed":
            ...
            if device_uuid and device_state:
                ...
                await self._on_state_update(device_uuid, is_locked, state_payload)
            return
        if msg_type == "integration_removed":
            reason = payload.get("reason", "unknown")
            LOGGER.warning("Server sent integration_removed: reason=%s", reason)
            if self._on_integration_removed is not None:
                await self._on_integration_removed(reason)
            return
