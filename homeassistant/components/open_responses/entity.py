...
def _adjust_schema(schema: dict[str, Any]) -> None:
    """Adjust the output schema to be compatible with the Responses API."""
...
def _format_structured_output(
    ...
    """Format the schema to be compatible with the Responses API."""
        ...
        for _iteration in range(max_iterations):
            try:
                ...
                if (
                    ...
                    LOGGER.error(
                        "Insufficient funds for Open Responses endpoint: %s", err
                    )
                    raise HomeAssistantError(
                        "Insufficient funds for Open Responses endpoint"
                    ) from err
    ...
    def append_files_to_content() -> ResponseInputMessageContentListParam:
        ...
        for file_path, mime_type in files:
            ...
            if not mime_type or not mime_type.startswith(("image/", "application/pdf")):
                raise HomeAssistantError(
                    "Only images and PDF are supported by the Responses API,"
