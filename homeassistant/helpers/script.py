...
class _ScriptRun:
    ...
    async def _async_step(self, log_exceptions: bool) -> None:
        ...
        with trace_path(str(self._step)):
            async with trace_action(
                self._hass, self, self._stop, self._variables.parallel_scope
            ) as trace_element:
                if self._stop.done():
                    return
                ...
                try:
                    ...
                    trace_element.update_variables(self._variables.parallel_scope)
    ...
    async def async_step_set_variables(self) -> None:
...
SCRIPT_STEPS = {
    ...
    cv.SCRIPT_ACTION_SET_VARIABLES: _ScriptRun.async_step_set_variables,
