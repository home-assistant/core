...
class ScriptRunVariables(UserDict[str, Any]):
    ...
    _parallel_scope: ChainMap[str, Any]
    ...
    def create_top_level(
        ...
        parallel_scope = full_scope = ChainMap(local_store)
        variables = cls(
            _local_store=local_store,
            _parallel_scope=parallel_scope,
    ...
    def enter_scope(self, parallel: bool = False) -> ScriptRunVariables:
        ...
        if not parallel:
            ...
            parallel_scope = self._parallel_scope
            full_scope = self._full_scope
        else:
            parallel_store = ({}, {})
            parallel_scope = ChainMap(parallel_store[1])
    ...
    def _ensure_local(self) -> None:
        if self._local_store is None:
            self._local_store = {}
            self._parallel_scope = self._parallel_scope.new_child(self._local_store)
    ...
    @property
    def parallel_scope(self) -> Mapping[str, Any]:
        """Return variables in parallel scope."""
        return self._parallel_scope
