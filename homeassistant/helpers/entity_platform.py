...
class EntityPlatform:
    ...
    async def _async_add_entity(
        ...
        if cast(str | None, entity.entity_id) is not None:
            if not valid_entity_id(entity.entity_id):
                entity.add_to_platform_abort()
                raise HomeAssistantError(f"Invalid entity ID: {entity.entity_id}")
            entity._overridden_object_id = split_entity_id(entity.entity_id)[1]  # noqa: SLF001
        ...
        if entity.unique_id is not None:
            ...
            # if the entity is already registered, as it will be ignored.
            if not registered_entity_id:
                suggested_object_id = _async_get_suggested_object_id(entity)
            ...
            if entity.entity_id is None or (
...
@callback
def _async_get_suggested_object_id(entity: Entity) -> str:
    """Get suggested object ID for an entity."""
    suggested_object_id = entity.suggested_full_object_id

    platform = entity.platform

    if not suggested_object_id:
        if entity.unique_id is not None and platform is not None:
...
@callback
def async_get_suggested_object_id(hass: HomeAssistant, entity_id: str) -> str:
    """Get suggested object id for an already existing entity.

    Raises HomeAssistantError if the entity is not in the registry or
    is not backed by an entity object.

    If user has renamed the entity, that name is prioritized.
    Otherwise, the suggested object id is calculated.
    """
    ...
    domain_entities = hass.data.get(DATA_DOMAIN_ENTITIES, {}).get(entity_entry.domain)
    if not (entity_obj := domain_entities.get(entity_id) if domain_entities else None):
        raise HomeAssistantError(f"Entity {entity_id} has no object.")

    return _async_get_suggested_object_id(entity_obj)
