...
- Set `PARALLEL_UPDATES` explicitly in every entity platform file: `0` for read-only platforms of coordinator-based integrations and for action platforms whose client already serializes its own requests; a bounded value (typically `1`) for any other action platform. (Silver: `parallel-updates`)
