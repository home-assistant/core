...
- Create the client in `async_setup_entry()` and store it on the typed `entry.runtime_data`, not `hass.data` — except a push-only integration (webhook, subscription) with nothing to hold between calls, which can legitimately have no coordinator or runtime data at all. If the integration has shared polled data, use a `DataUpdateCoordinator` in `coordinator.py` with a shared base entity in `entity.py`, and type the coordinator's `config_entry`. (Bronze: `runtime-data`, `common-modules`)
...
- During setup, raise `ConfigEntryNotReady` for transient failures (offline device, timeout), `ConfigEntryAuthFailed` for invalid credentials or another account state that needs user action to restore access (an expired subscription, a revoked or insufficient scope) — it starts a reauthentication flow — and `ConfigEntryError` for other non-retryable failures. (Bronze: `test-before-setup`)
...
- Test through the public surface: set the integration up via `hass` and assert entity state and the entity/device registries (`snapshot_platform` preferred). Do not reach into internal integration details (private attributes, coordinator internals) — calling `coordinator.async_refresh()` directly to force an immediate, deterministic update is fine. For polling integrations, prefer driving updates with `freezer` + `async_fire_time_changed` when testing polling-interval behavior itself; push integrations (webhook, subscription) must instead exercise their real update path (e.g. posting to the webhook), since time advancement doesn't trigger those.
- Patch the third-party library, not integration internals, and put shared fixtures in `conftest.py`. Every config-flow test must end in `CREATE_ENTRY`, `ABORT`, or a `FORM` that asserts a specific persistent error, and assert the `unique_id` when the flow assigns one (some integrations are legitimately exempt from `unique-config-entry`). (Bronze: `config-flow-test-coverage`)
