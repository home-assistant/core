...
- Do NOT allow users to set config entry names in config flows. Names are automatically generated or can be customized later in UI. Exceptions: helper integrations may allow custom names, subentry flows may legitimately ask for one (e.g. naming a conversation-agent subentry), and a synthetic source with no natural device identity may need one to tell multiple instances apart (e.g. naming a local-file camera source). Most other `CONF_NAME` fields still found in config flows are grandfathered legacy code marked as no longer allowed, not sanctioned examples to copy.
