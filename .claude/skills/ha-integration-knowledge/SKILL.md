...
- A coordinator update raises `ConfigEntryAuthFailed` for the same reasons. Reauth cannot fix an account limitation, such as an expired subscription or an exceeded quota. Raise a repair issue for those instead. (Gold: `repair-issues`)
