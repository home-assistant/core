...
async def test_track_template_with_time(hass):
    ...
    template_complex = Template(
        "{{ track_time_pattern() }}{{ states.switch.test.state and now() }}", hass
    )
    ...
    assert info.listeners == {
        ...
        "domains": set(),
        "entities": set(),
        "time_patterns": [{"hour": "*", "minute": "*", "second": "*"}],
    }
...
async def test_track_template_turned_off(hass):
    """Test tracking template with time pattern off."""
    hass.states.async_set("switch.test", "on")
    specific_runs = []
    template_complex = Template(
        '{{ track_time_pattern("off") }}{{ states.switch.test.state and now() }}', hass
    )
    ...
    await hass.async_block_till_done()
    now = dt_util.utcnow()
    async_fire_time_changed(hass, now + timedelta(hours=200000))
    await hass.async_block_till_done()
    assert len(specific_runs) == 0
    hass.states.async_set("switch.test", "off")
    await hass.async_block_till_done()
...
async def test_track_template_based_on_state_and_time_pattern_off(hass):
    """Test tracking template that starts with track state and switches to time pattern off."""
    hass.states.async_set("switch.test", "off")
    specific_runs = []
    template_complex = Template(
        """
        {% if states.switch.test.state == "on" %}
            {{ track_time_pattern("off") }}
            {{ states.switch.test.state }}
        {% else %}
            {{ states.switch.test.state }}
        {% endif %}
        """,
...
async def test_track_template_time_pattern_ignores_rate_limit(hass):
    """Test tracking template with time pattern disable rate limit."""
    specific_runs = []
    template_complex = Template(
        """
...
async def test_track_template_with_pattern_change(hass):
    """Test tracking template that starts with track state and switches to time pattern off."""
    hass.states.async_set("switch.test", "on")
    specific_runs = []
    template_complex = Template(
        """
        {% if states.switch.test.state == "on" %}
            {{ track_time_pattern(seconds=0,minutes=0,hours="*") }}
        {% else %}
            {{ track_time_pattern() }}
        {% endif %}
        {{ now() }}
    ...
    assert info.listeners == {
        ...
        "domains": set(),
        "entities": set(),
        "time_patterns": [{"hour": "*", "minute": 0, "second": 0}],
    }
    assert len(specific_runs) == 0

    now = dt_util.utcnow()

    async_fire_time_changed(hass, now + timedelta(hours=2))
    await hass.async_block_till_done()
    assert len(specific_runs) >= 1
    ...
    async_fire_time_changed(hass, now + timedelta(hours=4))
    await hass.async_block_till_done()
    ...
    async_fire_time_changed(hass, now + timedelta(hours=4, seconds=2))
    await hass.async_block_till_done()
    assert len(specific_runs) >= 3

    async_fire_time_changed(hass, now + timedelta(hours=4, seconds=4))
    await hass.async_block_till_done()
    assert len(specific_runs) >= 4
...
async def test_async_track_template_result_multiple_templates_mixing_listeners(hass):
    ...
    template_1 = Template("{{ states.switch.test.state == 'on' }}")
    template_2 = Template('{{ track_time_pattern(hours="23") }}{{ now() and True }}')
    ...
    assert info.listeners == {
        ...
        "domains": set(),
        "entities": {"switch.test"},
        "time_patterns": [{"hour": 23, "minute": "*", "second": "*"}],
    }
    hass.states.async_set("switch.test", "on")
    await hass.async_block_till_done()
