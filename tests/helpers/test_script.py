...
async def test_wait_for_trigger_trigger_defined_variables(
    hass: HomeAssistant,
) -> None:
    """Test variables defined on a wait_for_trigger trigger become available.

    Variables configured on a trigger inside wait_for_trigger must be
    available in the actions that run after the wait completes.
    """
    context = Context()
    wait_alias = "wait step"
    event = "test_event"
    ...
    actions = [
        {
            ...
            "wait_for_trigger": {
                ...
                "entity_id": "switch.test",
                "to": "off",
                "variables": {"trigger_var": "triggered_value"},
            },
        },
        {
            "event": event,
            "event_data": {"value": "{{ trigger_var }}"},
        },
    ]
    sequence = cv.SCRIPT_SCHEMA(actions)
    sequence = await script.async_validate_actions_config(hass, sequence)
    ...
    assert len(events) == 1
    assert events[0].data["value"] == "triggered_value"
