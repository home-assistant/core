...
async def test_assist_api_get_timer_tools(
    ...
    assert "intent.HassStartTimer" not in [tool.name for tool in api.tools]
    ...
    api = await llm.async_get_api(hass, "assist", llm_context)
    assert "intent.HassStartTimer" in [tool.name for tool in api.tools]
...
"""
    first_part_prompt = (
        ...
        "Use intent.HassTurnOn to lock and intent.HassTurnOff to unlock a lock. "
    ...
    dynamic_context_prompt = (
        ...
        " current state of\nthe home using the"
        " `homeassistant.GetLiveContext` tool. This is a primary"
        ...
        "    2.  You MUST call `homeassistant.GetLiveContext`. This"
...
{no_timer_prompt}"""
    ...
    # Verify that the homeassistant.GetLiveContext tool returns the same results
    # as the exposed_entities_prompt
    result = await api.async_call_tool(
        llm.ToolInput(tool_name="homeassistant.GetLiveContext", tool_args={})
    )
    assert result == {
        "success": True,
...
async def test_action_tool(
    ...
    tool = tools[0]
    assert tool.name == "script.test_script"
    assert (
        tool.description
    ...
    tool_input = llm.ToolInput(
        tool_name="script.script_with_no_fields",
        tool_args={},
    )
...
async def test_no_tools_exposed(hass: HomeAssistant) -> None:
    ...
    api = await llm.async_get_api(hass, "assist", llm_context)
    # homeassistant.GetLiveContext is always offered; it reports when nothing is exposed.
