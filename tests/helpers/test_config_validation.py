...
def test_zwave_network_key(caplog):
    """Test uuid validation."""
    schema = vol.Schema(cv.zwave_network_key)

    for value in ['Not a 16 byte value zwave networking key', '0', 0]:
        with pytest.raises(vol.Invalid):
            schema(value)

    with pytest.raises(vol.Invalid):
        schema('0x 01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, '
               '0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10')

    value = ('0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, '
             '0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10')
    assert schema(value) == value.lower()
