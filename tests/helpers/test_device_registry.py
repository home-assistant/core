...
async def test_loading_from_storage(hass, hass_storage):
    ...
    assert entry.disabled_by is device_registry.DeviceEntryDisabledBy.USER
...
async def test_loading_saving_data(hass, registry, area_registry):
    ...
    orig_light = registry.async_get_or_create(
        ...
        model="light",
        via_device=("hue", "0123"),
        disabled_by=device_registry.DeviceEntryDisabledBy.USER,
    )
...
async def test_update(registry):
    ...
    assert updated_entry.disabled_by is device_registry.DeviceEntryDisabledBy.USER
...
async def test_disable_config_entry_disables_devices(hass, registry):
    ...
    assert entry1.disabled_by is device_registry.DeviceEntryDisabledBy.CONFIG_ENTRY
    entry2 = registry.async_get(entry2.id)
    assert entry2.disabled
    assert entry2.disabled_by is device_registry.DeviceEntryDisabledBy.USER
