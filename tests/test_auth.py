...
async def test_create_new_user():
    ...
    step = await manager.login_flow.async_init(('unsecure_example', None))
...
async def test_login_as_existing_user():
    ...
    user.credentials.append(auth.Credentials(
        id='mock-id',
        auth_provider_type='unsecure_example',
        auth_provider_id=None,
        data={'username': 'test-user'}
    ))
    manager._store.users.append(user)
...
async def test_linking_user_to_two_auth_providers():
    ...
    step = await manager.login_flow.async_init(('unsecure_example', 'another-provider'))
    step = await manager.login_flow.async_configure(step['flow_id'], {
        'username': 'another-user',
        'password': 'another-password',
    })
