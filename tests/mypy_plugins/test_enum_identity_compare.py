...
def _run_mypy(paths: list[Path], tmp_path: Path) -> str:
    """Run mypy with the plugin over paths and return its stdout."""
    ...
    config.write_text(
        ...
        # Lets the stub package written next to the cases resolve to the
        # fullname the framework-guaranteed set matches on.
...
def mypy_errors(tmp_path_factory: pytest.TempPathFactory) -> dict[str, list[str]]:
    ...
    """
    tmp_path = tmp_path_factory.mktemp("enum_identity_compare")
