...
async def test_async_update_title_placeholders(hass: HomeAssistant) -> None:
    ...
    with mock_config_flow(domain, TestFlow):
        result = await hass.config_entries.flow.async_init(
            domain, context={"source": config_entries.SOURCE_USER}
        )
    assert result["type"] is FlowResultType.FORM
    assert result["context"]["title_placeholders"] == {"initial": "value"}
    ...
    flow.async_update_title_placeholders({"name": "updated"})
    await hass.async_block_till_done()

    # Verify placeholders were updated (preserving existing values)
    assert flow.context["title_placeholders"] == {"initial": "value", "name": "updated"}
    ...
    flow.async_update_title_placeholders({"initial": "new_value", "another": "key"})
    await hass.async_block_till_done()
