...
async def test_log_warning_custom_component(hass, caplog):
    ...
    assert (
        "You are using a custom integration custom_components.test_standalone "
        "for test_standalone" in caplog.text
    )

    await loader.async_get_integration(hass, "test")
    assert (
        "You are using a custom integration custom_components.test for test "
        in caplog.text
    )
