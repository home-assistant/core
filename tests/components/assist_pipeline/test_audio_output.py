...
async def test_consumer_cancellation_fails_producer(hass: HomeAssistant) -> None:
    """Test consumer cancellation remains distinct and terminates the output."""
    manager = PipelineAudioOutputManager(hass)
    output = manager.async_create("wav", "audio/wav")
    consumer = hass.async_create_task(_collect(output))
    await asyncio.sleep(0)
    ...
    with pytest.raises(asyncio.CancelledError):
        await consumer
    with pytest.raises(PipelineAudioOutputError, match="consumer stopped early"):
        await output.async_write(b"late")
...
async def test_audio_output_http_failure_before_response(
    hass: HomeAssistant,
    hass_client_no_auth: ClientSessionGenerator,
    init_components: None,
) -> None:
    """Test a failure before streaming starts returns an HTTP error."""
    output = hass.data[assist_pipeline.DOMAIN].audio_output_manager.async_create(
        "wav", "audio/wav"
    )
    output.async_fail(ValueError("generation failed"))
...
async def test_audio_output_http_failure_aborts_response(
    hass: HomeAssistant,
    hass_client_no_auth: ClientSessionGenerator,
    init_components: None,
) -> None:
    """Test a failure after streaming starts aborts the HTTP response."""
    output = hass.data[assist_pipeline.DOMAIN].audio_output_manager.async_create(
        "wav", "audio/wav"
    )
    await output.async_write(b"partial")
    client = await hass_client_no_auth()
...
class _UnexpectedFailureTestProcessor(_FailingStreamingTestProcessor):
    """Test processor that fails unexpectedly."""
...
async def test_unexpected_error_fails_audio_output(
    hass: HomeAssistant,
    init_components: None,
    mock_chat_session: chat_session.ChatSession,
) -> None:
    """Test an unexpected error terminates a controller-owned output."""
    processor: _UnexpectedFailureTestProcessor | None = None

    def create_processor(run) -> _UnexpectedFailureTestProcessor:
        nonlocal processor
        processor = _UnexpectedFailureTestProcessor(run)
        return processor
    ...
    with pytest.raises(PipelineAudioOutputError, match="unexpected failure"):
        await _collect(processor.response_audio)
...
async def test_cleanup_error_unregisters_pipeline(
    hass: HomeAssistant,
    init_components: None,
    mock_chat_session: chat_session.ChatSession,
) -> None:
    """Test cleanup failures cannot leave an active run registered."""
    processor: _UnexpectedFailureTestProcessor | None = None
