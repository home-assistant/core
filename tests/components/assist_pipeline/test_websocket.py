...
def test_setup_text_input_intent():
    """Test _setup_text_input helper function for INTENT stage."""
    from homeassistant.components.assist_pipeline.websocket_api import _setup_text_input
    from homeassistant.components.assist_pipeline.pipeline import PipelineStage
    
    msg = {"input": {"text": "Turn on the lights"}}
    input_args = {"device_id": "test_device"}
    
    result = _setup_text_input(PipelineStage.INTENT, msg, input_args)
    
    assert result["device_id"] == "test_device"
    assert result["intent_input"] == "Turn on the lights"


def test_setup_text_input_tts():
    """Test _setup_text_input helper function for TTS stage."""
    from homeassistant.components.assist_pipeline.websocket_api import _setup_text_input
    from homeassistant.components.assist_pipeline.pipeline import PipelineStage
    
    msg = {"input": {"text": "The lights are on"}}
    input_args = {"device_id": "test_device"}
    
    result = _setup_text_input(PipelineStage.TTS, msg, input_args)
    
    assert result["device_id"] == "test_device"
    assert result["tts_input"] == "The lights are on"


def test_setup_audio_pipeline_wake_word(
    hass: HomeAssistant,
    init_components,
):
    """Test _setup_audio_pipeline helper function for WAKE_WORD stage."""
    from homeassistant.components.assist_pipeline.websocket_api import _setup_audio_pipeline
    from homeassistant.components.assist_pipeline.pipeline import PipelineStage
    from unittest.mock import Mock
    
    # Mock connection
    connection = Mock()
    connection.async_register_binary_handler.return_value = (123, Mock())
    
    # Mock pipeline
    pipeline = Mock()
    pipeline.stt_language = "en"
    ...
    result_args, handler_id, unregister_handler, wake_word_settings, audio_settings = _setup_audio_pipeline(
        hass, connection, msg, pipeline, PipelineStage.WAKE_WORD, input_args
    )
    
    # Check wake word settings
    assert wake_word_settings is not None
    assert wake_word_settings.timeout == 5
    assert wake_word_settings.audio_seconds_to_buffer == 2
    
    # Check audio settings
    assert audio_settings.noise_suppression_level == 1
    assert audio_settings.auto_gain_dbfs == -30
    assert abs(audio_settings.volume_multiplier - 1.5) < 0.001
    assert not audio_settings.is_vad_enabled  # no_vad=True means vad disabled
    
    # Check handler registration
    assert handler_id == 123
    assert unregister_handler is not None
    
    # Check input args contain required keys
    assert "stt_metadata" in result_args
    assert "stt_stream" in result_args
    assert "wake_word_phrase" in result_args
    assert result_args["wake_word_phrase"] is None


def test_setup_audio_pipeline_stt(
    hass: HomeAssistant,
    init_components,
):
    """Test _setup_audio_pipeline helper function for STT stage."""
    from homeassistant.components.assist_pipeline.websocket_api import _setup_audio_pipeline
    from homeassistant.components.assist_pipeline.pipeline import PipelineStage
    from unittest.mock import Mock
    ...
    # Mock connection
    connection = Mock()
    connection.async_register_binary_handler.return_value = (456, Mock())
    ...
    result_args, handler_id, unregister_handler, wake_word_settings, audio_settings = _setup_audio_pipeline(
        hass, connection, msg, pipeline, PipelineStage.STT, input_args
    )
    
    # Check wake word settings is None for STT stage
    assert wake_word_settings is None
    
    # Check wake word phrase is set
    assert result_args["wake_word_phrase"] == "hey assistant"
    
    # Check default audio settings
    assert audio_settings.noise_suppression_level == 0
    assert audio_settings.auto_gain_dbfs == 0
    assert audio_settings.volume_multiplier == 1
    assert audio_settings.is_vad_enabled  # default is enabled
