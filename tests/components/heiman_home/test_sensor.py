...
def _get_sensor_entities(hass: HomeAssistant) -> list[HeimanSensorEntity]:
    """Return all Heiman sensor entities registered on hass."""
    platforms = entity_platform.async_get_platforms(hass, DOMAIN)
    return [entity for platform in platforms for entity in platform.entities.values()]
...
async def test_sensor_entity_creation(
    ...
    # Create a mock config entry and run the real setup flow with the
    # coordinator mocked out, so the component forwards to the real sensor
    # platform and entities are created through the standard framework path
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            "auth_implementation": DOMAIN,
    ...
    added_entities = _get_sensor_entities(hass)
...
async def test_sensor_skip_scan_on_no_structure_change(
    ...
    assert len(_get_sensor_entities(hass)) == 1
