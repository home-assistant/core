...
async def test_unload_with_no_runtime_data(hass: HomeAssistant) -> None:
    ...
    # Load the entry through the framework with component setup patched out,
    # so the entry is loaded but still has no runtime_data
    with patch(
        "homeassistant.components.heiman_home.async_setup_entry",
        return_value=True,
    ):
