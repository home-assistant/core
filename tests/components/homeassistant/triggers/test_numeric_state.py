        ...
        automation.DOMAIN,
        {
            automation.DOMAIN: {
                ...
                "action": {
                    "service": "test.automation",
                    "data_template": {
                        "some": "{{ trigger.platform }} - {{ trigger.entity_id }} - {{ trigger.below }} - {{ trigger.above }} - {{ trigger.from_state.state }} - {{ trigger.to_state.state }}"
                    },
                },
            }
        },
            ...
            automation.DOMAIN: {
                ...
                "action": [
                    {"wait_template": "{{ states(trigger.entity_id) | int < 10 }}"},
                    {
                        "service": "test.automation",
                        "data_template": {
                            "some": "{{ trigger.platform }} - {{ trigger.entity_id }} - {{ trigger.to_state.state }}"
                        },
                    },
                ],
            }
