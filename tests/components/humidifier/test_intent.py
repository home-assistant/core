...
async def test_intent_turn_on_off(hass):
    """Test the humidifier on/off intent."""
    hass.states.async_set(
        "humidifier.bedroom_humidifier", STATE_OFF, {ATTR_HUMIDITY: 40}
    )
    turn_on_calls = async_mock_service(hass, DOMAIN, SERVICE_TURN_ON)
    ...
    result = await hass.helpers.intent.async_handle(
        "test", intent.INTENT_ON, {"name": {"value": "Bedroom humidifier"}},
    )
    await hass.async_block_till_done()

    assert result.speech["plain"]["speech"] == "Turned bedroom humidifier on"
    ...
    result = await hass.helpers.intent.async_handle(
        "test", intent.INTENT_OFF, {"name": {"value": "Bedroom humidifier"}},
    )
    await hass.async_block_till_done()

    assert result.speech["plain"]["speech"] == "Turned bedroom humidifier off"
