...
async def test_play_media_music_library_album_artist(
    hass: HomeAssistant, soco_factory: SoCoMockFactory, async_autosetup_sonos, discover
) -> None:
    """Test that multiple albums are added to queue for A:ALBUMARTIST."""
    sock_mock = soco_factory.mock_list.get("192.168.42.2")
    sock_mock.music_library.get_music_library_information.return_value = [
        "Album_1",
        "Album_2",
    ]
    ...
    assert sock_mock.clear_queue.call_count == 1
    # Verify both albums are added to queue
    assert sock_mock.add_to_queue.call_count == 2
    assert sock_mock.add_to_queue.call_args_list[0].args[0] == "Album_1"
    assert sock_mock.add_to_queue.call_args_list[1].args[0] == "Album_2"
    # Verify queue is played from start.
    assert sock_mock.play_from_queue.call_count == 1
    assert sock_mock.play_from_queue.call_args_list[0].args[0] == 0
