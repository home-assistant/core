"""Tests for the HEMS definitions module."""
...
def test_all_decoder_specs_are_valid() -> None:
    ...
    for name, spec in decoders_data.items():
        ...
        assert isinstance(
            decoder, (NumericDecoderSpec, BinaryDecoderSpec, EnumDecoderSpec)
...
def test_decoder_types_match_platforms() -> None:
    ...
    sensor -> NumericDecoderSpec (signed/unsigned/temperature)
    ...
    platform_to_decoder_types = {
        "sensor": {"signed", "unsigned", "temperature"},
        "binary": {"binary"},
        "select": {"enum"},
    }
    ...
    for class_code, device in data.get("devices", {}).items():
        for entity in device.get("entities", []):
            platform = entity.get("platform", "sensor")
            decoder_name = entity.get("decoder", "")
            decoder_spec = decoders_data.get(decoder_name, {})
            decoder_type = decoder_spec.get("type", "unsigned")
...
def test_load_definitions_registry_resolves_all_decoders() -> None:
    ...
    # Verify each platform has resolved entities with correct decoder types
    for platform_type in ("sensor", "binary", "select"):
        resolved = registry.get_resolved_entities(platform_type)  # type: ignore[arg-type]
        for entities in resolved.values():
            for _entity_def, decoder_spec in entities:
                ...
                elif platform_type == "binary":
                    assert isinstance(decoder_spec, BinaryDecoderSpec)
                elif platform_type == "select":
