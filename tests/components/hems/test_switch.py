"""Tests for the HEMS switch platform."""
...
async def test_switch_not_created_for_climate_class(
    hass: HomeAssistant, init_integration
) -> None:
    """Test that switch entity is NOT created for climate class (0x0130).

    Air conditioners should use the climate platform for power control,
    not a separate switch entity.
    """
    entry = init_integration
    coordinator = entry.runtime_data.coordinator
    ...
    await coordinator._async_setup_device("010107", int("013001", 16))
    await hass.async_block_till_done()

    # Verify NO switch entity was created for the air conditioner
    entity_registry = er.async_get(hass)
    # Check with current unique_id format
    entity_id = entity_registry.async_get_entity_id(
