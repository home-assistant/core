"""Common fixtures for the HEMS integration tests."""
...
def mock_async_validate_network() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "homeassistant.components.hems.config_flow.create_multicast_socket",
...
@pytest.fixture
def mock_hems_client() -> Generator[AsyncMock]:
    ...
    with patch("homeassistant.components.hems.HemsClient", return_value=client):
        yield client
...
def patch_platforms(platforms: list[Platform]) -> Generator[None]:
    ...
    with patch("homeassistant.components.hems.PLATFORMS", platforms):
        yield
