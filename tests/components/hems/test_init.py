"""Tests for the HEMS initialization helpers."""
...
from homeassistant.components.hems import async_setup, async_unload_entry
...
async def test_async_setup_and_unload_entry(
    ...
    assert entry.runtime_data.client is mock_hems_client
    assert entry.runtime_data.property_poller is not None
    mock_hems_client.start.assert_awaited_once()

    await hass.config_entries.async_unload(entry.entry_id)
    await hass.async_block_till_done()

    mock_hems_client.stop.assert_awaited_once()
...
async def test_runtime_inactivity_issue_raised_and_cleared(
    ...
    with patch(
        "homeassistant.components.hems.async_track_time_interval",
...
async def test_runtime_error_restart_success_updates_health(
    ...
    await mock_hems_client._listener(error_event)
    await hass.async_block_till_done()
    ...
    assert mock_hems_client.start.await_count == 2
    assert mock_hems_client.stop.await_count == 1
...
async def test_runtime_error_restart_failure_creates_issue(
    ...
    mock_hems_client.start.side_effect = [None, OSError("socket busy")]
...
async def test_property_poller_requests_polled_properties(
    ...
    with patch(
        "homeassistant.components.hems.poller.async_track_time_interval",
        side_effect=_track_time_interval,
    ):
        await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
    ...
    mock_hems_client.async_get.side_effect = _async_get
    ...
    assert callbacks
    prev_count = mock_hems_client.async_send.await_count
    callbacks[0](dt_util.utcnow())
    await hass.async_block_till_done()
    assert mock_hems_client.async_send.await_count > prev_count
...
async def test_property_poller_falls_back_to_poll_when_notifications_disabled(
    ...
    assert any(
        ...
        for call in mock_hems_client.async_send.await_args_list
    )
...
async def test_node_created_with_property_maps(
    ...
    with patch(
        "homeassistant.components.hems.coordinator.time.monotonic",
...
async def test_property_poller_respects_get_property_map(
    ...
    for call in mock_hems_client.async_send.call_args_list:
        frame = call.args[1]
...
async def test_property_poller_requests_notifications_for_required_epcs(
    ...
    calls = mock_hems_client.async_send.await_args_list
    assert any(
...
async def test_property_poller_handles_sna_notification_response(
    ...
    assert not any(
        call.args[1].esv == 0x63 for call in mock_hems_client.async_send.await_args_list
