...
async def test_service_preset_mode_until_date(hass, config_entry, netatmo_auth):
    """Test service with preset mode with an end date for thermostats."""
    with selected_platforms(["climate"]):
        assert await hass.config_entries.async_setup(config_entry.entry_id)
    ...
    await hass.services.async_call(
        ...
        SERVICE_SET_PRESET_MODE_UNTIL,
        {
            ...
            ATTR_END_DATE: "2022-12-24T16:00",
        },
        blocking=True,
    )
    await hass.async_block_till_done()
