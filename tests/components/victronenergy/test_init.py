...
async def test_setup_unload_and_reload_entry(hass: HomeAssistant, mock_config_entry: MockConfigEntry, mock_mqtt_client: MagicMock) -> None:
...
async def test_setup_entry_connection_error(hass: HomeAssistant, mock_config_entry: MockConfigEntry) -> None:
    """Test entry setup with connection error."""
    mock_config_entry.add_to_hass(hass)
    
    with patch("homeassistant.components.victronenergy.mqtt_client.Client") as mock_client_class:
...
async def test_integration_cleanup_on_removal(hass: HomeAssistant, mock_config_entry: MockConfigEntry, mock_mqtt_client: MagicMock) -> None:
...
async def test_mqtt_connection_callbacks(hass: HomeAssistant, mock_config_entry: MockConfigEntry) -> None:
...
async def test_mqtt_worker_cleanup(hass: HomeAssistant, mock_config_entry: MockConfigEntry, mock_mqtt_client: MagicMock) -> None:
...
async def test_error_handling_during_cleanup(hass: HomeAssistant, mock_config_entry: MockConfigEntry) -> None:
