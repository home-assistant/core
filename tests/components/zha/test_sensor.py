...
@pytest.mark.parametrize(
    "attr_value, state_name",
    [
        (smartenergy.Metering.RegisteredTier.No_Tier, "No Tier"),
        (smartenergy.Metering.RegisteredTier.Tier_7, "Tier 7"),
        (smartenergy.Metering.RegisteredTier.Extended_Tier, "Extended Tier"),
    ],
)
async def test_metering_registered_tier_delivered(
    hass,
    zigpy_device_mock,
    zha_device_joined,
    attr_value,
    ...
    """Test ZHA electrical measurement receiving registered tier delivered updates."""
    zigpy_device = zigpy_device_mock(
        {
            1: {
                SIG_EP_INPUT: [
    ...
    cluster = zigpy_device.endpoints[1].smartenergy_metering
    zha_dev = await zha_device_joined(zigpy_device)
    zha_dev.available = True
    await hass.async_block_till_done()

    await async_enable_traffic(hass, [zha_dev])

    entity_ids = [
        ref.reference_id for ref in zha_dev.gateway.device_registry[zha_dev.ieee]
    ]
    entity_id = next(id for id in entity_ids if id.endswith("register_tier_delivered"))

    await send_attributes_report(
        hass,
        cluster,
        {
            smartenergy.Metering.attributes_by_name[
    ...
    hass_state = hass.states.get(entity_id)
    assert hass_state.state == state_name
