...
from homeassistant.components.indoor_air_quality.helpers import unique_id_from_sources
...
def test_unique_id_uses_methodology_and_sources() -> None:
    """Config-entry identity is independent of its display name."""
    sources = {CONF_TEMPERATURE: "sensor.temperature"}

    unique_id = unique_id_from_sources(sources, STANDARD_UK)

    assert unique_id == unique_id_from_sources(sources, STANDARD_UK)
    assert unique_id != unique_id_from_sources(
        {CONF_TEMPERATURE: "sensor.other_temperature"}, STANDARD_UK
    )


def test_device_sources_ignore_disabled_entities(hass: HomeAssistant) -> None:
    """Disabled source entities are not selected automatically."""
    device_id = _create_mock_device(hass)
    _create_mock_sensor(
        hass,
        device_id,
        "disabled_temperature",
        SensorDeviceClass.TEMPERATURE,
        disabled_by=er.RegistryEntryDisabler.USER,
    )

    assert _sources_from_device(hass, device_id) == {}
...
async def test_form_user_device_selects_only_pm25(hass: HomeAssistant) -> None:
    """Device detection keeps particle-size fractions separate."""
    device_id = _create_mock_device(hass)
    _create_mock_sensor(hass, device_id, "kitchen_pm1", SensorDeviceClass.PM1)
    _create_mock_sensor(hass, device_id, "kitchen_pm25", SensorDeviceClass.PM25)
    _create_mock_sensor(hass, device_id, "kitchen_pm10", SensorDeviceClass.PM10)
...
async def test_form_user_already_configured(hass: HomeAssistant) -> None:
    """Test user form aborts when the methodology and sources already exist."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
...
async def test_reconfigure_flow(hass: HomeAssistant) -> None:
    ...
    assert config_entry.unique_id == unique_id_from_sources(
        {CONF_TEMPERATURE: "sensor.new_temp"}, STANDARD_UK
    )
...
async def test_reconfigure_no_sources(hass: HomeAssistant) -> None:
    ...
    config_entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            CONF_SOURCES: {CONF_PM25: "sensor.pm"},
            CONF_STANDARD: STANDARD_UK,
        },
        title="Test",
        unique_id="test",
...
async def test_reconfigure_rejects_duplicate_sources(hass: HomeAssistant) -> None:
    """Reconfiguration cannot duplicate another methodology and source set."""
    original_sources = {CONF_TEMPERATURE: "sensor.original"}
    duplicate_sources = {CONF_TEMPERATURE: "sensor.duplicate"}
    config_entry = MockConfigEntry(
        domain=DOMAIN,
        data={CONF_SOURCES: original_sources, CONF_STANDARD: STANDARD_UK},
        unique_id=unique_id_from_sources(original_sources, STANDARD_UK),
    )
    duplicate_entry = MockConfigEntry(
        domain=DOMAIN,
        data={CONF_SOURCES: duplicate_sources, CONF_STANDARD: STANDARD_UK},
        unique_id=unique_id_from_sources(duplicate_sources, STANDARD_UK),
    )
    config_entry.add_to_hass(hass)
    ...
    result = await config_entry.start_reconfigure_flow(hass)
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], duplicate_sources
    )

    assert result["type"] is FlowResultType.ABORT
    assert result["reason"] == "already_configured"
    assert config_entry.data[CONF_SOURCES] == original_sources
...
async def test_form_user_device_ignores_tvoc_parts(hass: HomeAssistant) -> None:
    """Device detection ignores a parts-based tVOC sensor despite its name."""
    device_id = _create_mock_device(hass, "Air Monitor")
    _create_mock_sensor(
        hass,
        device_id,
    ...
    assert result["type"] is FlowResultType.FORM
    assert result["errors"] == {"base": "no_matching_sources"}
