...
from homeassistant.components.indoor_air_quality.const import (
    ...
    MOLAR_MASS_CO2,
    MOLAR_MASS_HCHO,
    STANDARD_IAQUK_2015 as STANDARD_UK,
    UNIT_MGM3,
    UNIT_PPM,
    UNIT_UGM3,
)
...
async def test_source_entity_rename_updates_config_entry(
    hass: HomeAssistant, entity_registry: er.EntityRegistry
) -> None:
    """A source entity registry rename is persisted in the helper entry."""
    source = entity_registry.async_get_or_create(
        "sensor", "test", "temperature", suggested_object_id="temperature"
    )
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            CONF_SOURCES: {CONF_TEMPERATURE: source.entity_id},
            CONF_STANDARD: STANDARD_UK,
        },
        title="Test",
        unique_id="test",
    ...
    entity_registry.async_update_entity(
        source.entity_id, new_entity_id="sensor.renamed_temperature"
    )
    await hass.async_block_till_done()

    assert entry.data[CONF_SOURCES] == {CONF_TEMPERATURE: "sensor.renamed_temperature"}
    assert entry.unique_id == unique_id_from_sources(
        {CONF_TEMPERATURE: "sensor.renamed_temperature"}, STANDARD_UK
    )


async def test_source_removal_preserves_selected_device(
    hass: HomeAssistant,
    device_registry: dr.DeviceRegistry,
    entity_registry: er.EntityRegistry,
) -> None:
    """Removing one source does not detach a multi-source helper from its device."""
    source_config_entry = MockConfigEntry(domain="test")
    source_config_entry.add_to_hass(hass)
    device = device_registry.async_get_or_create(
        config_entry_id=source_config_entry.entry_id,
        identifiers={("test", "air_monitor")},
        name="Air monitor",
    )
    temperature = entity_registry.async_get_or_create(
        "sensor",
        "test",
        "temperature",
    ...
    humidity = entity_registry.async_get_or_create(
        "sensor",
        "test",
        "humidity",
    ...
    hass.states.async_set(
        humidity.entity_id, "50", {ATTR_UNIT_OF_MEASUREMENT: PERCENTAGE}
    )
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            ...
            CONF_SOURCES: {
                CONF_TEMPERATURE: temperature.entity_id,
                CONF_HUMIDITY: humidity.entity_id,
            },
            CONF_STANDARD: STANDARD_UK,
        },
        title="Air monitor",
        unique_id="air-monitor",
    )
    entry.add_to_hass(hass)
    assert await hass.config_entries.async_setup(entry.entry_id)
    await hass.async_block_till_done()
    index = entity_registry.async_get("sensor.air_monitor_index")
    level = entity_registry.async_get("sensor.air_monitor_level")
    assert index is not None and index.device_id == device.id
    assert level is not None and level.device_id == device.id

    entity_registry.async_remove(temperature.entity_id)
    await hass.async_block_till_done()

    updated_index = entity_registry.async_get(index.entity_id)
    updated_level = entity_registry.async_get(level.entity_id)
    assert updated_index is not None and updated_index.device_id == device.id
    assert updated_level is not None and updated_level.device_id == device.id
...
async def test_controller_update_multi_source(hass: HomeAssistant) -> None:
    ...
    assert controller.extra_state_attributes == {
        ATTR_LIMITING_SOURCE: CONF_TEMPERATURE,
        ATTR_SOURCES_SET: 3,
        ATTR_SOURCES_USED: 3,
...
async def test_humidity_above_physical_range_is_unavailable(
    hass: HomeAssistant,
) -> None:
    """Relative humidity above 100% is not scored."""
    hass.states.async_set(
        "sensor.humidity", "101", {ATTR_UNIT_OF_MEASUREMENT: PERCENTAGE}
    )
    controller = IndoorAirQualityController(
        hass, "test", "Test", {CONF_HUMIDITY: "sensor.humidity"}
    )
...
async def test_invalid_pollutant_value_is_unavailable(
    hass: HomeAssistant, value: float
) -> None:
    """Physically invalid pollutant concentrations are not scored."""
    hass.states.async_set("sensor.co2", str(value), {ATTR_UNIT_OF_MEASUREMENT: "ppm"})
    controller = IndoorAirQualityController(
        hass, "test", "Test", {CONF_CO2: "sensor.co2"}
    )
...
async def test_pm25_levels(hass: HomeAssistant) -> None:
    """Test PM2.5 is normalized to µg/m³ before scoring."""
    controller = IndoorAirQualityController(
        hass, "test", "Test", {CONF_PM25: "sensor.pm25"}
    )

    hass.states.async_set("sensor.pm25", 0.02, {ATTR_UNIT_OF_MEASUREMENT: "mg/m³"})
    controller.update()
    assert (
        controller.extra_state_attributes[ATTR_SOURCE_INDEX_TPL.format(CONF_PM25)] == 5
    )
...
async def test_tvoc_parts_unit_is_unavailable(hass: HomeAssistant) -> None:
    """TVOC parts cannot be converted without a declared reference compound."""
    hass.states.async_set("sensor.tvoc", 1, {ATTR_UNIT_OF_MEASUREMENT: "ppm"})
    controller = IndoorAirQualityController(
        hass, "test", "Test", {CONF_TVOC: "sensor.tvoc"}
    )
...
@pytest.mark.parametrize(
    "source_unit",
    [
        UnitOfDensity.MICROGRAMS_PER_CUBIC_METER,  # canonical Greek mu
...
@pytest.mark.parametrize(
    "source_unit",
    [
        UnitOfDensity.MILLIGRAMS_PER_CUBIC_METER,  # canonical
        "mg/m³",
    ],
)
...
def test_convert_value_hcho_molar_mass() -> None:
    """Test the formaldehyde molar-mass constant is usable."""
    assert convert_value(
        1, "ppm", UNIT_MGM3, molar_mass=MOLAR_MASS_HCHO
    ) == pytest.approx(MOLAR_MASS_HCHO / 24.45)
...
async def test_state_unavailable_when_configured_source_unavailable(
    hass: HomeAssistant,
) -> None:
    """The score is unavailable when it cannot use every configured source."""
    hass.states.async_set(
        "sensor.t", 20, {ATTR_UNIT_OF_MEASUREMENT: UnitOfTemperature.CELSIUS}
    )
    hass.states.async_set("sensor.co2", 2000, {ATTR_UNIT_OF_MEASUREMENT: "ppm"})
    controller = IndoorAirQualityController(
        hass,
        "test",
        "Test",
        {CONF_TEMPERATURE: "sensor.t", CONF_CO2: "sensor.co2"},
    )
    controller.update()
    assert controller.iaq_index is not None

    hass.states.async_remove("sensor.co2")
    controller.update()
