...
async def test_get_opening_type_door(
    hass: HomeAssistant, mock_concord232_binary_sensor_client: MagicMock
) -> None:
    """Test zone type detection for door sensor."""
    mock_concord232_binary_sensor_client.list_zones.return_value = [
        {"number": 1, "name": "DOOR Sensor", "state": "Normal"},
    ]
    mock_concord232_binary_sensor_client.zones = (
        mock_concord232_binary_sensor_client.list_zones.return_value
    )
    ...
    state = hass.states.get("binary_sensor.door_sensor")
    assert state.attributes.get("device_class") == "door"


async def test_get_opening_type_window(
    hass: HomeAssistant, mock_concord232_binary_sensor_client: MagicMock
) -> None:
    """Test zone type detection for window sensor."""
    mock_concord232_binary_sensor_client.list_zones.return_value = [
        {"number": 1, "name": "WINDOW Sensor", "state": "Normal"},
    ]
    mock_concord232_binary_sensor_client.zones = (
        mock_concord232_binary_sensor_client.list_zones.return_value
    )
    ...
    state = hass.states.get("binary_sensor.window_sensor")
    assert state.attributes.get("device_class") == "window"
