...
async def test_reauth_success(hass: HomeAssistant, mock_setup_entry: AsyncMock) -> None:
    """Test reauthentication flow updates the password and reloads."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=USER_INPUT[CONF_USERNAME],
        data=USER_INPUT,
...
async def test_reauth_cannot_connect(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"], user_input={CONF_PASSWORD: "any-password"}
        )
...
async def test_reauth_unknown_error(hass: HomeAssistant) -> None:
    """Test reauthentication flow handles unexpected errors."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=USER_INPUT[CONF_USERNAME],
        data=USER_INPUT,
...
async def test_reauth_invalid_auth(hass: HomeAssistant) -> None:
    """Test reauthentication flow handles invalid credentials."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=USER_INPUT[CONF_USERNAME],
        data=USER_INPUT,
...
async def test_reconfigure_wrong_account(
    ...
    """Test reconfigure flow aborts when a different username is entered."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=USER_INPUT[CONF_USERNAME],
        data=USER_INPUT,
    ...
    different_user = {CONF_USERNAME: "other-user", CONF_PASSWORD: "some-password"}
    with patch(
        "homeassistant.components.freshr.config_flow.FreshrClient",
        return_value=_mock_client(),
    ):
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"], user_input=different_user
        )
...
async def test_reconfigure_invalid_auth(hass: HomeAssistant) -> None:
    """Test reconfigure flow handles invalid credentials."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=USER_INPUT[CONF_USERNAME],
        data=USER_INPUT,
