...
async def test_get_tools_no_exposed_todo(hass: HomeAssistant) -> None:
    ...
    assert "todo.get_items" not in [tool.name for tool in result.tools]
...
async def test_todo_get_items_tool(hass: HomeAssistant) -> None:
    ...
    tool = next((tool for tool in result.tools if tool.name == "todo.get_items"), None)
    ...
    result = await tool.async_call(
        hass,
        llm.ToolInput("todo.get_items", {"todo_list": "Mock Todo List Name"}),
        llm_context,
    )
...
async def test_todo_get_items_status_filter(
    ...
    llm_context = _llm_context()
    result = await llm_component.async_get_tools(hass, llm_context, "assist")
    tool = next(tool for tool in result.tools if tool.name == "todo.get_items")
    ...
    await tool.async_call(
        hass,
        llm.ToolInput(
            "todo.get_items", {"todo_list": "Mock Todo List Name", "status": status}
        ),
        llm_context,
    )
...
async def test_todo_list_intents_exposed(hass: HomeAssistant) -> None:
    ...
    assert "todo.HassListAddItem" in names
    assert "todo.HassListCompleteItem" in names
    assert "todo.HassListRemoveItem" in names
