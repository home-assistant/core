...
class TestEntrySetup:
    """Test cases for async_setup_entry."""
    ...
    async def test_unrecoverable_api_errors_create_new_flow(
        ...
        400 (bad request): Occurs when trying to update the app and it fails.
        ...
        422 (unprocessable entity): Occurs when trying to update the app and
            it fails.
        """
        api = smartthings_mock.return_value
        for error_status in (400, 401, 403, 422):
...
class TestEntryUnload:
    """Test cases for async_unload_entry."""
