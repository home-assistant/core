...
from homeassistant.components.denonavr.config_flow import CONF_RECEIVER_ID, CONF_SHOW_ALL_SOURCES, CONF_ZONE2, CONF_ZONE3, DEFAULT_SHOW_SOURCES, DEFAULT_TIMEOUT, DEFAULT_ZONE2, DEFAULT_ZONE3, DOMAIN
...
async def test_config_flow_manual_host_success(hass):
    ...
    with patch(
        ...
        "homeassistant.components.denonavr.receiver.denonavr.DenonAVR.model_name", TEST_MODEL,
    ), patch(
        "homeassistant.components.denonavr.receiver.denonavr.DenonAVR.serial_number", TEST_SERIALNUMBER,
    ), patch(
...
async def test_config_flow_settings(hass):
    ...
    with patch(
        ...
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], {CONF_TIMEOUT: 5, CONF_SHOW_ALL_SOURCES: True, CONF_ZONE2: True, CONF_ZONE3: True},
        )
