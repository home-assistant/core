...
async def test_sensors_without_corona_filter(
    ...
    state_w1 = hass.states.get("sensor.aach_severity_1")

    assert state_w1.state == "minor"

    state_w2 = hass.states.get("sensor.aach_severity_2")

    assert state_w2.state == "minor"

    state_w3 = hass.states.get("sensor.aach_severity_3")

    assert state_w3.state == STATE_UNAVAILABLE  # Warning expired

    state_w4 = hass.states.get("sensor.aach_severity_4")

    assert state_w4.state == STATE_UNAVAILABLE

    state_w5 = hass.states.get("sensor.aach_severity_5")
