...
async def test_reauth_flow_wrong_account(
    ...
    """Test reauth flow with wrong account (different GWID)."""
    mock_config_entry.add_to_hass(hass)

    # Change the GWID to a different one
    mock_waterfurnace_client.gwid = "DIFFERENT_GWID_67890"
    ...
    result = cast(
        ...
        await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
                ...
                CONF_PASSWORD: "different_password",
            },
        ),
    )
