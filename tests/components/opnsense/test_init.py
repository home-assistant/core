...
@pytest.mark.parametrize("status_code", [401, 403])
async def test_setup_entry_tracker_interface_api_auth_error(
    hass: HomeAssistant, mocked_opnsense: mock.MagicMock, status_code: int
) -> None:
    """Test auth API errors during tracker interface validation."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={**ENTRY_CONFIG, opnsense.CONF_TRACKER_INTERFACES: ["LAN"]},
    )
    entry.add_to_hass(hass)
    ...
    network_insight_client = mock.MagicMock()
    network_insight_client.get_interfaces.side_effect = APIException(
        status_code=status_code, resp_body="Unauthorized"
    )
    mocked_opnsense.NetworkInsightClient.return_value = network_insight_client
...
async def test_setup_entry_tracker_interface_api_non_auth_error(
    hass: HomeAssistant, mocked_opnsense: mock.MagicMock
) -> None:
    """Test non-auth API errors during tracker interface validation."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={**ENTRY_CONFIG, opnsense.CONF_TRACKER_INTERFACES: ["LAN"]},
    )
...
async def test_setup_entry_tracker_interface_connectivity_error(
    hass: HomeAssistant, mocked_opnsense: mock.MagicMock
) -> None:
    """Test connectivity errors during tracker interface validation are retried."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={**ENTRY_CONFIG, opnsense.CONF_TRACKER_INTERFACES: ["LAN"]},
    )
    ...
    network_insight_client = mock.MagicMock()
    network_insight_client.get_interfaces.side_effect = RequestException(
        "Connection failed"
    )
    mocked_opnsense.NetworkInsightClient.return_value = network_insight_client
