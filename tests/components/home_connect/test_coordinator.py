...
async def test_coordinator_disabling_updates_for_appliance(
    ...
    assert (
        appliance_ha_id
        in config_entry.options[CONF_DISABLE_UPDATES_ON_CONNECT_PAIRED_EVENT]
    )
    ...
    # Check that fixing the issue does not enable the updates
    ...
    # Run config flow to delete from the options the appliance
    ...
    new_options = set(
        config_entry.options[CONF_DISABLE_UPDATES_ON_CONNECT_PAIRED_EVENT]
    )
    new_options.remove(appliance_ha_id)

    await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={CONF_DISABLE_UPDATES_ON_CONNECT_PAIRED_EVENT: list(new_options)},
    )
