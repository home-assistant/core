"""Tests for Elke27 lights."""

from __future__ import annotations

from types import SimpleNamespace
from unittest.mock import AsyncMock

import pytest

from homeassistant.components.elke27 import light as light_module
from homeassistant.components.elke27.coordinator import Elke27DataUpdateCoordinator
from homeassistant.components.elke27.light import async_setup_entry
from homeassistant.components.elke27.models import Elke27RuntimeData
from homeassistant.const import CONF_HOST
from homeassistant.core import HomeAssistant
from homeassistant.exceptions import HomeAssistantError

from tests.common import MockConfigEntry


class _Hub:
    def __init__(self) -> None:
        self.is_ready = True
        self.panel_name = None
        self.async_set_light = AsyncMock(return_value=True)


async def test_light_entities_updates_and_actions(hass: HomeAssistant) -> None:
    """Test light entities are created, update, and delegate actions."""
    entry = MockConfigEntry(domain="elke27", data={CONF_HOST: "192.168.1.60"})
    entry.add_to_hass(hass)

    hub = _Hub()
    coordinator = Elke27DataUpdateCoordinator(hass, hub, entry)
    snapshot = SimpleNamespace(
        panel_info=SimpleNamespace(
            mac="aa:bb:cc:dd:ee:ff",
            name="Panel A",
            serial="1234",
        ),
        lights=[
            SimpleNamespace(light_id=1, name="Light 1", on=False),
            SimpleNamespace(light_id=2, name="Light 2", on=True),
        ],
    )
    coordinator.async_set_updated_data(snapshot)
    entry.runtime_data = Elke27RuntimeData(hub=hub, coordinator=coordinator)

    entities: list[light_module.Elke27Light] = []

    def _add_entities(new_entities):
        entities.extend(new_entities)

    await async_setup_entry(hass, entry, _add_entities)
    assert len(entities) == 2

    light_1 = next(entity for entity in entities if entity._light_id == 1)
    await light_1.async_turn_on()
    hub.async_set_light.assert_awaited_once_with(1, state=True, level=99)

    snapshot.lights[0].on = True
    coordinator.async_set_updated_data(snapshot)
    await hass.async_block_till_done()
    assert light_1.is_on is True


async def test_light_pin_required(hass: HomeAssistant) -> None:
    """Test PIN-required error surfaces as HomeAssistantError."""
    entry = MockConfigEntry(domain="elke27", data={CONF_HOST: "192.168.1.61"})
    entry.add_to_hass(hass)

    hub = _Hub()
    hub.async_set_light.side_effect = light_module.Elke27PinRequiredError

    coordinator = Elke27DataUpdateCoordinator(hass, hub, entry)
    snapshot = SimpleNamespace(
        panel_info=SimpleNamespace(
            mac="aa:bb:cc:dd:ee:ff",
            name="Panel A",
            serial="1234",
        ),
        lights=[SimpleNamespace(light_id=1, name="Light 1", on=False)],
    )
    coordinator.async_set_updated_data(snapshot)
    entry.runtime_data = Elke27RuntimeData(hub=hub, coordinator=coordinator)

    entities: list[light_module.Elke27Light] = []

    def _add_entities(new_entities):
        entities.extend(new_entities)

    await async_setup_entry(hass, entry, _add_entities)
    assert len(entities) == 1

    light_1 = next(entity for entity in entities if entity._light_id == 1)
    with pytest.raises(
        HomeAssistantError, match="PIN required to perform this action."
    ):
        await light_1.async_turn_on()


async def test_light_setup_edge_cases(hass: HomeAssistant) -> None:
    """Verify setup handles missing runtime data and snapshots."""
    entry = MockConfigEntry(domain="elke27", data={CONF_HOST: "192.168.1.62"})
    entry.add_to_hass(hass)
    entry.runtime_data = None

    entities: list[light_module.Elke27Light] = []

    def _add_entities(new_entities):
        entities.extend(new_entities)

    await async_setup_entry(hass, entry, _add_entities)
    assert entities == []

    hub = _Hub()
    coordinator = Elke27DataUpdateCoordinator(hass, hub, entry)
    coordinator.async_set_updated_data(None)
    entry.runtime_data = Elke27RuntimeData(hub=hub, coordinator=coordinator)
    await async_setup_entry(hass, entry, _add_entities)
    assert entities == []

    snapshot = SimpleNamespace(lights=[])
    coordinator.async_set_updated_data(snapshot)
    await async_setup_entry(hass, entry, _add_entities)
    assert entities == []

    snapshot.lights = [SimpleNamespace(light_id="x", name="Bad")]
    coordinator.async_set_updated_data(snapshot)
    await async_setup_entry(hass, entry, _add_entities)
    assert entities == []


def test_light_properties_when_missing() -> None:
    """Verify properties handle missing light data."""
    hub = _Hub()
    coordinator = SimpleNamespace(data=None)
    entry = MockConfigEntry(domain="elke27", data={CONF_HOST: "192.168.1.63"})
    light = light_module.Elke27Light(coordinator, hub, entry, 1, SimpleNamespace())
    assert light.is_on is None
    hub.is_ready = False
    assert light.available is False
    assert light.is_on is None


async def test_light_turn_off_pin_required(hass: HomeAssistant) -> None:
    """Verify turn_off handles PIN required."""
    entry = MockConfigEntry(domain="elke27", data={CONF_HOST: "192.168.1.64"})
    entry.add_to_hass(hass)
    hub = _Hub()
    hub.async_set_light.side_effect = light_module.Elke27PinRequiredError
    coordinator = Elke27DataUpdateCoordinator(hass, hub, entry)
    coordinator.async_set_updated_data(
        SimpleNamespace(lights=[SimpleNamespace(light_id=1, on=True)])
    )
    entry.runtime_data = Elke27RuntimeData(hub=hub, coordinator=coordinator)
    light = light_module.Elke27Light(
        coordinator, hub, entry, 1, SimpleNamespace(light_id=1)
    )
    with pytest.raises(
        HomeAssistantError, match="PIN required to perform this action."
    ):
        await light.async_turn_off()


def test_light_iter_lights_variants() -> None:
    """Verify light iteration for mapping and list."""
    assert list(light_module._iter_lights({"lights": {1: "x"}})) == []
    snapshot = SimpleNamespace(lights={1: "x"})
    assert list(light_module._iter_lights(snapshot)) == ["x"]
    snapshot.lights = ["a"]
    assert list(light_module._iter_lights(snapshot)) == ["a"]
    snapshot.lights = "bad"
    assert list(light_module._iter_lights(snapshot)) == []


def test_level_from_kwargs_defaults_to_full_when_off() -> None:
    """Verify explicit brightness maps to Elk level."""
    assert light_module._level_from_kwargs({"brightness": 255}) == 99
