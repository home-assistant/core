...
class WithingsFactoryConfig:
    ...
    def __init__(
        ...
        assert isinstance(self._withings_config, dict), \
            "withings_config must be a dict."
        assert isinstance(self._api_config, dict), \
            "api_config must be a dict."
        assert isinstance(self._http_config, dict), \
            "http_config must be a dict."

        self._hass_config = {
            'homeassistant': {
                CONF_UNIT_SYSTEM: unit_system or CONF_UNIT_SYSTEM_METRIC
            },
...
class WithingsFactoryData:
    ...
    def __init__(
        ...
        self._nokia_api_get_sleep_summary_mock = \
            nokia_api_get_sleep_summary_mock
...
WithingsFactory = Callable[
    [WithingsFactoryConfig],
    Awaitable[WithingsFactoryData]
]


@pytest.fixture(name='withings_factory')
def withings_factory_fixture(
        request,
        hass
) -> WithingsFactory:
    ...
    async def factory(config: WithingsFactoryConfig) -> WithingsFactoryData:
        ...
        await async_process_ha_core_config(
            hass,
            config.hass_config.get('homeassistant')
        )
        ...
        nokia_auth_get_credentials_mock = \
            nokia_auth_get_credentials_patch.start()

        nokia_api_request_patch = asynctest.patch(
            'nokia.NokiaApi.request',
            return_value=config.nokia_request_response
        )
        nokia_api_request_mock = nokia_api_request_patch.start()

        nokia_api_get_measures_patch = asynctest.patch(
            'nokia.NokiaApi.get_measures',
            return_value=config.nokia_measures_response
        )
        nokia_api_get_measures_mock = \
            nokia_api_get_measures_patch.start()

        nokia_api_get_sleep_patch = asynctest.patch(
            'nokia.NokiaApi.get_sleep',
            return_value=config.nokia_sleep_response
        )
        nokia_api_get_sleep_mock = \
            nokia_api_get_sleep_patch.start()
        ...
        with async_create_task_patch:
            ...
            flow_id = tasks[1].result()['flow_id']
