...
async def test_default_thermostat(hass, hk_driver, cls):
    ...
    acc = cls.thermostat(hass, hk_driver, 'Thermostat', entity_id, 2, None)
    ...
    hass.states.async_set(entity_id, 'auto',
                          {ATTR_OPERATION_MODE: 'auto',
                           ATTR_OPERATION_LIST: ['heat', 'cool'],
                           ATTR_TEMPERATURE: 22.0,
                           ATTR_CURRENT_TEMPERATURE: 18.0,
                           ATTR_UNIT_OF_MEASUREMENT: '°C'})
    await hass.async_block_till_done()
    ...
    # Set from HomeKit
    call_set_temperature = async_mock_service(hass, 'climate',
                                              'set_temperature')
    call_set_operation_mode = async_mock_service(hass, 'climate',
                                                 'set_operation_mode')
    ...
    assert call_set_operation_mode[0].data[ATTR_OPERATION_MODE] == 'heat'
    assert acc.char_target_heat_cool.value == 1
...
async def test_auto_thermostat(hass, hk_driver, cls):
    ...
    # support_auto = True
    hass.states.async_set(entity_id, 'off', {ATTR_SUPPORTED_FEATURES: 6})
    await hass.async_block_till_done()
    acc = cls.thermostat(hass, hk_driver, 'Thermostat', entity_id, 2, None)
    await hass.async_add_job(acc.run)
    await hass.async_block_till_done()
...
async def test_power_state(hass, hk_driver, cls):
    ...
    # Set from HomeKit
    call_turn_on = async_mock_service(hass, 'climate', 'turn_on')
    call_turn_off = async_mock_service(hass, 'climate', 'turn_off')
    call_set_operation_mode = async_mock_service(hass, 'climate',
                                                 'set_operation_mode')
...
async def test_thermostat_fahrenheit(hass, hk_driver, cls):
    ...
    hass.states.async_set(entity_id, 'off', {ATTR_SUPPORTED_FEATURES: 6})
    await hass.async_block_till_done()
    with patch.object(hass.config.units, CONF_TEMPERATURE_UNIT, new='°F'):
        acc = cls.thermostat(hass, hk_driver, 'Thermostat', entity_id, 2, None)
    await hass.async_add_job(acc.run)
    await hass.async_block_till_done()
