...
async def test_temperature(hass):
    ...
    acc = TemperatureSensor(hass, 'Temperature', entity_id, 2, {})
...
async def test_humidity(hass):
    ...
    acc = HumiditySensor(hass, 'Humidity', entity_id, 2, {})
    await hass.async_add_job(acc.run)
...
async def test_air_quality(hass):
    ...
    acc = AirQualitySensor(hass, 'Air Quality', entity_id, 2, {})
    await hass.async_add_job(acc.run)
...
async def test_co2(hass):
    ...
    acc = CarbonDioxideSensor(hass, 'CO2', entity_id, 2, {})
    await hass.async_add_job(acc.run)
...
async def test_light(hass):
    ...
    acc = LightSensor(hass, 'Light', entity_id, 2, {})
    await hass.async_add_job(acc.run)
...
async def test_binary(hass):
    ...
    acc = BinarySensor(hass, 'Window Opening', entity_id, 2, {})
    await hass.async_add_job(acc.run)
...
async def test_binary_device_classes(hass):
    ...
    for device_class, (service, char) in BINARY_SENSOR_SERVICE_MAP.items():
        ...
        acc = BinarySensor(hass, 'Binary Sensor', entity_id, 2, {})
