...
async def test_switch_set_state(
    ...
    hass.states.async_set(entity_id, AlarmControlPanelEntityState.ARMED_AWAY)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 1
    assert acc.char_current_state.value == 1

    hass.states.async_set(entity_id, AlarmControlPanelEntityState.ARMED_HOME)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 0
    assert acc.char_current_state.value == 0

    hass.states.async_set(entity_id, AlarmControlPanelEntityState.ARMED_NIGHT)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 2
    assert acc.char_current_state.value == 2

    hass.states.async_set(entity_id, AlarmControlPanelEntityState.DISARMED)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 3
    assert acc.char_current_state.value == 3

    hass.states.async_set(entity_id, AlarmControlPanelEntityState.TRIGGERED)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 3
...
async def test_arming(hass: HomeAssistant, hk_driver) -> None:
    ...
    hass.states.async_set(entity_id, AlarmControlPanelEntityState.ARMED_VACATION)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 1
    assert acc.char_current_state.value == 1
    ...
    hass.states.async_set(entity_id, AlarmControlPanelEntityState.ARMING)
    await hass.async_block_till_done()
    assert acc.char_target_state.value == 1
    assert acc.char_current_state.value == 3
