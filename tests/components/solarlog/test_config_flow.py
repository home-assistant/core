...
async def test_form(hass: HomeAssistant, mock_setup_entry: AsyncMock) -> None:
    ...
    result2 = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {CONF_HOST: HOST, CONF_NAME: NAME, "has_password": False},
    )
    ...
    assert result2["data"]["has_password"] is False
...
async def test_user(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], {CONF_HOST: HOST, CONF_NAME: NAME, "has_password": False}
    )
    await hass.async_block_till_done()
...
async def test_form_exceptions(
    ...
    result = await flow.async_step_user(
        {CONF_NAME: NAME, CONF_HOST: HOST, "has_password": False}
    )
    await hass.async_block_till_done()
    ...
    result = await flow.async_step_user(
        {CONF_NAME: NAME, CONF_HOST: HOST, "has_password": True}
    )
    await hass.async_block_till_done()
...
async def test_abort_if_already_setup(hass: HomeAssistant, test_connect: None) -> None:
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {CONF_HOST: HOST, CONF_NAME: "solarlog_test_7_8_9", "has_password": False},
    )
...
async def test_reconfigure_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], {"has_password": True, CONF_PASSWORD: password}
    )
    await hass.async_block_till_done()
    ...
    assert entry.data["has_password"] == has_password
