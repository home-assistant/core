...
async def prometheus_client(loop, hass, hass_client):
    ...
    await setup.async_setup_component(hass, sensor.DOMAIN, {
        'sensor': [{
            'platform': 'demo'
            }]
        })

    await setup.async_setup_component(hass, climate.DOMAIN, {
        'climate': [{
            'platform': 'demo'
            }]
        })

    sensor1 = DemoSensor("Television Energy", 74, None,
                         ENERGY_KILO_WATT_HOUR, None)
    sensor1.hass = hass
    sensor1.entity_id = "sensor.television_energy"
    await sensor1.async_update_ha_state()

    sensor2 = DemoSensor("Radio Energy", 14, DEVICE_CLASS_POWER,
                         ENERGY_KILO_WATT_HOUR, None)
    sensor2.hass = hass
    sensor2.entity_id = "sensor.radio_energy"
    await sensor2.async_update_ha_state()

    sensor3 = DemoSensor("Electricity price", 0.123, None,
                         "SEK/kWh", None)
    sensor3.hass = hass
...
def test_view(prometheus_client):  # pylint: disable=redefined-outer-name
    ...
    assert 'temperature_c{domain="sensor",' \
           'entity="sensor.outside_temperature",' \
           'friendly_name="Outside Temperature"} 15.6' in body

    assert 'battery_level_percent{domain="sensor",' \
           'entity="sensor.outside_temperature",' \
           'friendly_name="Outside Temperature"} 12.0' in body

    assert 'current_temperature_c{domain="climate",' \
           'entity="climate.heatpump",' \
           'friendly_name="HeatPump"} 25.0' in body

    assert 'humidity_percent{domain="sensor",' \
    ...
    assert 'power_kwh{domain="sensor",' \
