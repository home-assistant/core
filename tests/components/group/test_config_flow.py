...
async def test_options_member_of_own_group(
    hass: HomeAssistant, group_type, member_state
) -> None:
    """Test it's not possible to include group as member of itself."""
    members1 = [f"{group_type}.one", f"{group_type}.two"]
    members2 = [f"{group_type}.four", f"{group_type}.bed_room"]
    ...
    assert result["type"] == RESULT_TYPE_FORM
    assert result["errors"]["base"] == "own_entity_not_allowed"
