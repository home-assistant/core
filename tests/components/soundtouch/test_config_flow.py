...
async def test_user_flow_create_entry(
    ...
    result = await hass.config_entries.flow.async_configure(
        ...
        user_input={
            ...
            CONF_NAME: "MockDeviceName",
        },
    )

    assert result.get("type") == RESULT_TYPE_CREATE_ENTRY
    assert result.get("title") == "MockDeviceName"
...
async def test_zeroconf_flow_create_entry(
    ...
    assert result.get("description_placeholders") == {CONF_NAME: DEVICE_1_NAME}
