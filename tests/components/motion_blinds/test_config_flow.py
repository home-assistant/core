...
async def test_get_interfaces_disabled_adapter_excluded(hass: HomeAssistant) -> None:
    """Test that disabled adapters are not added to the interface list."""
    gateway = ConnectMotionGateway(hass)

    with patch(
        "homeassistant.components.motion_blinds.gateway.network.async_get_adapters",
        return_value=MOCK_ADAPTERS_WITH_DISABLED,
    ):
        interfaces = await gateway.async_get_interfaces()

    assert "10.0.0.1" not in interfaces
    assert "192.168.1.10" in interfaces
