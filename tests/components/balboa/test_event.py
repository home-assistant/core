...
async def test_event(hass: HomeAssistant, client: MagicMock) -> None:
    ...
    setattr(
        client,
        "fault",
        MagicMock(fault_datetime=datetime(2025, 2, 15, 13, 0), message="TEST"),
    )
    client.emit("")
    ...
    state = hass.states.get(ENTITY_EVENT)
    assert state.attributes[ATTR_EVENT_TYPES] == [FAULT]
    assert state.attributes[ATTR_EVENT_TYPE] == FAULT
    assert state.attributes[f"{FAULT}_date"] == "2025-02-15T13:00:00"
    assert state.attributes["message"] == "TEST"
