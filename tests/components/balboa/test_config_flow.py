"""Test the Balboa Spa config flow."""
from unittest.mock import patch

from homeassistant import config_entries, setup
from homeassistant.components.balboa.config_flow import AlreadyConfigured, CannotConnect
...
async def test_form(hass):
    ...
    with patch(
        "homeassistant.components.balboa.config_flow.validate_input",
        return_value=mock_coro({"title": "Test Title"}),
    ), patch(
        "homeassistant.components.balboa.async_setup", return_value=mock_coro(True)
    ) as mock_setup, patch(
