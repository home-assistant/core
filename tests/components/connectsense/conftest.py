...
from homeassistant.components.connectsense.const import CONF_NOTIFY_ENABLED
...
def mock_http(aioclient_mock, host: str, serial: str):
    ...
    aioclient_mock.get(f"{base}/control", json={"outlet_active": False})
    aioclient_mock.post(f"{base}/control", status=200, text="OK")
    aioclient_mock.post(f"{base}/config", status=200, text="OK")
    aioclient_mock.post(f"{base}/notify", status=200, text="OK")
...
async def setup_entry(
    ...
    entry = MockConfigEntry(
        ...
        options={CONF_NOTIFY_ENABLED: False},
    )
