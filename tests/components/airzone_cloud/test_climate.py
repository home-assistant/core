...
async def test_airzone_create_climates(hass: HomeAssistant) -> None:
    ...
    # Assume these entities defaults
    def _d_temp(value, unit=UnitOfTemperature.CELSIUS, precision=PRECISION_TENTHS):
        return display_temp(
            hass,
            value,
            unit,
    ...
    def _d_interval(unit=UnitOfTemperature.CELSIUS, precision=PRECISION_TENTHS):
    ...
    assert state.attributes[ATTR_CURRENT_TEMPERATURE] == _d_temp(21.0)
    ...
    assert state.attributes[ATTR_MAX_TEMP] == _d_temp(30)
    assert state.attributes[ATTR_MIN_TEMP] == _d_temp(15)
    assert state.attributes[ATTR_TARGET_TEMP_STEP] == _d_interval()
    assert state.attributes[ATTR_TEMPERATURE] == 22.0

    state = hass.states.get("climate.bron_pro")
    assert state.state == HVACMode.COOL
    assert ATTR_CURRENT_HUMIDITY not in state.attributes
    assert state.attributes[ATTR_CURRENT_TEMPERATURE] == _d_temp(20.0)
    ...
    assert state.attributes[ATTR_MIN_TEMP] == _d_temp(15)
    assert state.attributes[ATTR_TARGET_TEMP_STEP] == _d_interval()
    assert state.attributes.get(ATTR_TEMPERATURE) == _d_temp(22.0)

    # Groups
    state = hass.states.get("climate.group")
    assert state.state == HVACMode.COOL
    assert state.attributes[ATTR_CURRENT_HUMIDITY] == 27
    assert state.attributes[ATTR_CURRENT_TEMPERATURE] == _d_temp(22.5)
    ...
    assert state.attributes[ATTR_MIN_TEMP] == _d_temp(15)
    assert state.attributes[ATTR_TARGET_TEMP_STEP] == _d_interval()
    assert state.attributes[ATTR_TEMPERATURE] == _d_temp(24.0)

    # Installations
    state = hass.states.get("climate.house")
    assert state.state == HVACMode.COOL
    assert state.attributes[ATTR_CURRENT_HUMIDITY] == _d_temp(27)
    assert state.attributes[ATTR_CURRENT_TEMPERATURE] == _d_temp(21.5)
    assert ATTR_FAN_MODE not in state.attributes
    assert ATTR_FAN_MODES not in state.attributes
    assert state.attributes[ATTR_HVAC_ACTION] == HVACAction.COOLING
    assert state.attributes[ATTR_HVAC_MODES] == [
    ...
    assert state.attributes[ATTR_MIN_TEMP] == _d_temp(15)
    assert state.attributes[ATTR_TARGET_TEMP_STEP] == _d_interval()
    assert state.attributes[ATTR_TEMPERATURE] == _d_temp(23.0)

    # Zones
    state = hass.states.get("climate.dormitorio")
    assert state.state == HVACMode.OFF
    assert state.attributes[ATTR_CURRENT_HUMIDITY] == _d_temp(24)
    assert state.attributes[ATTR_CURRENT_TEMPERATURE] == _d_temp(25.0)
    assert ATTR_FAN_MODE not in state.attributes
    assert ATTR_FAN_MODES not in state.attributes
    assert state.attributes[ATTR_HVAC_ACTION] == HVACAction.OFF
    assert state.attributes[ATTR_HVAC_MODES] == [
