...
async def setup_test_entity_cover(
...
async def test_cover_config_entry(hass: HomeAssistant) -> None:
    """Test that the cover is added by config entry."""
...
async def test_no_covers(caplog: LogCaptureFixture, hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        await setup_test_entity_cover(hass, {})
...
async def test_no_poll_when_cover_has_no_command_state(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        await setup_test_entity_cover(hass, {"test": {}})
        async_fire_time_changed(hass, dt_util.utcnow() + SCAN_INTERVAL)
        await hass.async_block_till_done()
...
async def test_poll_when_cover_has_command_state(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        await setup_test_entity_cover(hass, {"test": {"command_state": "echo state"}})
        async_fire_time_changed(hass, dt_util.utcnow() + SCAN_INTERVAL)
        await hass.async_block_till_done()
