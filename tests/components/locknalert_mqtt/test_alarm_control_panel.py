...
from homeassistant.components.locknalert_mqtt.alarm_control_panel import (
...
async def test_discovery_update_unchanged_alarm(
    ...
    with patch(
        "homeassistant.components.locknalert_mqtt.alarm_control_panel.LocknAlertMqttAlarm.discovery_update"
    ) as discovery_update:
...
@pytest.mark.parametrize(
    "hass_config",
    [DEFAULT_CONFIG, {"locknalert_mqtt": [DEFAULT_CONFIG["locknalert_mqtt"]]}],
...
async def test_reload_after_invalid_config(
    ...
    with patch(
        "homeassistant.components.locknalert_mqtt.ir.async_delete_issue"
