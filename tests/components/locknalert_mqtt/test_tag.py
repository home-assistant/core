"""The tests for the locknalert_mqtt tag scanner."""
...
async def test_discover_bad_tag(
    ...
    assert device_registry.async_get_device(identifiers={(DOMAIN, "0AFFD2")}) is None
...
async def test_if_fires_on_mqtt_message_with_template(
    ...
    """Test tag scanning with value template."""
    await mqtt_mock_entry()
...
async def test_if_fires_on_mqtt_message_after_update_without_device(
    ...
    """Test tag scanning after update without device."""
    await mqtt_mock_entry()
...
async def test_if_fires_on_mqtt_message_after_update_with_template(
    ...
    """Test tag scanning after template update."""
    await mqtt_mock_entry()
...
async def test_no_resubscribe_same_topic(
    ...
    async_fire_mqtt_message(hass, "homeassistant/tag/bla1/config", json.dumps(config))
    await hass.async_block_till_done()
    assert device_registry.async_get_device(identifiers={(DOMAIN, "0AFFD2")})
...
async def test_not_fires_on_mqtt_message_after_remove_from_registry(
    ...
    """Test tag scanning after removal from registry."""
...
async def test_cleanup_tag(
    ...
    mqtt_entry = hass.config_entries.async_entries(DOMAIN)[0]
    ...
    device_entry2 = device_registry.async_get_device(identifiers={(DOMAIN, "hejhopp")})
