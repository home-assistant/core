...
from homeassistant.components.livisi.const import (
    DOMAIN,
    IS_OPEN,
...
async def test_setup_entities_and_unload(
    ...
    connection.async_set_state.assert_has_awaits(
        [
            ...
            call("switch-capability", key=ON_STATE, value=True),
            call(
                "setpoint-capability",
    ...
    connection.async_set_state.return_value = False
    ...
    on_data(
        LivisiWebsocketEvent(
            namespace="core.RWE",
            type=LIVISI_EVENT_STATE_CHANGED,
            source="window-capability",
            timestamp=None,
            properties={IS_OPEN: True},
        )
    )
    await hass.async_block_till_done()
    assert hass.states.is_state(binary_sensor_id, STATE_ON)
    ...
    on_data(
        LivisiWebsocketEvent(
            namespace="core.RWE",
            type=LIVISI_EVENT_STATE_CHANGED,
            source="setpoint-capability",
            timestamp=None,
            properties={target_temperature_property: 23.0},
        )
    )
    await hass.async_block_till_done()
    climate_state = hass.states.get(climate_id)
    assert climate_state is not None
    assert climate_state.attributes[ATTR_TEMPERATURE] == 23.0
    ...
    on_data(
        LivisiWebsocketEvent(
            namespace="core.RWE",
            type=LIVISI_EVENT_STATE_CHANGED,
            source="humidity-capability",
            timestamp=None,
            properties={"humidity": 50},
        )
    )
    await hass.async_block_till_done()
    climate_state = hass.states.get(climate_id)
    assert climate_state is not None
    assert climate_state.attributes[ATTR_CURRENT_HUMIDITY] == 50
    assert climate_state.state != STATE_UNAVAILABLE
    ...
    connection.async_get_devices.side_effect = LivisiException
    await config_entry.runtime_data.async_refresh()
    assert hass.states.is_state(binary_sensor_id, STATE_UNAVAILABLE)
    assert hass.states.is_state(climate_id, STATE_UNAVAILABLE)
...
@pytest.mark.parametrize(
    ("device", "platform"),
    [
        pytest.param(DEVICES[0], Platform.BINARY_SENSOR, id="binary-sensor"),
        pytest.param(DEVICES[1], Platform.SWITCH, id="switch"),
...
async def test_new_unreachable_event_wins_over_recovery(
    ...
    recovery_call_count = connection.async_get_value.await_count
    await config_entry.runtime_data.async_refresh()
    assert connection.async_get_value.await_count == recovery_call_count
    ...
    async_dispatcher_send(
        hass,
        f"{LIVISI_REACHABILITY_CHANGE}_switch-device",
        True,
        0,
    )
    await hass.async_block_till_done()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            LivisiException(), ConfigEntryState.SETUP_RETRY, id="connection-error"
        ),
    ],
)
async def test_setup_error(
    hass: HomeAssistant,
    exception: LivisiException,
