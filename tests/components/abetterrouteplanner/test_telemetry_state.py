...
* the four-surface ``forget_vehicle`` sweep;
...
# ---------- 6. forget_vehicle four-surface sweep ---------------------------


async def test_forget_vehicle_clears_all_four_surfaces_and_re_dispatches(
    hass: HomeAssistant,
    telemetry_coordinator: AbrpTelemetryCoordinator,
) -> None:
    """``forget_vehicle`` sweeps all four surfaces; re-adding re-fires dispatch.

    Populate ``data`` / ``last_reported_at`` / ``last_provider`` /
    ``_presence_seen`` for a vid, forget it, assert all four are clear for
    that vid while a second vehicle is retained, then re-apply a frame and
    assert the first-seen dispatch fires again (proving ``_presence_seen`` was
    swept, not just ``data``).
    """
    coordinator = telemetry_coordinator
    coordinator.register_presence_predicates({Metric.VOLTAGE})
    ...
    try:
        ...
        await hass.async_block_till_done()
        assert dispatched == [
            (MOCK_VEHICLE_ID, Metric.VOLTAGE),
            (MOCK_VEHICLE_ID_2, Metric.VOLTAGE),
        ]
        # Simulate the sensor platform marking both pairs seen after entity
        # creation so ``_presence_seen`` is populated (the coordinator itself
        # never self-adds — that is the platform's job).
        coordinator.mark_metric_seen(MOCK_VEHICLE_ID, Metric.VOLTAGE)
        coordinator.mark_metric_seen(MOCK_VEHICLE_ID_2, Metric.VOLTAGE)

        # Pre-check: all four surfaces populated for the vid we will forget.
        assert MOCK_VEHICLE_ID in coordinator.data
        assert MOCK_VEHICLE_ID in coordinator.last_reported_at
        assert MOCK_VEHICLE_ID in coordinator.last_provider
        assert (MOCK_VEHICLE_ID, Metric.VOLTAGE) in coordinator._presence_seen

        coordinator.forget_vehicle(MOCK_VEHICLE_ID)

        # All four cleared for the forgotten vid.
        assert MOCK_VEHICLE_ID not in coordinator.data
        assert MOCK_VEHICLE_ID not in coordinator.last_reported_at
        assert MOCK_VEHICLE_ID not in coordinator.last_provider
        assert (MOCK_VEHICLE_ID, Metric.VOLTAGE) not in coordinator._presence_seen

        # The other vehicle is untouched.
        assert MOCK_VEHICLE_ID_2 in coordinator.data
        assert (MOCK_VEHICLE_ID_2, Metric.VOLTAGE) in coordinator._presence_seen
...
async def test_forget_vehicle_unknown_id_is_noop(
    telemetry_coordinator: AbrpTelemetryCoordinator,
) -> None:
    """Forgetting a never-seen vid is a silent no-op (pop-with-default)."""
    coordinator = telemetry_coordinator

    coordinator.forget_vehicle(999999)

    assert coordinator.data == {}
    assert coordinator.last_reported_at == {}
    assert coordinator.last_provider == {}
