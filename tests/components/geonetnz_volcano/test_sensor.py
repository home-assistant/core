...
from homeassistant.components.geonetnz_volcano.sensor import (
    ATTR_STATUS,
    ATTR_LAST_UPDATE,
    ATTR_CREATED,
    ATTR_UPDATED,
...
async def test_setup(hass):
    ...
    with patch("homeassistant.util.dt.utcnow", return_value=utcnow), patch(
        ...
        state = hass.states.get("sensor.geonet_nz_volcano_32_87336_117_22743")
        assert state is not None
        assert int(state.state) == 3
        assert state.name == "GeoNet NZ Volcano (32.87336, -117.22743)"
        ...
        assert attributes[ATTR_UNIT_OF_MEASUREMENT] == "volcanos"
        assert attributes[ATTR_ICON] == "mdi:mountain"
