...
async def test_user_input_device_not_found(
    hass: HomeAssistant, mock_get_device_info_mc_exception, mock_get_source_ip: None
) -> None:
...
async def test_user_input_non_yamaha_device_found(
    hass: HomeAssistant, mock_get_device_info_invalid, mock_get_source_ip: None
) -> None:
...
async def test_user_input_device_already_existing(
    hass: HomeAssistant, mock_get_device_info_valid, mock_get_source_ip: None
) -> None:
...
async def test_user_input_unknown_error(
    hass: HomeAssistant, mock_get_device_info_exception, mock_get_source_ip: None
) -> None:
    """Test when user specifies an existing device, which does not provide the musiccast API."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
...
async def test_ssdp_discovery_failed(
    hass: HomeAssistant, mock_ssdp_no_yamaha, mock_get_source_ip: None
) -> None:
...
async def test_ssdp_discovery_successful_add_device(
    hass: HomeAssistant, mock_ssdp_yamaha, mock_get_source_ip: None
) -> None:
