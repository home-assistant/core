...
def mock_device_info(device_type: str) -> dict[str, Any]:
    ...
    return json_loads(load_fixture(get_device_file_name(device_type), DOMAIN))


def mock_device_profile(device_type: str) -> dict[str, Any]:
    """Load a mock device profile from json file."""
    return json_loads(load_fixture(get_profile_file_name(device_type), DOMAIN))


def mock_device_status(device_type: str) -> dict[str, Any]:
    """Load a mock device status from json file."""
    return json_loads(load_fixture(get_status_file_name(device_type), DOMAIN))
...
def mock_thinq(device_info: dict[str, Any]) -> MagicMock:
    ...
    thinq.async_get_device_status = AsyncMock(return_value=mock_thinq_api_response())
    return thinq
...
async def mock_lg_device(hass: HomeAssistant, device_info: dict[str, Any]) -> LGDevice:
