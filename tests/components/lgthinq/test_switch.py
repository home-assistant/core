...
async def test_switch(
    hass: HomeAssistant, init_integration_thinq: MockConfigEntry
) -> None:
    ...
    dehumidifier: LGDevice = get_mock_lg_device_for_type(
        init_integration_thinq, DEHUMIDIFIER
    )
    assert dehumidifier
    dehumidifier._thinq.async_get_device_status = AsyncMock(
        return_value=mock_thinq_api_response(
            status=200, body=mock_device_status(DEHUMIDIFIER)
        )
    )
