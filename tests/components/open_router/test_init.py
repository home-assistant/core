...
async def test_migrate_entry_from_v1_0_to_v1_1(
    ...
    assert conversation_subentry.data[CONF_CHAT_MODEL] == "openai/gpt-3.5-turbo"
    assert CONF_MODEL not in conversation_subentry.data
    ...
    assert conversation_subentry.data[CONF_PROVIDER] == []

    ai_task_subentry = entry.subentries["ai_task_subentry"]
    assert ai_task_subentry.data[CONF_CHAT_MODEL] == "openai/gpt-4"
    assert CONF_MODEL not in ai_task_subentry.data
    assert ai_task_subentry.data[CONF_WEB_SEARCH] is False
    assert ai_task_subentry.data[CONF_PROVIDER] == []
...
async def test_migrate_entry_already_migrated(
    ...
    entry = MockConfigEntry(
        ...
        subentries_data=[
            ConfigSubentryData(
                data={
                    CONF_CHAT_MODEL: "openai/gpt-3.5-turbo",
                    CONF_PROMPT: "You are a helpful assistant.",
    ...
    assert conversation_subentry.data[CONF_PROVIDER] == ["openai"]
