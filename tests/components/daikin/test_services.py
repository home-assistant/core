...
@pytest.mark.asyncio
async def test_service_target_entity_filters_devices(hass: HomeAssistant) -> None:
    """Ensure the service uses the targeted entity to select the coordinator."""
    config_entry_1 = MockConfigEntry(domain="daikin", entry_id="entry1")
    config_entry_1.add_to_hass(hass)
    config_entry_2 = MockConfigEntry(domain="daikin", entry_id="entry2")
    config_entry_2.add_to_hass(hass)

    device1 = FakeDevice("00:11:22:33:44:55")
    device2 = FakeDevice("66:77:88:99:AA:BB")
    coordinator1 = FakeCoordinator(device1)
    coordinator2 = FakeCoordinator(device2)
    hass.data.setdefault("daikin", {})["entry1"] = coordinator1
    ...
    await hass.services.async_call(
        ...
        {"zone_id": 0, "temperature": 20, "entity_id": "climate.entry2"},
        blocking=True,
    )

    assert coordinator1.device.lztemp_h.get(0, 22) == 22
    assert coordinator2.device.lztemp_h[0] == "20"
    assert coordinator2.device.lztemp_c[0] == "20"


@pytest.mark.asyncio
async def test_service_entry_id_not_found(hass: HomeAssistant) -> None:
    """Raise when a provided config entry id is not loaded."""

    hass.data.setdefault("daikin", {})
    await services.async_setup_services(hass)
    ...
    with pytest.raises(ServiceValidationError) as err:
        await hass.services.async_call(
            "daikin",
            services.SERVICE_SET_ZONE_TEMPERATURE,
            {"zone_id": 0, "temperature": 21, "entry_id": "missing"},
            blocking=True,
        )

    assert err.value.translation_key == "service_entry_not_found"


@pytest.mark.asyncio
async def test_service_requires_target_when_multiple_entries(
    hass: HomeAssistant,
) -> None:
    """Raise when multiple devices exist but no explicit target is provided."""
    ...
    coordinator1 = FakeCoordinator(FakeDevice("00:11:22:33:44:55"))
    coordinator2 = FakeCoordinator(FakeDevice("66:77:88:99:AA:BB"))
    hass.data.setdefault("daikin", {})["entry1"] = coordinator1
    hass.data["daikin"]["entry2"] = coordinator2
    ...
    assert err.value.translation_key == "service_target_not_found"
...
async def test_service_nonexistent_zone(setup_integration) -> None:
    ...
    hass, coordinator = setup_integration
    await services.async_setup_services(hass)
    service_data = {"zone_id": 2, "temperature": 22}
    with pytest.raises(ServiceValidationError) as err:
        await hass.services.async_call(
            "daikin", services.SERVICE_SET_ZONE_TEMPERATURE, service_data, blocking=True
        )
