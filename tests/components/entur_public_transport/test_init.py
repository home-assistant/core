...
async def test_setup_entry_expand_quays(
    ...
    """Test that expand_all_quays is called when enabled during setup."""
    mock_config_entry.add_to_hass(hass)
    await hass.config_entries.async_setup(mock_config_entry.entry_id)
    await hass.async_block_till_done()
...
async def test_setup_entry_expand_quays_error(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_entur_client: MagicMock,
    side_effect: Exception,
) -> None:
    """Test that expand_all_quays errors raise ConfigEntryNotReady."""
    mock_config_entry.add_to_hass(hass)
    mock_entur_client.expand_all_quays.side_effect = side_effect

    await hass.config_entries.async_setup(mock_config_entry.entry_id)
    await hass.async_block_till_done()

    # ConfigEntryNotReady is caught by Home Assistant and results in SETUP_RETRY
...
async def test_setup_entry_update_error(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_entur_client: MagicMock,
    side_effect: Exception,
) -> None:
    """Test that update errors raise ConfigEntryNotReady."""
    mock_config_entry.add_to_hass(hass)
    mock_entur_client.update.side_effect = side_effect
