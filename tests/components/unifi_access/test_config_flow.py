...
async def test_integration_discovery_no_update_when_online(
    hass: HomeAssistant,
    init_integration: MockConfigEntry,
) -> None:
    """Test discovery does not update host when the console is online."""
    hass.config_entries.async_update_entry(init_integration, unique_id="B4FBE4AABBCC")
    ...
    assert result["type"] is FlowResultType.ABORT
    assert result["reason"] == "already_configured"
    assert init_integration.data[CONF_HOST] == MOCK_HOST
...
async def test_integration_discovery_skips_non_matching_host(
    hass: HomeAssistant,
) -> None:
    """Test discovery skips entries with non-matching host before finding a match."""
    other_entry = MockConfigEntry(
        domain=DOMAIN,
        title="Other Console",
        data={
    ...
    assert result["type"] is FlowResultType.ABORT
    assert result["reason"] == "already_configured"
    assert matching_entry.unique_id == "B4FBE4AABBCC"
