...
SSDP = SsdpServiceInfo(
    ...
    upnp={
        ...
        ATTR_UPNP_PRESENTATION_URL: "http://192.168.1.2/",
        ATTR_UPNP_SERIAL: "08HRB8GJ3F019DD",
...
async def test_full_flow(
    ...
    assert result["result"].unique_id == "08HRB8GJ3F019DD"


async def test_already_configured(
    hass: HomeAssistant, mock_config_entry: MockConfigEntry, mock_syncthru: AsyncMock
) -> None:
...
async def test_syncthru_not_supported(
    ...
    assert result["type"] is FlowResultType.ABORT
    assert result["reason"] == "syncthru_not_supported"
...
async def test_unknown_state(hass: HomeAssistant, mock_syncthru: AsyncMock) -> None:
    ...
    assert result["type"] is FlowResultType.FORM
    assert result["step_id"] == "user"
    assert result["errors"] == {"base": "unknown_state"}
...
async def test_ssdp(
    ...
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_SSDP}, data=SSDP
    )
    ...
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"] == {CONF_URL: "http://192.168.1.2/"}
    assert result["title"] == "SEC84251907C415"
    assert result["result"].unique_id == "08HRB8GJ3F019DD"
