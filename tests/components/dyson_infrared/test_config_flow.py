...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            DysonDeviceType.FAN, "Dyson Fan via My Living Room Emitter", id="fan"
        ),
        pytest.param(
...
async def test_form_and_create_entry(
    hass: HomeAssistant, device_type: DysonDeviceType, title: str
) -> None:
    """Test that the user config flow creates an entry without further steps."""
    with (
        patch(
