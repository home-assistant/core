...
from . import (
    ...
    WOLOCK_SERVICE_INFO,
    WOMETERTHP_SERVICE_INFO_NOT_CONNECTABLE,
...
async def test_async_step_bluetooth_meter_pro_not_connectable(
    hass: HomeAssistant,
) -> None:
    """Test discovery via bluetooth for Meter Pro from a non-connectable source."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN,
        context={"source": SOURCE_BLUETOOTH},
        data=WOMETERTHP_SERVICE_INFO_NOT_CONNECTABLE,
    )
    assert result["type"] is FlowResultType.FORM
    assert result["step_id"] == "confirm"
    ...
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["title"] == "Meter Pro EEFF"
    assert result["data"] == {
        CONF_ADDRESS: "AA:BB:CC:DD:EE:FF",
        CONF_SENSOR_TYPE: "hygrometer_pro",
    }
