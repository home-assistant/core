...
async def test_user_setup_woroller_shade(hass: HomeAssistant) -> None:
    """Test the user-initiated discovery flow seeds the quiet mode option for roller shades."""
    ...
    with patch(
        "homeassistant.components.switchbot.config_flow.async_discovered_service_info",
        return_value=[ROLLER_SHADE_SERVICE_INFO],
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"next_step_id": "select_device"},
    ...
    assert result["options"] == {
        CONF_RETRY_COUNT: 3,
        CONF_ROLLER_SHADE_QUIET_MODE: False,
    }
...
async def test_options_flow_roller_shade_quiet_mode(hass: HomeAssistant) -> None:
    """Test updating the roller shade quiet mode option."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            CONF_ADDRESS: "aa:bb:cc:dd:ee:ff",
        ...
        options={CONF_RETRY_COUNT: 2, CONF_ROLLER_SHADE_QUIET_MODE: False},
        unique_id="aabbccddeeff",
    )
    entry.add_to_hass(hass)
    ...
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"][CONF_RETRY_COUNT] == 4
    assert result["data"][CONF_ROLLER_SHADE_QUIET_MODE] is True
