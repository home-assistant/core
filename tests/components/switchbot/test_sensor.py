...
async def test_presence_sensor(hass: HomeAssistant) -> None:
    """Test setting up creates the sensor for Presence Sensor."""
    await async_setup_component(hass, DOMAIN, {})
    ...
    light_sensor = hass.states.get("sensor.test_name_light_level")
    light_sensor_attrs = light_sensor.attributes
    assert light_sensor.state == "12"
    assert light_sensor_attrs[ATTR_FRIENDLY_NAME] == "test-name Light level"
    ...
    motion_sensor = hass.states.get("binary_sensor.test_name_occupancy")
    motion_sensor_attrs = motion_sensor.attributes
    assert motion_sensor.state == "on"
    assert motion_sensor_attrs[ATTR_FRIENDLY_NAME] == "test-name Occupancy"
