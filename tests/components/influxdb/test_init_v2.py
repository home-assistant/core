...
@mock.patch("homeassistant.components.influxdb.InfluxDBClientV2")
@mock.patch(
...
class TestInfluxDB(unittest.TestCase):
    ...
    def test_setup_config_full(self, mock_client):
        ...
        assert mock_client.return_value.write_api.return_value.write.call_count == 1

    def test_setup_minimal_config_v2(self, mock_client):
    ...
    def test_setup_missing_v2_required(self, mock_client):
        """Test the setup with v2 but missing token and organization."""
        config = {"influxdb": {"api_v2": True}}
    ...
    def _setup(self, mock_client, **kwargs):
        ...
        mock_client.return_value.write_api.return_value.write.reset_mock()
    ...
    def test_event_listener(self, mock_client):
        ...
        for in_, out in valid.items():
            ...
            assert mock_client.return_value.write_api.return_value.write.call_count == 1
            assert (
                mock_client.return_value.write_api.return_value.write.call_args
                == mock.call(bucket=influxdb.DEFAULT_BUCKET, record=body)
            )
            mock_client.return_value.write_api.return_value.write.reset_mock()
    ...
    def test_event_listener_states(self, mock_client):
        ...
        for state_state in (1, "unknown", "", "unavailable"):
            ...
            if state_state == 1:
                assert (
                    mock_client.return_value.write_api.return_value.write.call_count
                    == 1
                )
                assert (
                    mock_client.return_value.write_api.return_value.write.call_args
                ...
                assert not mock_client.return_value.write_api.return_value.write.called
            mock_client.return_value.write_api.return_value.write.reset_mock()
    ...
    def test_event_listener_whitelist(self, mock_client):
        ...
        self._setup(mock_client, include={"entities": ["fake.included"]})
    ...
    def test_event_listener_whitelist_domain(self, mock_client):
        """Test the event listener against a whitelist."""
        self._setup(mock_client, include={"domains": ["fake"]})
    ...
    def test_event_listener_whitelist_domain_and_entities(self, mock_client):
        """Test the event listener against a whitelist."""
        self._setup(
            mock_client, include={"domains": ["fake"], "entities": ["other.one"]}
        )
    ...
    def test_event_listener_unit_of_measurement_field(self, mock_client):
        ...
        self._setup(
            mock_client, override_measurement="state",
        )
    ...
    def test_event_listener_tags_attributes(self, mock_client):
        ...
        self._setup(
            mock_client, tags_attributes=["friendly_fake"],
        )
    ...
    def test_scheduled_write(self, mock_client):
        ...
        event = mock.MagicMock(data={"new_state": state}, time_fired=12345)
        mock_client.return_value.write_api.return_value.write.side_effect = IOError(
            "foo"
        )
        ...
        mock_client.return_value.write_api.return_value.write.assert_called_with(
            bucket=influxdb.DEFAULT_BUCKET, record=json_data
        )

        # Write works again
        mock_client.return_value.write_api.return_value.write.side_effect = None
        with mock.patch.object(influxdb.time, "sleep") as mock_sleep:
            self.handler_method(event)
            self.hass.data[influxdb.DOMAIN].block_till_done()
            assert not mock_sleep.called
        assert mock_client.return_value.write_api.return_value.write.call_count == 3
    ...
    def test_queue_backlog_full(self, mock_client):
        ...
        with mock.patch(
            ...
            assert mock_client.return_value.write_api.return_value.write.call_count == 0
