...
def create_client_mock(
    ...
    started_loop: Event,
    charge_point: dict,
    charge_cards: list[dict[str, Any]],
    status: dict,
    grid: dict,
) -> MagicMock:
    ...
    async def connect(receiver):
        ...
        await client_mock.get_charge_cards()
    ...
    async def get_charge_cards() -> None:
        """Send a list of charge cards to the callback."""
        await client_mock.receiver({"object": "CHARGE_CARDS", "cards": charge_cards})
...
async def init_integration(
    ...
    grid: dict | None = None,
    charge_cards: list[dict[str, Any]] | None = None,
    ...
    if charge_cards is None:
        charge_cards = [DEFAULT_CHARGE_CARD]
    ...
    client_mock = create_client_mock(
        hass, future_container, started_loop, charge_point, charge_cards, status, grid
    )
