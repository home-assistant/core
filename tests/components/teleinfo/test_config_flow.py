...
async def test_usb_discovery_success(
    ...
    config_entry = result["result"]
    assert config_entry.unique_id == "0403:6015_AB1234_FTDI_FT230X Basic UART"
    assert config_entry.data == {
        CONF_SERIAL_PORT: "/dev/ttyUSB0",
    }
...
async def test_usb_discovery_already_configured_updates_path(
    ...
    # Existing entry with same USB unique_id but old path
    existing_entry = MockConfigEntry(
        title="Teleinfo (/dev/ttyUSB-old)",
        domain=DOMAIN,
        data={CONF_SERIAL_PORT: "/dev/ttyUSB-old"},
        unique_id="0403:6015_AB1234_FTDI_FT230X Basic UART",
    )
    existing_entry.add_to_hass(hass)

    # Discover same dongle at new path
    new_discovery = UsbServiceInfo(
...
async def test_usb_discovery_manual_entry_duplicate(
    ...
    """Test USB discovery aborts when the port was already added manually."""

    # mock_config_entry has ADCO unique_id, matched via serial port data
    mock_config_entry.add_to_hass(hass)
