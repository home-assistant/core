...
async def test_device_registry(
    hass: HomeAssistant, async_autosetup_bluesound, soco
) -> None:
    """Test bluesound device registered in the device registry."""
    device_registry = dr.async_get(hass)
    reg_device = device_registry.async_get_device(
        identifiers={("bluesound", "RINCON_test")}
    )
...
async def test_entity_basic(
    hass: HomeAssistant, async_autosetup_bluesound, discover
) -> None:
