...
async def test_unique_id_new_entry(
    ...
    with (
        # We are not interested in the entry migration here
        patch(
        ...
        patch(
            "homeassistant.components.foscam.camera.HassFoscamCamera.async_added_to_hass",
        ),
    ):
...
async def test_switch_unique_id_migration_ok(
    ...
    entity_id = entity_registry.async_get_entity_id(
        SWITCH_DOMAIN, DOMAIN, "sleep_switch"
    )
...
async def test_unique_id_migration_not_needed(
    ...
    """Test that the unique ID for a sleep switch is not executed if already right format."""
    entry = MockConfigEntry(
        domain=config_flow.DOMAIN,
        data=VALID_CONFIG,
        entry_id=ENTRY_ID,
