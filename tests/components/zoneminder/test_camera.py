...
async def test_async_setup_entry(zoneminder_mock, hass: HomeAssistant) -> None:
    ...
    is_available_property = PropertyMock(return_value=False)
    ...
    type(monitor1).is_available = is_available_property
    ...
    await hass.services.async_call(
        HASS_DOMAIN, SERVICE_UPDATE_ENTITY, {ATTR_ENTITY_ID: camera1_entity_id}
    )
    await hass.async_block_till_done()
    ...
    assert state1.attributes["monitor_id"] == monitor1.id
    ...
    monitor1.is_recording = True
    await hass.services.async_call(
        CAMERA_DOMAIN, SERVICE_TURN_OFF, {ATTR_ENTITY_ID: camera1_entity_id}
    )
    await hass.services.async_call(
        HASS_DOMAIN, SERVICE_UPDATE_ENTITY, {ATTR_ENTITY_ID: camera1_entity_id}
    )
    ...
    monitor1.is_recording = True
    await hass.services.async_call(
        CAMERA_DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: camera1_entity_id}
    )
    await hass.services.async_call(
        HASS_DOMAIN, SERVICE_UPDATE_ENTITY, {ATTR_ENTITY_ID: camera1_entity_id}
    )
