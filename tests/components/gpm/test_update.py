...
async def test_integration_properties(mock_cloned_integration_manager) -> None:
    """Test properties of integration fetched during async_update."""
    entity = GPMUpdateEntity(mock_cloned_integration_manager)
    ...
    assert mock_cloned_integration_manager.fetch.await_count == 1
...
async def test_install(mock_cloned_integration_manager, version) -> None:
    """Test update installation."""
    entity = GPMUpdateEntity(mock_cloned_integration_manager)
    await entity.async_update()
    await entity.async_install(version=version, backup=False)
    assert mock_cloned_integration_manager.checkout.await_count == 1


async def test_install_same_version(mock_cloned_integration_manager) -> None:
    """Test failed update installation."""
    entity = GPMUpdateEntity(mock_cloned_integration_manager)
    await entity.async_update()
    with pytest.raises(HomeAssistantError):
        await entity.async_install(version="v0.9.9", backup=False)
    assert mock_cloned_integration_manager.checkout.await_count == 0
