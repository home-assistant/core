...
@pytest.mark.parametrize(
    ("serial", "expected_location"),
    [
        # Without a serial the location is the only discriminator and is kept
        # whole, bus number included
        ("", "I2C: PIIX4, address 0x70"),
        # With one, the bus is replaced and the fixed address is kept
        ("SERIAL123", "i2c_0, address 0x70"),
    ],
)
async def test_device_key_replaces_i2c_bus(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_openrgb_client: MagicMock,
    mock_openrgb_device: MagicMock,
    ...
    """Test that an I2C bus is replaced but its address is kept.

    Linux renumbers the I2C bus across reboots while the address stays fixed, so
    only the bus may be dropped. Dropping the address too would let two devices
    on different addresses collapse onto one key.
    """
    mock_openrgb_client.devices = [
        _device_at(mock_openrgb_device, serial, "I2C: PIIX4, address 0x70")
    ]
    ...
    assert _light_unique_ids(entity_registry, mock_config_entry) == {
        _key(mock_config_entry, serial or "none", expected_location)
    }


async def test_device_key_survives_renumbered_i2c_bus(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_openrgb_client: MagicMock,
    mock_openrgb_device: MagicMock,
    ...
    """Test that renumbering the I2C bus does not register the device again."""
    device = _device_at(mock_openrgb_device, "SERIAL123", "I2C: i2c-1, address 0x70")
    mock_openrgb_client.devices = [device]

    mock_config_entry.add_to_hass(hass)
    await hass.config_entries.async_setup(mock_config_entry.entry_id)
    await hass.async_block_till_done()
    before = _light_unique_ids(entity_registry, mock_config_entry)

    device.metadata.location = "I2C: i2c-3, address 0x70"
    await hass.config_entries.async_reload(mock_config_entry.entry_id)
    await hass.async_block_till_done()

    assert _light_unique_ids(entity_registry, mock_config_entry) == before
    assert before == {_key(mock_config_entry, "SERIAL123", "i2c_0, address 0x70")}


async def test_device_key_keeps_unsplittable_i2c_location(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_openrgb_client: MagicMock,
    mock_openrgb_device: MagicMock,
    ...
    """Test that an I2C location with no address part is left alone.

    There is nothing to keep once the bus is dropped, so replacing it would
    discard the only discriminator the device has.
    """
    location = "I2C: PIIX4"
    mock_openrgb_client.devices = [
        _device_at(mock_openrgb_device, "SERIAL123", location)
    ]
    ...
    assert _light_unique_ids(entity_registry, mock_config_entry) == {
        _key(mock_config_entry, "SERIAL123", location)
    }
...
async def test_migrate_hid_device_key(
    ...
    stable_key = _key(mock_config_entry, "IO2105F28204577", "hid_0")
...
@pytest.mark.usefixtures("mock_openrgb_client")
async def test_migrate_i2c_device_key(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    entity_registry: er.EntityRegistry,
    device_registry: dr.DeviceRegistry,
) -> None:
    """Test that a version 1 I2C key has its bus replaced and its address kept."""
    mock_config_entry.add_to_hass(hass)

    legacy_key = _key(mock_config_entry, "SERIAL123", "I2C: i2c-1, address 0x70")
    legacy_entity = entity_registry.async_get_or_create(
        Platform.LIGHT, DOMAIN, legacy_key, config_entry=mock_config_entry
    )
    device_registry.async_get_or_create(
    ...
    stable_key = _key(mock_config_entry, "SERIAL123", "i2c_0, address 0x70")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Nothing would be left to tell devices apart once the bus is dropped
        ("SERIAL123", "I2C: PIIX4"),
    ],
)
@pytest.mark.usefixtures("mock_openrgb_client")
async def test_migrate_leaves_other_keys_alone(
    ...
    """Test that a key is only rewritten when it has both a serial and an unstable part."""
    mock_config_entry.add_to_hass(hass)
...
async def test_migrate_device_with_two_openrgb_identifiers(
    ...
    for serial, bus in (("IO2105F28204577", "hid_0"), ("382042V02105657", "usb_0")):
        assert device_registry.async_get_device_by_identifier(
