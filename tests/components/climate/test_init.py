...
async def test_preset_mode_validation(
    hass: HomeAssistant, integration_helper: MockConfigEntry
) -> None:
    ...
    await hass.config_entries.async_setup(integration_helper.entry_id)
    await hass.async_block_till_done()
...
async def test_warning_not_implemented_turn_on_off_feature(
    hass: HomeAssistant,
    caplog: pytest.LogCaptureFixture,
    integration_helper: MockConfigEntry,
) -> None:
...
async def test_no_issue_no_aux_property(
    ...
    assert await hass.config_entries.async_setup(integration_helper.entry_id)
    await hass.async_block_till_done()
