...
async def test_temperature_validation_float_precision(
    hass: HomeAssistant, register_test_integration: MockConfigEntry
) -> None:
    """Test validation for temperature edge case when float precision causes limits to be miscalculated."""
    ...
    class MockClimateEntityTemp(MockClimateEntity):
        ...
        _attr_target_temperature = 25
        _attr_target_temperature_high = 30
        _attr_target_temperature_low = 20
        _attr_target_temperature_step = PRECISION_WHOLE
        _attr_min_temp = 16
    ...
    await hass.services.async_call(
        ...
        {
            "entity_id": "climate.test",
            ATTR_TARGET_TEMP_LOW: TemperatureConverter.convert(
                60.8, UnitOfTemperature.FAHRENHEIT, UnitOfTemperature.CELSIUS
            ),
            ATTR_TARGET_TEMP_HIGH: TemperatureConverter.convert(
                95.0, UnitOfTemperature.FAHRENHEIT, UnitOfTemperature.CELSIUS
            ),
        },
        blocking=True,
    )
    ...
    state = hass.states.get("climate.test")
    assert state.attributes.get(ATTR_TARGET_TEMP_LOW) == 16
    assert state.attributes.get(ATTR_TARGET_TEMP_HIGH) == 35
