...
async def test_flow_with_multiple_schema_errors_nested(
    hass: HomeAssistant, client
) -> None:
    """Test an config flow with multiple schema errors where fields are not in the schema."""
    mock_integration(
    ...
    with patch.dict(HANDLERS, {"test": TestFlow}):
        ...
        # Currently the config flow doesn't support nested schemas
        # as voluptuous_serialize doesn't support it.
        # Therefore we inject a nested schema here.
        hass.config_entries.flow._progress[flow_id].cur_step[
            "data_schema"
        ] = vol.Schema(
            {
                vol.Required("nested"): vol.Schema(
                    {
                        vol.Required(CONF_LATITUDE): cv.latitude,
                        vol.Required(CONF_LATITUDE): cv.latitude,
                    }
        ...
        assert data == {
            "errors": {
                "nested": {"latitude": "required key not provided"},
                "radius": "required key not provided",
            }
        }
