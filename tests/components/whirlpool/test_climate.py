...
OVEN_SINGLE = "climate.single_cavity_oven"
OVEN_UPPER = "climate.dual_cavity_oven_upper_oven"
OVEN_LOWER = "climate.dual_cavity_oven_lower_oven"


@pytest.fixture(
    params=[
        (OVEN_SINGLE, "mock_oven_single_cavity_api", whirlpool.oven.Cavity.Upper),
        (OVEN_UPPER, "mock_oven_dual_cavity_api", whirlpool.oven.Cavity.Upper),
        (OVEN_LOWER, "mock_oven_dual_cavity_api", whirlpool.oven.Cavity.Lower),
    ]
)
...
async def test_oven_dynamic_attributes(
    ...
    assert state is not None
    assert state.state == HVACMode.OFF
    assert state.attributes[ATTR_PRESET_MODE] == "bake"
    assert state.attributes[ATTR_CURRENT_TEMPERATURE] == 180
    assert state.attributes[ATTR_TEMPERATURE] == 200
    ...
    await trigger_attr_callback(hass, mock)
    assert hass.states.get(entity_id).attributes[ATTR_PRESET_MODE] == "broil"

    mock.get_online.return_value = False
    await trigger_attr_callback(hass, mock)
...
async def test_oven_set_temperature(
    ...
    entity_id, mock_fixture, cavity = oven_climate_entity
    mock = request.getfixturevalue(mock_fixture)
    await init_integration(hass)
...
async def test_oven_set_preset_mode(
    ...
    await hass.services.async_call(
        CLIMATE_DOMAIN,
        SERVICE_SET_PRESET_MODE,
        {ATTR_ENTITY_ID: entity_id, ATTR_PRESET_MODE: "broil"},
        blocking=True,
    )
    mock.set_cook.assert_called_once_with(
...
async def test_oven_turn_on(
    ...
    mock.set_cook.assert_called_once_with(
        target_temp=200, mode=whirlpool.oven.CookMode.Bake, cavity=cavity
    )
...
@pytest.mark.parametrize(
    ...
    [
        (SERVICE_TURN_OFF, {}),
        (SERVICE_SET_HVAC_MODE, {"hvac_mode": HVACMode.OFF}),
    ],
)
async def test_oven_turn_off(
    hass: HomeAssistant,
    service: str,
    service_data: dict,
    oven_climate_entity: tuple[str, str, whirlpool.oven.Cavity],
