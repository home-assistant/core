...
async def test_new_active_session_cancels_pending_delayed_retry(
    hass: HomeAssistant,
    freezer: FrozenDateTimeFactory,
    mock_config_entry: MockConfigEntry,
    mock_client: MagicMock,
) -> None:
    """Test a new active session cancels the previous session's delayed retry."""
    session_a_start = datetime(2026, 4, 6, 10, 15, tzinfo=dt_util.UTC)
    session_a_finish = datetime(2026, 4, 6, 12, 5, tzinfo=dt_util.UTC)
    session_b_start = datetime(2026, 4, 6, 12, 10, tzinfo=dt_util.UTC)
    ...
    mock_client.async_get_charge_session = _make_charge_session_side_effect(
        ...
        [
            {
                "status": ChargerStatus.CHARGING,
                "session_start": session_a_start,
                "session_finish": None,
            },
            {
                "status": ChargerStatus.FINISHED,
                "session_start": session_a_start,
                "session_finish": session_a_finish,
            },
            {
                "status": ChargerStatus.CHARGING,
