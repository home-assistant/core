...
from homeassistant.components.gardena_smart_local.config_flow import _async_try_connect
...
async def test_user_flow_handshake_error_non_401(hass: HomeAssistant) -> None:
    """A non-401 handshake error is reported as cannot_connect."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
    )
...
async def test_try_connect_success_closes_websocket(hass: HomeAssistant) -> None:
    """A successful probe closes the websocket and reports no error."""
    mock_ws = AsyncMock()
    ...
    with patch(
        "aiohttp.ClientSession.ws_connect", return_value=mock_ws_cm
    ) as ws_connect:
        error = await _async_try_connect(hass, MOCK_HOST, MOCK_PORT, MOCK_PASSWORD)
...
async def _start_user_flow(hass: HomeAssistant) -> tuple[dict, dict]:
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
    )
    return result, {}
...
async def _start_zeroconf_confirm_flow(hass: HomeAssistant) -> tuple[dict, dict]:
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_ZEROCONF}, data=_make_zeroconf_info()
    )
    return result, {}


async def _start_reconfigure_flow(hass: HomeAssistant) -> tuple[dict, dict]:
...
@pytest.mark.parametrize(
    ("start_flow", "expect_type", "expect_reason"),
    [
        pytest.param(_start_user_flow, FlowResultType.CREATE_ENTRY, None, id="user"),
        pytest.param(
            _start_zeroconf_confirm_flow,
            FlowResultType.CREATE_ENTRY,
            None,
            id="zeroconf_confirm",
        ),
        pytest.param(
            _start_reconfigure_flow,
...
async def test_cannot_connect_then_recovers(
    hass: HomeAssistant,
    start_flow,
    expect_type: FlowResultType,
    expect_reason: str | None,
) -> None:
    """After a connection error, resubmitting valid info still succeeds."""
    result, context = await start_flow(hass)
    ...
    assert result["type"] is expect_type
    if expect_reason is not None:
        assert result["reason"] == expect_reason
        assert context["entry"].data[CONF_HOST] == MOCK_HOST
    else:
        assert result["data"][CONF_HOST] == MOCK_HOST
...
async def test_zeroconf_already_configured(hass: HomeAssistant) -> None:
    """Zeroconf rediscovery aborts and refreshes the stored host and port.

    The gateway keeps its host but advertises a new port, so the unique-id
    match must run (and persist the update) before the host match aborts.
    """
    entry = MockConfigEntry(
        domain=DOMAIN,
