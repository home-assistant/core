...
async def test_trigger_with_delay(
    ...
    assert (
        state.attributes["post_pending_state"] == AlarmControlPanelEntityState.TRIGGERED
    )
...
async def test_arm_away_after_disabled_disarmed(
    ...
    assert state.state == AlarmControlPanelEntityState.PENDING
    assert (
        state.attributes["pre_pending_state"] == AlarmControlPanelEntityState.DISARMED
    )
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        ("payload_arm_away", AlarmControlPanelEntityState.ARMED_AWAY),
        ("payload_arm_custom_bypass", AlarmControlPanelEntityState.ARMED_CUSTOM_BYPASS),
        ("payload_arm_home", AlarmControlPanelEntityState.ARMED_HOME),
        ("payload_arm_night", AlarmControlPanelEntityState.ARMED_NIGHT),
        ("payload_arm_vacation", AlarmControlPanelEntityState.ARMED_VACATION),
    ],
)
...
async def test_state_changes_are_published_to_mqtt(
    ...
    mqtt_mock.async_publish.assert_called_once_with(
        "alarm/state", AlarmControlPanelEntityState.DISARMED, 0, True
    )
    ...
    await hass.async_block_till_done()
    mqtt_mock.async_publish.assert_called_once_with(
        "alarm/state", AlarmControlPanelEntityState.PENDING, 0, True
    )
    mqtt_mock.async_publish.reset_mock()
    # Fast-forward a little bit
    future = dt_util.utcnow() + timedelta(seconds=1)
    with patch(
        ...
        await hass.async_block_till_done()
    mqtt_mock.async_publish.assert_called_once_with(
        "alarm/state", AlarmControlPanelEntityState.ARMED_HOME, 0, True
    )
    mqtt_mock.async_publish.reset_mock()
    ...
    with patch(
        ...
        await hass.async_block_till_done()
    mqtt_mock.async_publish.assert_called_once_with(
        "alarm/state", AlarmControlPanelEntityState.ARMED_AWAY, 0, True
    )
    mqtt_mock.async_publish.reset_mock()
    ...
    with patch(
        ...
        await hass.async_block_till_done()
    mqtt_mock.async_publish.assert_called_once_with(
        "alarm/state", AlarmControlPanelEntityState.ARMED_NIGHT, 0, True
    )
    mqtt_mock.async_publish.reset_mock()
