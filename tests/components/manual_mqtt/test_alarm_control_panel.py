...
async def test_arm_vacation_no_pending(hass, mqtt_mock):
    ...
    assert STATE_ALARM_ARMED_VACATION == hass.states.get(entity_id).state


async def test_arm_vacation_no_pending_when_code_not_req(hass, mqtt_mock):
    """Test arm vacation method."""
    assert await async_setup_component(
        hass,
        alarm_control_panel.DOMAIN,
...
async def test_arm_vacation_with_pending(hass, mqtt_mock):
    """Test arm vacation method."""
    assert await async_setup_component(
        hass,
        alarm_control_panel.DOMAIN,
    ...
    state = hass.states.get(entity_id)
    assert state.attributes["post_pending_state"] == STATE_ALARM_ARMED_VACATION
...
async def test_arm_vacation_with_invalid_code(hass, mqtt_mock):
    ...
    await common.async_alarm_arm_vacation(hass, f"{CODE}2")
    await hass.async_block_till_done()
...
async def test_armed_vacation_with_specific_pending(hass, mqtt_mock):
    """Test arm home method."""
    assert await async_setup_component(
        hass,
        alarm_control_panel.DOMAIN,
...
async def test_arm_vacation_via_command_topic(hass, mqtt_mock):
    """Test arming vacation via command topic."""
    assert await async_setup_component(
        hass,
        alarm_control_panel.DOMAIN,
        {
    ...
    # Fire the arm command via MQTT; ensure state changes to pending
    async_fire_mqtt_message(hass, "alarm/command", "ARM_VACATION")
    await hass.async_block_till_done()
    assert STATE_ALARM_PENDING == hass.states.get(entity_id).state
...
async def test_state_changes_are_published_to_mqtt(hass, mqtt_mock):
    ...
    with patch(
        ...
        await hass.async_block_till_done()
    mqtt_mock.async_publish.assert_called_once_with(
        "alarm/state", STATE_ALARM_ARMED_VACATION, 0, True
    )
    mqtt_mock.async_publish.reset_mock()
