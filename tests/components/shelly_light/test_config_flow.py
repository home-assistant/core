"""Test the ShellyLighting config flow."""
...
from homeassistant.components.shelly_light.const import DOMAIN
...
async def test_config_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {"name": "My shelly_light", "entity_id": input_sensor_entity_id},
    )
    await hass.async_block_till_done()

    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["title"] == "My shelly_light"
    assert result["data"] == {}
    assert result["options"] == {
        "entity_id": input_sensor_entity_id,
        "name": "My shelly_light",
    }
    ...
    assert config_entry.title == "My shelly_light"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_shelly_light")
