...
async def test_end_time_with_microseconds_zeroed(hass, recorder_mock):
    ...
    start_of_today = dt_util.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    with freeze_time(time_600):
        ...
        await hass.async_block_till_done()
        assert hass.states.get("sensor.heatpump_compressor_today").state == "5.83"
    ...
    with freeze_time(rolled_to_next_day):
        ...
        await hass.async_block_till_done()
        assert hass.states.get("sensor.heatpump_compressor_today").state == "0.00"
