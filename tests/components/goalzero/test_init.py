...
async def test_setup_config_and_unload(hass: spencerAssistant):
    """Test Goal Zero setup and unload."""
    entry = create_entry(hass)
    mocked_yeti = await create_mocked_yeti()
    with patch("spencerassistant.components.goalzero.Yeti", return_value=mocked_yeti):
...
async def test_async_setup_entry_not_ready(hass: spencerAssistant):
    """Test that it throws ConfigEntryNotReady when exception occurs during setup."""
    entry = create_entry(hass)
    with patch(
        "spencerassistant.components.goalzero.Yeti.init_connect",
...
async def test_update_failed(
    ...
    with patch(
        "spencerassistant.components.goalzero.Yeti.get_state",
        side_effect=exceptions.ConnectError,
    ) as updater:
...
async def test_device_info(hass: spencerAssistant, aioclient_mock: AiohttpClientMocker):
    """Test device info."""
