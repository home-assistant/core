...
async def test_sync_set_active_tone(hass):
    """Test if async set_active_tone calls sync set_active_tone."""
    siren = MockSirenEntity()
    siren.hass = hass

    siren.set_active_tone = MagicMock()
    await siren.async_set_active_tone("fire")

    assert siren.set_active_tone.called
