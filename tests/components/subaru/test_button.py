...
async def test_remote_start(
    ...
    with (
        ...
        mock_remote_start.assert_called_once_with(vin, "Auto")
...
async def test_remote_start_with_selected_preset(
    hass: HomeAssistant,
    subaru_config_entry: MockConfigEntry,
    vin: str,
    vehicle_status: dict,
) -> None:
    """Test remote start uses selected climate preset from runtime_data."""
    await setup_subaru_config_entry(
        hass,
        subaru_config_entry,
        vehicle_list=[vin],
    ...
    subaru_config_entry.runtime_data.climate_presets[vin] = "Full Heat"
    with (
        patch(MOCK_API_REMOTE_START) as mock_remote_start,
        patch(MOCK_API_FETCH),
        patch(MOCK_API_GET_DATA, return_value=vehicle_status),
        ...
        await hass.async_block_till_done()
        mock_remote_start.assert_called_once_with(vin, "Full Heat")
