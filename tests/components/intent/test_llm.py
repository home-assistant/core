...
async def test_generic_intents_exposed(hass: HomeAssistant) -> None:
    ...
    assert "intent.HassTurnOn" in names
    assert "intent.HassTurnOff" in names
...
async def test_timer_intents_require_timer_device(hass: HomeAssistant) -> None:
    ...
    assert "intent.HassStartTimer" not in await _tool_names(hass)
...
async def test_timer_intents_offered_for_timer_device(hass: HomeAssistant) -> None:
    ...
    assert "intent.HassStartTimer" in names
    assert "intent.HassTimerStatus" in names


async def test_set_position_requires_exposed_cover(hass: HomeAssistant) -> None:
    """Test intent.HassSetPosition is only exposed when a cover/valve is exposed."""
    assert "intent.HassSetPosition" in await _tool_names(hass)

    async_expose_entity(hass, "conversation", COVER_ENTITY_ID, False)
    assert "intent.HassSetPosition" not in await _tool_names(hass)
