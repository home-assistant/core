...
from spencerassistant.components import dhcp, ssdp
...
async def test_form(hass: spencerAssistant, nvr: NVR) -> None:
    ...
    with patch(
        "spencerassistant.components.unifiprotect.config_flow.ProtectApiClient.get_nvr",
        return_value=nvr,
    ), patch(
        "spencerassistant.components.unifiprotect.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
async def test_form_version_too_old(hass: spencerAssistant, old_nvr: NVR) -> None:
...
async def test_form_reauth_auth(hass: spencerAssistant, nvr: NVR) -> None:
...
async def test_form_options(hass: spencerAssistant, ufp_client: ProtectApiClient) -> None:
...
async def test_discovered_by_ssdp_or_dhcp(
    hass: spencerAssistant, source: str, data: dhcp.DhcpServiceInfo | ssdp.SsdpServiceInfo
) -> None:
...
async def test_discovered_by_unifi_discovery_direct_connect_updated_but_not_using_direct_connect(
    ...
    with _patch_discovery(), patch(
        "spencerassistant.components.unifiprotect.config_flow.async_console_is_alive",
        return_value=False,
    ):
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
...
async def test_discovered_by_unifi_discovery(hass: spencerAssistant, nvr: NVR) -> None:
    """Test a discovery from unifi-discovery."""
...
async def test_discovery_can_be_ignored(hass: spencerAssistant) -> None:
