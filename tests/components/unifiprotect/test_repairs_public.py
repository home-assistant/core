...
async def test_public_stream_repair_no_permission(
    hass: HomeAssistant,
    ufp: MockUFPFixture,
    camera: Camera,
    hass_client: ClientSessionGenerator,
    ...
    """A permission error while creating a stream lands on the confirm step."""
    for channel in camera.channels:
        channel.is_rtsp_enabled = False
    ...
    ufp.api.create_camera_rtsps_streams = AsyncMock(
        side_effect=NotAuthorized("missing write permission")
    )
...
async def test_public_stream_no_repair_if_third_party(
    hass: HomeAssistant,
    ufp: MockUFPFixture,
    camera: Camera,
    issue_registry: ir.IssueRegistry,
) -> None:
    """No repair for third-party cameras."""
    for channel in camera.channels:
        channel.is_rtsp_enabled = False
    camera.is_third_party_camera = True
...
async def test_public_stream_no_repair_if_globally_disabled(
    hass: HomeAssistant,
    ufp: MockUFPFixture,
    camera: Camera,
    issue_registry: ir.IssueRegistry,
) -> None:
    """No repair when RTSP is globally disabled."""
    for channel in camera.channels:
        channel.is_rtsp_enabled = False
