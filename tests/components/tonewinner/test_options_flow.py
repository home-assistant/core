...
async def test_options_flow_does_not_modify_entry_data(
    hass: HomeAssistant,
) -> None:
    """Test options flow only updates source mappings, not entry data."""
    mock_config_entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            CONF_SERIAL_PORT: "/dev/ttyUSB0",
    ...
    # Entry data (serial port / baud) must be untouched by the options flow.
    assert mock_config_entry.data[CONF_SERIAL_PORT] == "/dev/ttyUSB0"
    assert mock_config_entry.data[CONF_BAUD_RATE] == 9600

    source_mappings = result["data"][CONF_SOURCE_MAPPINGS]
    assert source_mappings["HD1"]["enabled"] is False
    assert source_mappings["HD1"]["name"] == "Living Room TV"
