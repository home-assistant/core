...
from homeassistant.components.tonewinner.const import CONF_SOURCE_MAPPINGS, DOMAIN
...
async def test_media_player_handle_power_off(
    ...
    with patch("asyncio.create_task"), patch.object(entity, "schedule_update_ha_state"):
        ...
        assert entity.state == MediaPlayerState.OFF
        assert entity.source is None  # Source should be cleared
...
async def test_media_player_handle_volume_response(
    ...
    with patch.object(entity, "schedule_update_ha_state"):
        # Handle volume response (50.0 = 50% = 0.5)
...
async def test_media_player_handle_mute_on(
    ...
    with patch.object(entity, "schedule_update_ha_state"):
        # Handle mute on response
        entity.handle_response("AMT ON")
...
async def test_media_player_handle_sound_mode(
    ...
    with patch.object(entity, "schedule_update_ha_state"):
        # Handle sound mode response
        entity.handle_response("MODE 01 Direct")
...
async def test_media_player_cleanup_on_removal(
    ...
    entity._refresh_task.cancel = MagicMock()

    entity._source_check_task = MagicMock()
    entity._source_check_task.cancel = MagicMock()
    ...
    entity._source_check_task.cancel.assert_called_once()
...
async def test_media_player_has_entity_name(
    ...
    assert entity.name is None  # Uses device name
