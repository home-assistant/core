...
async def test_service_registration_and_unregistration(
    ...
    """Test that services are registered during setup and unregistered during unload."""
    with patch("homeassistant.components.kiosker.KioskerAPI") as mock_api_class:
        # Setup mock API
        mock_api = MagicMock()
        mock_api.host = "10.0.1.5"
        ...
        with patch(
            ...
            # Before setup - services should not exist
            assert not hass.services.has_service(DOMAIN, "navigate_url")
            assert not hass.services.has_service(DOMAIN, "blackout_set")

            # Setup integration
            await setup_integration(hass, mock_config_entry)

            # After setup - services should exist
            assert hass.services.has_service(DOMAIN, "navigate_url")
            assert hass.services.has_service(DOMAIN, "navigate_refresh")
            assert hass.services.has_service(DOMAIN, "navigate_home")
            assert hass.services.has_service(DOMAIN, "navigate_backward")
            assert hass.services.has_service(DOMAIN, "navigate_forward")
            assert hass.services.has_service(DOMAIN, "print")
            assert hass.services.has_service(DOMAIN, "clear_cookies")
            assert hass.services.has_service(DOMAIN, "clear_cache")
            assert hass.services.has_service(DOMAIN, "screensaver_interact")
            assert hass.services.has_service(DOMAIN, "blackout_set")
            assert hass.services.has_service(DOMAIN, "blackout_clear")

            # Unload integration
            assert await hass.config_entries.async_unload(mock_config_entry.entry_id)
            await hass.async_block_till_done()

            # After unload - services should be removed (if this was the last entry)
            assert not hass.services.has_service(DOMAIN, "navigate_url")
            assert not hass.services.has_service(DOMAIN, "blackout_set")
