...
from homeassistant.components.recorder.models import StatisticsMeta, process_timestamp
...
async def test_validate_statistics_supported_device_class(
    ...
    async def assert_validation_result(client, expected_result):
        await client.send_json({"id": next_id(), "type": "history/validate_statistics"})
        response = await client.receive_json()
        assert response["success"]
...
async def test_validate_statistics_unsupported_device_class(
    ...
    await hass.async_add_executor_job(hass.data[recorder.DATA_INSTANCE].block_till_done)
    client = await hass_ws_client()
    rec = hass.data[recorder.DATA_INSTANCE]
