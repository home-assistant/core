...
DEVICE_FACTORY = "homeassistant.components.linknlink.device.llk.gendevice"
...
async def test_device_setup_linknlink_exception(hass: HomeAssistant) -> None:
    """Test we handle a LinknLink exception."""
    device = get_device("Living Room")
    mock_api = device.get_mock_api()
...
async def test_device_setup_update_network_timeout(hass: HomeAssistant) -> None:
    ...
    device = get_device("Living Room")
    mock_api = device.get_mock_api()
    mock_api.check_sensors.side_effect = llke.NetworkTimeoutError()
...
async def test_device_setup_update_authentication_error(hass: HomeAssistant) -> None:
    ...
    mock_api = device.get_mock_api()
    mock_api.check_sensors.side_effect = llke.AuthorizationError()
    mock_api.auth.side_effect = (None, llke.AuthenticationError())
...
async def test_device_setup_update_linknlink_exception(hass: HomeAssistant) -> None:
    """Test we handle a LinknLink exception in the update step."""
    device = get_device("Bedroom")
    mock_api = device.get_mock_api()
    mock_api.check_sensors.side_effect = llke.LinknLinkException()
...
async def test_device_setup_get_fwversion_linknlink_exception(
    hass: HomeAssistant,
) -> None:
    """Test we load the device even if we cannot read the firmware version."""
    device = get_device("Living Room")
    mock_api = device.get_mock_api()
    mock_api.get_fwversion.side_effect = llke.LinknLinkException()
