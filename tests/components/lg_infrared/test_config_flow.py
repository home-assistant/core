...
@pytest.mark.parametrize(
    ("tv_config", "expected_title", "unique_id_entity_id"),
    [
        pytest.param(
...
@pytest.mark.usefixtures("mock_infrared_emitter_entity")
async def test_user_flow_tv_already_configured(
...
async def test_user_flow_ac_success(hass: HomeAssistant) -> None:
    ...
    assert result["result"].unique_id == f"lg_ir_ac_{mock_infrared_emitter_entity_id}"
...
@pytest.mark.usefixtures("mock_infrared_emitter_entity")
async def test_reconfigure_ac_update_modes(
    hass: HomeAssistant,
    mock_ac_config_entry: MockConfigEntry,
) -> None:
    """Test reconfiguring AC entry to add heat mode."""
    mock_ac_config_entry.add_to_hass(hass)

    result = await hass.config_entries.flow.async_init(
        DOMAIN,
        context={"source": "reconfigure", "entry_id": mock_ac_config_entry.entry_id},
    )
    ...
    assert mock_ac_config_entry.data[CONF_HVAC_MODES] == [
        HVACMode.COOL,
        HVACMode.HEAT,
        HVACMode.DRY,
    ]
...
async def test_reconfigure_tv(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
) -> None:
    """Test reconfiguring TV entry updates emitter/receiver."""
    mock_config_entry.add_to_hass(hass)
