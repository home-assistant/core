...
async def test_sensors_200(hass: HomeAssistant, setup_rainforest_200) -> None:
    ...
    demand = hass.states.get("sensor.eagle_200_mockhwaddress_power_demand")
    assert demand is not None
    assert demand.state == "1.152000"
    assert demand.attributes["unit_of_measurement"] == "kW"

    delivered = hass.states.get("sensor.eagle_200_mockhwaddress_total_energy_delivered")
    assert delivered is not None
    assert delivered.state == "45251.285000"
    assert delivered.attributes["unit_of_measurement"] == "kWh"

    received = hass.states.get("sensor.eagle_200_mockhwaddress_total_energy_received")
    ...
    price = hass.states.get("sensor.eagle_200_mockhwaddress_energy_price")
