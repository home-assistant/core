...
async def test_fossil_energy_consumption_no_co2(hass, hass_ws_client):
    ...
    assert response["result"] == {
        period1.isoformat(): 22.0,
        period2.isoformat(): 33.0,
        period3.isoformat(): 44.0,
        period4.isoformat(): 55.0,
    }
    ...
    assert response["result"] == {
        period1.isoformat(): 22.0,
        period2_day_start.isoformat(): 33.0,
        period3.isoformat(): 44.0,
        period4_day_start.isoformat(): 55.0,
    }
    ...
    response = await client.receive_json()
    assert response["success"]
    assert response["result"] == {period1.isoformat(): 33.0, period3.isoformat(): 55.0}
