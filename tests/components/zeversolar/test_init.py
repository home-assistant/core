...
async def test_setup_without_power_limit_api(hass: HomeAssistant) -> None:
    """Entry loads even when the power limit API probe fails."""
    entry = await init_integration(hass, power_limit_supported=False)
    assert entry.state is ConfigEntryState.LOADED

    coordinator = entry.runtime_data
    assert coordinator.power_limit_supported is False
