...
async def test_binary_sensors_update_data(
    ...
    assert hass.states.get("binary_sensor.mock_title_unusual_flow").state == STATE_ON

    mock_droplet.get_low_leak.return_value = False

    mock_droplet.listen_forever.call_args_list[0][0][1]({})

    assert hass.states.get("binary_sensor.mock_title_unusual_flow").state == STATE_OFF
