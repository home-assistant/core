...
async def test_setup_integration_update_fail(
    ...
    mock_k1_connector.side_effect = K1.K1ConnectionError
    assert await async_setup_component(hass, DOMAIN, {})
    await hass.async_block_till_done()
    assert (
...
async def test_setup_integration_exception(
    ...
    mock_k1_connector.side_effect = Exception
...
async def test_setup_integration_with_data(
    ...
    mock_k1_connector.return_value = MOCK_DEVICE_STATUS_DATA
    assert await async_setup_component(hass, DOMAIN, {})
    await hass.async_block_till_done()
...
async def test_configure_platforms_dynamically(
    ...
    mock_k1_connector.return_value = initial_status_data
    assert await async_setup_component(hass, DOMAIN, {})
    await hass.async_block_till_done()
    ...
    mock_k1_connector.return_value = updated_status_data
