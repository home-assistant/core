"""Test the lawn mower config flow."""
...
async def test_config_flow(hass: HomeAssistant, platform) -> None:
    ...
    with patch(
        "homeassistant.components.lawn_mower.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"name": "My lawn_mower", "entity_id": input_sensor_entity_id},
        )
        await hass.async_block_till_done()

    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert result["title"] == "My lawn_mower"
    assert result["data"] == {}
    ...
    assert config_entry.title == "My lawn_mower"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_lawn_mower")
