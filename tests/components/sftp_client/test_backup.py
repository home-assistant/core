"""Test the backups for SFTPClient."""
...
from homeassistant.components.sftp_client.backup import (
    async_register_backup_agents_listener,
)
from homeassistant.components.sftp_client.const import (
    ...
    assert response["result"]["backups"] == [
        {
            "addons": [],
            "agents": {
                "sftp_client.01JKXV07ASC62D620DGYNG2R8H": {
                    "protected": False,
                    "size": 34519040,
                }
            },
...
async def test_agents_delete(
    ...
    assert response["success"]
    assert response["result"] == {"agent_errors": {}}
    assert sftp_client.client.remove.call_count == 2
...
async def test_error_on_agents_download(
    ...
    client = await hass_client()
    backup_id = BACKUP_METADATA["backup_id"]
    sftp_client.client.listdir.return_value = []
...
async def test_delete_error(
    ...
    sftp_client.client.remove.side_effect = side_effect
