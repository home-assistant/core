...
async def test_generic_switch_multi_node(
    ...
    # check button 2
    state_button_2 = hass.states.get("event.mock_generic_switch_button_fancy")
    assert state_button_2
    assert state_button_2.state == "unknown"
    # name should be 'DeviceName (Fancy)' due to the appended label set to 'Fancy'
    assert state_button_2.name == "Mock Generic Switch Button (Fancy)"
