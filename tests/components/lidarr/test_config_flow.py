...
from spencerassistant.components.lidarr.const import DEFAULT_NAME, DOMAIN
from spencerassistant.config_entries import SOURCE_REAUTH, SOURCE_USER
from spencerassistant.const import CONF_API_KEY, CONF_SOURCE
...
def _patch_client():
    return patch(
        "spencerassistant.components.lidarr.config_flow.LidarrClient.async_get_system_status"
    )
...
async def test_flow_user_form(
    ...
    with patch(
        "spencerassistant.components.lidarr.config_flow.LidarrClient.async_try_zeroconf",
...
async def test_wrong_app(hass: spencerAssistant) -> None:
...
async def test_zero_conf_failure(hass: spencerAssistant) -> None:
