...
from homeassistant.components import iaqualink
...
async def test_setup_systems_exception_message_uses_fallback_detail(
    hass: HomeAssistant,
    config_entry: MockConfigEntry,
) -> None:
    """Test setup retry message includes detail for empty exception strings."""
    with (
        patch(
            "homeassistant.components.iaqualink.AqualinkClient.login",
            return_value=None,
        ...
        pytest.raises(
            ConfigEntryNotReady,
            match="Error while attempting to retrieve systems list: TimeoutError",
        ),
    ):
        await iaqualink.async_setup_entry(hass, config_entry)
...
async def test_setup_devices_exception_message_uses_fallback_detail(
    hass: HomeAssistant,
    config_entry: MockConfigEntry,
    client: AqualinkClient,
) -> None:
    """Test device setup retry message includes detail for empty exception strings."""
    system = get_aqualink_system(client, cls=IaquaSystem)
    system.online = True
    system.update = AsyncMock()
    systems = {system.serial: system}
    ...
    with (
        ...
        patch(
            "homeassistant.components.iaqualink.AqualinkDataUpdateCoordinator.async_config_entry_first_refresh",
            new_callable=AsyncMock,
        ),
        patch.object(system, "get_devices", side_effect=TimeoutError()),
        pytest.raises(
            ConfigEntryNotReady,
            match="Error while attempting to retrieve devices list: TimeoutError",
        ),
    ):
        await iaqualink.async_setup_entry(hass, config_entry)
