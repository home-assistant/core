...
async def test_bandwidth_sensors(hass, aioclient_mock, mock_unifi_websocket):
    ...
    mock_unifi_websocket(
        data={"meta": {"message": MESSAGE_CLIENT}, "data": [wireless_client],}
    )
    ...
    async_dispatcher_send(
        hass, controller.signal_update, clients_connected, devices_connected,
    )
    await hass.async_block_till_done()
...
async def test_dbm_sensors(hass, aioclient_mock, mock_unifi_websocket):
    """Verify that dbm sensors are working as expected."""
    wired_client = {
        "hostname": "Wired client",
        "is_wired": True,
        "mac": "00:00:00:00:00:01",
    ...
    # Not sure how to continue here; there is no "global" signal sensor.
    # assert hass.states.get("sensor.signal").state == "-34"

    # Verify state update

    # wireless_client["signal"] = -15
    ...
    # assert hass.states.get("sensor.signal").state == "-15"

    # Disable option

    options[CONF_ALLOW_DBM_SENSORS] = False
    hass.config_entries.async_update_entry(config_entry, options=options.copy())
    await hass.async_block_till_done()
    ...
    options[CONF_ALLOW_DBM_SENSORS] = True
    hass.config_entries.async_update_entry(config_entry, options=options.copy())
    await hass.async_block_till_done()
...
async def test_uptime_sensors(hass, aioclient_mock, mock_unifi_websocket):
    ...
    with patch("homeassistant.util.dt.now", return_value=now):
        config_entry = await setup_unifi_integration(
            hass, aioclient_mock, options=options, clients_response=[client1, client2],
        )
    ...
    mock_unifi_websocket(
        data={"meta": {"message": MESSAGE_CLIENT}, "data": [client1],}
    )
    await hass.async_block_till_done()
...
async def test_remove_sensors(hass, aioclient_mock, mock_unifi_websocket):
    ...
    mock_unifi_websocket(
        data={"meta": {"message": MESSAGE_CLIENT_REMOVED}, "data": [wired_client],}
    )
    await hass.async_block_till_done()
