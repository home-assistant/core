...
async def test_switch_change_state(
    ...
    await hass.services.async_call(
        LIGHT_DOMAIN,
        SERVICE_TURN_OFF,
        {ATTR_ENTITY_ID: "light.wled_light"},
        blocking=True,
    )
    await hass.async_block_till_done()
    state = hass.states.get("light.wled_light")
    ...
    await hass.services.async_call(
        LIGHT_DOMAIN,
        SERVICE_TURN_ON,
        {ATTR_ENTITY_ID: "light.wled_light", ATTR_COLOR_TEMP: 400},
        blocking=True,
    )
    await hass.async_block_till_done()
    state = hass.states.get("light.wled_light")
    ...
    # Test with RGBW strip
    entity = hass.data[LIGHT_DOMAIN].get_entity("light.wled_light")
    entity._rgbw = True  # pylint: disable=protected-access
    await hass.services.async_call(
        LIGHT_DOMAIN,
        SERVICE_TURN_ON,
        {ATTR_ENTITY_ID: "light.wled_light", ATTR_COLOR_TEMP: 400},
...
async def test_light_error(
    ...
    await hass.services.async_call(
        LIGHT_DOMAIN,
        SERVICE_TURN_ON,
        {ATTR_ENTITY_ID: "light.wled_light_1"},
        blocking=True,
    )
    await hass.async_block_till_done()
    state = hass.states.get("light.wled_light_1")
