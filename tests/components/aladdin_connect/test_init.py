...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ClientResponseError(MOCK_REQUEST_INFO, (), status=500),
            ConfigEntryState.SETUP_RETRY,
        ),
        (
            ClientResponseError(MOCK_REQUEST_INFO, (), status=401),
...
async def test_setup_entry_token_error(
    ...
    mock_session.async_ensure_token_valid.side_effect = side_effect
...
async def test_setup_entry_api_error(
    ...
    mock_client = AsyncMock()
    mock_client.get_doors.side_effect = side_effect
