...
@pytest.mark.parametrize(
    ("service", "mock_method", "translation_key"),
    [
        (SERVICE_OPEN_COVER, "open_door", "open_door_failed"),
        (SERVICE_CLOSE_COVER, "close_door", "close_door_failed"),
    ],
)
async def test_cover_action_exceptions(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_aladdin_connect_api: AsyncMock,
    service: str,
    ...
    """Test cover action raises HomeAssistantError on API failure."""
    await _setup(hass, mock_config_entry)
    getattr(mock_aladdin_connect_api, mock_method).side_effect = aiohttp.ClientError()
    ...
    assert exc_info.value.translation_domain == "aladdin_connect"
