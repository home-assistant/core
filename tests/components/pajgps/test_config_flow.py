...
from homeassistant.components.pajgps.config_flow import CustomFlow, OptionsFlowHandler
...
def _make_mock_config_entry(data: Dict[str, Any], options: Dict[str, Any] | None = None) -> MagicMock:
...
def _make_options_flow(data: Dict[str, Any], options: Dict[str, Any] | None = None) -> OptionsFlowHandler:
...
class TestCustomFlow(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_valid_input_creates_entry(self):
        ...
        with patch("homeassistant.components.pajgps.config_flow._validate_credentials", new=AsyncMock(return_value=None)):
        ...
        data = result["data"]
        self.assertEqual(data["entry_name"], VALID_USER_INPUT["entry_name"])
        self.assertEqual(data["email"], VALID_USER_INPUT["email"])
        self.assertEqual(data["password"], VALID_USER_INPUT["password"])
        self.assertEqual(data["mark_alerts_as_read"], VALID_USER_INPUT["mark_alerts_as_read"])
        self.assertEqual(data["fetch_elevation"], VALID_USER_INPUT["fetch_elevation"])
        self.assertEqual(data["force_battery"], VALID_USER_INPUT["force_battery"])
    ...
    async def test_creates_entry_with_valid_guid(self):
        ...
        self.assertEqual(str(parsed), generated_guid)
    ...
    async def test_empty_entry_name_returns_form_with_error(self):
        ...
        self.assertEqual(result["type"], "form")
        self.assertEqual(result["errors"]["base"], "entry_name_required")
    ...
    async def test_empty_email_returns_form_with_error(self):
        ...
        self.assertEqual(result["type"], "form")
        self.assertEqual(result["errors"]["base"], "email_required")
    ...
    async def test_empty_password_returns_form_with_error(self):
        ...
        self.assertEqual(result["type"], "form")
        self.assertEqual(result["errors"]["base"], "password_required")
    ...
    async def test_duplicate_check_uses_email_as_key(self):
        ...
        with patch.object(flow, "_async_abort_entries_match") as mock_abort_match, \
             patch("homeassistant.components.pajgps.config_flow._validate_credentials", new=AsyncMock(return_value=None)):
            await flow.async_step_user(user_input=dict(VALID_USER_INPUT))
...
class TestOptionsFlowHandler(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_form_defaults_come_from_entry_data(self):
        ...
        self.assertEqual(defaults["entry_name"], VALID_ENTRY_DATA["entry_name"])
        self.assertEqual(defaults["email"], VALID_ENTRY_DATA["email"])
        self.assertEqual(defaults["password"], VALID_ENTRY_DATA["password"])
        self.assertEqual(defaults["mark_alerts_as_read"], VALID_ENTRY_DATA["mark_alerts_as_read"])
        self.assertEqual(defaults["fetch_elevation"], VALID_ENTRY_DATA["fetch_elevation"])
        self.assertEqual(defaults["force_battery"], VALID_ENTRY_DATA["force_battery"])
    ...
    async def test_options_override_data_defaults(self):
        ...
        self.assertEqual(defaults["entry_name"], overriding_options["entry_name"])
        self.assertEqual(defaults["email"], overriding_options["email"])
        self.assertEqual(defaults["password"], overriding_options["password"])
        self.assertEqual(defaults["mark_alerts_as_read"], overriding_options["mark_alerts_as_read"])
        self.assertEqual(defaults["fetch_elevation"], overriding_options["fetch_elevation"])
        self.assertEqual(defaults["force_battery"], overriding_options["force_battery"])
    ...
    async def test_valid_update_creates_entry(self):
        ...
        self.assertEqual(result["type"], "create_entry")
        data = result["data"]
        self.assertEqual(data["entry_name"], VALID_OPTIONS_INPUT["entry_name"])
        self.assertEqual(data["email"], VALID_OPTIONS_INPUT["email"])
        self.assertEqual(data["password"], VALID_OPTIONS_INPUT["password"])
        self.assertEqual(data["mark_alerts_as_read"], VALID_OPTIONS_INPUT["mark_alerts_as_read"])
        self.assertEqual(data["fetch_elevation"], VALID_OPTIONS_INPUT["fetch_elevation"])
        self.assertEqual(data["force_battery"], VALID_OPTIONS_INPUT["force_battery"])
    ...
    async def test_valid_update_calls_async_update_entry(self):
        ...
        passed_data = call_kwargs.kwargs.get("data") or call_kwargs.args[1] if len(call_kwargs.args) > 1 else call_kwargs.kwargs.get("data")
        self.assertEqual(passed_data["guid"], VALID_ENTRY_DATA["guid"])
...
class TestValidateCredentials(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_none_on_successful_login(self):
        ...
        from homeassistant.components.pajgps.config_flow import _validate_credentials
