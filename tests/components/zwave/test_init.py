...
from homeassistant.components.zwave import (
    const, CONFIG_SCHEMA, CONF_DEVICE_CONFIG_GLOB, DATA_ZWAVE_NETWORK)
...
def test_network_options(hass, mock_openzwave):
    ...
    network = hass.data[zwave.DATA_ZWAVE_NETWORK]
...
def test_setup_platform(hass, mock_openzwave):
    ...
    hass.data[DATA_ZWAVE_NETWORK] = MagicMock()
    hass.data[zwave.DATA_ZWAVE_DEVICES] = {456: mock_device}
...
def test_zwave_ready_wait(hass, mock_openzwave):
    ...
    with patch.object(zwave.time, 'sleep') as mock_sleep:
        with patch.object(zwave, '_LOGGER') as mock_logger:
            hass.data[DATA_ZWAVE_NETWORK].state = MockNetwork.STATE_STARTED
...
class TestZWaveServices(unittest.TestCase):
    ...
    def setUp(self):
        ...
        setup_component(self.hass, 'zwave', {'zwave': {}})
        self.hass.block_till_done()
        self.zwave_network = self.hass.data[DATA_ZWAVE_NETWORK]
