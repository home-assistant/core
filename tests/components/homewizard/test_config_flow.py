...
async def test_manual_flow_works(hass, aioclient_mock, mock_homewizard_energy):
    ...
    assert len(mock_homewizard_energy.close.mock_calls) == 1
    assert len(mock_setup_entry.mock_calls) == 1


async def test_discovery_flow_works(hass, aioclient_mock, mock_homewizard_energy):
...
async def test_config_flow_imports_entry(aioclient_mock, hass, mock_homewizard_energy):
...
async def test_discovery_disabled_api(hass, aioclient_mock, mock_homewizard_energy):
...
async def test_check_detects_unexpected_api_response(
    hass, aioclient_mock, mock_homewizard_energy
):
    """Test check detecting device endpoint failed fetching data."""

    mock_homewizard_energy.device = None
