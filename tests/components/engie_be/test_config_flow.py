...
@pytest.mark.usefixtures("mock_config_flow_client", "mock_setup_entry")
async def test_reauth_success(
    ...
    assert mock_config_entry.data["access_token"] == "new-access-token"
...
async def test_reauth_invalid_auth_recovery(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_config_flow_client: MagicMock,
    mock_setup_entry: AsyncMock,
) -> None:
    """Test reauthentication recovers after an authentication failure."""
    mock_config_entry.add_to_hass(hass)

    result = await mock_config_entry.start_reauth_flow(hass)

    mock_config_flow_client.async_start_authentication.side_effect = (
