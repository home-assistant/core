...
async def test_manual_empty_form(
    hass: HomeAssistant,
    mock_setup_entry: AsyncMock,
) -> None:
    """Test we get the form."""
...
async def test_manual_discovery_exception_success(
    hass: HomeAssistant, mock_airtouch_discovery: AsyncMock
) -> None:
    """Test we handle cannot connect error."""
    ...
    with (
        patch.object(
            ...
            "_discover_device_by_ip",
            new_callable=AsyncMock,
            side_effect=Exception("Device not found during test"),
        ),
        patch.object(
            AirTouch5ConfigFlow,
            "_discovery",
...
async def test_manual_discovery_exception_no_host(
    hass: HomeAssistant, mock_airtouch_discovery: AsyncMock
) -> None:
    """Test we handle cannot connect error."""
