...
async def test_coordinator_live_error(hass: HomeAssistant, mock_live_status) -> None:
    ...
    async_fire_time_changed(hass, utcnow() + TESSIE_FLEET_API_SYNC_INTERVAL)
    await hass.async_block_till_done()
    mock_live_status.assert_called_once()
    assert hass.states.get("sensor.energy_site_solar_power").state == STATE_UNAVAILABLE


async def test_coordinator_info_error(hass: HomeAssistant, mock_site_info) -> None:
