...
async def test_async_step_bluetooth_valid_device(hass):
    ...
    with patch(
        "spencerassistant.components.govee_ble.async_setup_entry", return_value=True
    ):
...
async def test_async_step_user_with_found_devices(hass):
    ...
    with patch(
        "spencerassistant.components.govee_ble.config_flow.async_discovered_service_info",
