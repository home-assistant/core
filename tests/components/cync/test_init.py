...
async def test_migrate_overlapping_unique_ids(
    ...
    merged_device = device_registry.async_get_or_create(
        config_entry_id=mock_config_entry.entry_id,
        identifiers={(DOMAIN, "10000-1111"), (DOMAIN, "10000-1101")},
    )
    ...
    assert interrupted_chained_entity.previous_unique_id == "10000-1111"
    ...
    assert interrupted_first_entity.previous_unique_id == "10000-1101"
    ...
    assert migrated_chained_entity.previous_unique_id == "10000-1111"
    ...
    assert migrated_first_entity.previous_unique_id == "10000-1101"
...
async def test_migration_failure_closes_client_and_keeps_marker(
    ...
    assert interrupted_light.unique_id == "10000-1101"
    ...
    assert migrated_light.unique_id == "10000-1"
    assert "mesh_unique_ids_migration_pending" not in mock_config_entry.data
