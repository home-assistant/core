...
from homeassistant.components.verisure.const import CONF_FORCE_ARM
...
@pytest.mark.parametrize("force_arm", [True, False])
...
async def test_arm_passes_force_arm_option(
    hass: HomeAssistant,
    ...
    """Arming passes the configured force_arm option through to the library."""
    mock_config_entry.add_to_hass(hass)
    hass.config_entries.async_update_entry(
        mock_config_entry, options={CONF_FORCE_ARM: force_arm}
    )
    with patch(
    ...
    getattr(mock_verisure, method).assert_called_once_with("1234", force_arm=force_arm)
