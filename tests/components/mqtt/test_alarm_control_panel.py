...
async def test_update_state_via_state_topic(hass, mqtt_mock_entry):
...
async def test_ignore_update_state_if_unknown_via_state_topic(hass, mqtt_mock_entry):
...
async def test_publish_mqtt_no_code(hass, mqtt_mock_entry, service, payload):
...
async def test_publish_mqtt_with_code(hass, mqtt_mock_entry, service, payload):
...
async def test_publish_mqtt_with_remote_code(hass, mqtt_mock_entry, service, payload):
...
async def test_publish_mqtt_with_remote_code_text(
    hass, mqtt_mock_entry, service, payload
):
...
async def test_publish_mqtt_with_code_required_false(
    hass, mqtt_mock_entry, service, payload, disable_code
):
    """Test publishing of MQTT messages when code is configured.
...
async def test_disarm_publishes_mqtt_with_template(hass, mqtt_mock_entry):
...
async def test_update_state_via_state_topic_template(hass, mqtt_mock_entry):
...
async def test_attributes_code_number(hass, mqtt_mock_entry):
...
async def test_attributes_remote_code_number(hass, mqtt_mock_entry):
    """Test attributes which are not supported by the vacuum."""
...
async def test_attributes_code_text(hass, mqtt_mock_entry):
    """Test attributes which are not supported by the vacuum."""
    config = copy.deepcopy(DEFAULT_CONFIG)
...
async def test_unique_id(hass, mqtt_mock_entry):
    ...
    await help_test_unique_id(hass, mqtt_mock_entry, alarm_control_panel.DOMAIN, config)


async def test_discovery_removal_alarm(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_alarm_topic_and_template(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_alarm_template(hass, mqtt_mock_entry, caplog):
    """Test update of discovered alarm_control_panel."""
    config1 = copy.deepcopy(DEFAULT_CONFIG[alarm_control_panel.DOMAIN])
    config2 = copy.deepcopy(DEFAULT_CONFIG[alarm_control_panel.DOMAIN])
    config1["name"] = "Beer"
...
async def test_discovery_update_unchanged_alarm(hass, mqtt_mock_entry, caplog):
    """Test update of discovered alarm_control_panel."""
    config1 = copy.deepcopy(DEFAULT_CONFIG[alarm_control_panel.DOMAIN])
    config1["name"] = "Beer"
...
async def test_discovery_broken(hass, mqtt_mock_entry, caplog):
    ...
    await help_test_discovery_broken(
        hass, mqtt_mock_entry, caplog, alarm_control_panel.DOMAIN, data1, data2
    )
...
async def test_encoding_subscribable_topics(
    hass, mqtt_mock_entry, caplog, topic, value
):
