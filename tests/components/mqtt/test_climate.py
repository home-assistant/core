...
class TestMQTTClimate(unittest.TestCase):
    ...
    def test_set_target_temperature_low_high(self):
        ...
        assert 21 == state.attributes.get('target_temp_high')
        ...
        self.mock_publish.async_publish.reset_mock()
        common.set_temperature(self.hass, target_temp_low=47,
                               target_temp_high=49,
                               entity_id=ENTITY_CLIMATE)
        self.hass.block_till_done()
        state = self.hass.states.get(ENTITY_CLIMATE)
        assert 47 == state.attributes.get('target_temp_low')
        assert 49 == state.attributes.get('target_temp_high')
        self.mock_publish.async_publish.assert_has_calls([
        ...
        # also test directly supplying the operation mode to set_temperature
        self.mock_publish.async_publish.reset_mock()
        common.set_temperature(self.hass, target_temp_low=21,
                               target_temp_high=23,
                               operation_mode="auto",
                               entity_id=ENTITY_CLIMATE)
        self.hass.block_till_done()
