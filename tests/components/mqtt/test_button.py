...
async def test_command_template(hass, mqtt_mock_entry):
...
async def test_unique_id(hass, mqtt_mock_entry):
    ...
    await help_test_unique_id(hass, mqtt_mock_entry, button.DOMAIN, config)


async def test_discovery_removal_button(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_button(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_unchanged_button(hass, mqtt_mock_entry, caplog):
    """Test update of discovered button."""
    data1 = (
        '{ "name": "Beer",'
    ...
    with patch(
        ...
        await help_test_discovery_update_unchanged(
            hass, mqtt_mock_entry, caplog, button.DOMAIN, data1, discovery_update
        )
...
async def test_publishing_with_custom_encoding(
    hass, mqtt_mock_entry, caplog, service, topic, parameters, payload, template
):
