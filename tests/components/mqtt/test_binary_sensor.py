...
async def test_invalid_sensor_value_via_mqtt_message(hass, mqtt_mock_entry, caplog):
    """Test the setting of the value via MQTT."""
    assert await async_setup_component(
        hass,
        binary_sensor.DOMAIN,
...
async def test_off_delay(hass, mqtt_mock_entry):
...
async def test_unique_id(hass, mqtt_mock_entry):
    ...
    await help_test_unique_id(hass, mqtt_mock_entry, binary_sensor.DOMAIN, config)


async def test_discovery_removal_binary_sensor(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_binary_sensor_template(hass, mqtt_mock_entry, caplog):
    """Test update of discovered binary_sensor."""
    config1 = copy.deepcopy(DEFAULT_CONFIG[binary_sensor.DOMAIN])
    config2 = copy.deepcopy(DEFAULT_CONFIG[binary_sensor.DOMAIN])
    config1["name"] = "Beer"
...
async def test_discovery_update_unchanged_binary_sensor(hass, mqtt_mock_entry, caplog):
    """Test update of discovered binary_sensor."""
    config1 = copy.deepcopy(DEFAULT_CONFIG[binary_sensor.DOMAIN])
    config1["name"] = "Beer"
    ...
    with patch(
        ...
        await help_test_discovery_update_unchanged(
            hass, mqtt_mock_entry, caplog, binary_sensor.DOMAIN, data1, discovery_update
        )
...
async def test_discovery_broken(hass, mqtt_mock_entry, caplog):
    ...
    await help_test_discovery_broken(
        hass, mqtt_mock_entry, caplog, binary_sensor.DOMAIN, data1, data2
    )
...
async def test_cleanup_triggers_and_restoring_state(
    hass, mqtt_mock_entry, caplog, tmp_path, freezer, payload1, state1, payload2, state2
):
