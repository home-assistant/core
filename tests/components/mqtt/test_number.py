...
async def test_run_number_setup(hass, mqtt_mock_entry):
...
async def test_value_template(hass, mqtt_mock_entry):
...
async def test_run_number_service_optimistic(hass, mqtt_mock_entry):
...
async def test_run_number_service(hass, mqtt_mock_entry):
...
async def test_run_number_service_with_command_template(hass, mqtt_mock_entry):
...
async def test_unique_id(hass, mqtt_mock_entry):
    ...
    await help_test_unique_id(hass, mqtt_mock_entry, number.DOMAIN, config)


async def test_discovery_removal_number(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_number(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_unchanged_number(hass, mqtt_mock_entry, caplog):
    """Test update of discovered number."""
    data1 = (
    ...
    with patch(
        ...
        await help_test_discovery_update_unchanged(
            hass, mqtt_mock_entry, caplog, number.DOMAIN, data1, discovery_update
        )
...
async def test_min_max_step_attributes(hass, mqtt_mock_entry):
...
async def test_invalid_min_max_attributes(hass, caplog, mqtt_mock_entry):
...
async def test_mqtt_payload_not_a_number_warning(hass, caplog, mqtt_mock_entry):
...
async def test_mqtt_payload_out_of_range_error(hass, caplog, mqtt_mock_entry):
