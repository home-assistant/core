...
async def add_banned_ips(manager: IpBanManager, banned_ips: list[str]) -> None:
    """Add list of banned IPs to the manager."""
    for banned_ip in banned_ips:
        await manager.async_add_ban(ip_address(banned_ip))
...
async def test_access_from_banned_ip(
    ...
    await add_banned_ips(app[KEY_BAN_MANAGER], BANNED_IPS)
...
async def test_access_from_banned_ip_with_invalid_ip_entry(
    ...
    """Test that invalid IP addresses in ban storage are skipped gracefully.
    ...
    # Store the data directly to simulate loading from storage with invalid entries
    await app[KEY_BAN_MANAGER].store.async_save(data)
...
async def test_ban_migration(
    ...
    legacy_ip_bans_file_path = hass.config.path(LEGACY_IP_BANS_FILE)

    # Create a legacy ban file with banned IPs
    async with aiofiles.open(legacy_ip_bans_file_path, "w") as fp:
        await fp.write(
            yaml.dump(
                {
                    banned_ip: {"banned_at": "2016-11-16T19:20:03"}
                    for banned_ip in BANNED_IPS
                }
            )
        )
    ...
    with pytest.raises(FileNotFoundError):
        os.stat(legacy_ip_bans_file_path)
