"""Test the StreamWorker.

Exercise the StreamWorker functionality by mocking av.open calls to return a
fake media container as well a fake decoded stream in the form of a series of
packets.

The SteamWorker opens the stream source (typically a URL) and gets back a
...
from homeassistant.components.stream.worker import StreamWorker
...
async def async_decode_stream(hass, packets, py_av=None):
    ...
    stream_worker = StreamWorker(hass, stream)
    ...
    with patch("av.open", new=py_av.open), patch(
        ...
        stream_worker.run(threading.Event())
