...
def mock_stream_settings(hass: HomeAssistant):
...
def run_worker(hass: HomeAssistant, stream, stream_source, stream_settings=None):
...
async def async_decode_stream(
    hass: HomeAssistant, packets, py_av=None, stream_settings=None
):
