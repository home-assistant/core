...
@pytest.fixture
def storage_setup_named_categorized_tag(
    hass,
    hass_storage,
):
    """Storage setup for test case of named tags."""
                    ...
                    "items": [
                        {
                            "id": TEST_TAG_ID,
                            CONF_NAME: TEST_TAG_NAME,
                            TAG_CATEGORY: TEST_TAG_CATEGORY,
                        },
                    ]
                },
            }
...
async def test_named_tag_with_category_scanned_event(
    hass: HomeAssistant,
    hass_ws_client: WebSocketGenerator,
    freezer: FrozenDateTimeFactory,
    storage_setup_named_categorized_tag,
) -> None:
    """Test scanning named tag triggering event."""
    assert await storage_setup_named_categorized_tag()
    ...
    assert event_data["device_id"] == TEST_DEVICE_ID
    assert event_data["tag_id"] == TEST_TAG_ID
    assert event_data["category"] == TEST_TAG_CATEGORY
