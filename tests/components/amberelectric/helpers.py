...
def generate_actual_interval(channel_type: ChannelType, end_time: datetime) -> Interval:
    ...
    return Interval(
        ActualInterval(
            ...
            spike_status=SpikeStatus.NONE.value,
            descriptor=PriceDescriptor.LOW.value,
            advanced_price=AdvancedPrice(low=10, high=20),
        )
    )
...
def generate_forecast_interval(
    ...
    return Interval(
        ForecastInterval(
            ...
            channel_type=channel_type.value,
            spike_status=SpikeStatus.NONE.value,
            descriptor=PriceDescriptor.VERYLOW.value,
            estimate=True,
            advancedPrice=AdvancedPrice(low=10, high=20),
        )
    )
