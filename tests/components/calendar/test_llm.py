...
async def test_get_tools_no_exposed_calendar(hass: HomeAssistant) -> None:
    ...
    assert "calendar.get_events" not in [tool.name for tool in result.tools]
...
async def test_calendar_get_events_tool(hass: HomeAssistant) -> None:
    ...
    tool = next(
        (tool for tool in result.tools if tool.name == "calendar.get_events"), None
    )
...
async def test_calendar_get_events_tool_not_found(hass: HomeAssistant) -> None:
    ...
    llm_context = _llm_context()
    result = await llm_component.async_get_tools(hass, llm_context, "assist")
    tool = next(tool for tool in result.tools if tool.name == "calendar.get_events")
    ...
    response = await tool.async_call(
        hass,
        llm.ToolInput(
            "calendar.get_events", {"calendar": "Mock Calendar Name", "range": "today"}
        ),
        llm_context,
    )
