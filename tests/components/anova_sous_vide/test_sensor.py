...
async def test_no_config_entry_coordinator(hass: HomeAssistant):
    ...
    with patch(
        "homeassistant.components.anova_sous_vide.coordinator._LOGGER.error"
    ) as logger_error:
        AnovaCoordinator(hass, None, "")
        assert logger_error.called


async def test_update_failed(hass: HomeAssistant):
