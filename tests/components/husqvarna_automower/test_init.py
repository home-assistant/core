...
#     print("values Add mower 1 and check if it worked", values)
#     mock_automower_client.get_status.return_value = values
#     freezer.tick(SCAN_INTERVAL)
#     async_fire_time_changed(hass)
#     await hass.async_block_till_done()
...
async def test_add_device(
    ...
    """Test add device."""
    original_values = deepcopy(values)
    values_copy1 = deepcopy(values)
    del values_copy1["1234"]
    mock_automower_client.get_status.return_value = values_copy1
    await setup_integration(hass, mock_config_entry)
    entry = hass.config_entries.async_entries(DOMAIN)[0]
    await hass.async_block_till_done()

    entities_mower_one = len(
        er.async_entries_for_config_entry(entity_registry, entry.entry_id)
    )
    assert entities_mower_one == 37
    current_devices = len(
        dr.async_entries_for_config_entry(device_registry, entry.entry_id)
    )
    ...
    state = hass.states.get("switch.test_mower_2_enable_schedule")
    assert state.name == "Test Mower 2 Enable schedule"
    assert (
        len(er.async_entries_for_config_entry(entity_registry, entry.entry_id))
        == entities_mower_one + 12
    )
    assert (
        len(dr.async_entries_for_config_entry(device_registry, entry.entry_id))
        == current_devices + 1
...
async def test_add_device2(
    ...
    values_copy1 = deepcopy(values)
    del values_copy1["1234"]
    values_copy2 = deepcopy(values)
    del values_copy2[TEST_MOWER_ID]
    mock_automower_client.get_status.return_value = values_copy2
    await setup_integration(hass, mock_config_entry)
    entry = hass.config_entries.async_entries(DOMAIN)[0]
    await hass.async_block_till_done()

    entities_mower_two = len(
        er.async_entries_for_config_entry(entity_registry, entry.entry_id)
    )
    assert entities_mower_two == 12
    current_devices = len(
        dr.async_entries_for_config_entry(device_registry, entry.entry_id)
    )
    assert current_devices == 1
