...
from custom_components.mill_wifi.api import (
    AuthenticationError,
    MillApiClient,
    MillApiError,
)
...
MOCK_DEVICES_DATA = [
    ...
    },
    {
        ...
        "name": "Socket 1",
        "type": EDeviceType.SOCKET_GEN3.value,
        "data": "other_data",
    },
]
...
def mock_api_client():
    ...
    client.homes = AsyncMock(return_value={})
...
async def coordinator(hass: HomeAssistant, mock_api_client: MillApiClient):
    ...
    with patch(
        ...
        return MillDataCoordinator(hass, mock_api_client)
...
async def test_coordinator_successful_first_refresh(
    hass: HomeAssistant,
...
async def test_coordinator_update_with_no_devices(
    hass: HomeAssistant,
    coordinator: MillDataCoordinator,
    mock_api_client: MillApiClient,
):
...
async def test_coordinator_update_raises_authentication_error(
    hass: HomeAssistant,
    coordinator: MillDataCoordinator,
    mock_api_client: MillApiClient,
):
    ...
    with pytest.raises(
        UpdateFailed, match="Error fetching devices: Auth failed"
    ):  # Or UpdateFailed if it's caught and re-raised
        await coordinator._async_update_data()


async def test_coordinator_update_raises_mill_api_error(
    hass: HomeAssistant,
    coordinator: MillDataCoordinator,
    mock_api_client: MillApiClient,
):
    """Test data update when API raises MillApiError."""
    mock_api_client.get_all_devices.side_effect = MillApiError(
...
async def test_coordinator_first_refresh_raises_config_entry_not_ready_on_auth_error(
    ...
    mock_api_client.get_all_devices.side_effect = AuthenticationError(
        "Auth failed during first refresh"
    )
...
async def test_coordinator_first_refresh_raises_config_entry_not_ready_on_api_error(
    ...
    mock_api_client.get_all_devices.side_effect = MillApiError(
        "API error during first refresh"
    )
    coordinator_instance = MillDataCoordinator(hass, mock_api_client)
