...
def mock_responses(mock, single=False):
    ...
    mock.put(base_url + "bed/-31/sleepNumber?_k=0987&side=R")
    mock.get(
    ...
    mock.get(
        base_url + "bed/-31/foundation/status?_k=0987",
        text=load_fixture("foundation.json", "sleepiq"),
    )
    mock.get(
        base_url + "bed/-31/foundation/system?_k=0987",
        text=load_fixture("foundation-system.json", "sleepiq"),
    )
    mock.get(
        base_url + "bed/-31/foundation/outlet?_k=0987&outletId=3",
        text=load_fixture("outlet.json", "sleepiq"),
    )
