...
async def test_setup(
    ...
    entry = entity_registry.async_get(f"switch.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_pause_mode")
    assert entry
...
async def test_switch_get_states(hass: HomeAssistant, mock_asyncsleepiq) -> None:
    ...
    assert (
        hass.states.get(f"switch.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_pause_mode").state
        == STATE_OFF
    )
