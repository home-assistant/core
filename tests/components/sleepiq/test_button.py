...
async def test_button_calibrate(
    ...
    state = hass.states.get(f"button.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_calibrate")
    assert (
        state.attributes.get(ATTR_FRIENDLY_NAME) == f"{BED_NAME} SleepNumber {BED_NAME} Calibrate"
    )

    entity = entity_registry.async_get(f"button.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_calibrate")
    assert entity
...
async def test_button_stop_pump(
    ...
    state = hass.states.get(f"button.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_stop_pump")
    assert (
        state.attributes.get(ATTR_FRIENDLY_NAME) == f"{BED_NAME} SleepNumber {BED_NAME} Stop Pump"
    )

    entity = entity_registry.async_get(f"button.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_stop_pump")
    assert entity
