...
async def test_setup(
    ...
    entry = entity_registry.async_get(f"light.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_light_1")
    assert entry
    assert entry.original_name == f"SleepNumber {BED_NAME} Light 1"
    assert entry.unique_id == f"{BED_ID}-light-1"

    entry = entity_registry.async_get(f"light.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_light_2")
    assert entry
...
async def test_switch_get_states(hass: HomeAssistant, mock_asyncsleepiq) -> None:
    ...
    assert (
        hass.states.get(f"light.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_light_1").state
        == STATE_OFF
    )
    ...
    assert (
        hass.states.get(f"light.{BED_NAME_LOWER}_sleepnumber_{BED_NAME_LOWER}_light_1").state == STATE_ON
    )
