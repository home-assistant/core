...
pytestmark = [
    ...
    pytest.mark.usefixtures("device_source_mock_retry"),
]
...
async def test_catch_request_error_unavailable(
    hass: HomeAssistant, ssdp_scanner_mock: Mock, caplog: pytest.LogCaptureFixture
) -> None:
...
async def test_catch_upnp_connection_error(
    hass: HomeAssistant, dms_device_mock: Mock, caplog: pytest.LogCaptureFixture
) -> None:
    ...
    # Clear the error, but the device should reconnect
