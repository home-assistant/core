...
from spencerassistant.components.dlna_dms.const import CONF_SOURCE_ID, DOMAIN
from spencerassistant.const import CONF_DEVICE_ID, CONF_HOST, CONF_URL
...
MOCK_DISCOVERY: Final = ssdp.SsdpServiceInfo(
    ...
    x_spencerassistant_matching_domains={DOMAIN},
)
...
def mock_setup_entry() -> Iterable[Mock]:
    ...
    with patch(
        "spencerassistant.components.dlna_dms.async_setup_entry",
        return_value=True,
    ) as mock:
        yield mock


async def test_user_flow(hass: spencerAssistant, ssdp_scanner_mock: Mock) -> None:
...
async def test_ssdp_flow_unavailable(
    hass: spencerAssistant, upnp_factory_mock: Mock
) -> None:
...
async def test_ssdp_flow_bad_data(hass: spencerAssistant) -> None:
