...
async def test_add_invalid_dataset(hass: HomeAssistant) -> None:
    ...
    assert len(await dataset_store.async_list_datasets(hass)) == 0
...
async def test_add_dataset_twice(hass: HomeAssistant) -> None:
    ...
    assert len(await dataset_store.async_list_datasets(hass)) == 1
    created = (await dataset_store.async_list_datasets(hass))[0].created

    await dataset_store.async_add_dataset(hass, "new_source", DATASET_1)
    assert len(await dataset_store.async_list_datasets(hass)) == 1
    assert (await dataset_store.async_list_datasets(hass))[0].created == created
...
async def test_delete_dataset_twice(hass: HomeAssistant) -> None:
    ...
    dataset_id = (await dataset_store.async_list_datasets(hass))[1].id

    await dataset_store.async_delete_dataset(hass, dataset_id)
    assert len(await dataset_store.async_list_datasets(hass)) == 1
...
async def test_delete_preferred_dataset(hass: HomeAssistant) -> None:
    ...
    dataset_id = (await dataset_store.async_list_datasets(hass))[0].id
...
async def test_get_preferred_dataset(hass: HomeAssistant) -> None:
    ...
    assert (await dataset_store.async_get_preferred_dataset(hass)).tlv == DATASET_1
...
async def test_dataset_properties(hass: HomeAssistant) -> None:
    ...
    for dataset in await dataset_store.async_list_datasets(hass):
    ...
    dataset = await dataset_store.async_get_dataset(hass, dataset_1.id)
    ...
    dataset = await dataset_store.async_get_dataset(hass, dataset_2.id)
    assert dataset == dataset_2
    assert dataset.extended_pan_id == "1111111122222222"
    assert dataset.network_name == "HomeAssistant!"
    assert dataset.pan_id == "1234"

    dataset = await dataset_store.async_get_dataset(hass, dataset_3.id)
