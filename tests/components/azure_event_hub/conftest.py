...
@pytest.fixture(autouse=False, name="mock_aeh_cs")
def mock_aeh_cs_fixture():
    """Mock the azure event hub client."""
    with patch(
        f"{AZURE_EVENT_HUB_PATH}.client.AzureEventHubClientConnectionString",
        return_value=Mock(spec=AzureEventHubClientConnectionString),
    ) as mock_aeh_client:
        yield mock_aeh_client


@pytest.fixture(autouse=False, name="mock_aeh_sas")
def mock_aeh_sas_fixture():
    """Mock the azure event hub client."""
    with patch(
        f"{AZURE_EVENT_HUB_PATH}.client.AzureEventHubClientSAS",
        return_value=Mock(spec=AzureEventHubClientSAS),
    ) as mock_aeh_client:
        yield mock_aeh_client
