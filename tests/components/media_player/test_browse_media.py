...
from spencerassistant.components.media_player.browse_media import (
...
def fixture_mock_sign_path():
    """Mock sign path."""
    with patch(
        "spencerassistant.components.media_player.browse_media.async_sign_path",
...
async def test_process_play_media_url(hass, mock_sign_path):
    ...
    with pytest.raises(spencerAssistantError), patch(
        "spencerassistant.components.media_player.browse_media.get_url",
...
async def test_process_play_media_url_for_addon(hass, mock_sign_path):
    ...
    assert (
        ...
        != "http://spencerassistant:8123/path?authSig=bla"
    )
    ...
    assert (
        async_process_play_media_url(hass, "/path", for_supervisor_network=True)
        != "https://spencerassistant:8123/path?authSig=bla"
    )
