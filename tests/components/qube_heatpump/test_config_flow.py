...
async def test_form_cannot_connect(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {CONF_HOST: "1.1.1.1", CONF_NAME: "qube 1"},
        )
...
async def test_form_already_configured(
    ...
    with patch(
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {CONF_HOST: "1.2.3.4", CONF_NAME: "qube 2"},
        )
...
async def test_form_duplicate_ip(
    ...
    with (
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {CONF_HOST: "qube.local", CONF_NAME: "qube 2"},
        )
...
async def test_reconfigure_duplicate_unique_id(
    ...
    entry2 = MockConfigEntry(
        domain=DOMAIN,
        data={CONF_HOST: "5.6.7.8", CONF_PORT: 502, CONF_NAME: "qube 2"},
