...
async def test_reauth_wrong_account(hass):
    ...
    with patch(
        "pyhoma.client.TahomaClient.login", side_effect=BadCredentialsException
        ...
        mock_entry2 = MockConfigEntry(
            domain=config_flow.DOMAIN,
            unique_id=TEST_EMAIL2,
            data={"username": TEST_EMAIL2, "password": TEST_PASSWORD, "hub": TEST_HUB},
        )
    ...
    with patch("pyhoma.client.TahomaClient.login", return_value=True), patch(
        ...
        assert result["reason"] == "already_configured"
        assert mock_entry.data["username"] == TEST_EMAIL
        assert mock_entry.data["password"] == TEST_PASSWORD
