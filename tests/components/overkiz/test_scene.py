...
async def test_scene_entities_snapshot(
    ...
    mock_client.get_action_groups = AsyncMock(return_value=scenarios)
...
async def test_scene_activate(
    ...
    """Test activating a scene calls execute_persisted_action_group with the correct OID."""
    scenarios = load_scenarios_fixture("scenarios/tahoma_switch.json")
    mock_client.get_action_groups = AsyncMock(return_value=scenarios)
    mock_client.execute_persisted_action_group = AsyncMock(return_value="exec-1")
    ...
    mock_client.execute_persisted_action_group.assert_awaited_once_with(
...
async def test_scene_activate_multiple(
    ...
    mock_client.execute_persisted_action_group.reset_mock()
