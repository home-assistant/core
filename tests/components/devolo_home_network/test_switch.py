...
async def test_update_enable_guest_wifi(
    ...
    entry = configure_integration(hass)
    device_name = entry.title.replace(" ", "_").lower()
    entity_id = f"{PLATFORM}.{device_name}_enable_guest_wi_fi"
    ...
    await hass.services.async_call(
        PLATFORM, SERVICE_TURN_OFF, {ATTR_ENTITY_ID: entity_id}, blocking=True
    )
    ...
    await hass.services.async_call(
        PLATFORM, SERVICE_TURN_ON, {ATTR_ENTITY_ID: entity_id}, blocking=True
    )
...
async def test_update_enable_leds(
    ...
    entry = configure_integration(hass)
    device_name = entry.title.replace(" ", "_").lower()
    entity_id = f"{PLATFORM}.{device_name}_enable_leds"
...
async def test_auth_failed(
    ...
    with pytest.raises(HomeAssistantError):
        await hass.services.async_call(
            PLATFORM, SERVICE_TURN_OFF, {"entity_id": entity_id}, blocking=True
        )
    flows = hass.config_entries.flow.async_progress()
    assert len(flows) == 1
