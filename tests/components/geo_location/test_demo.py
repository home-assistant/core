...
class TestDemoPlatform(unittest.TestCase):
    ...
    def test_setup_platform(self):
        ...
        with patch('homeassistant.util.dt.utcnow', return_value=utcnow):
            ...
            assert abs(state_first_entry.attributes['latitude']-self.hass.config.latitude) < 1.0
            assert abs(state_first_entry.attributes['longitude']-self.hass.config.longitude) < 1.0
