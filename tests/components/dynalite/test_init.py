...
async def test_service_request_area_preset(hass):
    ...
    with patch(
        ...
        "homeassistant.components.dynalite.bridge.DynaliteDevices.request_area_preset",
        return_value=True,
        ...
        await hass.async_block_till_done()
        mock_req_area_pres.assert_called_once_with(2, None)
        mock_req_area_pres.reset_mock()
        await hass.services.async_call(
            dynalite.DOMAIN, "request_area_preset", {"host": "", "area": 3},
        )
        await hass.async_block_till_done()
        assert mock_req_area_pres.mock_calls == [call(3, None), call(3, None)]
        mock_req_area_pres.reset_mock()
        await hass.services.async_call(
            dynalite.DOMAIN, "request_area_preset", {"host": "5.6.7.8", "area": 4},
        )
        await hass.async_block_till_done()
        mock_req_area_pres.assert_called_once_with(4, None)
        mock_req_area_pres.reset_mock()
        await hass.services.async_call(
