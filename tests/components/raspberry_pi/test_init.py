...
from spencerassistant.components.raspberry_pi.const import DOMAIN
...
def mock_rpi_power():
    ...
    with patch(
        "spencerassistant.components.rpi_power.async_setup_entry",
        return_value=True,
    ):
        yield
...
async def test_setup_entry(hass: spencerAssistant) -> None:
    ...
    with patch(
        "spencerassistant.components.raspberry_pi.get_os_info",
