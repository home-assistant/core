...
async def test_hmip_acceleration_sensor(hass, default_mock_hap):
    ...
    assert ha_entity.attributes[ATTR_ACCELERATION_SENSOR_MODE] == "FLAT_DECT"
    assert ha_entity.attributes[ATTR_ACCELERATION_SENSOR_NEUTRAL_POSITION] == "VERTICAL"
    assert (
        ha_entity.attributes[ATTR_ACCELERATION_SENSOR_SENSITIVITY] == "SENSOR_RANGE_4G"
    )
    assert ha_entity.attributes[ATTR_ACCELERATION_SENSOR_TRIGGER_ANGLE] == 45
...
async def test_hmip_sunshine_sensor(hass, default_mock_hap):
    ...
    assert ha_entity.state == STATE_ON
    assert ha_entity.attributes["today_sunshine_duration_in_minutes"] == 100
...
async def test_hmip_security_zone_sensor_group(hass, default_mock_hap):
    ...
    ha_entity = hass.states.get(entity_id)
    assert ha_entity.state == STATE_ON
    assert ha_entity.attributes[ATTR_MOTION_DETECTED] is True
...
async def test_hmip_security_sensor_group(hass, default_mock_hap):
    ...
    assert ha_entity.state == STATE_OFF
    assert not ha_entity.attributes.get("low_bat")
    await async_manipulate_test_data(hass, hmip_device, "lowBat", True)
    ha_entity = hass.states.get(entity_id)
    assert ha_entity.state == STATE_ON
    assert ha_entity.attributes[ATTR_LOW_BATTERY] is True
    ...
    assert ha_entity.state == STATE_ON
    assert (
        ha_entity.attributes["smoke_detector_alarm"]
        == SmokeDetectorAlarmType.PRIMARY_ALARM
    )
