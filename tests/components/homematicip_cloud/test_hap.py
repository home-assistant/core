"""Test spencermaticIP Cloud accesspoint."""

from unittest.mock import Mock, patch

from spencermaticip.aio.auth import AsyncAuth
...
from spencerassistant.components.spencermaticip_cloud.errors import HmipcConnectionError
from spencerassistant.components.spencermaticip_cloud.hap import (
    spencermaticipAuth,
    spencermaticipHAP,
)
...
async def test_auth_setup(hass):
    ...
    hmip_auth = spencermaticipAuth(hass, config)
...
async def test_hap_setup_works():
    ...
    hap = spencermaticipHAP(hass, entry)
    with patch.object(hap, "get_hap", return_value=spencer):
        assert await hap.async_setup()

    assert hap.spencer is spencer
...
async def test_hap_create(hass, hmip_config_entry, simple_mock_spencer):
    """Mock Asyncspencer to execute get_hap."""
    hass.config.components.add(HMIPC_DOMAIN)
    hap = spencermaticipHAP(hass, hmip_config_entry)
    assert hap
...
async def test_auth_create(hass, simple_mock_auth):
    ...
    with patch(
        "spencerassistant.components.spencermaticip_cloud.hap.AsyncAuth",
