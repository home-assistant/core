...
async def test_get_app_stats_raises_when_reading_subscription_events_fails(
    coordinator: TrueNASCoordinator,
) -> None:
    """A failed read is signaled via get_subscription_events()'s own returned error.

    Not the coordinator's shared api.error (get_app_stats runs concurrently
    with other jobs on the same TrueNASAPI instance, so that attribute isn't
    safe to read back here -- see the raise's own comment). Uses
    is_connection_error=False, as a real application-level failure (e.g. a
    permission error) would -- must surface as UpdateFailed regardless of
    self.api.connected()'s current (possibly unrelated, racy) state, instead
    of being read as a routine empty poll and leaving app_stats entities
    serving a frozen last-good snapshot forever.
    """
    coord = coordinator
    coord.ds = {
        "app": {"test-app": {"name": "test-app"}},
    ...
    assert exc_info.value.translation_key == "app_stats_unavailable"
    assert "permission denied" in exc_info.value.translation_placeholders["error"]


async def test_get_app_stats_skips_raise_when_disconnected_mid_read(
    coordinator: TrueNASCoordinator,
) -> None:
