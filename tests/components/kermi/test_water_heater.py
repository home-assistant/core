...
def test_init(kermi_water_heater):
...
def test_set_temperature(kermi_water_heater):
...
def test_set_operation_mode(kermi_water_heater):
...
async def test_async_setup_entry(mock_kermi_water_heater, mock_entry, mock_coordinator):
