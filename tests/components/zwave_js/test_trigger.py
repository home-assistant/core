...
async def test_invalid_trigger_configs(hass: HomeAssistant) -> None:
    ...
    with pytest.raises(vol.Invalid):
        await TRIGGERS[f"{DOMAIN}.event"].async_validate_config(
            hass,
            {
    ...
    with pytest.raises(vol.Invalid):
        await TRIGGERS[f"{DOMAIN}.value_updated"].async_validate_config(
            hass,
            {
...
async def test_zwave_js_trigger_config_entry_unloaded(
    ...
    assert await TRIGGERS[f"{DOMAIN}.value_updated"].async_validate_config(
        hass,
        {
            "platform": f"{DOMAIN}.value_updated",
            "entity_id": SCHLAGE_BE469_LOCK_ENTITY,
    ...
    assert await TRIGGERS[f"{DOMAIN}.event"].async_validate_config(
        hass,
        {
            "platform": f"{DOMAIN}.event",
            "entity_id": SCHLAGE_BE469_LOCK_ENTITY,
