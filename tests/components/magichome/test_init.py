...
class TestLight(unittest.TestCase):
    ...
    def test_default_profiles_light(self):
        ...
        dev = next(
            filter(lambda x: x.entity_id == "light.dc4f22e3c446", platform.ENTITIES)
        )
...
async def test_intent_set_color(hass):
    ...
    hass.states.async_set(
        "light.dc4f22e3c446", "off", {ATTR_SUPPORTED_FEATURES: light.SUPPORT_COLOR}
    )
    hass.states.async_set("switch.c44f339fff3e", "off")
...
async def test_light_context(hass, hass_admin_user):
    ...
    state2 = hass.states.get("light.dc4f22e3c446")
