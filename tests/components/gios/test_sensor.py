...
async def test_availability(hass: HomeAssistant) -> None:
    ...
    state = hass.states.get("sensor.station_name_pm2_5_index")
    assert state
    assert state.state == "good"

    state = hass.states.get("sensor.station_name_air_quality_index")
    assert state
    assert state.state == "good"
...
async def test_missing_index_data(
    ...
    state = hass.states.get("sensor.station_name_nitrogen_dioxide_index")
    assert state
    assert state.state == STATE_UNAVAILABLE

    state = hass.states.get("sensor.station_name_ozone_index")
    assert state
    assert state.state == STATE_UNAVAILABLE

    state = hass.states.get("sensor.station_name_pm10_index")
    assert state
    assert state.state == STATE_UNAVAILABLE
    ...
    state = hass.states.get("sensor.station_name_sulphur_dioxide_index")
    assert state
    assert state.state == STATE_UNAVAILABLE
...
async def test_unique_id_migration(
    ...
    entity_registry.async_get_or_create(
        ...
        DOMAIN,
        "123-pm2.5",
        suggested_object_id="station_name_pm2_5",
        disabled_by=None,
    )

    await setup_integration(hass, mock_config_entry)

    entry = entity_registry.async_get("sensor.station_name_pm2_5")
    assert entry
