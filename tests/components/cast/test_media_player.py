...
from tests.async_mock import ANY, MagicMock, Mock, patch
...
async def test_device_attribute(hass: HomeAssistantType):
    ...
    uuid = info.uuid.replace("-", "")
    device_entry = device_reg.async_get_device({("cast", uuid)}, set())
    assert device_entry is not None
    assert device_entry.identifiers == {("cast", uuid)}
    assert device_entry.manufacturer == "Google Inc."
    assert device_entry.name == "Speaker"
    assert device_entry.model == "Some model"
