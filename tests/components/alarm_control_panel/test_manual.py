...
class TestAlarmControlPanelManual(unittest.TestCase):
    ...
    def test_keypress_arm(self):
        """Test arming with a valid code sent via keypress"""
        self.assertTrue(setup_component(
            self.hass, alarm_control_panel.DOMAIN,
            {'alarm_control_panel': {
                'platform': 'manual',
        ...
        alarm_control_panel.alarm_keypress(self.hass, CODE)
        self.hass.block_till_done()

        self.assertEqual(STATE_ALARM_ARMED_AWAY,
                         self.hass.states.get(entity_id).state)

    def test_keypress_disarm(self):
        """Test disarming with a valid code sent via keypress"""
        self.assertTrue(setup_component(
            self.hass, alarm_control_panel.DOMAIN,
            {'alarm_control_panel': {
                'platform': 'manual',
