...
from homeassistant.components.tewke.data import TewkeConfigEntry, TewkeData
...
async def test_async_setup_observe_success(
    ...
    mock_config_entry.runtime_data = TewkeData(
        ...
        tap=mock_tap,
        scenes={"scene1": Scene(id="scene1", name="Scene 1", isActive=False, brightness=0),},
        pending_scenes={},
    )
    ...
    with patch("homeassistant.components.tewke.util._TewkeObserver") as mock_observer:
        ...
        mock_observer_instance.on_scene_update.assert_called_once_with(coordinator.data["scenes_all"])
...
async def test_tewke_observer(
    ...
    mock_config_entry.runtime_data = TewkeData(
        ...
        coordinator=AsyncMock(),
        tap=mock_tap,
        scenes={"scene1": Scene(id="scene1", name="Scene 1", isActive=False, brightness=0), "scene2": Scene(id="scene2", name="Scene 2", isActive=False, brightness=0)},
        pending_scenes={"pending1": Scene(id="pending1", name="Scene pending 1", isActive=False, brightness=0)},
    )
    ...
    observer.on_scene_update({"scene1": Scene.model_construct(id="scene1", name="Scene 1")})
    ...
    observer.on_config_update(ConfigData.model_construct(device_name="New Name", tewke_os_version="1.1.0"))
