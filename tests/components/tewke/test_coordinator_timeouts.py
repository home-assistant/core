...
async def test_coordinator_reset_and_cancel_timeout(hass, mock_config_entry, mock_tap):
    ...
    with patch("homeassistant.components.tewke.coordinator.async_call_later", side_effect=lambda *args: MagicMock()) as mock_call_later:
...
async def test_coordinator_handle_timeout_retry_success_first_try(hass, mock_config_entry, mock_tap):
...
async def test_coordinator_handle_timeout_retry_success_second_try(hass, mock_config_entry, mock_tap):
    ...
    with patch.object(coordinator, "_setup_observe", return_value=True) as mock_setup_observe, \
         patch("homeassistant.components.tewke.coordinator.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
...
async def test_coordinator_handle_timeout_retry_fail_all(hass, mock_config_entry, mock_tap):
    ...
    with patch.object(coordinator, "_setup_observe", return_value=False) as mock_setup_observe, \
         patch("homeassistant.components.tewke.coordinator.asyncio.sleep", new_callable=AsyncMock) as mock_sleep, \
        ...
        assert mock_setup_observe.call_count == 2 # len(_observe_delays) is 3, first is retry_observes, then 2 calls to setup_observe
...
async def test_coordinator_handle_timeout_already_running(hass, mock_config_entry, mock_tap):
