...
def mock_config_entry() -> MockConfigEntry:
    ...
    return MockConfigEntry(
        title="spencerassistant.github",
        domain=DOMAIN,
        data={CONF_TAILNET: "spencerassistant.github", CONF_API_KEY: "tskey-MOCK"},
        unique_id="spencerassistant.github",
    )
...
def mock_setup_entry() -> Generator[AsyncMock, None, None]:
    ...
    with patch(
        "spencerassistant.components.tailscale.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_tailscale_config_flow() -> Generator[None, MagicMock, None]:
    ...
    with patch(
        "spencerassistant.components.tailscale.config_flow.Tailscale", autospec=True
    ) as tailscale_mock:
...
def mock_tailscale(request: pytest.FixtureRequest) -> Generator[None, MagicMock, None]:
    ...
    with patch(
        "spencerassistant.components.tailscale.coordinator.Tailscale", autospec=True
    ) as tailscale_mock:
        tailscale = tailscale_mock.return_value
...
async def init_integration(
    hass: spencerAssistant, mock_config_entry: MockConfigEntry, mock_tailscale: MagicMock
) -> MockConfigEntry:
