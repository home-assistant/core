...
from homeassistant.components.omie.util import CET, _is_published, get_market_dates
...
class TestGetMarketDates:
    ...
    def test_lisbon_midday_spans_cet_dates(self) -> None:
        ...
        market_dates = get_market_dates(local_time)

        # A Lisbon day (00:00-23:59 Lisbon) converts to (01:00 CET day1 - 00:59 CET day2)
        # So it spans 2 CET dates: June 15 (already published) and June 16 (not yet published, not returned)
        assert market_dates == {date(2024, 6, 15)}

    def test_lisbon_afternoon_spans_cet_dates(self) -> None:
        """Test that Lisbon day spans CET dates due to timezone conversion."""
        tz = ZoneInfo("Europe/Lisbon")
        local_time = datetime(2024, 6, 15, 13, 0, tzinfo=tz)  # 13:00 Lisbon = 14:00 CET
    ...
    def test_dst_spring_transition(self) -> None:
        ...
        assert market_dates == {date(2024, 3, 31)}
