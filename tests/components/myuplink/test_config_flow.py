...
@pytest.mark.usefixtures("current_request_with_host")
async def test_flow_reauth_bad_scope(
    hass: HomeAssistant,
    hass_client_no_auth: ClientSessionGenerator,
    aioclient_mock: AiohttpClientMocker,
...
async def test_flow_reauth_account_mismatch(
    ...
    """Test reauth step with mismatching account."""
    ...
    assert hass.config_entries.async_update_entry(
        mock_config_entry, data=CURRENT_TOKEN, unique_id="wrong_uid"
    )
    assert len(hass.config_entries.async_entries(DOMAIN)) == 1
    ...
    assert result.get("type") is FlowResultType.ABORT
    assert result.get("reason") == "account_mismatch"
...
async def test_flow_reauth(
    ...
    """Test reauth step with correct input."""
