...
async def test_option_flow(hass: HomeAssistant) -> None:
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        data=CONFIG,
        options={CONF_UNIT_OF_MEASUREMENT: UnitOfVolumeConcentration.MG_DL},
    )
    entry.add_to_hass(hass)
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={CONF_UNIT_OF_MEASUREMENT: UnitOfVolumeConcentration.MMOL_L},
    )
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"] == {
        CONF_UNIT_OF_MEASUREMENT: UnitOfVolumeConcentration.MMOL_L,
    }
