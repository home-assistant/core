...
async def test_user_flow_pair_updates_existing_entry(hass: HomeAssistant) -> None:
    """User-initiated flow with same unique_id updates host/port of existing entry."""
    ...
    with patch(
        ...
        res = await hass.config_entries.flow.async_configure(
            res["flow_id"], {CONF_HOST: "192.0.2.10", CONF_PORT: 9123}
        )
        res = await hass.config_entries.flow.async_configure(
            res["flow_id"], {"code": "1234"}
        )
...
async def test_manage_saved_snapshot_error_shows_form(hass: HomeAssistant) -> None:
    ...
    snapshot_ok = {
        "entities": [{"id": "light.a", "isSaved": True}],
        "quick_bars": [{"name": "QB"}],
    }
...
async def test_qb_manage_ensure_snapshot_false_returns_error_form(
    hass: HomeAssistant,
) -> None:
...
async def test_options_manage_saved_pick_shows_form_on_none_input(
    hass: HomeAssistant,
) -> None:
...
async def test_options_manage_saved_shows_form_on_none_input(
    hass: HomeAssistant,
) -> None:
    """Test manage_saved shows form when user_input is None."""
    entry = await _loaded_entry(hass)
    snapshot = {
        "entities": [{"id": "light.a", "isSaved": True, "friendlyName": "Light A"}],
        "quick_bars": [{"name": "QB"}],
    }
