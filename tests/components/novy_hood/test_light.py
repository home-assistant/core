...
from homeassistant.components.novy_hood.commands import NovyHoodLight
...
ENTITY_ID = "light.novy_hood_light"
...
async def test_turn_on_and_off_send_light_once_each(
    ...
    assert isinstance(mock_rf_entity.send_command_calls[0].command, NovyHoodLight)
    ...
    assert isinstance(mock_rf_entity.send_command_calls[1].command, NovyHoodLight)
