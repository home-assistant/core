...
async def test_validate_user_for_token_creation_inactive_user(mock_hass) -> None:
    """Test _validate_user_for_token_creation with inactive user."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = MockUser(is_active=False).add_to_auth_manager(manager)
    
    with pytest.raises(ValueError, match="User is not active"):
        manager._validate_user_for_token_creation(
            user, CLIENT_ID, auth_models.TOKEN_TYPE_NORMAL
        )


async def test_validate_user_for_token_creation_system_user_with_client(
    mock_hass,
) -> None:
    """Test _validate_user_for_token_creation with system user and client_id."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = await manager.async_create_system_user("Test System User")
    
    with pytest.raises(
        ValueError,
        match="System generated users cannot have refresh tokens connected to a client",
    ):
        manager._validate_user_for_token_creation(
            user, CLIENT_ID, auth_models.TOKEN_TYPE_SYSTEM
        )


async def test_validate_user_for_token_creation_normal_user_without_client(
    mock_hass,
) -> None:
    """Test _validate_user_for_token_creation with normal user without client_id."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = MockUser().add_to_auth_manager(manager)
    
    with pytest.raises(ValueError, match="Client is required to generate a refresh token"):
        manager._validate_user_for_token_creation(
            user, None, auth_models.TOKEN_TYPE_NORMAL
        )


async def test_validate_user_for_token_creation_system_user_wrong_token_type(
    mock_hass,
) -> None:
    """Test _validate_user_for_token_creation with system user and wrong token type."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = await manager.async_create_system_user("Test System User")
    
    with pytest.raises(
        ValueError,
        match="System generated users can only have system type refresh tokens",
    ):
        manager._validate_user_for_token_creation(
            user, None, auth_models.TOKEN_TYPE_NORMAL
        )
...
async def test_validate_user_for_token_creation_success(mock_hass) -> None:
    """Test _validate_user_for_token_creation with valid parameters."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = MockUser().add_to_auth_manager(manager)
...
async def test_validate_long_lived_token_without_client_name(mock_hass) -> None:
    """Test _validate_long_lived_token without client_name."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = MockUser().add_to_auth_manager(manager)
    
    with pytest.raises(
        ValueError, match="Client_name is required for long-lived access token"
    ):
        manager._validate_long_lived_token(user, None)


async def test_validate_long_lived_token_duplicate_client_name(mock_hass) -> None:
    """Test _validate_long_lived_token with duplicate client_name."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = MockUser().add_to_auth_manager(manager)
    
    # Create a long-lived token first
    await manager.async_create_refresh_token(
        user,
        client_name="GPS Logger",
        token_type=auth_models.TOKEN_TYPE_LONG_LIVED_ACCESS_TOKEN,
    ...
    # Try to validate with the same client_name
    with pytest.raises(ValueError, match="GPS Logger already exists"):
        manager._validate_long_lived_token(user, "GPS Logger")


async def test_validate_long_lived_token_success(mock_hass) -> None:
    """Test _validate_long_lived_token with valid parameters."""
    manager = await auth.auth_manager_from_config(mock_hass, [], [])
    user = MockUser().add_to_auth_manager(manager)
    
    # Should not raise any exception
    manager._validate_long_lived_token(user, "New Client")
