...
def version_to_branch(version):
    """
    Convert the project version into the GitHub branch or tag.
    """
...
code_branch = version_to_branch(__version__)
...
def extract_github_path(fn):
    """
    Convert a filesystem path to its GitHub equivalent.
    """

    p = re.compile('{}homeassistant-.*-.*\.egg{}'.format(
        re.escape(os.sep), re.escape(os.sep)))
    match = p.search(fn)
...
def linkcode_resolve(domain, info):
    ...
    if lineno:
        ...
        linespec = ""
    fn = relpath(fn, start='../')
    path = extract_github_path(fn)

    return '{}/blob/{}/{}{}'.format(GITHUB_URL, code_branch, path, linespec)
