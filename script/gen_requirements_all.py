...
def gather_requirements_from_modules(errors: list[str], reqs: dict[str, list[str]]) -> None:
    ...
    for package in sorted(packages):
        try:
            ...
            print(f"⚠️ Import error in {package}: {err}")
...
def main(validate: bool, ci: bool) -> int:
    ...
    if not Path("requirements_all.txt").is_file():
        print("❌ Run this script from the Home Assistant root directory.")
        return 1

    print("🔍 Gathering module data...")
    data = gather_modules()
    if data is None:
        return 1

    print("⚙️ Generating requirements files...")
    ...
    if validate:
        ...
        if differences:
            print("❌ Differences detected! Please run `python3 -m script.gen_requirements_all` to update.")
            print("\n\n".join(differences))
            return 1
        print("✅ Validation successful. No differences found.")
        return 0
