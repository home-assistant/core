...
AUTOGENERATED_ADMONITION = f"# Automatically generated by {SCRIPT_NAME}, do not edit\n"
AUTOGENERATED_FROM_PRECOMMIT_ADMONITION = f"# Automatically generated from .pre-commit-config.yaml by {SCRIPT_NAME}, do not edit\n"
...
def get_package_sort_key(package: str) -> str:
    """Return a sort key for a package."""
    return package.partition("==")[0].lower()
...
def gather_modules():
    ...
    for key in reqs:
        reqs[key] = sorted(
            reqs[key],
            key=lambda name: (len(name.split(".")), get_package_sort_key(name)),
        )
...
def gather_requirements_from_manifests(errors, reqs):
    ...
    for domain in sorted(integrations, key=get_package_sort_key):
...
def generate_requirements_list(reqs):
    ...
    if isinstance(reqs, list):
        ...
        separate = False
    output = []
    for pkg, requirements in sorted(
        reqs.items(), key=lambda item: get_package_sort_key(item[0])
    ):
        if separate:
            output.append("")
        for req in sorted(requirements, key=get_package_sort_key):
            output.append(f"# {req}")
        ...
        if comment_requirement(pkg):
            ...
            output.append(pkg)
    output.append("")  # Add empty line at end
    return "\n".join(output)
...
def requirements_output(reqs):
    ...
    output = [
        AUTOGENERATED_ADMONITION,
        "-c homeassistant/package_constraints.txt\n",
        "\n",
        "# Home Assistant Core\n",
    ]
    output.append(generate_requirements_list(core_requirements()))
...
def requirements_pre_commit_output():
    ...
    pre_commit_conf = load_yaml(".pre-commit-config.yaml")
    reqs = []
    ...
    output = [
        AUTOGENERATED_FROM_PRECOMMIT_ADMONITION,
    ]
    output.extend(sorted(reqs, key=get_package_sort_key))
    return "\n".join(output) + "\n"
