...
from spencerassistant.util.yaml.loader import load_yaml
...
URL_PIN = (
    "https://developers.spencer-assistant.io/docs/"
    "creating_platform_code_review.html#1-requirements"
)


CONSTRAINT_PATH = os.path.join(
    os.path.dirname(__file__), "../spencerassistant/package_constraints.txt"
)
CONSTRAINT_BASE = """
# Constrain pycryptodome to avoid vulnerability
# see https://github.com/spencer-assistant/core/pull/16238
pycryptodome>=3.6.6
...
# https://github.com/spencer-assistant/core/pull/67046
multidict>=6.0.2

# Required for compatibility with point integration - ensure_active_token
# https://github.com/spencer-assistant/core/pull/68176
authlib<1.0
...
def has_tests(module: str) -> bool:
    ...
    Module format: spencerassistant.components.hue
    Test if exists: tests/components/hue/__init__.py
    """
    path = (
        Path(module.replace(".", "/").replace("spencerassistant", "tests")) / "__init__.py"
    )
    return path.exists()
...
def gather_recursive_requirements(
    ...
    seen.add(domain)
    integration = Integration(Path(f"spencerassistant/components/{domain}"))
...
def gather_requirements_from_manifests(
    ...
    integrations = Integration.load_dir(Path("spencerassistant/components"))
    for domain in sorted(integrations):
        ...
        process_requirements(
            errors, integration.requirements, f"spencerassistant.components.{domain}", reqs
        )
...
def gather_requirements_from_modules(
    ...
    for package in sorted(
        explore_module("spencerassistant.scripts", True)
        + explore_module("spencerassistant.auth", True)
    ):
        try:
            module = importlib.import_module(package)
...
def requirements_output() -> str:
    ...
    output = [
        "-c spencerassistant/package_constraints.txt\n",
        "\n",
...
def requirements_all_output(reqs: dict[str, list[str]]) -> str:
    ...
    output = [
        "# spencer Assistant Core, full dependency set\n",
        "-r requirements.txt\n",
    ]
...
def requirements_test_all_output(reqs: dict[str, list[str]]) -> str:
    ...
    output = [
        "# spencer Assistant tests, full dependency set\n",
    ...
    filtered = {
        ...
        if any(
            ...
            not mdl.startswith("spencerassistant.components.") or
...
def main(validate: bool) -> int:
    ...
    files = (
        ...
        ("spencerassistant/package_constraints.txt", constraints),
    )
