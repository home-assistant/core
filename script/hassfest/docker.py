...
DOCKERFILE_TEMPLATE = (
    r"""# syntax=docker/dockerfile@sha256:{dockerfile_syntax}
...
LABEL \
    ...
    r""""The Home Assistant Authors" \
    org.opencontainers.image.description="""
    r""""Open-source home automation"""
    r""" platform running on Python 3" \
    org.opencontainers.image.documentation="""
    r""""https://www.home-assistant.io/docs/" \
    org.opencontainers.image.licenses="Apache-2.0" \
    org.opencontainers.image.title="Home Assistant" \
    org.opencontainers.image.url="""
    r""""https://www.home-assistant.io/"
...
# Add go2rtc binary
COPY --from=ghcr.io/alexxit/go2rtc@sha256:{go2rtc}"""
    r""" /usr/local/bin/go2rtc /bin/go2rtc

## Setup Home Assistant Core dependencies
COPY --parents requirements.txt"""
    r""" homeassistant/package_constraints.txt homeassistant/
RUN \
    # Verify go2rtc can be executed
    go2rtc --version \
    # Install uv at the version pinned in the requirements file
    && pip3 install --no-cache-dir"""
    r""" "uv==$(awk -F'==' '/^uv==/{{print $2}}'"""
...
COPY requirements_all.txt"""
    r""" home_assistant_frontend-*"""
    r""" home_assistant_intents-* homeassistant/
RUN \
    if ls homeassistant/home_assistant_*.whl"""
    r""" 1> /dev/null 2>&1; then \
        uv pip install"""
    r""" homeassistant/home_assistant_*.whl; \
    fi \
    && uv pip install \
        --no-build \
...
COPY --parents LICENSE* README*"""
    r""" homeassistant/ pyproject.toml homeassistant/
RUN \
    uv pip install \
        -e ./homeassistant \
...
ENV \
    UV_SYSTEM_PYTHON=true \
    UV_EXTRA_INDEX_URL="""
    r""""https://wheels.home-assistant.io/musllinux-index/"

SHELL ["/bin/sh", "-o", "pipefail", "-c"]
ENTRYPOINT ["""
    r""""/usr/src/homeassistant/script/hassfest/docker/entrypoint.sh"]
WORKDIR "/github/workspace"

COPY --parents requirements.txt"""
    r""" homeassistant/ script /usr/src/homeassistant/

# Uv creates a lock file in /tmp
RUN --mount=type=tmpfs,target=/tmp \
    --mount=type=bind,"""
    r"""source=requirements_test.txt,"""
    r"""target=/tmp/requirements_test.txt,readonly \
    --mount=type=bind,"""
    r"""source=requirements_test_pre_commit.txt,"""
    r"""target=/tmp/requirements_test_pre_commit.txt,"""
    r"""readonly \
    ...
    && uv pip install \
        ...
        --no-cache \
        -c """
    r"""/usr/src/homeassistant/homeassistant/"""
    r"""package_constraints.txt \
        -r /usr/src/homeassistant/requirements.txt \
        "pipdeptree==$(awk -F'=='"""
    r""" '/^pipdeptree==/{{print $2}}'"""
    r""" /tmp/requirements_test.txt)" \
        "tqdm==$(awk -F'=='"""
    r""" '/^tqdm==/{{print $2}}'"""
    r""" /tmp/requirements_test.txt)" \
        "ruff==$(awk -F'=='"""
    r""" '/^ruff==/{{print $2}}'"""
    r""" /tmp/requirements_test_pre_commit.txt)"

LABEL "name"="hassfest"
LABEL "maintainer"="""
    r""""Home Assistant <hello@home-assistant.io>"

LABEL "com.github.actions.name"="hassfest"
LABEL "com.github.actions.description"="""
    r""""Run hassfest to validate"""
    r""" standalone integration repositories"
